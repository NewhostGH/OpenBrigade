...
class ConsumableController extends Controller
{
    ...
    {
        return [
            ['key' => 'type', 'label' => 'Type', 'type' => 'text', 'value' => fn ($c) => $c->TC_LIBELLE ?? __('common.empty_value'), 'alwaysVisible' => true, 'mobile' => true],
            ['key' => 'description', 'label' => 'Description', 'type' => 'text', 'value' => fn ($c) => $c->C_DESCRIPTION ?: __('common.empty_value'), 'alwaysVisible' => true, 'mobile' => true],
            ...
            ['key' => 'lieu', 'label' => 'Lieu', 'type' => 'text', 'value' => fn ($c) => $c->C_LIEU_STOCKAGE ?: __('common.empty_value'), 'mobile' => false, 'exportable' => true, 'exportValue' => fn ($c) => $c->C_LIEU_STOCKAGE ?? ''],
            ['key' => 'peremption', 'label' => 'Péremption', 'type' => 'html', 'value' => fn ($c) => $c->C_DATE_PEREMPTION ? (($c->alert_level === 'expired' || $c->alert_level === 'expiring') ? '<i class="fas fa-exclamation-triangle text-warning me-1" title="Attention"></i>' : '').e(Carbon::parse($c->C_DATE_PEREMPTION)->format('d/m/Y')) : __('common.empty_value'), 'mobile' => false, 'exportable' => true, 'exportValue' => fn ($c) => $c->C_DATE_PEREMPTION ? Carbon::parse($c->C_DATE_PEREMPTION)->format('d/m/Y') : ''],
