...
@section('title', __('admin.references.consumable_category.title') . ' — ' . config('app.name'))
...
<x-ob-breadcrumb :items="[
    ...
    ['label' => __('admin.references.consumable_category.title')],
]"/>
...
<div class="mx-3 mt-3">
    ...
    <div class="ob-widget-card mb-3">
        <div class="ob-widget-card-header">
            <div class="ob-widget-card-title"><i class="fas fa-plus me-2"></i>{{ __('admin.references.consumable_category.new_title') }}</div>
        </div>
        <div class="p-3">
            <form method="POST" action="{{ route('admin.references.consumable-category.store') }}">
                ...
                <div class="row g-2 align-items-end">
                    <div class="col-auto">
                        <label class="form-label form-label-sm">{{ __('admin.references.consumable_category.col_code') }} <span class="text-danger">*</span></label>
                        <input type="text" name="CC_CODE" value="{{ old('CC_CODE') }}"
                               class="form-control form-control-sm text-uppercase" style="width:110px;"
                               maxlength="12" placeholder="{{ __('admin.references.consumable_category.ph_code') }}" required
                               pattern="[A-Za-z0-9_]+" title="{{ __('admin.references.consumable_category.code_pattern_title') }}">
                    </div>
                    <div class="col-auto">
                        <label class="form-label form-label-sm">{{ __('admin.references.consumable_category.col_name') }} <span class="text-danger">*</span></label>
                        <input type="text" name="CC_NAME" value="{{ old('CC_NAME') }}"
                               class="form-control form-control-sm" style="width:160px;" maxlength="40" required>
                    </div>
                    <div class="col">
                        <label class="form-label form-label-sm">{{ __('admin.references.consumable_category.col_desc') }}</label>
                    ...
                    <div class="col-auto">
                        <label class="form-label form-label-sm">{{ __('admin.references.consumable_category.col_order') }}</label>
                ...
                <div class="mt-2" style="font-size:var(--font-size-xs);color:var(--text-muted);">
                    {!! __('admin.references.consumable_category.icon_hint') !!}
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="ob-widget-card">
        <div class="ob-widget-card-header">
            <div class="ob-widget-card-title"><i class="fas fa-layer-group me-2"></i>{{ __('admin.references.consumable_category.list_title', ['count' => $items->count()]) }}</div>
        </div>
        <div class="table-responsive">
            <table class="table table-sm table-hover mb-0">
                <thead class="table-light">
                    <tr>
                        <th style="width:110px;">{{ __('admin.references.consumable_category.col_code') }}</th>
                        <th style="width:160px;">{{ __('admin.references.consumable_category.col_name') }}</th>
                        <th>{{ __('admin.references.consumable_category.col_desc') }}</th>
                        <th style="width:110px;">{{ __('admin.references.consumable_category.col_icon') }}</th>
                        <th style="width:70px;">{{ __('admin.references.consumable_category.col_order') }}</th>
                ...
                @empty
                    <tr><td colspan="6" class="text-center text-muted py-4">{{ __('admin.references.consumable_category.empty') }}</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
