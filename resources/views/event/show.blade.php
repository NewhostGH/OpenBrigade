            ...
            <div class="row g-4">
                ...
                <div class="col-md-5">
                    <dl class="mb-0" style="display:grid; grid-template-columns:auto 1fr; gap:5px 16px;
                        ...
                        <dd class="mb-0">{{ $typeLabel ?? $event->TE_CODE ?? __('common.empty_value') }}</dd>

                        <dt class="text-muted fw-normal">{{ __('event.field_lieu') }}</dt>
                        <dd class="mb-0">{{ $event->E_LIEU ?: __('common.empty_value') }}</dd>

                        @feature('multi_site')
                        <dt class="text-muted fw-normal">{{ __('event.field_section') }}</dt>
                        <dd class="mb-0">{{ $event->section?->S_DESCRIPTION ?? __('common.empty_value') }}</dd>
                        @endfeature
                        ...
                        @foreach($participants as $p)
                            <tr>
                                ...
                                <td style="font-size:var(--font-size-xs);color:var(--text-muted-soft)">
                                    {{ $p->P_GRADE ?? __('common.empty_value') }}
                                </td>
                                @endfeature
                                <td style="font-size:var(--font-size-xs);color:var(--text-muted-soft)">
                                    {{ $p->TP_LIBELLE ?? __('common.empty_value') }}
                                </td>
                                ...
                                @else
                                    <td style="font-size:var(--font-size-xs);color:var(--text-muted-soft)">
                                        {{ $p->EE_NAME ?? __('common.empty_value') }}
                                    </td>
                                @endif
                            ...
                            <tr>
                                <td class="fw-semibold" style="font-size:var(--font-size-sm)">
                                    <a href="{{ route('vehicle.show', $v->V_ID) }}" class="text-decoration-none">
                                        {{ $v->V_IMMATRICULATION ?: __('common.empty_value') }}
                                    </a>
                                </td>
                                ...
                                <td class="text-end" style="font-size:var(--font-size-xs);color:var(--text-muted-soft)">
                                    {{ $v->EV_KM ? number_format($v->EV_KM, 0, ',', "\u{202f}") . ' km' : __('common.empty_value') }}
                                </td>
                                @if(auth()->user()->hasPermission(15))
                                    <td class="text-end pe-2">
                                        <form method="POST"
                    ...
                    <tbody>
                        @foreach($materiels as $m)
                            <tr>
                                ...
                                <td style="font-size:var(--font-size-xs);color:var(--text-muted-soft)">
                                    {{ $m->MA_NUMERO_SERIE ?: __('common.empty_value') }}
                                </td>
                    ...
                    <tbody>
                        @foreach($renforts as $r)
                            <tr>
                                ...
                                <td style="font-size:var(--font-size-xs);color:var(--text-muted-soft)">
                                    {{ $r->E_LIEU ?: __('common.empty_value') }}
                                </td>
                                <td class="text-center" style="font-size:var(--font-size-sm)">
                ...
                <table class="table table-sm table-hover mb-0 align-middle">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="font-size:var(--font-size-sm)">{{ $opt->EOG_TITLE ?? __('common.empty_value') }}</td>
                            <td style="font-size:var(--font-size-sm)">
                ...
                <tbody>
                    ...
                    <tr @if($log->EL_IMPORTANT) class="table-warning" @endif>
                        <td style="font-size:var(--font-size-sm);white-space:nowrap">
                            {{ $log->EL_DEBUT ? \Carbon\Carbon::parse($log->EL_DEBUT)->format('d/m H:i') : __('common.empty_value') }}
                            @if($log->EL_IMPORTANT)
                                <i class="fas fa-exclamation-circle text-danger ms-1" title="{{ __('event.log_important_title') }}"></i>
                            @endif
                        </td>
                        <td style="font-size:var(--font-size-sm)">
                            {{ $log->TEL_DESCRIPTION ?? $log->TEL_CODE ?? __('common.empty_value') }}
                        </td>
                        <td style="font-size:var(--font-size-sm)">
                            @if($log->EL_TITLE)
                        ...
                        <td style="font-size:var(--font-size-sm)">
                            {{ $log->P_PRENOM ? $log->P_PRENOM.' '.$log->P_NOM : __('common.empty_value') }}
                        </td>
                        @if(auth()->user()->hasPermission(15))
                        <td>
