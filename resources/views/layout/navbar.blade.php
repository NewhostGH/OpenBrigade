<div class="container-fluid noprint">
    <nav class="navbar navbar-expand-lg fixed-top noprint navbar-ebrigade">
        ...
        <div class="nav-siglets" id="navSiglets">
            @forelse ($pinnedShortcuts ?? [] as $shortcut)
                <a class="siglet" href="{{ url($shortcut['url']) }}" title="{{ $shortcut['label'] }}"
                    data-key="{{ $shortcut['key'] }}">
                    @if ($shortcut['icon'] !== '')
                        <i class="fas fa-{{ $shortcut['icon'] }}"></i>
                    @endif
                    <span>{{ $shortcut['label'] }}</span>
                    <button class="siglet-unpin" data-key="{{ $shortcut['key'] }}" title="Désépingler" aria-label="Désépingler">×</button>
                </a>
            @empty
        ...
        <div class="nav-border">
            <button class="navbar-toggler custom-toggler button-left noboxshadow" type="button"
                ...
                <i class="fas fa-chevron-down py-1 text-violet"></i>
            </button>
        </div>
        ...
        <div class="collapse navbar-collapse nav-right" id="navbarMain">
            <ul class="navbar-nav nav-top">
                ...
                @endphp
                @if ($canAdd)
                    <li class="nav-item dropdown nav-top-item navtop-hover margin-li" style="position:relative;">
                            ...
                            @if (auth()->user()->hasPermission(1))
                                <li>
                                    <a class="dropdown-item dropdown-item-profil"
                                        ...
                                        <i class="fas fa-user-plus fa-fw nav-item-icon" style="color:#28A745;"></i> Personnel
                                    </a>
                                </li>
                            @endif
                            @if (auth()->user()->hasPermission(15))
                                <li>
                                    <a class="dropdown-item dropdown-item-profil"
                                        ...
                                        <i class="fas fa-calendar-plus fa-fw nav-item-icon" style="color:#28A745;"></i> Activité
                                    </a>
                                </li>
                            @endif
                            @if (auth()->user()->hasPermission(17))
                                <li>
                                    <a class="dropdown-item dropdown-item-profil" href="{{ url('/ins_vehicule.php') }}">
                                        <i class="fas fa-truck fa-fw nav-item-icon" style="color:#28A745;"></i> Véhicule
                                    </a>
                                </li>
                            @endif
                            @if (auth()->user()->hasPermission(70))
                                <li>
                                    <a class="dropdown-item dropdown-item-profil"
                                        ...
                                        <i class="fas fa-toolbox fa-fw nav-item-icon" style="color:#28A745;"></i> Matériel
                                    </a>
                                </li>
                            @endif
                            @if (auth()->user()->hasPermission(71))
                                <li>
                                    <a class="dropdown-item dropdown-item-profil"
                                        ...
                                        <i class="fas fa-boxes fa-fw nav-item-icon" style="color:#28A745;"></i> Consommable
                                    </a>
                                </li>
                            @endif
                        </ul>
                ...
                {{-- Help --}}
                <ul class="nav-text navtop-hover margin-li pt-2" href="{{ route('about') }}" title="A propos" role="button">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end nav-dropdown-menu">
                        <li>
                            <a class="dropdown-item dropdown-item-profil"
                                ...
                                <i class="fas fa-user fa-fw nav-item-icon"></i> Ma fiche
                            </a>
                        </li>
                        <li>
                            <a class="dropdown-item dropdown-item-profil"
                                href="{{ url('/legacy/change_password.php') }}">
                                <i class="fas fa-key fa-fw nav-item-icon"></i> Mot de passe
                            </a>
                        </li>
                        <li>
                            <a class="dropdown-item dropdown-item-profil"
                                href="{{ url('/legacy/preferences.php') }}">
                                <i class="fas fa-sliders-h fa-fw nav-item-icon"></i> Mes préférences
                            </a>
                        </li>
                        <li>
                            <a class="dropdown-item dropdown-item-profil"
                                href="{{ url('/legacy/upd_section.php?S_ID=' . (auth()->user()->P_SECTION ?? 0)) }}">
                                <i class="fas fa-building fa-fw nav-item-icon"></i> Ma section
                            </a>
                        </li>
                        ...
                        <li>
                            <form method="POST" action="{{ route('logout') }}" class="d-inline">
                                ...
                                <button type="submit" class="dropdown-item dropdown-item-profil">
                                    <i class="fa fa-power-off fa-fw nav-item-icon" style="color:red;"></i> Déconnexion
                                </button>
                            </form>
                        </li>
                    </ul>
