    ...
    <div id="section-info" data-veh-section data-nav-icon="fas fa-info-circle" data-nav-label="{{ __('vehicle.nav_label_info') }}" class="ob-widget-card mb-3">
        <div class="ob-widget-card-header">
            <div class="ob-widget-card-title">
                ...
                <span>{{ $vehicule->V_IMMATRICULATION ?: __('common.empty_value') }}</span>
                ...
                <div class="col-md-5">
                    <dl class="mb-0"
                        ...
                        <dd class="mb-0">
                            ...
                            {{ $vehicleType->TV_LIBELLE ?? ($vehicule->TV_CODE ?: __('common.empty_value')) }}
                        </dd>

                        @if($vehicule->V_MODELE || $vehicule->V_ANNEE)
                            <dt class="text-muted fw-normal">{{ __('vehicle.dt_model') }}</dt>
                            <dd class="mb-0">
                                {{ trim(($vehicule->V_MODELE ?? '') . ($vehicule->V_ANNEE ? ' (' . $vehicule->V_ANNEE . ')' : '')) ?: __('common.empty_value') }}
                            </dd>
                        @endif

                        <dt class="text-muted fw-normal">{{ __('vehicle.dt_indicatif') }}</dt>
                        <dd class="mb-0 fw-semibold">{{ $vehicule->V_INDICATIF ?: __('common.empty_value') }}</dd>

                        @feature('multi_site')
                        <dt class="text-muted fw-normal">{{ __('vehicle.dt_section') }}</dt>
                        <dd class="mb-0">{{ $vehicule->section?->S_DESCRIPTION ?? __('common.empty_value') }}</dd>
                        @endfeature
                        ...
                        @foreach($expCards as $card)
                            <div class="col-6">
                                <div style="border-left: 3px solid {{ $card['color'] }};
                                    ...
                                    <div
                                        style="font-size:1rem; font-weight:700; color:{{ $card['text'] === __('common.empty_value') ? '#94a3b8' : $card['color'] }}; letter-spacing:.01em;">
                                        {{ $card['text'] }}
                                    </div>
                                    @if($card['sub'])
                                        <div
                        ...
                        @foreach($eventHistory as $ev)
                            <tr>
                                ...
                                <td style="font-size:var(--font-size-xs);color:var(--text-muted-soft)">
                                    {{ $ev->TFV_NAME ?? __('common.empty_value') }}
                                </td>
                                <td style="font-size:var(--font-size-xs);color:var(--text-muted-soft)">
                                    {{ $ev->EH_DATE_DEBUT ? \Carbon\Carbon::parse($ev->EH_DATE_DEBUT)->format('d/m/Y') : __('common.empty_value') }}
                                </td>
                                <td class="text-end" style="font-size:var(--font-size-xs);color:var(--text-muted-soft)">
                                    {{ $ev->EV_KM ? number_format($ev->EV_KM, 0, ',', "\u{202f}") . ' km' : __('common.empty_value') }}
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                    ...
                    <select name="MA_ID" class="form-select form-select-sm" style="max-width:280px;" required>
                        ...
                        @foreach($availableEquipment as $eq)
                            ...
                            <option value="{{ $eq->MA_ID }}">
                                {{ $eq->MA_MODELE ?: __('common.empty_value') }}
                ...
                <table class="table table-sm table-hover mb-0">
                    ...
                    <tbody>
                        @foreach($materiels as $mat)
                            <tr>
                                <td style="font-size:var(--font-size-sm)">{{ $mat->TM_DESCRIPTION ?? $mat->TM_CODE ?? __('common.empty_value') }}</td>
                                <td style="font-size:var(--font-size-sm)">{{ $mat->MA_MODELE ?: __('common.empty_value') }}</td>
                                <td style="font-size:var(--font-size-xs);color:var(--text-muted-soft)">
                                    {{ $mat->MA_NUMERO_SERIE ?: __('common.empty_value') }}</td>
                                <td style="font-size:var(--font-size-xs);color:var(--text-muted-soft)">
                                    {{ $mat->MA_INVENTAIRE ?: __('common.empty_value') }}</td>
                    ...
                    <tbody>
                        @foreach($documents as $doc)
                            <tr>
                                ...
                                <td style="font-size:var(--font-size-xs);color:var(--text-muted-soft)">
                                    {{ $doc->TD_LIBELLE ?? __('common.empty_value') }}</td>
                                <td style="font-size:var(--font-size-xs);color:var(--text-muted-soft)">
                                    {{ $doc->D_CREATED_DATE ? \Carbon\Carbon::parse($doc->D_CREATED_DATE)->format('d/m/Y') : __('common.empty_value') }}
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
