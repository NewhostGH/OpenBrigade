        ...
        @if ($hasBirthdays)
            ...
            @foreach ($birthdays['days'] as $day)
                @foreach ($day['rows'] as $p)
                    <div class="ob-dash-birthday-row">
                        <img src="{{ $p->avatarSrc }}" class="ob-dash-birthday-avatar"
                             onerror="this.src='{{ asset('images/autre.png') }}'">
                        <div class="ob-dash-birthday-name">
                            {{ ucfirst(strtolower($p->P_PRENOM)) }} {{ strtoupper($p->P_NOM) }}
                        </div>
                        <span class="ob-dash-day-label day-label-{{ $day['color'] }}">
                            {{ $day['label'] }}
                        </span>
                    </div>
                @endforeach
