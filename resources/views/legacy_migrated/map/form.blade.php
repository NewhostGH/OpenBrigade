    ...
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    <h1 class="h4 mb-4">Map Form</h1>

                    @php($itemKey = $item?->getKey() ?? ($item?->id ?? ($item?->P_ID ?? null)))
                    <form method="POST" action="{{ ($item && $itemKey) ? route('legacy_migrated.map.update', $itemKey) : route('legacy_migrated.map.store') }}">
                        @csrf
                        @if($item)
                            @method('PUT')
                        @endif
        ...
        <div class="mb-3">
            ...
            @enderror
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <a href="{{ route('legacy_migrated.map.index') }}" class="btn btn-outline-secondary">Back</a>
                        </div>
                    </form>
                </div>
            </div>
