...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="h4 mb-0">Responsable List</h1>
        <a href="{{ route('legacy_migrated.upd_responsable.create') }}" class="btn btn-primary">New</a>
    </div>
            ...
            <div class="table-responsive">
                <table class="table table-striped table-hover mb-0">
                    ...
                    <tr>
                        ...
                        <td>{{ $item->p_prenom ?? '' }}</td>
                        <td>{{ $item->p_nom ?? '' }}</td>
                        <td>{{ $item->s_codeps_code ?? '' }}</td>
                        <td>
                            @if($itemKey)
                                <a href="{{ route('legacy_migrated.upd_responsable.edit', $itemKey) }}" class="btn btn-sm btn-outline-primary">Edit</a>
                                <form method="POST" action="{{ route('legacy_migrated.upd_responsable.destroy', $itemKey) }}" class="d-inline">
                                    @csrf
                                    @method('DELETE')
