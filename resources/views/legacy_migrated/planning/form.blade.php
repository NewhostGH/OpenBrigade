...
<div class="container py-4">
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="card shadow-sm">
                <div class="card-header">
                    Legacy migration source: planning.php | This view stems from a legacy migration and requires functional verification.
                </div>
                <div class="card-body">
                    <h1 class="h4 mb-4">Planning Form</h1>

                    @php($itemKey = $item?->getKey() ?? ($item?->id ?? ($item?->P_ID ?? null)))
                    <form method="POST" action="{{ ($item && $itemKey) ? route('legacy_migrated.planning.update', $itemKey) : route('legacy_migrated.planning.store') }}">
                        @csrf
                        @if($item)
                            @method('PUT')
                        @endif
        ...
        <div class="mb-3">
            <label for="day_planning" class="form-label">Day Planning</label>
            <textarea id="day_planning" name="day_planning" class="form-control" rows="4">{{ old('day_planning', $item?->day_planning) }}</textarea>
            @error('day_planning')
                <div class="text-danger small">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            @enderror
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <a href="{{ route('legacy_migrated.planning.index') }}" class="btn btn-outline-secondary">Back</a>
                        </div>
                    </form>
                </div>
            </div>
