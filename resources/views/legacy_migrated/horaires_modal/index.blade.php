...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <a href="{{ route('legacy_migrated.horaires_modal.create') }}" class="btn btn-primary">New</a>
    </div>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-striped table-hover mb-0">
                    ...
                    <tr>
                        <td>{{ $item->count1 ?? '' }}</td>
                        <td>{{ $item->h_comment ?? '' }}</td>
                        <td>
                            @if($itemKey)
                                <a href="{{ route('legacy_migrated.horaires_modal.edit', $itemKey) }}" class="btn btn-sm btn-outline-primary">Edit</a>
                                <form method="POST" action="{{ route('legacy_migrated.horaires_modal.destroy', $itemKey) }}" class="d-inline">
                                    @csrf
                                    @method('DELETE')
