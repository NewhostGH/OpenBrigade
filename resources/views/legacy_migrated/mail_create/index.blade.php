...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="h4 mb-0">MailCreate List</h1>
        <a href="{{ route('legacy_migrated.mail_create.create') }}" class="btn btn-primary">New</a>
    </div>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-striped table-hover mb-0">
                    ...
                    <tr>
                        <td>{{ $item->p_id ?? '' }}</td>
                        <td>{{ $item->upperp_nom ?? '' }}</td>
                        <td>{{ $item->p_prenom ?? '' }}</td>
                        <td>{{ $item->p_email ?? '' }}</td>
                        <td>
                            @if($itemKey)
                                <a href="{{ route('legacy_migrated.mail_create.edit', $itemKey) }}" class="btn btn-sm btn-outline-primary">Edit</a>
                                <form method="POST" action="{{ route('legacy_migrated.mail_create.destroy', $itemKey) }}" class="d-inline">
                                    @csrf
                                    @method('DELETE')
