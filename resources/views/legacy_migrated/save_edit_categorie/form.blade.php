...
<div class="container py-4">
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="card shadow-sm">
                <div class="card-header">
                    Legacy migration source: save_edit_categorie.php | This view stems from a legacy migration and requires functional verification.
                </div>
                <div class="card-body">
                    <h1 class="h4 mb-4">EditCategorie Form</h1>

                    @php($itemKey = $item?->getKey() ?? ($item?->id ?? ($item?->P_ID ?? null)))
                    <form method="POST" action="{{ ($item && $itemKey) ? route('legacy_migrated.save_edit_categorie.update', $itemKey) : route('legacy_migrated.save_edit_categorie.store') }}">
                        @csrf
                        @if($item)
                            @method('PUT')
                        @endif
        ...
        <div class="mb-3">
            <label for="TM_USAGE_PREV" class="form-label">Tm Usage Prev</label>
            <input type="text" id="TM_USAGE_PREV" name="TM_USAGE_PREV" class="form-control" value="{{ old('TM_USAGE_PREV', $item?->TM_USAGE_PREV) }}">
            @error('TM_USAGE_PREV')
        ...
        <div class="mb-3">
            ...
            @enderror
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <a href="{{ route('legacy_migrated.save_edit_categorie.index') }}" class="btn btn-outline-secondary">Back</a>
                        </div>
                    </form>
                </div>
            </div>
