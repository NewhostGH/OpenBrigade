...
.table-nav { padding: 4px 12px 0 12px; }
.buttons-container { display: flex; align-items: center; gap: 4px; padding: 4px 0; }
</style>
@endpush
...
<div class="table-responsive table-nav noprint" style="border-bottom: solid 1px #dee2e6;" id="breadcrumb">
    <nav aria-label="breadcrumb" id="navbreadcrumb">
        <ol class="breadcrumb noprint">
            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}" style="color:#666;font-size:0.87rem;font-weight:bold;">Accueil</a></li>
            <li class="breadcrumb-item" style="font-size:0.87rem;font-weight:bold;color:#666;">Personnel</li>
            <li class="breadcrumb-item active" aria-current="page" style="color:#2b224f;font-size:0.87rem;font-weight:bold;">Liste</li>
        </ol>
    </nav>
    <div class="buttons-container noprint">
        ...
        <a class="btn btn-default" href="{{ route('personnel.index', array_merge(request()->query(), ['export' => 'xls'])) }}">
            <i class="far fa-file-excel fa-1x excel-hover" title="Exporter la liste dans un fichier Excel"></i>
        </a>
        <a class="btn btn-success" href="#" title="Ajouter du personnel" onclick="window.location='{{ route('dashboard.legacy') }}?redirect=ins_personnel.php%3Fcategory%3DINT%26suggestedcompany%3D-1';">
...
<div class="container-fluid noprint" id="toolbar" align="left">
    ...
    <select id="sectionFilter" name="filter"
        class="selectpicker" data-style="btn-default" data-container="body" data-live-search="true"
        ...
        @foreach ($sections as $sec)
            <option value="{{ $sec->S_ID }}" {{ $sectionId === (int)$sec->S_ID ? 'selected' : '' }}>{{ $sec->S_CODE }} - {{ $sec->S_DESCRIPTION }}</option>
        @endforeach
    </select>
    ...
    <select id="category_filter" name="category_filter" title="" class="selectpicker" data-style="btn-default" data-container="body"
        ...
        <option value="INT" {{ $category === 'INT' ? 'selected' : '' }} class="option-ebrigade">Tous sauf externes</option>
        @foreach ($categories as $statut)
            <option value="{{ $statut }}" {{ $category === $statut ? 'selected' : '' }} class="option-ebrigade">{{ $statut }}</option>
        @endforeach
    </select>

    <select id="position_filter" name="position_filter" title="" class="selectpicker" data-style="btn-default" data-container="body"
    ...
    @forelse ($items as $item)
        ...
        <tr style="cursor:pointer;" onclick="window.location='{{ route('personnel.show', $item) }}'">
            ...
            <td onclick="event.stopPropagation()" class="text-nowrap">
                <a href="{{ route('personnel.edit', $item) }}" class="btn btn-default btn-sm" title="Modifier" onclick="event.stopPropagation()">
                    <i class="fas fa-edit"></i>
                </a>
            </td>
        </tr>
    @empty
        <tr><td colspan="11" class="text-center py-4">Aucun personnel trouvé</td></tr>
    @endforelse
    </tbody>
</table>

<span style="height:36px;line-height:30px;color:#333;margin-right:1.6em;float:right;" title="Nombre de personnes">{{ $items->total() }} lignes</span>
</div>

<style type="text/css">
table { border-collapse: collapse !important; }
.hide_mobile { }
@media (max-width: 768px) { .hide_mobile { display: none; } }
</style>
