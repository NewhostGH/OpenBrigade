...
if (isset($_GET['operation']) and $_GET['operation'] == 'Ajouter') {
    $operation = 'insert';
} elseif (isset($_GET['operation']) and $_GET['operation'] == 'Sauvegarder') {
    $operation = 'update';
} elseif (isset($_GET['operation']) and $_GET['operation'] == 'Supprimer') {
    $operation = 'delete';
...
if ($operation == 'update' or $operation == 'insert') {
    ...
    if (isset($_GET['PH_HIDE_LOWER'])) {
        $PH_HIDE_LOWER = intval($_GET['PH_HIDE_LOWER']);
    } else {
        $PH_HIDE_LOWER = 0;
    }
    if (isset($_GET['PH_UPDATE_LOWER_EXPIRY'])) {
        $PH_UPDATE_LOWER_EXPIRY = intval($_GET['PH_UPDATE_LOWER_EXPIRY']);
    } else {
        $PH_UPDATE_LOWER_EXPIRY = 0;
    }
    if (isset($_GET['PH_UPDATE_MANDATORY'])) {
        $PH_UPDATE_MANDATORY = intval($_GET['PH_UPDATE_MANDATORY']);
    } else {
        $PH_UPDATE_MANDATORY = 0;
    }
}
