...
if ($id != $P_ID) {
    check_all(70);
    if (! check_rights($id, 70, "$section")) {
        check_all(24);
    }
}

if ($id == $P_ID and (! check_rights($id, 70, "$section"))) {
    $update_my_size_only = true;
} else {
    $update_my_size_only = false;
}
/*
echo "<pre>";
print_r($_POST);
    ...
    if ($key != 'pompier') {
        ...
        if ($key == 'NB_'.$_MID) {
            ...
            if ($update_my_size_only) {
                ...
                if ($annee == 0 or $annee < 1900 or $annee > 2050) {
                    $annee = 'null';
                }

                if (isset($_POST['NEW_'.$_MID]) and $value > 0) {
                    $query2 = 'insert into materiel (TM_ID, MA_MODELE, MA_ANNEE, MA_NB, MA_UPDATE_BY, MA_UPDATE_DATE, AFFECTED_TO, S_ID, TV_ID,VP_ID)
