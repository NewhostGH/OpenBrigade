...
$html .= '</table>';
$html .= "<form action='evenement_competences.php' method='POST'>";
$html .= "<div class='table-responsive' ><div class='container-fluid'>";
...
if ($nbrows > 0) {
    ...
    while (custom_fetch_array($resultm)) {
        ...
        if ($poste == 0) {
            ...
            if ($nb == 0) {
                $pic = "<i class='fa fa-check-circle fa-lg' style='color:$widget_fggreen;' title='Pas de limite sur le nombre de personnel inscrit'></i>";
            } elseif ($inscrits > $nb) {
                if ($TE_CODE == 'GAR') {
                    $pic = "<i class='fa fa-check-circle fa-lg' style='color:$widget_fgorange;' title='Trop de personnel inscrit'></i>";
                } else {
                    $pic = "<i class='fa fa-check-circle fa-lg' style='color:$widget_fgblue;' title='Plus de personnel inscrit que nécessaire '></i>";
                }
            } elseif ($inscrits == $nb) {
                $pic = "<i class='fa fa-check-circle fa-lg' style='color:$widget_fggreen;' title='Nombre suffisant de personnel inscrit'></i>";
            } else {
                $pic = "<i class='fa fa-exclamation-circle fa-lg' style='color:$widget_fgred;' title='Pas assez de personnel inscrit'></i>";
            }
            $html .= '<tr>
            <td><b>Nombre total</b></td>';
            if ($evenement > 0) {
                $html .= '<td align=center><b>'.$inscrits.'</b> '.$pic.'</td>';
            }
            $html .= "<td align=center>
            <input name='global' class='form-control form-control-sm' style='width:auto; text-align:center' type='text' title='Nombre global de personnes' size=1 maxlength='3' value='$nb'
        ...
        // DETAIL PAR COMPETENCE
            ...
            if ($nb > 0) {
                if ($evenement > 0) {
                    ...
                    if ($inscrits > $nb + 2) {
                        $pic = "<i class='fa fa-check-circle fa-lg' style='color:$widget_fgblue;' title='Plus de personnel inscrit que nécessaire pour cette compétence'></i>";
                    } elseif ($inscrits >= $nb) {
                        $pic = "<i class='fa fa-check-circle fa-lg' style='color:$widget_fggreen;' title='Nombre suffisant de personnel inscrit pour cette compétence'></i>";
                    } else {
                        $pic = "<i class='fa fa-exclamation-circle fa-lg' style='color:$widget_fgred;'  title='Pas assez de personnel inscrit pour cette compétence'></i>";
                    }
                } else {
                    $inscrits = '';
                    $pic = '';
                }
                $html .= "<tr><td><div style='margin-left:5px;'> ".$type.' - '.$desc.'</div></td>';
                if ($evenement > 0) {
                    $html .= '<td align=center><b>'.$inscrits.'</b> '.$pic.'</td>';
                }
                $html .= "<td align=center>
                <input name='P".$poste."' class='form-control form-control-sm' style='width:auto; text-align:center' type=text title='Nombre requis' size='1' maxlength='3' value='$nb'
...
if ($competences == 1) {
    ...
    while (custom_fetch_array($resultm)) {
        if ($prevEQ != $EQ_ID) {
            $html .= "<option class='categorie' value='".$EQ_ID."'>".$EQ_NOM."</option>\n";
            $prevEQ = $EQ_ID;
        }
        $html .= "<option value='".$PS_ID."'>".$TYPE.' - '.$DESCRIPTION.'</option>';
    }
    $html .= '</select></td>';
    $html .= "<td align=center>
        <input name='new_nb'
            class='form-control form-control-sm' style='width:auto; text-align:center'
            type=text  
            title='Nombre requis' 
            size=1 
            value='1' 
...
if (isset($_POST['global'])) {
    if ($evenement > 0) {
        ...
        echo "<body onload=\"redirect_garde('".$garde."');\">";
    }
    exit;
} else {
    ...
    if ($evenement > 0) {
        $html .= " <input type=button class='btn btn-secondary' value='Retour' onclick=\"redirect('".$evenement."');\">";
    }
    if ($garde > 0) {
        $html .= " <input type=button class='btn btn-secondary' value='Retour' onclick=\"redirect_garde('".$garde."');\">";
    }
    $html .= '</form></div>';
    echo $html;
}
