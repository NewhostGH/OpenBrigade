...
if (isset($_GET['saved'])) {
    ...
    if ($errcode == 'nothing') {
        $html .= "<div class='alert alert-info' role='alert'> Aucun changement à sauver.</div></div><p>";
    } elseif ($errcode == 0) {
        $html .= "<div class='alert alert-success' role='alert'> Paramètres de configuration sauvés.</div></div><p>";
    } else {
        $html .= "<div class='alert alert-danger' role='alert'> Erreur lors de la sauvegarde des paramètres de configuration.</div></div><p>";
    }
}
...
function fillTableau($query)
{
    ...
    while ($row = @mysqli_fetch_array($result)) {
        ...
        $YESNO = $row['YESNO'];
        $IS_FILE = $row['IS_FILE'];
        if ($NAME == 'disponibilites' and $VALUE == '1') {
            $current_dispos = 1;
        }
        if ($NAME == 'sms_provider' and intval($VALUE) > 0) {
            $current_sms = intval($VALUE);
        }
        if ($NAME == 'geolocalize_enabled' and intval($VALUE) > 0) {
            $current_geolocalize_enabled = intval($VALUE);
        }
        if ($NAME == 'import_api' and intval($VALUE) > 0) {
            $current_import_api = intval($VALUE);
        }
        $DESCRIPTION = $row['DESCRIPTION'];

        if ($current_dispos == 0 and $ID == 47) {
            $style = "style='display:none'";
            $i++;
        }
        if ($current_geolocalize_enabled == 0 and ($ID == 57 or $ID == 60)) {
            $style = "style='display:none'";
        }
        if ($current_import_api == 0 and ($ID == 65 or $ID == 66)) {
            $style = "style='display:none'";
        }
        if ($current_sms == 0 and ($ID == 10 or $ID == 11 or $ID == 12)) {
            $style = "style='display:none'";
            $i++;
        }
        if (($current_sms == 1 or $current_sms == 2) and $ID == 12) {
            $style = "style='display:none'";
            $i++;
        }
        if (($current_sms == 3 or $current_sms == 4) and $ID == 4) {
            $style = "style='display:none'";
            $i++;
        }
        ...
        if ($HIDDEN == 0) {
            ...
            if ($ID == 9) {
                $H .= " <a href='".$wikiurl."/SMS' target=_blank><i class='fa fa-question-circle fa-lg' title='Information sur la configuration des comptes SMS'></a></td>";
            } else {
                $H .= '</td>';
            }
            ...
            if ($ID == 1) {
                ...
                if ($nbs > 1) {
                    $H .= 'Non';
                } elseif (! $debug and $VALUE == 1) {
                    $H .= 'Oui';
                } elseif (! $debug and $VALUE == 0) {
                    $H .= 'Non';
                } else {
                    if ($VALUE == '1') {
                        $checked = 'checked';
                ...
                foreach ([0, 30, 60, 90, 150, 250, 365, 600, 1000] as $k) {
                    if ($VALUE == $k) {
                        $selected = 'selected';
                    } else {
                        $selected = '';
...
if ($tab == 'conf7') {
    ...
    while ($row = @mysqli_fetch_array($result)) {
        if ($row['P_NOM'] != '') {
            $who = "<a href='upd_personnel.php?pompier=".$row['VH_BY']."' title='Voir la fiche'>".my_ucfirst($row['P_PRENOM']).' '.strtoupper($row['P_NOM']).'<a>';
        } else {
            $who = '';
        }
        $html .= '<tr>
