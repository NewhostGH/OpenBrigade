...
function get_sms_config($section)
{
    ...
    return [$SMS_SECTION_ACCOUNT, $SMS_LOCAL_PROVIDER, $SMS_LOCAL_USER, $SMS_LOCAL_PASSWORD, $SMS_SPECIFIC, $SMS_URL, $SMS_PROVIDER_NAME];
}
...
function get_sms_credits($section)
{
    ...
    if ($provider == 5) {
        ...
        if ($conn[0] == 'KO') {
            ...
            $credits = getSMSCredit_3("$conn[1]");
        }
    } elseif ($provider == 4) {
        if ($api != '' and $passwd != '') {
            $credits = getSMSCredit_4($api);
        } else {
            $credits = 'ERREUR';
        }
    } elseif ($provider == 6) {
        if ($api != '' and $passwd != '') {
            $credits = getSMSCredit_6($api);
        } else {
            $credits = 'ERREUR';
        }
    } elseif ($provider == 7) {
        if ($passwd != '' and $api != '') {
            $credits = getSMSCredit_7($passwd, $api);
        } else {
            $credits = 'ERREUR';
        }
    } elseif ($provider == 8) {
        if ($passwd != '' and $api != '' and $user != '') {
            $credits = getSMSCredit_8($api);
        } else {
            $credits = 'ERREUR';
        }
    }
...
function show_sms_account_balance($section, $credits)
{
    ...
    if ($SMS_CONFIG[1] > 0) {
        ...
        } elseif ($SMS_CONFIG[1] == 4) {
            echo ' SMS Gateway - <b> '.$credits.'</b>';
        } elseif ($SMS_CONFIG[1] == 8) {
            echo ' SMSEagle - <b> '.$credits.'</b>';
        } elseif ($SMS_CONFIG[1] == 5) {
            echo ' SMSMode - <b> '.$credits.'</b>';
        } else {
...
{
    if (! test_with_curl($url)) {
        return 'ERREUR1';
    }
    $buffer = @file_get_contents('http://'.$url);
    if (strpos($buffer, 'Welcome to') !== false) {
...
function getSMSCredit_8($url)
{
    ...
    $buffer = @file_get_contents('http://'.$url);
    if (strpos($buffer, 'SMSEagle') !== false) {
        return 'Solde illimité';
    } else {
        return 'ERREUR2';
    }
...
function sendSMS_7($to, $message, $token, $deviceID)
{
    ...
    if (isset($result['response'][0]['status'])) {
        if (in_array($result['response'][0]['status'], ['success', 'pending'])) {
            return 1;
        }
    }
...
function sendSMS_5($to, $message, $from, $user, $password)
{
    ...
    $path = 'https://api.smsmode.com/http/1.6/sendSMS.do';
    $request = '?pseudo='.urlencode($user);
    $request .= '&pass='.urlencode($password);
...
function send_sms($id, $dest, $message, $section, $create_page)
{
    ...
    if ($provider == 2) {
        ...
        $description = ['OK' => 'Message envoyé avec succès',
    ...
    if ($provider == 5) {
        /* Messages correspondants aux différents retours possibles de l'API */
        $description = ['0' => 'Accepté - le message a été accepté par le système et est en cours de traitement',
            '31' => 'Erreur interne',
