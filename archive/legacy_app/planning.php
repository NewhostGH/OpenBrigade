...
$possibleorders = ['G_LEVEL', 'P_STATUT', 'P_NOM', 'P_SECTION'];
...
for ($i = 1; $i <= $nbjoursdumois; $i++) {
    ...
    if ($day_planning == $i) {
        $d = "<span class='badge badge-pill badge3' title='Seules les personnes ayant une activité ce jour sont prises en compte, cliquer pour désactiver ce filtre'>
                    <a href=calendar.php?tab=2&day_planning=0 style='color:white;'>".$i.'</a></span>';
    } else {
        $d = "<a title='cliquer ici pour filtrer sur les personnes participant à une activité ce jour' 
...
while (custom_fetch_array($result)) {
    ...
    for ($i = 1; $i <= $nbjoursdumois; $i++) {
        [$status, $title, $style] = get_status($P_ID, $year, $month, $i);
