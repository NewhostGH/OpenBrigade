...
if ($syndicate == 1) {
    $query .= ' and sf.NIV in(1,3)';
} else {
    $query .= ' and sf.NIV in(0,1,3)';
}
...
while (custom_fetch_array($result)) {
    if (isset($EFFECTIFS[$S_ID])) {
        $EFFECTIFS[$S_ID] = $EFFECTIFS[$S_ID] + $NB;
    } else {
        $EFFECTIFS[$S_ID] = $NB;
    }
    $NBADH[$S_ID][$PROF] = $NB;
...
while (custom_fetch_array($result)) {
    ...
    foreach ($TP as $TP_ID => $TP_DESCRIPTION) {
        if (isset($COTIS[$PROF][$TP_ID])) {
            $VAL = floatval($COTIS[$PROF][$TP_ID]);
        } else {
            $VAL = 0;
        }
        $SUBTOTAL += $VAL;
