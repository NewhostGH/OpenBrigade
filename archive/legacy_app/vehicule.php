...
if ($tab == 1) {
    ...
    $possibleorders = ['TV_CODE', 'V_IMMATRICULATION', 'V_INDICATIF', 'V_MODELE', 'V_COMMENT', 'VP_OPERATIONNEL',
    ...
    $possibleorders = ['evenement', 'vehicule', 'dtdb', 'statut'];
    if (! in_array($order, $possibleorders) or $order == '') {
        $order = 'evenement';
    }
}
...
if ($tab == 1) {
    ...
    $querycnt .= $queryadd;
    $query .= $queryadd." \norder by ".$order;
    if ($order == 'VP_OPERATIONNEL') {
        $query .= ',VP_LIBELLE';
    }
    if ($order == 'TV_USAGE' || $order == 'V_FLAG1' || $order == 'V_FLAG2' || $order == 'V_FLAG3' || $order == 'V_FLAG4'
    || $order == 'AFFECTED_TO' || $order == 'V_EXTERNE' || $order == 'V_ANNEE' || $order == 'V_KM') {
        $query .= ' desc';
    }
    ...
    while ($row = @mysqli_fetch_array($result2)) {
        ...
        if ($small_device) {
            $TV_LIBELLE = substr($TV_LIBELLE, 0, 45);
        }
...
if ($tab == 2) {
    ...
    if ($number > 0) {
        ...
        while (custom_fetch_array($result)) {
            ...
            } elseif ($assur_perim or $ct_perim) {
                $fgcolor = $widget_fgred;
                $bgcolor = $widget_bgred;
