...
if (isset($_GET['data'])) {
    ...
    while (custom_fetch_array($result)) {
        ...
        } elseif (my_date_diff(getnow(), $MA_REV_DATE1) < 0) {
        ...
        // Icones
        if ($TM_LOT == 1) {
            $img1 = "<i class='fa fa-check' title='Lot de matériel'></i>";
        } else {
            $img1 = '';
        }

        if ($MA_EXTERNE == 1) {
            $img2 = "<i class='fa fa-check' title='matériel mis à disposition par $cisname'></i>";
        } else {
            $img2 = '';
        }
        ...
        if ($AFFECTED_TO != '') {
            ...
            if ($P_OLD_MEMBER > 0) {
                $owner = "<span style='color:black;' title='ancien membre'><b>".$owner.'</b><span>';
            }
        } else {
            $owner = '';
        }
        ...
        // Dans Véhicule/lot
        if (intval($V_ID) > 0) {
            $contenu_dans = '<a href=upd_vehicule.php?vid='.$V_ID." title='$TV_CODE $V_MODELE $V_INDICATIF $V_IMMATRICULATION'>".$V_MODELE.' '.$V_INDICATIF.'</a>';
        } elseif ($MA_PARENT != '') {
