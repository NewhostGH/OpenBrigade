...
if (isset($_POST['operation'])) {
    $operation = secure_input($dbc, $_POST['operation']);
}
...
if ($operation == 'update' || $operation == 'insert') {
    ...
    if ($operation == 'insert') {
        verify_csrf('insert_personnel');
    }

    $query = 'select c.CT_ID, c.CONTACT_TYPE, c.CT_ICON, p.CONTACT_VALUE, p.CONTACT_DATE 
            from contact_type c left join personnel_contact p on(p.CT_ID=c.CT_ID and p.P_ID='.$P_ID.')';
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        if (isset($_POST[$c])) {
            ...
            if ($value == '' and $CONTACT_VALUE != '') {
                $query2 = 'delete from personnel_contact where P_ID='.$P_ID.' and CT_ID='.$CT_ID;
            } elseif ($CONTACT_VALUE == '' and $value != '') {
                $query2 = 'insert into personnel_contact (CT_ID,P_ID,CONTACT_VALUE,CONTACT_DATE) values ('.$CT_ID.','.$P_ID.',"'.$value.'",NOW())';
            } elseif ($CONTACT_VALUE != $value and $value != '') {
                $query2 = 'update personnel_contact set CONTACT_VALUE="'.$value.'",CONTACT_DATE=NOW() where P_ID='.$P_ID.' and CT_ID='.$CT_ID;
            }
            if ($query2 != '') {
                $result2 = mysqli_query($dbc, $query2);
                insert_log('UPDP12', $P_ID, $complement = $CONTACT_TYPE.': '.$CONTACT_VALUE.' -> '.$value);
            }
        }
    }
}
...
if ($operation == 'document' or $operation == 'update') {
    if ($id != $P_ID) {
        if (get_statut($P_ID) == 'EXT') {
            check_all(37);
            if (! check_rights($id, 37, "$section")) {
                check_all(24);
            }
        } else {
            check_all(2);
            if (! check_rights($id, 2, "$section")) {
                check_all(24);
            }
        }
    }
...
if ($operation == 'document') {
    ...
    if (isset($_FILES['userfile'])) {
        for ($i = 0; $i < count($_FILES['userfile']['name']); $i++) {
            ...
            if ($error > 0) {
                ...
                $result = mysqli_query($dbc, $query);
                insert_log('ADDOC', $P_ID, "$file_name", $P_ID);
            }
        }
        ...
        $query = 'update document set DS_ID='.$security.' where P_ID='.$P_ID.'  and D_NAME="'.$doc.'"';
        $result = mysqli_query($dbc, $query);
        insert_log('SECDOC', $P_ID, "$doc - $security", $P_ID);
    ...
    $date_suspendu = 'null';
    if (isset($_POST['date_suspendu'])) {
        $date_suspendu = secure_input($dbc, $_POST['date_suspendu']);
    }
    if ($date_suspendu == '') {
        $date_suspendu = 'null';
    }
    ...
    if (isset($_POST['date_fin_suspendu'])) {
        $date_fin_suspendu = secure_input($dbc, $_POST['date_fin_suspendu']);
    }
    ...
    $date_npai = 'null';
    if (isset($_POST['date_npai'])) {
        $date_npai = secure_input($dbc, $_POST['date_npai']);
    }
    if ($date_npai == '') {
        $date_npai = 'null';
    }
    ...
    $fin_f = 'null';
    if (isset($_POST['fin'])) {
        $fin_f = secure_input($dbc, $_POST['fin']);
    }
    if ($fin_f == '') {
        $fin_f = 'null';
    }
    ...
    if (isset($_POST['licnum'])) {
        $licnum = str_replace('"', '', $_POST['licnum']);
    } else {
        $licnum = '';
    }
    ...
    if (isset($_POST['birth'])) {
        $birth = secure_input($dbc, $_POST['birth']);
    }
    $birth1 = $birth;
    if ($birth == '') {
        $birth = 'null';
    ...
    if (isset($_POST['motif_radiation'])) {
        $motif_radiation = str_replace('"', '', $_POST['motif_radiation']);
    } else {
        $motif_radiation = '';
    }
    ...
    if (isset($_POST['grade'])) {
        $grade = secure_input($dbc, $_POST['grade']);
    } else {
        $grade = '-';
    }
    if ($grade == '') {
    ...
    if (isset($_POST['profession'])) {
        $profession = secure_input($dbc, $_POST['profession']);
    } else {
        $profession = 'SPP';
    }
    if (isset($_POST['statut'])) {
        $statut = secure_input($dbc, $_POST['statut']);
    } else {
        $statut = 'SPV';
    }
    if (isset($_POST['regime_travail'])) {
        $regime_travail = secure_input($dbc, $_POST['regime_travail']);
    } else {
        $regime_travail = '';
    }
    ...
    if (isset($_POST['prenom2'])) {
        $prenom2 = str_replace('"', '', $_POST['prenom2']);
    } else {
        $prenom2 = '';
    }
    if (isset($_POST['nom'])) {
    ...
    if (isset($_POST['nom_naissance'])) {
        $nom_naissance = str_replace('"', '', $_POST['nom_naissance']);
    } else {
        $nom_naissance = '';
    }
    ...
    if (isset($_POST['birthdep'])) {
        $birthdep = secure_input($dbc, $_POST['birthdep']);
    } else {
        $birthdep = '';
    }
    if (isset($_POST['abbrege'])) {
        $abbrege = str_replace('"', '', $_POST['abbrege']);
    } else {
        $abbrege = '';
    }
    if (isset($_POST['relation_nom'])) {
        $relation_nom = secure_input($dbc, $_POST['relation_nom']);
    } else {
        $relation_nom = '';
    }
    if (isset($_POST['relation_prenom'])) {
        $relation_prenom = secure_input($dbc, $_POST['relation_prenom']);
    } else {
        $relation_prenom = '';
    }
    if (isset($_POST['relation_phone'])) {
        $relation_phone = secure_input($dbc, $_POST['relation_phone']);
    } else {
        $relation_phone = '';
    }
    if (isset($_POST['relation_email'])) {
        $relation_email = secure_input($dbc, $_POST['relation_email']);
    } else {
        $relation_email = '';
    }
    if (isset($_POST['type_salarie'])) {
        $type_salarie = secure_input($dbc, $_POST['type_salarie']);
    } else {
        $type_salarie = '';
    }
    if (isset($_POST['matricule'])) {
        $matricule = secure_input($dbc, ($_POST['matricule']));
    } else {
        $matricule = '-1';
    }
    if (isset($_POST['pays'])) {
    ...
    if (isset($_POST['abbrege'])) {
        $abbrege = secure_input($dbc, $_POST['abbrege']);
    } else {
        $abbrege = '';
    }

    if (isset($_POST['civilite'])) {
        $civilite = intval($_POST['civilite']);
    } else {
        $civilite = 0;
    }
    if ($civilite == 0) {
    ...
    $insurl .= "&phone2=$phone2&service=$service&address=$address&hide=$hide&birthplace=$birthplace&birthdep=$birthdep";
    ...
    if ($operation == 'update' and $nom != '' and $activite == 0) {
        ...
        $query = ' select count(1) as NB from pompier
             where P_SECTION in ('.rtrim($hissection.','.$list, ',').')
             and P_NOM="'.$nom.'"
             and P_PRENOM="'.$prenom.'"
    ...
    if ($operation == 'update') {
        ...
        if ($OLD_GROUPE != $habilitation or $OLD_GROUPE2 != $habilitation2) {
            if ((! check_rights($id, 25)) or $habilitation == 4 or $habilitation2 == 4) {
                @check_all(9);
            }
            ...
            while ($row_s = @mysqli_fetch_array($rs)) {
                ...
                if ($row_s['F_TYPE'] == 3 and $row_s['GP_USAGE'] == 'externes') {
                    check_all(46);
                }
            }
        }
        // cas ancien membre
        if ($PREVIOUSPOLDMEMBER == 0 and $activite > 0) {
            $habilitation = -1;
        }
        if ($PREVIOUSPOLDMEMBER > 0 and $activite == 0 and $statut != 'EXT') {
            $habilitation = 0;
        }
        ...
        if ($syndicate == 1 and (! check_rights($id, 1))) {
            $very_big_permission = false;
        } else {
            $very_big_permission = $big_permission;
        }
        ...
        if ($small_permission) {
            if ($grades and $syndicate == 1) {
                $m = $m + update_field_personnel($P_ID, 'P_GRADE', "$grade", "$OLD_GRADE", 'UPDP16');
            }
            if ($syndicate == 0) {
                $m = $m + update_field_personnel($P_ID, 'P_CODE', "$matricule", "$OLD_MATRICULE", 'UPDP5');
            }
        ...
        if ($big_permission) {
            ...
            if ($very_big_permission) {
                if ($syndicate == 1) {
                    $m = $m + update_field_personnel($P_ID, 'P_PROFESSION', "$profession", "$OLD_PROFESSION", 'UPDP17');
                }
            ...
            if (check_rights($id, 2, "$OLD_S_PARENT")) {
                $flag_allowed = true;
            } else {
                $flag_allowed = false;
            }
        ...
        if ($OLD_STATUT == 'EXT' and $statut != 'EXT' and $activite == 0) {
            ...
            $query = 'update pompier set P_MDP="'.$hash.'", P_MDP_EXPIRY=NOW() where P_ID='.$P_ID;
            $result = mysqli_query($dbc, $query);
            ...
            if ($email != '') {
                ...
                if ($nb == 1) {
                    ...
                    write_msgbox('OK', $star_pic, ucfirst($prenom).' '.strtoupper($nom)." peut maintenant se connecter.<br>Merci de lui communiquer ces infos:<p>identifiant: <b>$matricule</b><br>mot de passe: <b>$mypass</b><p align=center><a href=upd_personnel.php?from=created&pompier=$P_ID ><input type='submit' class='btn btn-secondary' value='Retour'></a>", 30, 0);
                    exit;
                }
            } else {
                write_msgbox('OK', $star_pic, ucfirst($prenom).' '.strtoupper($nom)." peut maintenant se connecter.<br>Merci de lui communiquer ces infos:<p>identifiant: <b>$matricule</b><br>mot de passe: <b>$mypass</b><p align=center><a href=upd_personnel.php?from=created&pompier=$P_ID ><input type='submit' class='btn btn-secondary' value='Retour'></a>", 30, 0);
        ...
        while (custom_fetch_array($result1)) {
            if (($id == $P_ID and $CF_USER_MODIFIABLE == 1 and $CF_USER_VISIBLE == 1) or check_rights($id, 2)) {
                ...
                if (isset($_POST['custom_'.$CF_ID])) {
                    if ($CF_TYPE == 'checkbox') {
                        $value = intval($_POST['custom_'.$CF_ID]);
                    } else {
    ...
    if ($operation == 'insert') {
        if ($statut == 'EXT') {
            check_all(37);
            if (! check_rights($id, 37, "$hissection")) {
                check_all(24);
            }
        } else {
            check_all(1);
            if (! check_rights($id, 1, "$hissection")) {
                check_all(24);
            }
        }
        if ($habilitation > 0) {
        ...
        if ($statut == 'EXT') {
            $matricule = generate_identifiant($prenom, $nom, $birthdep);
        }
        ...
        $query = 'insert into pompier 
                (P_CODE,P_PRENOM,P_PRENOM2,P_NOM,P_NOM_NAISSANCE,P_SEXE,P_CIVILITE,P_GRADE,P_PROFESSION,P_STATUT,P_REGIME, P_MDP, P_DATE_ENGAGEMENT, P_BIRTHDATE, 
                 ...
                 P_RELATION_NOM,P_RELATION_PRENOM,P_RELATION_PHONE,P_RELATION_MAIL, P_HIDE,TS_CODE,
                 P_CREATE_DATE, TP_ID, P_PAYS, P_FAVORITE_SECTION)
           values ("'.$matricule.'",LOWER("'.$prenom.'"),LOWER("'.$prenom2.'"),LOWER("'.$nom.'"),LOWER("'.$nom_naissance."\"),'".$sexe."','".$civilite."','".$grade."','".$profession."',
                   ...
                   "'.$relation_nom.'","'.$relation_prenom.'","'.$relation_phone.'","'.$relation_email."\",'".$hide."','".$type_salarie."',
                   CURDATE(), ".$type_paiement.', '.$pays.','.$hissection.')';
        $result = mysqli_query($dbc, $query);
        $m = 1;
        ...
        if ($statut == 'EXT') {
            ...
            write_msgbox('OK', $star_pic, $texte.'<p align=center><a href='.$url." ><input type='submit' class='btn btn-secondary' value='Retour'></a>",30,0);
            exit;
        } else {
