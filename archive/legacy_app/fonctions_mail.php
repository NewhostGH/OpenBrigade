...
function mysendmail($ids, $fromid, $Subject, $Mailcontent, $Attachment = 'None')
{
    ...
    if (intval($mail_allowed) == 0) {
        return 0;
    } elseif (isset($_SERVER['HTTP_HOST'])) {
        if ($_SERVER['HTTP_HOST'] == '127.0.0.1' or $_SERVER['HTTP_HOST'] == 'localhost' or $_SERVER['HTTP_HOST'] == 'ebrigade') {
            return 0;
        }
    }

    if (intval($donotreply) == 0) {
        $SenderMail = get_email($fromid);
    } else {
        $SenderMail = 'donotreply';
        if (isset($_SERVER['HTTP_HOST'])) {
            $SenderMail .= '@'.str_replace('www.', '', $_SERVER['HTTP_HOST']);
        } elseif (isset($_ENV['HTTP_HOST'])) {
            $SenderMail .= '@'.str_replace('www.', '', $_ENV['HTTP_HOST']);
        }
    }
    ...
    if ($cisurl != 'ebrigade.fr') {
        if (substr($cisurl, 0, 4) == 'http') {
            $Mailcontent .= '<p><font face=arial>'.$cisname.' - '.$cisurl;
        } else {
            $Mailcontent .= '<p><font face=arial>'.$cisname.' - http://'.$cisurl;
        }
    }
    $MailTo = '';
    $ids = str_replace(',,,,', ',', $ids);
...
function mysendmail2($MailTo, $Subject, $Mailcontent, $SenderName, $SenderMail, $Attachment = 'None')
{
    ...
    if ($cisurl != 'ebrigade.fr') {
        if (substr($cisurl, 0, 4) == 'http') {
            $Mailcontent .= '<p>'.$cisname.' - '.$cisurl;
        } else {
            $Mailcontent .= '<p>'.$cisname.' - http://'.$cisurl;
        }
    }
    require_once 'lib/PHPMailer/class.phpmailer.php';
    $mail = new PHPMailer;
...
function clean_mail_data($string)
{
    ...
    $s = str_replace('%u0024', 'dollars', $s);
    $s = str_replace('%u00A3','livres',$s);
    $s = str_replace('%u00A5','yens',$s);
