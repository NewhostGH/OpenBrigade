...
$possibleorders = ['S_CODE', 'S_DESCRIPTION', 'S_PARENT', 'NIV', 'SECTION_PARENT', 'S_ID_RADIO', 'S_AFFILIATION'];
if (! in_array($order, $possibleorders) or $order == '') {
    $order = 'S_CODE';
}

if ($order == 'S_AFFILIATION' or $order == 'S_ID_RADIO') {
    $order .= ' desc';
}
$disabled = 'disabled';
...
if ($nbsections == 0) {
    ...
    for ($i = 1; $i < $nbmaxlevels; $i++) {
        ...
        $T = $levels[$i];
        if (substr($levels[$i], -1) != 's') {
            $T .= 's';
        }
        if ($i == 0 or $i >= $filterniv) {
            echo "<option value='".$i."' $selected>les ".$T.' seulement</option>';
        }
    }
    echo '</select>';
    echo '</div>';
