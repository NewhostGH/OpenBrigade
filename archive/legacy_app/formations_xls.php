...
if ($id == $pompier) {
    $allowed = true;
} elseif ($mycompany == get_company($pompier) and check_rights($_SESSION['id'], 45) and $mycompany > 0) {
    $allowed = true;
} else {
    check_all(40);
}
...
$columns_title = ['Type', 'date', 'Type de formation', 'N°diplôme', 'Lieu', 'Délivré par', 'Commentaire'];
...
while ($row = @mysqli_fetch_array($result)) {
    ...
    $columns_data = [$TYPE, $PF_DATE, $TF_LIBELLE, $PF_DIPLOME, $PF_LIEU, $PF_RESPONSABLE, $PF_COMMENT];
    foreach ($columns as $c => $letter) {
