...
if (isset($_POST['statistiques'])) {
    $statistiques = intval($_POST['statistiques']);
} elseif ($save == 0) {
    $statistiques = 1;
} else {
    $statistiques = 0;
}
if (isset($_POST['show_vehicules'])) {
    $show_vehicules = intval($_POST['show_vehicules']);
} elseif ($save == 0) {
    $show_vehicules = 1;
} else {
    $show_vehicules = 0;
}
if (isset($_POST['show_materiel'])) {
    $show_materiel = intval($_POST['show_materiel']);
} elseif ($save == 0) {
    $show_materiel = 1;
} else {
    $show_materiel = 0;
}
if (isset($_POST['show_cav'])) {
    $show_cav = intval($_POST['show_cav']);
} elseif ($save == 0) {
    $show_cav = 1;
} else {
    $show_cav = 0;
    ...
    while (custom_fetch_array($result)) {
        if ($TEL_CODE == 'I') {
            if ($EL_IMPORTANT == 1) {
                ...
                $img = "class='fa fa-medkit' title='intervention'";
            }
        } else {
            if ($EL_IMPORTANT == 1) {
                $img = "class='far fa-file-text' style='color:red' title='message important'";
            } else {
                $img = "class='far fa-file-text' title='message'";
            }
        }
        if ($EL_IMPRIMER == 1) {
            $checked = 'checked';
