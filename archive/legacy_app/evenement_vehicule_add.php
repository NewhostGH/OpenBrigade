...
if (isset($_GET['V_ID'])) {
    $vehicule = intval($_GET['V_ID']);
} else {
    $vehicule = intval($_POST['V_ID']);
}
if (isset($_POST['km'])) {
    $km = intval($_POST['km']);
} else {
...
if (isset($_GET['filtervehicule'])) {
    $filtervehicule = $_GET['filtervehicule'];
} else {
    $filtervehicule = 'ALL';
}
...
if (! is_chef_evenement($id, $evenement)) {
    ...
    if (intval($E_OPEN_TO_EXT) == 0 and ! check_rights($id, 17, "$S_ID") and ! check_rights($id, 15, "$S_ID")) {
        check_all(24);
    }
}
...
if ($from == 'evenement') {
    ...
    echo "<body onload=redirect('vehicule.php?tab=2&vehicule=".$filtervehicule.'&date='.$date.'&order='.$order."');>";
}
?>
