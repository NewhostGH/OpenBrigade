...
class PDFEB extends FPDI
{
    public function Header()
    {
        ...
        if (! isset($no_header)) {
            ...
            if (file_exists($attestation_fiscale_pdf) and isset($attestation_fiscale)) {
                $fondpdf = $attestation_fiscale_pdf;
            } elseif (file_exists($asa_pdf) and isset($asa)) {
                $fondpdf = $asa_pdf;
            } elseif (file_exists($carte_adherent_pdf) and isset($carte_adherent)) {
                $fondpdf = $carte_adherent_pdf;
            } elseif (file_exists($attestation_gqs) and isset($gqs)) {
                $fondpdf = $attestation_gqs;
            } elseif (file_exists($customlocal) && $cursection['PDF_PAGE'] != '') {
                $fondpdf = $customlocal;
            } elseif (file_exists($customdefault)) {
                $fondpdf = $customdefault;
            } else {
                $fondpdf = $generic;
            }
    ...
    public function Footer()
    {
        ...
        $customdefault = $basedir.'/images/user-specific/pdf_page.pdf';
        $generic = $basedir.'/lib/fpdf/pdf_page.pdf';
        if (file_exists($customlocal) && $cursection['PDF_PAGE'] != '') {
            $fondpdf = $customlocal;
        } elseif (file_exists($customdefault)) {
            $fondpdf = $customdefault;
        } else {
    ...
    public function FicheSection($id = 0)
    {
        global $cisname;
        global $dbc;
        $section = [];
    ...
    public function FormatCommentaire($string)
    {
        $out = str_replace("\r\n", "\n", $string);
        $out = str_replace(".\n", '. ', $out);
        $out = str_replace("\n", ', ', $out);
    ...
    public function WriteBigTextBlock($text, $x, $y)
    {
        $mrl = 95; // max row length (compte rendu de réunion)
        $this->SetXY($x, $y);
        $lines = explode("\n", $text);
        $nblines = count($lines);
        if ($y < 265 and ($y + $nblines * 4.5 > 265)) {
            $nb1 = (265 - $y) / 5;
            $text = implode("\n", array_slice($lines, 0, $nb1));
            $nblines = $nblines - $nb1;
            $this->MultiCell(170, 4, $text, '', 'L');
            $this->AddPage();
            $y = 50;
            $this->SetXY($x, $y);
            $text = implode("\n", array_slice($lines, $nb1));
            $this->MultiCell(170, 4, $text, '', 'L');
        } else {
            $this->MultiCell(170, 4, $text, '', 'L');
        }
        // calculate new $y
        $estimated_num_rows = strlen($text) / $mrl;
        if ($nblines < $estimated_num_rows) {
            $nblines = $estimated_num_rows;
        }
        $y = $y + $nblines * 4.5 + 6;
