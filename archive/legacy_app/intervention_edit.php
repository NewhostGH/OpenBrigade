...
if (isset($_GET['modeinter'])) {
    $modeinter = $_GET['modeinter'];
} elseif (isset($_POST['modeinter'])) {
    $modeinter = $_POST['modeinter'];
} else {
    $modeinter = 'infos';
}
...
if ($numinter > 0) {
    ...
    if (check_rights($_SESSION['id'], 15, $S_ID)) {
        $granted_update = true;
    }
} elseif ($evenement > 0) {
    $S_ID = get_section_organisatrice($evenement);
...
if (isset($_POST['numinter']) and ($action == 'update' or $action == 'insert') and $granted_update) {
    ...
    $EL_RESPONSABLE = intval($_POST['responsable']);
    if ($EL_RESPONSABLE == 0) {
        $EL_RESPONSABLE = 'null';
    }
    $TEL_CODE = secure_input($dbc, $_POST['type']);
    if (isset($_POST['important'])) {
        $EL_IMPORTANT = intval($_POST['important']);
    } else {
        $EL_IMPORTANT = 0;
    }
    if (isset($_POST['imprimer'])) {
        $EL_IMPRIMER = intval($_POST['imprimer']);
    } else {
        $EL_IMPRIMER = 0;
    }
    ...
    if ($EL_TITLE == '' or $DATE_DEBUT == '' or $HEURE_DEBUT == '') {
        if ($EL_TITLE == '') {
            ...
            $msg = "L'heure de début doit être renseignée ";
        }
...
if ($modeinter == 'infos') {
    ...
    if ($TEL_CODE == 'I') {
        ...
        if ($TE_VICTIMES == 0) {
            $t = 'Elément de compte rendu de réunion';
        } else {
            $t = 'Message pour le rapport';
        }
        $img = 'file-text-o';
        $tit = 'Titre';
        $pdf = '';
    ...
    if ($TE_VICTIMES == 0) {
        echo "<td colspan=2><input type=hidden name='important' value='0'></td>";
    } else {
        if ($EL_IMPORTANT == 1) {
            $checked = 'checked';
        } else {
    ...
    if ($rowm['NB'] == 1 and $geolocalize_enabled == 1) {
        $map = '<a href=sitac.php?evenement='.$evenement."><i class='fa fa-map fa-lg' style='color:green;' title='Voir la carte Google Maps' border=0></i></a>";
    } else {
        $map = '';
    }
...
where ee.E_CODE='.$evenement.' order by ee.EE_ORDER ';
    ...
    if ($nb_equipes > 0) {
        ...
        while (custom_fetch_array($result2)) {
            ...
            if ($EE_ID_RADIO != '') {
                $radio = " <span class=small2 style='color:green' title=\"identifiant radio ".$EE_ID_RADIO.'">'.$EE_ID_RADIO.'</span> ';
            } else {
                $radio = '';
            }
    ...
    while ($row3 = @mysqli_fetch_array($result3)) {
        ...
        if (in_array($_eid, $equipes_engagees)) {
    ...
    if ($TEL_CODE == 'I' and $action != 'insert') {
        ...
        while (custom_fetch_array($result)) {
            ...
            if ($VI_DETRESSE_VITALE == 1) {
                $comments .= "<a title='Détresse vitale (Hémorragie, inconscience, ACR)' >détresse</a> ";
            }
            if ($VI_TRAUMATISME == 1) {
                $comments .= "<a title='Traumatisme' >Traumatisme</a> ";
            }
            if ($VI_INFORMATION == 1) {
                $comments .= "<a title='La personne a été assistée, ou des renseignements et informations lui ont été donnés' >assistée</a> ";
            }
            if ($VI_DECEDE == 1) {
                $comments .= "<a title='La victime est décédée' >décédé</a> ";
            }
            if ($VI_MALAISE == 1) {
                $comments .= "<a title='La victime eu un malaise avec ou sans perte de connaissance' >malaise</a> ";
            }
            if ($VI_SOINS == 1) {
                $comments .= "<a title=\"Des soins ont été réalisés par l'équipe de secouristes\" >soins</a> ";
            }
            if ($VI_MEDICALISE == 1) {
                $comments .= '<a title="La victime a été médicalisée" >médicalisée</a> ';
            }
            if ($VI_TRANSPORT == 1) {
                $comments .= '<a title="La victime a été transportée par '.$T_NAME.', destination: '.$D_NAME.'">transport</a> ';
            }
            if ($VI_VETEMENT == 1) {
                $comments .= '<a title="Des vêtements ou une couverture ont été offerts à la victime" >vêtements</a> ';
            }
            if ($VI_ALIMENTATION == 1) {
                $comments .= '<a title="Des aliments ou une boisson ont été offerts à la victime" >alimentation</a> ';
            }
            if ($VI_REFUS == 1) {
                $comments .= "<a title=\"La victime a refusé d'être prise en charge\" >refus</a> ";
            }
            if ($VI_IMPLIQUE == 1) {
                $comments .= '<a title="La personne est seulement impliquée, indemne" >impliqué</a> ';
            }
    ...
    if ($granted_update) {
        ...
        if ($numinter > 0) {
            echo " <input type='button' class='btn btn-warning' value='Supprimer' onclick=\"deleteIt('".$numinter."','".$TEL_CODE."');\">";
        }
    }
    if ($from == 'map') {
...
if ($modeinter == 'doc') {
    ...
    if ($NBdocs > 0 and $disabled == '') {
        ...
        if (! isset($_GET['order']) || (! in_array($_GET['order'], $possibleorders) or $_GET['order'] == '')) {
            $order = 'date';
        } else {
            $order = $_GET['order'];
        }
