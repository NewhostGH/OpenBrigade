...
$html_colors1 = ['#F7D4DC', '#E4C2AF', '#E0D4A3', '#DBE2A7', '#BCE2A7', '#A3E0C1', '#A3D9E0', '#A3CBE0', '#A3B0E0', '#D3A3E0', '#E0A3C3', '#D279A0', '#bb99ff', '#ffffcc', '#80aaff', '#00ccff', '#70dbdb', '#33ffd6', '#66ff66', '#ff8080', '#ecb3ff', '#e0e0d1', '#c6c6ec', '#99ffff'];
$html_colors2 = ['#EA8AA0', '#cc8b66', '#ccb866', '#b0bf40', '#8acc66', '#53c68c', '#40b0bf', '#4093bf', '#536cc6', '#a440bf', '#c6538e', '#ac396b', '#661aff', '#e6e600', '#3377ff', '#00a3cc', '#2eb8b8', '#00cca3', '#00cc00', '#ff3333', '#d24dff', '#adad85', '#6666cc', '#00e6e6'];
...
function print_backgroundColor($number = 0)
{
    global $html_colors1;
    if ($number == 1) {
        return 'backgroundColor: color(window.chartColors.blue).alpha(0.5).rgbString()';
    } elseif ($number == 2) {
        $colors = [$html_colors1[0], $html_colors1[5]];
    } else {
        $colors = array_merge($html_colors1, $html_colors1, $html_colors1, $html_colors1);
    }
...
function print_borderColor($number = 0)
{
    global $html_colors2;
    if ($number == 1) {
        return 'borderColor: window.chartColors.blue';
    } elseif ($number == 2) {
        $colors = [$html_colors2[0], $html_colors2[5]];
    } else {
        $colors = array_merge($html_colors2, $html_colors2, $html_colors2, $html_colors2);
    }

    return 'borderColor: '.implode_array($colors, $quotes = true);
}

function print_graphic($data, $labels, $type, $title, $maxwidth = 75, $one_color_only = 0)
{
...
function print_stacked_bar($labels, $datasets, $title, $type = 'bar')
{
    ...
    if ($labelX != '') {
        $scaleLabelX = ",scaleLabel: {display: true, labelString: '".$labelX."' }";
    } else {
        $scaleLabelX = '';
    }
    if ($labelY != '') {
        $scaleLabelY = ",scaleLabel: {display: true, labelString: '".$labelY."' }";
    } else {
        $scaleLabelY = '';
    }
    ...
    if ($type == 'bar') {
        echo "<canvas id='myChart' style='max-width:".$maxw."%;'></canvas>";
    } else {
        echo "<canvas id='myChart' style='max-width: ".$maxw.'%;'.$height."'></canvas>";
    }
    echo '<script>
...
function print_line_chart($dataset, $title, $label)
{
    $len = count($dataset) - 1;
    if ($len > 0) {
        echo "
        ...
        Type graphique
        <select id='typechart' onchange='change_chart_type();'>
            <option value='line'>Courbe</option>
            <option value='bar'>Histogramme</option>
        </select>";

        echo "<script>
            var myChartTitle='".$title."';
            var myChartLabel='".$label."';
            function generateData() {
                var dataset = [];\n";

        foreach ($dataset as $dt => $value) {
            $real_date = date_create($dt);
            $t = @date_timestamp_get($real_date) * 1000;
            if (intval($t > 0)) {
                echo '            dataset.push({t:'.$t.', y:'.$value."});\n";
            }
        }
        echo "            return dataset;
            }
...
function repo_connexion_heure_journee($section, $subsections)
{
    ...
    } elseif ($subsections == 0) {
        $query .= ' and p.P_SECTION = 0';
    }
...
function repo_nb_events($section, $dtdb, $dtfn, $category)
{
    ...
    if ($section == 0) {
        ...
        $condition = ' and e.S_ID in ('.get_family($section).','.$section.')';
    }
    ...
    if ($category == 'ALL') {
        ...
        $col2 = 'te.TE_LIBELLE';
        if ($category == 'C_SEC') {
            $title = 'Opérations secours';
        } else {
            $title = 'Autres activités opé.';
        }
    }
...
function repo_participations_par_jour($section, $subsections, $dtdb, $dtfn, $type = 'ALL')
{
    ...
    $query = "select eh.eh_date_debut Date, count(1) 'Nombre de participants'
    ...
    if ($type != 'ALL') {
        $query .= " and e.TE_CODE  = '".$type."'";
    }
    if ($section == 0 and $subsections == 0) {
        $query .= ' and p.P_SECTION = 0';
    } elseif ($section > 0) {
        $query .= ' and p.P_SECTION  in ('.$list.')';
    }
    ...
    $begin = new DateTime("$year1-$month1-$day1");
    $end = new DateTime("$year2-$month2-$day2");

    // initialize values tp zero for each date
...
function repo_evenements_par_jour($section, $subsections, $dtdb, $dtfn, $type = 'ALL')
{
    ...
    $query = "select eh.eh_date_debut Date, count(1) 'Nombre événements'
        ...
        and e.E_CANCELED = 0 and e.te_code <> 'MC'";
    if ($type != 'ALL') {
        $query .= " and e.TE_CODE  = '".$type."'";
    }
    if ($section == 0 and $subsections == 0) {
...
function repo_stats($section, $dtdb, $dtfn, $category)
{
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        for ($z = 1; $z < $k; $z++) {
            if (isset($libs[$z])) {
                $nb = intval($row2[$z]);
                if (! array_key_exists($libs[$z], $labeled_data)) {
                    $labeled_data[$libs[$z]] = $nb;
                } else {
                    $labeled_data[$libs[$z]] = $labeled_data[$libs[$z]] + $nb;
                }
            }
        }
    }
...
function repo_events_by_month($section, $subsections, $type, $year, $canceled)
{
    ...
    if ($canceled == 1) {
        $title = "Nombre d'événements annulés";
    } else {
        $title = "Nombre d'événements (non annulés)";
    }
...
function repo_cancelled($section, $subsections, $year, $type)
{
    global $dbc;
...
function repo_formations($section, $subsections, $year, $type, $competence = 'ALL', $tf = 'ALL')
{
    ...
    if ($subsections == 1 and $section > 0) {
        $query1 .= "\n and e.S_ID in (".$list.')';
    } elseif ($subsections == 0 and $section == 0) {
        $query1 .= "\n and e.S_ID =0";
    } elseif ($subsections == 0 and $section > 0) {
        $query1 .= "\n and e.S_ID =".$section;
    }
...
function repo_flux_members($section, $subsections, $year, $period = 'month', $cat_personnel = 'interne')
{
    ...
    if ($cat_personnel == 'interne' and $syndicate == 1) {
        $title = "Evolution du nombre d'adhérents";
    } elseif ($cat_personnel == 'interne') {
        $title = 'Flux de personnel';
    } else {
        $title = 'Personnel externe ajouté';
    }

    if ($period == 'year') {
        $title .= ' (annuel)';
    } else {
        $title .= ' (par mois) en '.$year;
    }
...
function repo_nombre_personnes_par_mois($section, $subsections, $year)
{
    global $dbc, $html_colors1,$html_colors2, $syndicate,$cisname, $months;
    ...
    for ($m = 1; $m <= 12; $m++) {
        $query = "select count(1) as NB 
            from pompier 
            where P_CREATE_DATE <= '".$year.'-'.$m."-01'
            and (P_FIN is null or P_FIN > '".$year.'-'.$m."-01' )
            and P_STATUT <> 'EXT'
            and P_SECTION in (".$list.')';
        $result = mysqli_query($dbc, $query);
        $row = mysqli_fetch_array($result);
    ...
    print_graphic($data, $months, 'bar', $title);
}
...
function repo_members_year($section, $subsections, $cat_personnel = 'INT')
{
    ...
    for ($Y = $min; $Y <= $max; $Y++) {
        $query = " select count(1) as NB from pompier
             where ( P_DATE_ENGAGEMENT is null or P_DATE_ENGAGEMENT <= '".$Y."-12-31' )
             and ( P_FIN is null or P_FIN > '".$Y."-12-31' )
             and P_SECTION in (".$list.') ';

        if ($cat_personnel == 'SAL') {
            $query .= " and P_STATUT = 'SAL'";
        } elseif ($cat_personnel == 'EXT') {
            $query .= " and P_STATUT = 'EXT'";
        } else {
            $query .= " and P_STATUT in ('BEN','SPV','SPP','RES','ACT','ADH')";
        }
    ...
    if ($cat_personnel == 'SAL') {
        $t = 'Nombre de salariés';
    } elseif ($cat_personnel == 'EXT') {
        $t = "Nombre d'externes";
    } elseif ($syndicate == 1) {
        $t = "Nombre d'adhérents";
    } elseif ($assoc == 1) {
        $t = 'Nombre de bénévoles';
    } elseif ($army == 1) {
        $t = 'Nombre de personnels';
    } else {
        $t = 'Nombre de Sapeurs pompiers';
    }
...
function repo_duree_engagement($section, $subsections)
{
    ...
    $query = "select round(avg(YEAR(NOW()) - YEAR(P_DATE_ENGAGEMENT)),2), count(1) 
    from pompier where P_OLD_MEMBER=0 and P_STATUT <> 'EXT' and P_SECTION in (".$list.')';
    $result = mysqli_query($dbc, $query);
    $row = mysqli_fetch_array($result);
    $data[] = round($row[0], 1);
    $labels[] = 'Actifs ('.$row[1].')';

    $query = "select round(avg(YEAR(P_FIN) - YEAR(P_DATE_ENGAGEMENT)),2), count(1) 
    from pompier where P_OLD_MEMBER > 0 and P_STATUT <> 'EXT' and P_SECTION in (".$list.')';
    $result = mysqli_query($dbc, $query);
    $row = mysqli_fetch_array($result);
    $data[] = round($row[0], 1);
...
function repo_duree_engagement_par_age($section, $subsections)
{
    global $dbc, $colors;
    if ($subsections == 1) {
        $list = get_family("$section");
    ...
    // The age groups
    $lower = [16, 25, 35, 45, 55, 65, 75];
    $upper = [24, 34, 44, 54, 64, 74, 99];
    ...
    for ($i = 0; $i < $nb_tranches; $i++) {
        $query = "select round(avg(YEAR(NOW()) - YEAR(P_DATE_ENGAGEMENT)),2), count(1) 
        from pompier where P_OLD_MEMBER=0 and P_STATUT <> 'EXT' and P_SECTION in (".$list.')
        and EXTRACT(YEAR FROM (FROM_DAYS(DATEDIFF(NOW(),P_BIRTHDATE))))+0 >='.$lower[$i].'
    ...
    while ($row1 = mysqli_fetch_array($result1)) {
        ...
        for ($m = 1; $m <= 12; $m++) {
            $query = "select round(sum(facture_montant),0) as CA 
                ...
                and eh.EH_DATE_DEBUT < DATE_ADD('".$year.'-'.$m."-01', INTERVAL 1 MONTH)
                and e.E_CODE = eh.E_CODE
...
function repo_age_stagiaires($section, $subsections, $year, $competence)
{
    ...
    // The age groups
    $lower = [0, 19, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75];
    $upper = [18, 24, 29, 34, 39, 44, 49, 54, 59, 64, 69, 74, 99];
    $nb_tranches = count($lower);
...
{
    global $dbc,$html_colors1,$html_colors2, $gardes, $pompiers;
...
{
    global $dbc,$html_colors1,$html_colors2, $nbsections, $mydarkcolor, $grades;
    ...
    if ($nb == 0) {
        echo '<p><b>Aucune personnel trouvé</b>';
    ...
    if (count($T) == 0) {
        echo '<p><b>Aucune disponibilité trouvée</b>';
    ...
    echo "<p><b>Total jours <span class='badge' style='background-color:#ffd480;color:".$mydarkcolor.";'>$allJ</span> nuits <span class='badge' style='background-color:#66a3ff;color:".$mydarkcolor.";'>$allN</span></b>";
...
function repo_bilan_participations($section, $year, $month, $mode_garde = 0, $groupJN = 0, $c1 = 1, $c2 = 1, $c3 = 1)
{
    global $dbc,$html_colors1,$html_colors2, $nbsections, $gardes,$mydarkcolor, $grades;
    ...
    $query_j = "select distinct P_GRADE, P_ID, P_NOM, P_PRENOM, P_SECTION, P_STATUT
            ...
            and P_NOM <> 'admin' ";
    if ($section != 0) {
        $query_j .= ' and P_SECTION in ('.get_family($section).')';
    }
    ...
    if ($mode_garde == 1) {
        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            if (! isset($$c)) {
                ...
                if ($gardes) {
                    $plus = " and TE_CODE not in ('GAR','MC')";
                } else {
                    $plus = " and TE_CODE <> 'MC'";
                }
                $$cla = " and e.TE_CODE in (select TE_CODE from type_evenement where CEV_CODE = '".$row['CEV_CODE']."' ".$plus.') ';
            }
            $i++;
        }
    }
    ...
    while ($row_j = @mysqli_fetch_array($result_j)) {
        ...
        $queryJ1 = 'select count(*) as NB, sum(ep.EP_DUREE) HOURS
               from evenement_participation ep, evenement e, evenement_horaire eh
                 ...
                 and YEAR(eh.EH_DATE_DEBUT) ='.$year;
        if ($mode_garde) {
            $queryJ1 .= ' and ep.EH_ID = 1';
        }
        if ($month != 100) {
            $queryJ1 .= ' and MONTH(eh.EH_DATE_DEBUT)='.$month.' ';
        }
        ...
        if ($mode_garde) {
            $queryJ1A = 'select count(*) as NB, sum(ep.EP_DUREE) HOURS
               from evenement_participation ep, evenement e, evenement_horaire eh
                 ...
                 and YEAR(eh.EH_DATE_DEBUT) ='.$year;
            if ($mode_garde) {
                $queryJ1A .= ' and ep.EH_ID = 1';
            }
            if ($month != 100) {
                $queryJ1A .= ' and MONTH(eh.EH_DATE_DEBUT)='.$month.' ';
            }
            ...
            if ($month != 100) {
                $queryN1 .= ' and MONTH(eh.EH_DATE_DEBUT)='.$month.' ';
            }
            ...
            if ($month != 100) {
                $queryN1A .= ' and MONTH(eh.EH_DATE_DEBUT)='.$month.' ';
            }
        ...
        $queryJ2 = 'select count(*) as NB, sum(ep.EP_DUREE) HOURS
                ...
                '.$clause5.'
                 and YEAR(eh.EH_DATE_DEBUT) ='.$year;
        if ($month != 100) {
            $queryJ2 .= ' and MONTH(eh.EH_DATE_DEBUT)='.$month.' ';
        }
        ...
        $query3 = 'select count(*) as NB, sum(ep.EP_DUREE) HOURS
                ...
                and YEAR(eh.EH_DATE_DEBUT) ='.$year;
        if ($month != 100) {
            $query3 .= ' and MONTH(eh.EH_DATE_DEBUT)='.$month.' ';
        }
        $result3 = mysqli_query($dbc, $query3);
        ...
        if ($mode_garde) {
            ...
            if ($month != 100) {
                $queryD .= ' and MONTH(d.D_DATE)='.$month.' ';
            }
        ...
        if ($mode_garde) {
            ...
            $K[] = $rowJ2['HOURS'];
            $L[] = $rowJ3['HOURS'];
            $total = $rowJ1['HOURS'] + $rowN1['HOURS'] + $rowJ1A['HOURS'] + $rowN1A['HOURS'] + $rowJ2['HOURS'] + $rowJ3['HOURS'];
            $T[] = $total;
        } else {
            $J[] = $rowJ1['NB'];
            $N[] = 0;
            ...
            $K[] = $rowJ2['NB'];
            $L[] = $rowJ3['NB'];
            $total = $rowJ1['NB'] + $rowJ2['NB'] + $rowJ3['NB'];
            $T[] = $total;
        }
        $nom = my_ucfirst($row_j['P_PRENOM']).' '.strtoupper($row_j['P_NOM']);
        if ($grades == 1) {
            $nom = $row_j['P_GRADE'].' - '.$nom;
        } // AICI
        if ($row_j['P_STATUT'] == 'SPP') {
            $nom .= ' - SPP';
        }
        $P_NOM[] = $nom;
        $P_GRADE[] = $row_j['P_GRADE']; // AICI
    ...
    if ($groupJN == 1) {
        echo "<p><b>Total gardes <span class='badge' style='background-color:#F7D4DC;color:$mydarkcolor;'>$allJ $g</span></b>";
    } elseif ($mode_garde == 1) {
        echo "<p><b>Total jours <span class='badge' style='background-color:#ffd480;color:$mydarkcolor;'>$allJ $g</span> nuits <span class='badge' style='background-color:#66a3ff;color:$mydarkcolor;'>$allN $g</span></b>";
    }
