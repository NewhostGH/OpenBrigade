...
if ($type == 'Habillement' or $usage == 'Habillement') {
    $habillement = true;
} else {
    $habillement = false;
}
...
if ($habillement) {
    ...
    $columns_title = ['Catégorie', 'Type', 'Nb', 'Section', 'Modèle',
...
if ($mid > 0) {
    ...
    if ($filter != 0) {
        $cmt = ' de '.get_section_name("$filter");
    } else {
        $cmt = ' de '.$cisname;
    }
...
while (custom_fetch_array($result1)) {
    ...
    if ($habillement) {
        $columns_data = [$TM_USAGE, $TM_CODE, $MA_NB, $S_CODE, $MA_MODELE,
