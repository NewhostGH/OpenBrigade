...
function load_reference_schema()
{
    ...
    mysqli_query($dbc, 'ALTER DATABASE `'.$database."` CHARACTER SET latin1 COLLATE 'latin1_swedish_ci'") or exit(mysqli_error($dbc));
...
function load_specific_data($organisation)
{
    ...
    if ($organisation == 0) {
        load_sql_file('sql/specific/sans_preconfiguration.sql');
    }
    if ($organisation == 1) {
        load_sql_file('sql/specific/secouristes.sql');
    }
    if ($organisation == 2) {
    ...
    if ($organisation == 4) {
        load_sql_file('sql/specific/syndicate.sql');
    }
    if ($organisation == 5) {
        load_sql_file('sql/specific/army.sql');
    }
    if ($organisation == 6) {
        load_sql_file('sql/specific/sslia.sql');
    }
    if ($organisation == 7) {
        load_sql_file('sql/specific/hospital.sql');
    }
...
function load_sql_file($file)
{
    ...
    for ($i = 0; $i < count($query) - 1; $i++) {
        mysqli_query($dbc, $query[$i]) or exit(mysqli_error($dbc));
    }
}
