...
if (isset($_GET['suggestedcompany'])) {
    $suggestedcompany = intval($_GET['suggestedcompany']);
} else {
    $suggestedcompany = 0;
}
...
if ($statut == 'EXT') {
    ...
    } elseif (check_rights($id, 37, $filter)) {
        $suggestedsection = $filter;
    }
} else { // internes
    check_all(1);
    $mysection = get_highest_section_where_granted($id, 1);
    if (check_rights($id, 1, $filter)) {
        $suggestedsection = $filter;
    }
    $suggestedcompany = 0;
}
...
if (isset($_GET['nom_naissance'])) {
    $nom_naissance = $_GET['nom_naissance'];
} else {
    $nom_naissance = '';
}
if (isset($_GET['birth1'])) {
    $birth1 = $_GET['birth1'];
} else {
    $birth1 = '';
}
...
if (isset($_GET['birthdep'])) {
    $birthdep = $_GET['birthdep'];
} else {
    $birthdep = '';
}
...
if (isset($_GET['habilitation'])) {
    $habilitation = $_GET['habilitation'];
} else {
    $habilitation = '0';
}
if (isset($_GET['type_salarie'])) {
    $type_salarie = $_GET['type_salarie'];
} else {
    $type_salarie = '';
}
...
if (isset($_GET['section'])) {
    $suggestedsection = $_GET['section'];
}
if (isset($_GET['statut'])) {
    $statut = $_GET['statut'];
}
if (isset($_GET['regime_travail'])) {
    $regime_travail = $_GET['regime_travail'];
} else {
    $regime_travail = '0';
}
...
echo insert_csrf('insert_personnel');
...
if ($full) {
    ...
    while (custom_fetch_array($result2)) {
        ...
        if ($statut == $S_STATUT) {
            $selected = 'selected';
        } elseif ($statut == 'EXT' and $S_STATUT == 'EXT') {
            $selected = 'selected';
        } elseif ($statut == '' and $S_STATUT == 'RES' and $army) {
            $selected = 'selected';
        } elseif ($statut == '' and $S_STATUT == 'ADH' and $syndicate) {
            $selected = 'selected';
        } elseif ($statut == '' and ($S_STATUT == 'BEN' or $S_STATUT == 'SPV')) {
            $selected = 'selected';
        } else {
            $selected = '';
        }
...
if ($evenement > 0) {
    $target = 'evenement_display.php?evenement='.$evenement.'&from=inscription';
} else {
    $target = 'personnel.php';
}
