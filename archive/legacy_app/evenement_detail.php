...
if (in_array($id, $chefs) or in_array($id, $chefs_parent)) {
    ...
    $chef = true;
} elseif (check_rights($id, 26, $organisateur)) {
    $veille = true;
    $SECTION_CADRE = get_highest_section_where_granted($id, 26);
}

if (check_rights($id, 15, $organisateur)) {
    $granted_event = true;
}
if (check_rights($id, 17, $organisateur) or $granted_event) {
    $granted_vehicule = true;
}
...
if ($what == 'personnel' or $what == 'personnelexterne') {
    ...
    if (check_rights($id, 10, $organisateur) or check_rights($id, 15, $organisateur)) {
        $granted_personnel = true;
    } elseif (! $granted_personnel) {
        check_all(10);
    }
    ...
    if (check_rights($id, 24)) {
        ...
        if ($what == 'personnelexterne') {
            $highestlevel = get_highest_section_where_granted($id, 37);
        } else {
            $highestlevel = get_highest_section_where_granted($id, 10);
        }

        $allowed1 = $highestlevel;
        if ($chef or check_rights($id, 15, "$organisateur")) {
            $allowed1 .= ','.$departement;
        }
        if ($veille) {
            $allowed1 .= ','.get_family("$SECTION_CADRE");
        }

        $allowed1 = str_replace(',,', ',', $allowed1);
        if (substr($allowed1, 0, 1) == ',') {
            $allowed1 = substr($allowed1, 1);
        }
    }
    ...
    if ($nbsections == 0) {
        ...
        if ($highestlevel == $sectioninscription) {
            $selected = 'selected';
        } else {
            $selected = '';
        }
        ...
        if (get_children("$sectioninscription") == '' or $sectioninscription == -1) {
    ...
    if ($gardes == 1 and $TE_CODE == 'GAR') {
        $jsfunction = 'inscrirePGarde';
    } else {
        $jsfunction = 'inscrireP';
    }
    ...
    if ($nbsections == 0) {
        if ($what == 'personnelexterne') {
            ...
            if ($company >= 0) {
                $query .= " and company.C_ID = $company";
            }
            if ($E_OPEN_TO_EXT == 1) {
                $sectionfilter = false;
            }
    ...
    if ($sectionfilter) {
        if ($allowed1 != '') {
            $query .= ' and P_SECTION in ('.$allowed1.')';
        }
        if ($allowed2 != '') {
            $query .= ' and P_SECTION in ('.$allowed2.')';
        }
    }
    if ($nbsections == 0) {
        if ($sectioninscription >= 0) {
            if ($subsections == 1 and $sectioninscription > 0) {
                $query .= ' and P_SECTION in ('.get_family($sectioninscription).')';
            } elseif ($subsections == 0) {
                $query .= ' and P_SECTION = '.$sectioninscription;
            }
        }
    }
    ...
    if ($what != 'personnelexterne' and ($sectioninscription > 0 or $subsections == 0)) {
        ...
        if ($subsections == 1) {
            $list = get_family("$sectioninscription");
        } else {
            $list = $sectioninscription;
        }
    ...
    if ($limit > 0 and $num == $limit) {
        echo "<tr><td><i class= 'fa fa-exclamation-triangle fa-lg' style='color:orange'></i><small> Seules les $limit premiers noms sont affichés, choisissez des filtres pour restreindre la liste</small></td></tr>";
    }
    echo '</table>';
    echo '</div>';
...
elseif ($what == 'vehicule' and $granted_vehicule) {
    $sectionfilter = true;
    ...
    if (check_rights($id, 17)) {
        $section17 = get_highest_section_where_granted($id, 17);
    } else {
        $section17 = -1000;
    }
    ...
    if ($chef or check_rights($id, 24)) {
        ...
        if ($SECTION_CADRE == 0) {
            ...
            $allowed = get_family("$SECTION_CADRE").','.$departement;
        }
        ...
        if ($mysection == 0) {
            ...
            $allowed = get_family("$mysection");
        }
    }

    if ($sectionfilter) {
        $query .= '     and s.S_ID in ('.$allowed.')';
    }
    $query .= '    and s.S_ID = v.S_ID';
    if ($nbsections == 0) {
        $query .= ' order by s.S_CODE, v.TV_CODE';
    } else {
        $query .= ' order by v.TV_CODE';
    }
    $result = mysqli_query($dbc, $query);

    echo '<div align=center>';
    echo "<table class='noBorder'>";
    echo "<tr><td><span class='left10'><b>Engager Véhicule</b></span></td></tr><tr></td>";
    echo "<td><select id='addvehicule' name='addvehicule' class='selectpicker' data-live-search='true' data-container='body' data-style='btn btn-default'
        ...
        <option value='0' selected class='option-ebrigade'>choix du véhicule</option>\n";

    $prevS_ID = -1;
    while (custom_fetch_array($result)) {
        if (($prevS_ID != $S_ID) and ($nbsections == 0)) {
            echo "<OPTGROUP LABEL='".$S_CODE.' - '.$S_DESCRIPTION."' class='section'>";
        }
        $prevS_ID = $S_ID;
...
// inscrire matériel
// =====================================================================
elseif ($what == 'materiel' and $granted_vehicule) {
    $sectionfilter = true;
    ...
    if ($nbsections == 0) {
        ...
        $level = get_level($dep_organisateur);
        if ($dep_organisateur == $sectioninscription) {
            $selected = 'selected';
        } else {
            $selected = '';
        }
    ...
    echo "<option value='ALL' $selected class='option-ebrigade'>Tous types de matériel</option>";
    ...
    if (intval($sectioninscription) >= 0) {
        $query .= ' and s.S_ID = '.$sectioninscription;
    }
    if ($type != 'ALL') {
        $query .= " and (tm.TM_ID='".$type."' or tm.TM_USAGE='".$type."')";
    }
    if ($nbsections == 0) {
        $query .= ' order by s.S_CODE, tm.TM_USAGE, tm.TM_CODE, m.MA_MODELE';
    } else {
        $query .= ' order by tm.TM_USAGE, tm.TM_CODE, m.MA_MODELE';
    }
    $result = mysqli_query($dbc, $query);

    echo "<tr><td><span class='left10'>Matériel à engager</span></td></tr>";
    ...
    while (custom_fetch_array($result)) {
        ...
        $prevS_ID = $S_ID;
        if ($prevTM_USAGE != $TM_USAGE) {
            echo "<OPTGROUP LABEL='...".$TM_USAGE."' class='categorie'>";
        }
        ...
        if ($elements >= 0) {
            $add2 = ' ('.$elements.' éléments dans ce lot)';
        } else {
            $add2 = '';
        }
        if ($MA_NUMERO_SERIE != '') {
...
elseif ($what == 'consommables' and $granted_vehicule) {
    $sectionfilter = true;
    $evts = get_event_and_renforts($evenement, false);
    ...
    if ($stockonly == 1) {
        ...
        if ($nbsections == 0) {
            $query .= ' order by s.S_CODE, cc.CC_NAME, tc.TC_DESCRIPTION';
        } else {
            $query .= ' order by cc.CC_NAME, tc.TC_DESCRIPTION';
        }
    } else {
    ...
    if ($nb > 0) {
        ...
        while (custom_fetch_array($result)) {
            ...
            if ($TCO_CODE == 'PE') {
                $label = $TC_DESCRIPTION.' ('.$TUM_DESCRIPTION.'s)';
            } elseif ($TUM_CODE != 'un' or $TC_QUANTITE_PAR_UNITE != 1) {
                $label = $TC_DESCRIPTION.' ('.$TCO_DESCRIPTION.' '.$TC_QUANTITE_PAR_UNITE.' '.$TUM_DESCRIPTION.')';
            } else {
                $label = $TC_DESCRIPTION;
            }
            ...
            if ($stockonly == 1) {
                ...
                if ($prevS_ID != $S_ID and $nbsections == 0) {
                    echo "<OPTGROUP LABEL='".$S_CODE.' - '.$S_DESCRIPTION."' class='categorie'>";
                }
                $prevS_ID = $S_ID;
                $label .= ' (stock '.$C_NOMBRE;
                if ($C_DATE_PEREMPTION != '') {
                    $label .= ' - péremption '.$C_DATE_PEREMPTION.')';
                } else {
                    $label .= ' )';
                }
        ...
        echo "<span class='left10'>Aucun produit trouvé</span>";
    }
    echo '</td>';
    echo '</tr>';
    echo '</table>';
...
if ($newpage) {
    if ($what == 'materiel') {
        $child = 2;
    } elseif ($what == 'consommables') {
        $child = 3;
    } elseif ($what == 'vehicule') {
        $child = 1;
    } elseif ($what == 'responsable') {
        $tab = 1;
    } else {
        $tab = 2;
    }
