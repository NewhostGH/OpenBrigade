...
$possibleorders = ['TM_USAGE', 'MA_NB', 'S_CODE', 'MA_MODELE', 'MA_NUMERO_SERIE', 'VP_OPERATIONNEL',
...
if ($type_materiel == 'Habillement' or $usage == 'Habillement') {
    $habillement = true;
} else {
    $habillement = false;
}
...
if ($tab == 1) {
    ...
    if ($order == 'AFFECTED_TO' || $order == 'MA_EXTERNE' || $order == 'V_ID' || $order == 'TM_LOT' || $order == 'MA_PARENT' || $order == 'MA_INVENTAIRE') {
        $query .= ' desc';
    }
    ...
    <thead>
        <tr class="widget-title">
            ...
            <th title='' data-field="serie" data-sortable="true" class="hide_mobile"><?php if ($habillement) { ?>Taille<?php } else { ?>N°Série<?php } ?></th>
...
if ($tab == 2) {
    ...
    if (get_children("$filter") != '') {
        ...
        echo "<input type='hidden' name='sub' id='sub' value='0'>";
    }
    ...
    if ($assoc) {
        ...
        echo "<input type='hidden' name='mad' id='mad' value='0'>";
    }
    ...
    if ($number > 0) {
        ...
        while (custom_fetch_array($result)) {
            ...
            if ($E_CANCELED == 1) {
                $label = "<span class='badge' style='color:$widget_fgred; background-color:$widget_bgred; margin-left: -4px;'>Activité annulée</span>";
            } elseif ($E_CLOSED == 1) {
                $label = "<span class='badge' style='color:$widget_fgorange; background-color:$widget_bgorange; margin-left: -4px;'>Inscriptions fermées</span>";
            } else {
                $label = "<span class='badge' style='color:$widget_fggreen; background-color:$widget_bggreen; margin-left: -4px;'>Inscriptions ouvertes</span>";
            }
            ...
            if ($MA_EXTERNE == 1) {
                $img3 = "<i class='fa fa-check' title=\"matériel mis à disposition par $cisname\"></i>";
            } else {
                $img3 = '';
            }
