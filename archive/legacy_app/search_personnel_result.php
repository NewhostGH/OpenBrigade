...
if ($envoisEmail) {
    ...
    $frmEmailDeb .= "<label for='CheckAll' class='label2 left10'>Tout cocher <input type=\"checkbox\" name=\"CheckAll\" id=\"CheckAll\" onclick=\"checkAll(document.frmPersonnel.SendMail,this.checked);\" ></label>";
    $frmEmailFin = '</form>';
}
// FIN EMAIL
...
where s_email2 like '$trouve%'
    ...
    case 'nom':
        ...
        $envoisEmail = false;
        if (strlen($trouve) > 0 or intval($trouve) > 0) {
            $ok = true;
        } else {
            $of = false;
        }
        ...
        $listA = get_family("$section");
        if ($local_only) {
            $listB = get_family("$mysection");
        }
        $queryadd1 = '';
        if ($section > 0) {
            $queryadd1 = ' AND p.P_SECTION in ('.$listA.')';
        }
        if ($local_only) {
            $queryadd1 .= '    and p.P_SECTION in ('.$listB.')';
        }
        $queryfull = $query.$queryadd1;

        if ($section > 0) {
            $role = get_specific_outside_role();
            $queryadd2 = ' and p.P_ID in ( select P_ID from section_role where S_ID in ('.$listA.') and GP_ID='.$role.') and P_SECTION not in ('.$listA.')';
            if ($local_only) {
                $queryadd2 .= '    and p.P_ID in ( select P_ID from section_role where S_ID in ('.$listB.') and GP_ID='.$role.') and P_SECTION not in ('.$listB.')';
            }
            $queryfull .= ' union '.$query.$queryadd2;
        }
        $query = $queryfull.' order by NOM, prenom asc ';
    ...
    case 'et':
        ...
        TO_DAYS(z.q_expiration) - TO_DAYS(NOW()) 'Reste' ";
            ...
            if ($statut == 'BENSAL') {
                ...
                $query .= " AND p.P_STATUT='".$statut."'";
            }
            if ($nb == 1) {
                $query .= ' AND z.P_ID = p.P_ID and z.PS_ID = '.$q[0];
            }
            if (! $externe) {
                $query .= " and p.p_statut <> 'EXT'";
            }
            ...
            $queryfull .= ' order by NOM, PRENOM asc ';
            $queryfull .= ' Limit '.$maxnumrows;
    ...
    case 'ou':
        ...
        if ($q) {
            $qualif = $_POST['qualif'];
            for ($i = 0; $i < count($q); $i++) {
                $queryA = " select distinct p.P_ID 'ID', p.P_EMAIL 'Email', ".phone_display_mask('P_PHONE')." 'numero', p.P_NOM 'NOM', p.p_statut, po.TYPE, po.DESCRIPTION,
                 p.P_PRENOM 'prenom', concat(s_code,' - ',s_description) 'section',
                case
                ...
                if ($statut == 'BENSAL') {
                    $queryA .= " AND p.P_STATUT in ('BEN','SAL')";
                } elseif ($statut != 'ALL') {
                    $queryA .= " AND p.P_STATUT='".$statut."'";
                }
                $queryA .= ' and p.P_OLD_MEMBER=0 ';
                if (! $externe) {
                    $queryA .= " and p.p_statut <> 'EXT'";
                }
    ...
    if ($number > 0) {
        ...
        while ($row = @mysqli_fetch_array($result)) {
            ...
            if ($P_ID != $prevPID) {
                ...
                if ($P_NOM_NAISSANCE != '') {
                    $P_NOM = $P_NOM.' né(e) '.$P_NOM_NAISSANCE;
                }
                ...
                if ($critere == 'ou' and $qualif == '7,6' and $assoc == 1) {
                    continue;
                }
                // exception 2 pour montrer seulement PAE PS si possède PAE PS et PAE PSC
                elseif ($critere == 'ou' and $qualif == '11,10' and $assoc == 1) {
                    continue;
                }
                $new = false;
                $P_PRENOM = '';
            ...
            if ($ancien >= 1) {
                $ft = "<span style='background:#A0A0A0;color:black;' >";
            } elseif ($p_statut == 'EXT') {
                $ft = "<span style='color:#0b660b;'>";
            } else {
                $ft = '<span>';
            }
            if ($flag1 == 1) {
            ...
            if ($P_ID > 0) {
                $hint = $hint."\n"."<tr class='newTable-tr' height=10 onclick=\"this.bgColor='#33FF00'\">";
                if ($envoisEmail) {
                    if ($new) {
                ...
                if ($Rejet > 0) {
                    $img_rejet = " <i class='fa fa-exclamation-triangle fa-lg' style='color:orange;' title='ATTENTION: il y a un ou des rejets non régularisés pour cette personne'></i>";
                } elseif ($Mode_Paiement == 4 and $syndicate == 1) {
                    ...
                    if ($montant == 0) {
                        $img_rejet = " <i class='fa fa-exclamation-circle' style='color:red;' title=\"ATTENTION: Adhérent payant par chèque mais aucun paiement enregistré\" ></i>";
                    } elseif ($annee < date('Y')) {
                        $img_rejet = " <i class='fa fa-exclamation-triangle' style='color:orange;' title=\"ATTENTION: Adhérent payant par chèque mais aucun paiement enregistré pour ".date('Y').'" ></i>';
                    } else {
                        $img_rejet = " <i class='fa fa-check-square' style='color:green;' title=\"Adhérent payant par chèque, ".$comment.'" ></i>';
                    }
                } else {
                    $img_rejet = '';
                }

                $hint .= "<td class='widget-text' onclick=\"displaymanager($P_ID);\">".$ft.strtoupper($P_NOM).' '.ucfirst($P_PRENOM).$ftend.' '.$img_rejet.'</td>';
                if ($identifiant != '0') {
                    $hint .= "<td onclick=\"displaymanager($P_ID);\">".$identifiant.'</td>';
                }
                $hint .= "<td class='widget-text' onclick=\"displaymanager($P_ID);\">".$S_CODE.'</td>';
                if ($p_statut != '0') {
                    $hint .= "<td class='widget-text hide_mobile' onclick=\"displaymanager($P_ID);\" >".$p_statut.'</td>';
                }
                if ($show_address) {
                    $hint .= "<td class='widget-text hide_mobile' onclick=\"displaymanager($P_ID);\" >".$zipcode.'</td>';
                    $hint .= "<td class='widget-text' onclick=\"displaymanager($P_ID);\">".$ville.'</td>';
                }
                if ($show_num) {
                    $hint .= "<td class='widget-text hide_mobile' >".$numero.'</td>';
                }
                if ($show_mail) {
                    $hint .= "<td class='widget-text'>".$email.'</td>';
                }
                if ($groupe1 != '') {
                    $hint .= "<td class='widget-text'>".$groupe1.$flag1.'</td>';
                }
                if ($groupe2 != '') {
                    $hint .= "<td class='widget-text hide_mobile' >".$groupe2.$flag2.'</td>';
                }
                if ($poste != '') {
                    $hint .= "<td class='widget-text'>".$poste.'</td>';
                }
                if ($bic != '0') {
                    $hint .= "<td class='widget-text hide_mobile'>".$bic.'</td>';
                }
                if ($iban != '0') {
                    $hint .= "<td class='widget-text' hide_mobile'>".$iban.'</td>';
                }

                if ($expire != '') {
                    if ($expire == '-') {
                        $expire = "<font color=$blue>".$expire.'</font>';
                    } elseif ($reste > 90) {
                        $expire = "<font color=$green><b>".$expire.'</b></font>';
                    } elseif ($reste > 0) {
                        $expire = "<font color=$orange><b>".$expire.'</b></font>';
                    } else {
                        $expire = "<font color=$red><b>".$expire.'</b></font>';
                    }
                    $hint .= "<td align=center class='widget-text hide_mobile'>".$expire.'</td>';
                }
                $hint .= '</tr>';
            }
        }
...
echo "<div class='col-sm-12'>$response</div>";
?>
