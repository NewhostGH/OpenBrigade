...
} elseif (isset($_POST['numinter'])) {
    $numinter = intval($_POST['numinter']);
} else {
    $numinter = '0';
}

if (isset($_GET['numcav'])) {
    $numcav = intval($_GET['numcav']);
} elseif (isset($_POST['numcav'])) {
    $numcav = intval($_POST['numcav']);
} else {
    $numcav = '0';
}
...
} elseif (isset($_POST['victime'])) {
    $victime = intval($_POST['victime']);
} else {
    $victime = '0';
}
...
if ($victime > 0) {
...
} elseif (isset($_POST['type_victime'])) {
    ...
    $evenement = $row[0];
    $responsable = intval($row[1]);
} elseif ($evenement_victime > 0) {
    $evenement = $evenement_victime;
}

$section_victime = get_section_organisatrice($evenement);
if ($responsable == $id) {
    $granted_update = true;
} elseif (check_rights($id, 15, $section_victime)) {
    $granted_update = true;
} elseif (is_chef_evenement($id, $evenement)) {
    $granted_update = true;
} elseif (is_operateur_pc($id, $evenement)) {
    $granted_update = true;
}
...
if (isset($_GET['victime']) and $action == 'delete' and $granted_update) {
    ...
    if ($numinter > 0) {
        echo "<body onload=\"ready('".$numinter."');\">";
    } else {
        echo "<body onload=\"readyliste2('".$evenement."');\">";
    }
    exit;
}
...
if (isset($_POST['victime']) and ($action == 'update' or $action == 'insert') and $granted_update) {
    ...
    if ($_POST['age'] == '') {
        ...
        $VI_AGE = intval($_POST['age']);
    }
    if (isset($_POST['sexe'])) {
        $VI_SEXE = secure_input($dbc, $_POST['sexe']);
    } else {
        $VI_SEXE = 'M';
    }
    if (isset($_POST['pays'])) {
        $VI_PAYS = intval($_POST['pays']);
    } else {
    ...
    if (! $store_confidential_data) {
        $VI_NOM = substr($VI_NOM, 0, 1);
    }
    if ($VI_NOM == '' and $VI_PRENOM == '') {
        $VI_NOM = '?';
    }
    if (isset($_POST['address'])) {
        $VI_ADDRESS = secure_input($dbc, $_POST['address']);
    } else {
        $VI_ADDRESS = '';
    }
    if (isset($_POST['numinter'])) {
        $EL_ID = intval($_POST['numinter']);
    } else {
        $EL_ID = 0;
    }
    if (isset($_POST['numerotation'])) {
        $VI_NUMEROTATION = intval($_POST['numerotation']);
    } else {
        $VI_NUMEROTATION = 0;
    }

    if ($VI_NUMEROTATION == '0') {
        $VI_NUMEROTATION = 'null';
    }
    if (isset($_POST['modevictime'])) {
        $modevictime = $_POST['modevictime'];
    } else {
        $modevictime = 'simple';
    }
    if (isset($_POST['detresse_vitale'])) {
        $VI_DETRESSE_VITALE = 1;
    } else {
        $VI_DETRESSE_VITALE = 0;
    }
    if (isset($_POST['decede'])) {
        $VI_DECEDE = 1;
    } else {
        $VI_DECEDE = 0;
    }
    ...
    if (isset($_POST['implique'])) {
        $VI_IMPLIQUE = 1;
    } else {
        $VI_IMPLIQUE = 0;
    }
    if (isset($_POST['commentaire'])) {
        $VI_COMMENTAIRE = secure_input($dbc, $_POST['commentaire']);
    } else {
        $VI_COMMENTAIRE = '';
    }
    if (isset($_POST['destination'])) {
        $D_CODE = secure_input($dbc, $_POST['destination']);
    } else {
        $D_CODE = 'NR';
    }
    if (isset($_POST['transporteur'])) {
        $T_CODE = secure_input($dbc, $_POST['transporteur']);
    } elseif ($nbsections > 0) {
        $T_CODE = 'SP';
    } else {
        $T_CODE = 'ASS';
    ...
    if (isset($_POST['type_victime'])) {
        $type_victime = intval($_POST['type_victime']);
    } else {
        $type_victime = 0;
    }
    if (isset($_POST['regulated'])) {
    ...
    } elseif ($type_victime > 0) {
        $date_in = date('Y-m-d');
    } else {
        $date_in = '';
    }
    if (isset($_POST['time_in']) and $_POST['time_in'] != '') {
        $time_in = secure_input($dbc, $_POST['time_in']);
    } else {
        $time_in = date('H:i');
    }
    if (isset($_POST['date_out']) and $_POST['date_out'] != '') {
        $date_out = secure_input($dbc, $_POST['date_out']);
        $tmp = explode('-', $date_out);
        $year = $tmp[2];
        $month = $tmp[1];
        $day = $tmp[0];
    ...
    if (isset($_POST['time_out'])) {
        $time_out = secure_input($dbc, $_POST['time_out']);
    } else {
        $time_out = '';
    }
    if (isset($_POST['heure_hopital'])) {
        $heure_hopital = secure_input($dbc, $_POST['heure_hopital']);
    } else {
        $heure_hopital = '';
    }
    if ($heure_hopital == '') {
    ...
    if (isset($_POST['raison'])) {
        $raison = secure_input($dbc, $_POST['raison']);
    } else {
        $raison = '';
    }
    if (isset($_POST['identification'])) {
        $identification = secure_input($dbc, $_POST['identification']);
    } else {
        $identification = '';
    }
    ...
    if ($type_victime > 0) {
        ...
        if ($entree_cav == ' ') {
            ...
            $entree_cav = "'".$entree_cav."'";
        }
        if ($sortie_cav == ' ') {
            $sortie_cav = 'null';
        } else {
            $sortie_cav = "'".$sortie_cav."'";
        }
    } else {
        $entree_cav = 'null';
        $sortie_cav = 'null';
    ...
    if ($from == 'list') {
        echo "<body onload=\"readyliste('".$evenement."','".$type_victime."');\">";
    } elseif ($from == 'intervention') {
        echo "<body onload=\"ready('".$EL_ID."');\">";
    } elseif ($from == 'evenement') {
...
if ($numcav > 0 or $numcav == 'cav') {
    $t = "Victime au centre d'accueil";
} else {
    $t = 'Victime ou Personne prise en charge';
}
...
if ($modevictime == 'full') {
    $icon = 'heartbeat';
} elseif ($modevictime == 'pssp') {
    $icon = 'user-md';
} elseif ($modevictime == 'doc') {
    $icon = 'folder-open';
} else {
    $icon = 'user';
}
...
if ($modevictime == 'simple') {
    ...
    if ($nbcav == 0 or ($numinter > 0 and $numcav == 0)) {
...
if ($modevictime == 'full' or $modevictime == 'pssp') {
    ...
    $nonred = ['Consciente', 'Ventilation', 'Pouls carotidien'];
    ...
    if ($modevictime == 'full') {
        $query2 .= " and bvc.BVC_PAGE='PSE'";
    } else {
        $query2 .= " and bvc.BVC_PAGE='PSSP'";
    }
    ...
    while (custom_fetch_array($result2)) {
        if ($DOC_ONLY == 1) {
            if ($modevictime == 'full') {
                $BVP_TITLE = $BVP_TITLE." <i class='fa fa-user-md fa-lg' title='A renseigner par un médecin'></i>";
            } elseif ($modevictime == 'pssp') {
                $BVP_TITLE = $BVP_TITLE." <i class='fa fa-plus-square' style='color:red;' title='Si coché, soins médico-psychologiques'></i>";
            }
        }
        if ($BVP_ID == 500 or $BVP_ID == 531 or $BVP_ID == 535) {
            $BVP_TITLE = '<b>'.$BVP_TITLE.'<b>';
        }
        if ($prevBVC_CODE != $BVC_CODE) {
            if ($i % 2 == 1 and $prevBVP_TYPE != 'textarea') {
        ...
        // checkbox
        elseif ($BVP_TYPE == 'checkbox') {
            echo "<td><label for='bilan".$BVP_ID."' id='label_bilan".$BVP_ID."' class='$class'>".$BVP_TITLE.'</label></td>';
            if (intval($BVP_VALUE) == '1') {
                $checked = 'checked';
            } else {
                $checked = '';
            }

            $gestes_soins = ['610', '620', '630', '635', '640', '650', '660', '670', '680', '687', '688', '690', '700', '710', '720', '722', '730', '740', '750', '780'];
            if (in_array($BVP_ID, $gestes_soins)) {
                $class = "class='gestes'";
            } else {
                $class = '';
            }
        ...
        elseif ($BVP_TYPE == 'radio') {
            ...
            if (in_array($BVP_TITLE, $nonred)) {
                $non = '<span class=RedClass><b>Non</b></span>';
            } else {
                $non = 'Non';
            }
...
if ($modevictime == 'doc') {
    ...
    if ($NB1 > 0 and $disabled == '') {
        $possibleorders = ['date', 'file', 'security', 'type', 'author', 'extension'];
        ...
        if (is_dir($mypath)) {
            ...
            for ($i = 0; $i < $number; $i++) {
                ...
                if ($document_security == 1) {
                    echo '<td>';
                    if ($s_arr[$i] > 1) {
                        $img = "<i class='fa fa-lock' style='color:orange;' title=\"".$s_lib_arr[$i].'" ></i>';
                    } else {
                        $img = "<i class='fa fa-unlock' title=\"".$s_lib_arr[$i].'"></i>';
                    }
                    if ($disabled == '') {
                        echo write_modal($url, 'doc_'.$is_folder[$i].'_'.$id_arr[$i], $img);
                    } else {
                        echo $img;
                    }
                    echo '</td>';
...
if ($modevictime != 'doc') {
    ...
    if ($granted_update) {
        if ($action != 'insert') {
            if ($numinter > 0) {
                echo " <input type='button' class='btn btn-danger' value='Supprimer' onclick=\"deleteIt('".$victime."','".$numinter."');\">";
            } elseif ($numcav > 0) {
                echo " <input type='button' class='btn btn-danger' value='Supprimer' onclick=\"deleteIt2('".$victime."','".$numcav."');\">";
            }
        }
    ...
    if ($from == 'export') {
        ...
        if ($numinter > 0) {
            echo "<a class='dropdown-item' onclick=\"ready('".$numinter."');\">Retour intervention</a>";
        }
        if ($numcav > 0 or $evenement_victime > 0) {
            echo "<a class='dropdown-item' onclick=\"readyliste('".$evenement_victime."','".$type_victime."');\">Retour Victime</a>";
        }
