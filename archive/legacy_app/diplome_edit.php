...
if (check_rights($id, 54, '0')) {
    $thislevel = 0;
} else {
    $thislevel = get_highest_section_where_granted($id, 54);
}
if (isset($_GET['aml'])) {
    $filter = $thislevel;
}
test_permission_level(54);
if (! check_rights($id, 54, $filter)) {
    $filter = $thislevel;
}
if (isset($_GET['psid'])) {
...
$pos_y = [];
$annexe = [];
$style_org = ['Normal', 'Gras', 'Italique', 'Gras et Italique'];
$taille_org = [8, 9, 10, 11, 12, 14, 16, 18];
$police_org = ['Courrier', 'Arial', 'Times'];
extract($_POST);
...
$FILE_MIMES = ['image/jpeg', 'image/jpg', 'image/x-png', 'image/pjpeg', 'image/gif', 'image/png'];
// Allowable file ext. names. you may add more extension names.
$FILE_EXTS = ['.jpg', '.png', '.gif'];
        ...
        if ($_FILES['userfile']['size'] != 0) {
            ...
            elseif (! in_array($file_type, $FILE_MIMES) && ! in_array($file_ext, $FILE_EXTS)) {
                ...
                if (! is_dir($upload_dir)) {
                    if (! mkdir($upload_dir)) {
                        exit("Le répertoire d'upload n'existe pas et sa création a échoué.");
                    }
                    if (! chmod($upload_dir, 0755)) {
                        exit('Echec lors de la mise à jour des permissions.');
                    }
                }
...
for ($i = 1; $i <= $numfields_org; $i++) {
    ...
    if (! $modifiable and $data['ACTIF'] == 0) {
        $local_disabled2 = '';
    }
    if (! $modifiable) {
