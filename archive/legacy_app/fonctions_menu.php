...
function evaluate_condition($id, $current, $type, $value, $menu = '')
{
    ...
    // > 0 display
    // <= 0 do not display
    $configs = ['vehicules', 'materiel', 'consommables', 'competences', 'externes', 'disponibilites', 'evenements', 'notes', 'client',
    ...
    if ($type == '') {
        ...
        if ($value == 45) {
            if (! isset($_SESSION['SES_COMPANY'])) {
                $ret = 0;
            } elseif (intval($_SESSION['SES_COMPANY']) == 0) {
                $ret = 0;
            }
        } elseif (check_rights($id, $value)) {
            $ret = 1;
        } else {
            $ret = 0;
        }
    } elseif ($type == 'not_permission') {
...
function write_menu()
{
    ...
    if ($logomenu == 1) {
        ...
        $out .= "<a class='navbar-brand logo-small' href='index_d.php' title=\"Aller a la page d'accueil de ".$cisname."\" ><i class='fas fa-home fa-lg' style='color: rgb(188, 188, 207);'></i></a>";
    }
    ...
    if ($evenements and check_rights($id, 41) and $btpref['button_even'] == '1') {
        if ($desktop) {
            $tit = 'Voir les activités prévues';
        }
    ...
    if ($btpref['button_search'] == '1') {
        if ($desktop) {
            $tit = 'Recherche sur les fiches personnel';
        }
    ...
    if (check_rights($id, 1) or (check_rights($id, 15) and $evenements == 1) or (check_rights($id, 17) and $vehicules == 1) or (check_rights($id, 70) and $materiel == 1) or (check_rights($id, 71) and $consommables == 1) or (check_rights($id, 77) and check_rights($id, 59))) {
        ...
        if (check_rights($id, 1)) {
            $out .= "<a class='dropdown-item dropdown-item-profil' href='./ins_personnel.php?category=INT&suggestedcompany=-1'>
            <i class=\"fas fa-plus-circle\" style='color:#28A745'></i>
            $label </a>";
        }
        if (check_rights($id, 15) and $evenements == 1) {
            $out .= "<a class='dropdown-item dropdown-item-profil' href='./evenement_edit.php?action=create'>
            <i class=\"fas fa-plus-circle\" style='color:#28A745'></i>
            Activité</a>";
        }
        if (check_rights($id, 17) and $vehicules == 1) {
            $out .= "<a class='dropdown-item dropdown-item-profil' href='./ins_vehicule.php'>
            <i class=\"fas fa-plus-circle\" style='color:#28A745'></i>
            Véhicule</a>";
        }
        if (check_rights($id, 70) and $materiel == 1) {
            $out .= "<a class='dropdown-item dropdown-item-profil' href='./ins_materiel.php?usage=ALL&type=ALL'>
            <i class=\"fas fa-plus-circle\" style='color:#28A745'></i>
            Matériel</a>";
        }
        if (check_rights($id, 71) and $consommables == 1) {
            $out .= "<a class='dropdown-item dropdown-item-profil' href='./upd_consommable.php?action=insert&type_conso=ALL'>
            <i class=\"fas fa-plus-circle\" style='color:#28A745'></i>
            Consommable</a>";
        }
        if (check_rights($id, 77) and $notes == 1) {
            $out .= "<a class='dropdown-item dropdown-item-profil' href='./note_frais_edit.php'>
            <i class=\"fas fa-plus-circle\" style='color:#28A745'></i>
            Note de frais</a>";
        }
        $out .= '</div>';
        ...
        if ($MG_IS_LEFT == 0) {
            if ($MI_CODE != $previtem) {
                ...
                if ($MI_ICON != '' and $MI_ICON != 'null') {
                    if ($MI_ICON == 'power-off') {
                        $MI_NAME = "<i class='fa fa-".$MI_ICON." fa-lg' style='color:red;' ></i> ".$MI_NAME;
                    } else {
                        $MI_NAME = "<i class='fa fa-".$MI_ICON." fa-lg'></i> ".$MI_NAME;
                    }
                }
                if ($MI_CODE == 'MASECTION') {
                    $MI_URL .= '?S_ID='.$mysection;
                }
                if ($MI_NAME == 'divider' and $items > 0) {
                    $menuitem = "\n<div role='separator' class='dropdown-divider'></div>";
                } elseif ($MI_CODE == 'COMMU') {
            ...
            if ($MG_CODE != $prevgroup) {
                ...
                // prepare next group
                if ($MG_ICON != '') {
                    $GROUP_NAME = "<i class='fas fa-".$MG_ICON." fa-lg'></i> ".$MG_NAME;
                } else {
                    $GROUP_NAME = $MG_NAME;
                }
                if ($MG_CODE == 'ME') {
    ...
    if ($display_item > 0) {// flush last item of the group, avoid 2 dividers
        if (substr($menuitem, -14, 7) != 'divider' or substr($menugroup, -14, 7) != 'divider') {
            $menugroup .= $menuitem;
        }
    }
    ...
    while ($row = @mysqli_fetch_array($result)) {
        ...
        if ($MG_IS_LEFT == 1) {
            if ($MI_CODE != $previtem) {
                ...
                if ($MI_ICON != '' and $MI_ICON != 'null') {
                    if ($MI_ICON == 'power-off') {
                        ...
                        $MI_NAME = "<i class='far fa-".$MI_ICON." fa-lg'></i> ".$MI_NAME;
                    }
                }
                if ($MI_NAME != 'divider') {
            ...
            if ($MG_CODE != $prevgroup) {
                ...
                if (! $display_item and $MI_CODE == 'CONF') {
                    $boldlink = "href='#'";
                } else {
                    $boldlink = "href='".$MI_URL."'";
                }
                $menugroup .= ' <script>
...
function write_pointage_links($person)
{
    ...
    if ($finished2) {
        ...
        $color = '#FFA800';
        $pulse = '';
    } elseif ((! $started1 and ! $finished1 and ! $started2) or (! $started2 and ! $finished2 and $finished1)) {
...
function writeBreadCrumb($pageName = null, $pageName2 = null, $index2 = null, $buttons = null)
{
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if ($endresult['MG_CODE'] == 'ME' and $endresult['MG_NAME'] == '') {
            $parentName = $label_personnel;
        }
    }
    if (end($arr) == 'upd_personnel.php') {
        $parentName = $label_personnel;
        if (isset($_GET['pompier'])) {
            $idpompier = intval($_GET['pompier']);
        } elseif (isset($_GET['pid'])) {
            $idpompier = intval($_GET['pid']);
        } else {
            $idpompier = $_SESSION['id'];
        }
        ...
        if (is_file($P_PHOTO)) {
            ...
            if ($P_CIVILITE == 4 or $P_CIVILITE == 5) {
                $pic = $defaultdog;
            }
        }
    ...
    } elseif (end($arr) == 'evenement_display.php') {
    ...
    if ($index2 == '') {
        $index2 = "<a style='color:#666' href='javascript:history.go(-1);'>";
    } else {
        $index2 = "<a style='color:#666' href='".$index2."'>";
    }
