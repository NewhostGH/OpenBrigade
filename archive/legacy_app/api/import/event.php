...
if (isset($input['token'])) {
    $token = secure_input($dbc, $input['token'], $strict = true);
} else {
    return_error(30, 'token not provided');
}
if ($token != $import_api_token) {
    return_error(40, 'Wrong token, access to this webservice forbidden expected');
}

// test other parameters
if (isset($input['event_code'])) {
    $event_code = intval($input['event_code']);
} else {
    return_error(1050, 'Missing event_code');
}

if (isset($input['event_name'])) {
    $event_name = secure_input($dbc, str_replace('"', '', utf8_decode($input['event_name'])));
} else {
    return_error(1060, 'Missing event_name');
}

if (isset($input['event_type'])) {
    $event_type = secure_input($dbc, $input['event_type']);
} else {
    return_error(1070, 'Missing event_type');
}

if (isset($input['location'])) {
    $location = secure_input($dbc, str_replace('"', '', utf8_decode($input['location'])));
} else {
    return_error(1080, 'Missing location');
}

if (isset($input['address'])) {
    $address = secure_input($dbc, str_replace('"', '', utf8_decode($input['address'])));
} else {
    return_error(1090, 'Missing address');
}

if (isset($input['section'])) {
    $section = intval($input['section']);
} else {
    return_error(1100, 'Missing section');
}

if (isset($input['comment'])) {
    $comment = secure_input($dbc, str_replace('"', '', utf8_decode($input['comment'])));
} else {
    $comment = '';
}

if (isset($input['tarif'])) {
    $tarif = floatval($input['tarif']);
} else {
    $tarif = 'null';
}

if (isset($input['url'])) {
    $url = secure_input($dbc, str_replace('"', '', utf8_decode($input['url'])));
} else {
    $url = '';
}
...
if (isset($input['competence'])) {
    $competence = intval($input['competence']);
} else {
    $competence = 0;
}
if ($competence > 0) {
    ...
    if ($NB == 0) {
        return_error(1120, 'Wrong value for competence');
    }
}

if (isset($input['contact_entreprise'])) {
    $contact_entreprise = secure_input($dbc, str_replace('"', '', utf8_decode($input['contact_entreprise'])));
} else {
    $contact_entreprise = '';
}

if (isset($input['contact_tel'])) {
    $contact_tel = secure_input($dbc, str_replace('"', '', $input['contact_tel']));
} else {
    $contact_tel = '';
}
...
if (isset($input['telephone'])) {
    $telephone = secure_input($dbc, str_replace('"', '', $input['telephone']));
} else {
    $telephone = '';
}
...
if (isset($input['type_formation'])) {
    $type_formation = secure_input($dbc, $input['type_formation']);
} else {
    $type_formation = '';
}
if ($type_formation != '') {
    ...
    if ($NB == 0) {
        return_error(1150, "Wrong value for type_formation, try 'I' or 'R'");
    }
}

if (isset($input['stagiaires'])) {
    $stagiaires = intval($input['stagiaires']);
} else {
    $stagiaires = '0';
}
...
if (isset($input['event_sessions'])) {
    ...
    if (! $found_1) {
        return_error(1200, 'No_session 1');
    }
} else {
    return_error(1210, 'Missing event_sessions');
}
...
if (isset($input['people'])) {
    ...
    foreach ($input['people'] as $j) {
        if (! isset($j['user_id'])) {
            return_error(1300, 'Wrong values for people');
        }
        if (! isset($j['function_id'])) {
            return_error(1310, 'Wrong values for people');
        }
        $user = intval($j['user_id']);
        if ($user == 0) {
            return_error(1320, 'Wrong values for people, user '.$j['user_id']);
        }
