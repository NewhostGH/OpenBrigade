...
if (! $webservice_key) {
    return_error(10, 'Export API not activated');
}
...
if ($token != $webservice_key) {
    return_error(40, "Wrong token $token instead of $webservice_key, access to this webservice forbidden");
}

$sq = [];
if (isset($input['qstrict'])) {
    $qstrict = intval($input['qstrict']);
} else {
    $qstrict = 0;
}
if (isset($input['exclude_old'])) {
    $exclude_old = intval($input['exclude_old']);
} else {
    $exclude_old = 0;
}
if (isset($input['id'])) {
