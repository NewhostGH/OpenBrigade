...
$possibleorders = ['EQ_ID', 'S_CODE', 'EQ_NOM', 'NB_POSTES', 'EQ_JOUR', 'EQ_NUIT', 'EQ_VEHICULES', 'EQ_SPP', 'EQ_PERSONNEL1', 'EQ_PERSONNEL2', 'EQ_ORDER'];
...
if ($order == 'EQ_NOM' || $order == 'EQ_ID' || $order == 'EQ_ORDER') {
    $query1 .= ' asc';
} else {
    $query1 .= ' desc';
}
...
if ($number == 0) {
    ...
    while (custom_fetch_array($result1)) {
        ...
        if ($EQ_DEFAULT == 1) {
            echo "<i class='fa fa-star ml-2' title='Type de garde par défaut'></i>";
        }
        echo '</td>';
        if ($EQ_JOUR == 1) {
            $jour = "<i class='fa fa-circle fa-lg' style='color:green;' title='actif'></i>";
        } else {
            $jour = "<i class='fa fa-circle fa-lg' style='color:red;' title='pas actif'></i>";
        }
        if ($EQ_NUIT == 1) {
            $nuit = "<i class='fa fa-circle fa-lg' style='color:green;' title='actif'></i>";
        } else {
            $nuit = "<i class='fa fa-circle fa-lg' style='color:red;' title='pas actif'></i>";
        }
