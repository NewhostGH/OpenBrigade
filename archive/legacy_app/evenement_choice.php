...
// sinon recherche par critères
    ...
    if ($type_evenement == 'ALLBUTGARDE') {
        $query .= "\n and TE.TE_CODE <> 'GAR'";
    } elseif ($type_evenement != 'ALL' and $ec_mode == 'default') {
    ...
    $errCal = '';
    $cbcalendar = '';
    $ChxCalendar = (isset($_GET['btGo']) ? (isset($_GET['chxCal']) ? $_GET['chxCal'] : []) : $chxCal); // utilise les données du formulaire ou de la session
    if (count($ChxCalendar) == 0) {
        $_SESSION['chxCal'] = [];
    }
    ...
    if (isset($_GET['delCal'])) {
        $delCal = intval($_GET['delCal']);
    } else {
        $delCal = 0;
    }
    if ($delCal > 0) {
    ...
    foreach ($ChxCalendar as $k => $v) {
        $ChxCalendar[$k] = intval($v, 10);
    }
    ...
    if ($search != '') {
        ...
        } elseif (strtolower($search) == 'renfort') {
...
if ($number > 0) {
    ...
    while (custom_fetch_array($result)) {
        ...
        if ($E_COLONNE_RENFORT > 0 and strtolower(substr($E_LIBELLE, 0, 7)) != 'colonne') {
            $E_LIBELLE = 'Colonne de renfort '.$E_LIBELLE;
        }
        if ($E_VISIBLE_INSIDE == 0) {
            $E_LIBELLE .= " <i class='fa fa-exclamation-triangle' style='color:orange;' title='ATTENTION événement caché, seules les personnes ayant la permission n°9 peuvent le voir'></i>";
        }
        ...
        if ($EQ_ICON != '' and is_file($EQ_ICON)) {
            $b1 = '<img  src='.$EQ_ICON.' title="'.$EQ_NOM."\"  class='img-max-35' style='$style'>";
        } elseif (is_file('images/evenements/'.$TE_ICON)) {
            $b1 = '<img src=images/evenements/'.$TE_ICON." title='".$TE_LIBELLE."' class='img-max-35' style='$style'>";
        } else {
            $b1 = '';
        }
        ...
        $b2 = '';
        if ($NR > 0) {
            $b2 .= "<i class='fa fa-plus-circle' style='color:green;' title='$NR ".$renfort_label."(s)' ></i>";
        }
        if ($NR > 1) {
            $b2 .= " <b><font color=green>$NR</font></b>";
        }

        if ($nbsessions > 1) {
            if ($EH_DESCRIPTION != '') {
                $dp = ' - <i>'.$EH_DESCRIPTION.'</i>';
            } else {
                $dp = '';
            }
        ...
        if ($ec_mode == 'MC') {
            ...
            $row2 = mysqli_fetch_array($result2);
            $nb = intval($row2[0]);
            if ($row2[2] == date('d-m-Y')) {
                $new = " <i class='fa fa-star' style='color:orange'  title=\"Dernier message ajouté aujourd'hui\" ></i>";
            } else {
                $new = '';
            }
            if ($nb > 0) {
