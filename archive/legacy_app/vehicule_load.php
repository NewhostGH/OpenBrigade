...
if (isset($_GET['data'])) {
    ...
    while (custom_fetch_array($result)) {
        ...
        if ($TV_ICON == '') {
            ...
            $vimg = "<img src='".$TV_ICON."' class='img-max-35'>";
        }

        // Icones
        if ($V_FLAG1 == 1) {
            $img1 = "<i class='fa fa-check' title='ce véhicule est équipé pour rouler sur la neige'></i>";
        } else {
            $img1 = '';
        }
        if ($V_FLAG2 == 1) {
            $img2 = "<i class='fa fa-check' title='ce véhicule est climatisé'></i>";
        } else {
            $img2 = '';
        }
        if ($V_FLAG3 == 1) {
            $img3 = "<i class='fa fa-check' title='ce véhicule est équipé public alert'></i>";
        } else {
            $img3 = '';
        }
        if ($V_FLAG4 == 1) {
            $img4 = "<i class='fa fa-check' title='ce véhicule est équipé d&apos;un attelage pour tracter une remorque'></i>";
        } else {
            $img4 = '';
        }
        if ($V_EXTERNE == 1) {
            $img5 = "<i class='fa fa-check' title='véhicule mis à disposition par $cisname'></i>";
        } else {
            $img5 = '';
        }
        ...
        if ($AFFECTED_TO != '') {
            ...
            if ($P_OLD_MEMBER == 1) {
                $owner = "<font color=black title='ancien membre'><b>".$owner.'</b><font>';
            }
        } else {
            $owner = '';
        }
        ...
        } elseif (my_date_diff(getnow(), $V_ASS_DATE1) < 0) {
            $fgcolor = $widget_fgred;
            $bgcolor = $widget_bgred;
            $VP_LIBELLE = 'Assurance périmée';
        } elseif (my_date_diff(getnow(), $V_TITRE_DATE1) < 0) {
            $fgcolor = $widget_fgred;
            $bgcolor = $widget_bgred;
            $VP_LIBELLE = "Titre d'accès périmé";
        } elseif (my_date_diff(getnow(), $V_CT_DATE1) < 0) {
            $fgcolor = $widget_fgred;
            $bgcolor = $widget_bgred;
            $VP_LIBELLE = 'Contrôle technique périmé';
        } elseif ((my_date_diff(getnow(), $V_REV_DATE1) < 0) and ($VP_OPERATIONNEL != 1)) {
            $fgcolor = $widget_fgorange;
            $bgcolor = $widget_bgorange;
        ...
        if ($row2['NB'] > 0) {
            $mat = "<span title='$row2[NB] élément(s) embarqué'>$row2[NB]</span>";
        } else {
            $mat = '';
        }

        // Assurance
        if (my_date_diff(getnow(), $V_ASS_DATE1) < 0) {
            $assurance = $widget_fgred;
        } elseif (my_date_diff(getnow(), $V_ASS_DATE1) < 30) {
        ...
        // Controle
        if (my_date_diff(getnow(), $V_CT_DATE1) < 0) {
            $controle = $widget_fgred;
        } elseif (my_date_diff(getnow(), $V_CT_DATE1) < 30) {
            $controle = $widget_fgorange;
        } else {
            $controle = $mydarkcolor;
        }

        // Révision
        if (my_date_diff(getnow(), $V_REV_DATE1) < 0) {
            $rev = $widget_fgorange;
        } else {
            $rev = $mydarkcolor;
        }
        ...
        // Accès
        if (my_date_diff(getnow(), $V_TITRE_DATE1) < 0) {
            $titre = $widget_fgred;
        } elseif (my_date_diff(getnow(), $V_TITRE_DATE1) < 30) {
