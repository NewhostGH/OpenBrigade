...
$row = mysqli_fetch_array($result);
if ($row['NB'] != 0) {
    $code_conducteur_active = true;
} else {
    $code_conducteur_active = false;
}
...
while ($row = mysqli_fetch_array($result)) {
    if (is_iphone()) {
        $reports[$row['R_CODE']] = substr($row['R_NAME'], 0, 46);
    } else {
        $reports[$row['R_CODE']] = substr($row['R_NAME'], 0, 60);
    }
}
