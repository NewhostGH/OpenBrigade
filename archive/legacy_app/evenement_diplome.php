...
if (isset($_POST['expiration'])) {
    $expiration = secure_input($dbc, $_POST['expiration']);
} else {
    $expiration = '';
}
if (isset($_POST['comment'])) {
    $comment = secure_input($dbc, str_replace('"', '', $_POST['comment']));
} else {
    $comment = '';
}
if (isset($_POST['update_hierarchy'])) {
    $update_hierarchy = intval($_POST['update_hierarchy']);
} else {
    $update_hierarchy = 0;
}
...
if (! in_array($id, $chefs) and ! check_rights($id, 4, "$S_ID")) {
    check_all(24);
}
...
$destid = '';
if (count($chefs) > 0) {
    $resp = ucfirst(get_prenom($chefs[0])).' '.strtoupper(get_nom($chefs[0]));
} else {
    $resp = '';
}
