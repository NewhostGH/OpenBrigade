...
if (isset($_GET['exp'])) {
    ...
    if ($filter == 0 and $subsections == 1) {
        unset($list);
    } elseif ($list == 0 and $subsections == 1) {
        unset($list);
    }
    ...
    $show = '0';
    if (check_rights($id, 2, intval($filter))) {
        $show = '1';
    }
    if ($ischef) {
        $show = '1';
...
as 'Tél'";
    ...
    switch ($exp) {
        ...
        case '1intervictime':
           ...
           evenement e';
            ...
            $SommeSur = ['interventions', 'personnes prises en charge'];
            break;
        ...
        case '1intervictimeparevt':
           ...
           evenement e';
            ...
            $orderby = 'el.EL_DEBUT';
            $groupby = 'e.E_CODE';
            $SommeSur = ['interventions', 'personnes'];
            break;
        ...
        case '1statdetailvictime':
            ...
            $orderby = 'date1';
            $groupby = 'date1';
            $SommeSur = ['personnes prises en charge', 'détresses', 'décès', 'malaises', 'assistées', 'soins', 'médicalisées', 'refus', 'impliqués', 'transports', 'vetements', 'alimentation', 'repos', 'traumatismes'];
            break;
        ...
        case '1statdetailvictimeparevt':
            ...
            $orderby = 'date1';
            $groupby = 'e.E_CODE';
            $SommeSur = ['personnes prises en charge', 'détresses', 'malaises', 'assistées', 'soins', 'médicalisées', 'refus', 'impliqués', 'transports', 'vetements', 'alimentation', 'repos', 'DCD', 'traumatismes'];
            break;
        ...
        case '1listevictime':
            ...
            $where .= (isset($list) ? '  and e.S_ID in('.$list.') ' : '');
            $orderby = 'date1';
            $SommeSur = ['détr.', 'décès', 'malaise', 'assist.', 'soins', 'refus', 'impliqués', 'vet.', 'alim.', 'transporté', 'repos.', 'trauma.'];
            break;
        ...
        mys.s_city 'Ville'";
            if ($assoc == 1) {
                $select .= ", mys.S_AFFILIATION 'Num Affiliation'";
            }
    ...
    and eh.eh_id = 1";
            if (isset($list)) {
                $table .= ' and e.s_id in('.$list.')';
            }
            if ($exp == '1cadps_sansR') {
                $table .= ' and ( e.E_PARENT=0 or e.E_PARENT is null )';
            }
            $table .= ') as e
    ';
            $where = " $evenemententredeuxdate ";
    ...
    ';
            ...
            $SommeSur = ['Montant devis', 'Montant facture'];
            break;
    ...
    ";
            ...
            $groupby = ' e.e_code';
            $SommeSur = ['Montant devis', 'Montant facturé'];
            break;
    ...
    ';
            ...
            $SommeSur = ['Kilometres.', 'Participants.', 'Heures.', 'Secours.', 'Assist.', 'Evac.', 'Total', 'Actions.', 'Repas.', 'Textiles.'];
            break;
    ...
    ";
            ...
            $orderby = ' e.te_code, e.eh_date_debut';
            $groupby = ' e.te_code, e.e_code, e.eh_id';
            $RuptureSur = ['Evénement'];
            $SommeSur = ['Participants.', 'Heures.', 'Total'];
            break;

        case  $exp == 'personneldisponiblea' or $exp == 'personneldisponibled':
            $tomorrow = mktime(0, 0, 0, date('m'), date('d') + 1, date('Y'));
            if ($exp == 'personneldisponiblea') {
                $export_name .= ' le '.date('d-m-Y');
            } else {
                $export_name .= ' le '.date('d-m-Y', $tomorrow);
            }
        ...
        ";
            ...
            if ($exp == 'personneldisponiblea') {
                $where .= " and d.d_date = '".date('Y-m-d')."'";
            } else {
                $where .= " and d.d_date = '".date('Y-m-d', $tomorrow)."'";
            }
            $where .= " and p.p_statut <> 'EXT' ";
            $orderby = 'p.p_nom, p.p_prenom, p.p_id, s.s_code, s.s_description';
            $groupby = '';
    ...
    ';
        ...
        case  $exp == '1ah' or $exp == '1soutienpopulations' or $exp == '1heuresparticipations':
            ...
            } elseif ($exp == '1soutienpopulations') {
    ...
    ';
            ...
            $orderby = ' e.te_code';
            $groupby = ' e.te_code';
            $SommeSur = ['Participations', 'Total_Heures'];
        ...
        case  $exp == '1dps' or $exp == '1garde' or $exp == '1alsan' or $exp == '1dpsre' or $exp == '1gardere' or $exp == '1alsanre' or $exp == '1mar':
            if ($exp == '1dps' or $exp == '1dpsre') {
                $tecode = 'DPS';
                $TE = "'DPS'";
            } elseif ($exp == '1alsan' or $exp == '1alsanre') {
            ...
            // build smart query for stats
            $SommeSur = ['Durée', 'Inscrits', 'Heures'];
            $q1 = 'select TB_NUM, TB_LIBELLE from type_bilan where TE_CODE='.$TE.' order by TB_NUM';
            $r1 = mysqli_query($dbc, $q1);
            $i = 1;
            $query_join = '';
    ...
    sum(k.ep_duree) 'Heures'";
        ...
        case '1alsanpardep':
            $TE = "'ALSAN'";
            // build smart query for stats
            $SommeSur = ['Inscrits', 'Durée événements', 'Heures participation'];
            $q1 = 'select TB_NUM, TB_LIBELLE from type_bilan where TE_CODE='.$TE.' order by TB_NUM';
            $r1 = mysqli_query($dbc, $q1);
            $i = 1;
            $query_join = '';
    ...
    ';
            ...
            $SommeSur = ['Hrs_Stagiaires', 'Encadrants', 'Hrs_Encadrants', 'Total', 'Réel'];
            break;
    ...
    ";
            ...
            if ($exp == '1conventions') {
                ...
                $where .= " and( e.e_convention is null or e.e_convention = '')";
            }
    ...
    ";
        ...
        ";
            if ($licences) {
                $select .= ",p.P_LICENCE 'Licence', p.ID_API 'Num API'";
            }
        ...
        case  $exp == 'presidents' or $exp == 'responsablesformations' or $exp == 'responsablesoperationnels' :
            if ($exp == 'presidents') {
                $pattern = 'Président (e)';
            } elseif ($exp == 'responsablesformations') {
        ...
        case 'montantactuel':
            $select = "concat('<a href=\"upd_section.php?from=export&status=cotisations&S_ID=',s.s_id,'\" target=_blank>',s.s_code,'</a>') 'Département',
                s.s_description 'Nom',";
            if ($syndicate == 1) {
                $select .= " tp.TP_DESCRIPTION 'Profession',";
            }
        ...
        case '2sommecotisations':
            ...
            $SommeSur = ['Nombre', 'Somme Cotisation nette'];
            break;
        ...
        case '2sommecotisationsprevues':
            ...
            if ($filter > 1) {
                $subtable1 .= (isset($list) ? ' and p_section in('.$list.') ' : '');
            }
            $subtable1 .= ')';
            ...
            $SommeSur = ['Nombre', 'Somme Cotisation prévue'];
            break;
        ...
        ";
            ...
            $orderby = 'Département, sf.s_code, p.p_nom, p.p_prenom ';
            $RuptureSur = ['Département'];
            $SommeSur = ['Rejeté', 'Régul.'];
            break;
        ...
        case  $exp == 'adhpayantparcheque' or $exp == 'adhpayantparvirement' or $exp == 'adhpayantparprelevement' :
            if ($exp == 'adhpayantparcheque') {
                $tp = 4;
            } elseif ($exp == 'adhpayantparvirement') {
                $tp = 2;
            } else {
                $tp = 1;
            }
        ...
        pc.montant 'Montant',
        date_format(pc.pc_date,'%d-%m-%Y') 'Date',";
            if ($exp == '1cotisationCheque') {
                $select .= " pc.num_cheque 'Numéro Chèque'";
            } else {
                $select .= " tp.TP_DESCRIPTION 'Payé par'";
            }
        ...
        ";
            ...
            $where = (isset($list) ? ' p.p_section in('.$list.') and' : '');
            if ($exp == 'adhtournee_off') {
                $where .= " g.G_CATEGORY='SP' and g.G_TYPE in ('officiers','service de santé') and ";
            }
            if ($exp == 'adhtournee_non_off') {
                $where .= " g.G_CATEGORY='SP' and g.G_TYPE in ('caporaux et sapeurs','sous-officiers') and ";
            }
            if ($exp == 'adhtournee_pats') {
                $where .= " (g.G_CATEGORY='PATS' or p.P_PROFESSION = 'PATS' ) and ";
            }
            $where .= ' p.p_section = sf.s_id ';
            $where .= " and ( p.P_STATUT <> 'SAL' or p.TS_CODE not in ('TC','TP','VNP')) and p.P_NOM <> 'admin'";
            $where .= " and p.P_OLD_MEMBER=0 and p.P_NOM <> 'admin'";
        ...
        case  $exp == 'nbadherentspardep' or $exp == 'nbadherentspardepS' :
            ...
            $where = ' p.P_OLD_MEMBER=0';
            if ($syndicate == 1) {
                $where .= ' and sf.NIV in(1,3)';
            } else {
                $where .= ' and sf.NIV in(0,1,3)';
            }
            $where .= (isset($list) ? ' and p.p_section in('.$list.') ' : '');
            $where .= ' and p.P_SECTION in (select S_ID from section where (S_PARENT=sf.S_ID or S_ID=sf.S_ID))';
            if ($syndicate == 1) {
                ...
                $where .= " and ( p.P_STATUT in ('SAL','BEN'))";
            }
            $where .= " and p.P_NOM <>'admin'";
            $groupby = ' sf.s_code';
            $orderby = ' sf.s_code';
        ...
        ";
            ...
            $where .= ' and p.P_OLD_MEMBER=0';
            $where .= ' and p.SUSPENDU=0';
            if ($exp == 'cordonneesAdherentsparGTetServicesansNPAI') {
                $where .= ' and p.NPAI=0';
            }
            $where .= " and ( p.P_STATUT <> 'SAL' or p.TS_CODE = 'ADH')";
            $where .= " and p.P_NOM <>'admin'";
    ...
    ";
            ...
            $orderby = ' e.te_code, e.eh_date_debut';
            $groupby = ' e.te_code, e.e_code';
            $SommeSur = ['Participants.', 'Vehicules.'];
            break;
        ...
        case '1tcd_activite_annee':
            ...
            $dbCols = mysqli_query($dbc, $sqlColonnes) or exit('Erreur :'.mysqli_error($dbc));
    ...
    ";
            ...
            $RuptureSur = ['Section'];
            $SommeSur = ['Heures prévues', 'Heures réalisées'];
            break;
            // -------------------
            // heures / personne
            // -------------------
    ...
    case 
        when count(distinct P.E_CODE) = 0 then 0
            ...
            $orderby = 'F.S_CODE';
            $groupby = 'F.S_CODE';
            $SommeSur = ['Nombre formations', 'Stagiaires'];
            break;
    ...
    count(distinct e.E_CODE) 'Evenements'";
        ...
        ";
            ...
            if (isset($list)) {
                ...
                if ($role > 0) {
                    $where .= '  or p.P_ID in ( select P_ID from section_role where S_ID in ('.$list.') and GP_ID='.$role.' ) ';
                }
                $where .= ' )';
            }
            $where .= " and p.p_old_member= 0 and p.P_STATUT <> 'EXT'";
    ...
    Z.Section";
        ...
        and o.p_section = s.s_id";
            ...
            if (isset($list)) {
                ...
                if ($role > 0) {
                    $table .= '  or o.P_ID in ( select P_ID from section_role where S_ID in ('.$list.') and GP_ID='.$role.' ) ';
                }
                $table .= ' )';
            }
    ...
    s.s_code 'Section'";
            ...
            if (isset($list)) {
                ...
                if ($role > 0) {
                    $where .= '  or o.P_ID in ( select P_ID from section_role where S_ID in ('.$list.') and GP_ID='.$role.' ) ';
                }
                $where .= ' )';
            }
            $orderby = 'p.type';
    ...
    count(*) 'Nombre'";
            ...
            if (isset($list)) {
                ...
                if ($role > 0) {
                    $select .= '  or o.P_ID in ( select P_ID from section_role where S_ID in ('.$list.') and GP_ID='.$role.' ) ';
                }
                $select .= ' )';
            }
    ...
    date_format(p.p_create_date,'%d-%m-%Y') 'Ajouté le'";
        ...
        case  $exp == 'horairesavalider' or $exp == '1horaires' :
            ...
            if ($exp == 'horairesavalider') {
                if ($syndicate == 0) {
                    $where .= " and hv.HS_CODE ='ATTV'";
                } else {
                    $where .= " and hv.HS_CODE in ('SEC','ATTV')";
                }
            }
            if ($exp == '1horaires') {
        ...
        case  $prefix == '1note' or $prefix == '1notN' :
            ...
            if ($suffix == 'REMB' and $assoc) {
                $export_name .= " (ou dons à l'association)";
            }
            ...
            if ($suffix != 'toutes') {
                if ($suffix == 'VAL') {
                    $where .= " and n.FS_CODE in ('VAL','VAL1')";
                } else {
                    $where .= " and n.FS_CODE='".$suffix."'";
                }
            }
            $where .= (isset($list) ? '  and s.S_ID in('.$list.') ' : '');
            $orderby = 'n.NF_CREATE_DATE desc';
            $SommeSur = ['montant '.$default_money_symbol];
            break;
        ...
        case  $exp == 'maincourantejour' or $exp == 'maincourantehier' :
           ...
           evenement e, section s, type_evenement te';
            if ($exp == 'maincourantejour') {
                $where = " date_format(el.EL_DEBUT,'%Y-%m-%d') = '".date('Y-m-d')."'";
            } else {
                $where = " date_format(el.EL_DEBUT,'%Y-%m-%d') = '".date('Y-m-d', $yesterday)."'";
            }
            ...
            $where .= (isset($list) ? '  and e.S_ID in('.$list.') ' : '');
            $groupby = 'e.E_CODE';
            $SommeSur = ['interventions ou messages', 'victimes'];
            break;
        ...
        case '1reports':
            check_all(14);
            if ($days_log > 0) {
                $comment = "<i class='fas fa-exclamation-triangle' style='color:orange;'></i> ce reporting contient les données des $days_log derniers jours";
            }
