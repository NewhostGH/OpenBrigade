...
if (isset($_GET['addmarker'])) {
    $addmarker = intval($_GET['addmarker']);
} else {
    $addmarker = 0;
}

if (isset($_GET['addflag'])) {
    $addflag = intval($_GET['addflag']);
} else {
    $addflag = 0;
}
...
$EVTLAT = $row['LAT'];
$EVTLNG = $row['LNG'];
if (isset($_SESSION['centerMapFor'])) {
    $centerMapFor = intval($_SESSION['centerMapFor']);
} else {
    $centerMapFor = 0;
}

if ($centerMapFor == $evenement) {
    if (isset($_SESSION['centerlat'])) {
        $LAT = $_SESSION['centerlat'];
    }
    if (isset($_SESSION['centerlng'])) {
        $LNG = $_SESSION['centerlng'];
    }
}
...
#equipesdiv{overflow:auto; height: 800px;margin-top:60px}
...
if (! $table_params) {
    ...
    <td><span class='ebrigade-h4'>".get_info_evenement($evenement).' </span></td>';

    $url = 'sitac_options.php?evenement='.$evenement.'&gps='.$gps.'&autorefresh='.$autorefresh.'&showlist='.$showlist;
    echo "<td><div class='noprint'>";
    echo write_modal($url, 'options_'.$evenement, "<i class='fa fa-cog fa-2x' title='Voir ou modifier les options de la carte'></i>");
    echo '</td>';
...
if ($nbrow == 0) {
    ...
    echo "<div class='row'>
    <div id='canvas' class='".$class."' style='width:100%;height:100%'></div>";
    echo write_modal($url."' style='position:absolute;margin-left:76%;margin-top:1%'", 'options_'.$evenement, "<input type='button' class='btn btn-primary' value='Options' style='position:absolute'>");
}

if ($showlist == 1) {
    echo "<div id='equipesdiv' align = right class='col' style='position: absolute;
    height: 600px;
    width: 315px;
    background-color: white;
    float: right;
    ...
    while (custom_fetch_array($result)) {
        if ($EE_ICON == '') {
            if ($assoc == 1) {
                $EE_ICON = 'images/sitac/protectioncivile.png';
            } else {
                $EE_ICON = 'images/gardes/GAR.png';
            }
        }
        if ($ELAT == '') {
            $ELAT = $LAT;
        ...
        $nb = $row2[0];
        if ($nb > 0) {
            $personnel = ' '.$nb." <i class='fa fa-user' title='".$nb." personnes' ></i>";
        } else {
            $personnel = '';
        }
        ...
        $nb2 = $row2[0];
        if ($nb2 > 0) {
            $vehicules = ' '.$nb2." <i class='fa fa-car' title='".$nb2." véhicules' ></i>";
        } else {
            $vehicules = '';
        }
        ...
        $result2 = mysqli_query($dbc, $query2);
        while (custom_fetch_array($result2)) {
            $inside .= "<br><i class='fa fa-user'></i> <a href=upd_personnel.php?pompier=".$P_ID." 
                    title='voir fiche personnel' class=small2>".strtoupper($P_NOM).' '.my_ucfirst($P_PRENOM).'</a> <span class=small2>'.$TP_LIBELLE.'</span>';
        }
        $inside .= "<br><input type=text class='noboxshadow' style=\"border:0;color:".$mydarkcolor.';font-size: 10px;width:270px;" id="address'.$EE_ID.'" value="'.$EE_ADDRESS.'" size=70 readonly>';
        ...
        echo "<div class='panel panel-default' style='width:300px;'>
                    ...
                    <div id='collapse".$EE_ID."' class='panel-collapse collapse' role='tabpanel' aria-labelledby='heading".$EE_ID."'>
                        <div class='panel-body' align=left class='left10'>".$inside.'</div>
                    </div>
            </div>';
    }
    echo '</div></table>';
...
while (custom_fetch_array($result)) {
    ...
    if (substr($EE_ICON, 0, 16) == 'images/vehicules') {
        $w = 60;
    } else {
        $w = 45;
    }
...
while ($row = @mysqli_fetch_array($result)) {
    ...
    if ($EL_ADDRESS == '') {
        $EL_ADDRESS = "Placez l'icône sur la carte";
    }
...
while ($row = @mysqli_fetch_array($result)) {
    ...
    if ($CAV_ADDRESS == '') {
        $CAV_ADDRESS = "Placez l'icône sur la carte";
    }
...
while ($row = @mysqli_fetch_array($result)) {
    ...
    if (strlen($COMMENT) > 25) {
        if (strlen($COMMENT) > 100) {
            $class = 'labelswhiteHuge';
        } else {
            $class = 'labelswhiteBig';
        }
        $iconflag = 'iconFlag2';
    } else {
        $class = 'labelswhite';
