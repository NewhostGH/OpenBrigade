...
if (isset($_POST['EO_TITLE'])) {
    $EO_TITLE = secure_input($dbc, $_POST['EO_TITLE']);
}
if (isset($_POST['EOG_TITLE'])) {
    $EOG_TITLE = secure_input($dbc, $_POST['EOG_TITLE']);
}
if (isset($_POST['EO_TYPE'])) {
    $EO_TYPE = secure_input($dbc, $_POST['EO_TYPE']);
}
if (isset($_POST['EO_COMMENT'])) {
    $EO_COMMENT = secure_input($dbc, $_POST['EO_COMMENT']);
}
if (isset($_POST['EO_ORDER'])) {
    $EO_ORDER = intval($_POST['EO_ORDER']);
} else {
    $EO_ORDER = 1;
}
if (isset($_POST['EOG_ORDER'])) {
    $EOG_ORDER = intval($_POST['EOG_ORDER']);
} else {
    $EOG_ORDER = 1;
}
...
$action_requiring_permissions = ['save_update', 'save_insert', 'delete', 'add_choice', 'del_choice', 'save_groupe_insert'];
...
if ($update_allowed) {
    if ($action == 'save_update' and $EO_TITLE != '') {
        ...
        $action = 'display';
    } elseif ($action == 'add_choice' and $option > 0) {
        if (isset($_GET['val'])) {
            $val = intval($_GET['val']);
    ...
    } elseif ($action == 'del_choice' and $option > 0) {
        if (isset($_GET['choix'])) {
...
if ($update_allowed) {
    echo "<form action='evenement_options.php' method='POST'>";
}
$num = 0;
...
elseif ($action == 'insert') {
    ...
    echo '<div align=center>';
    if ($update_allowed) {
        echo " <input type=submit class='btn btn-success' name='OK' value='Sauvegarder'>";
    }
...
elseif ($action == 'display') {
    ...
    if ($num == 0) {
        echo "<span class=small>Aucune option n'a été créée</span><p>";
    } else {
        ...
        while (custom_fetch_array($resultm)) {
            if ($evenement == $renfort) {
                ...
                $complement = ' and P_ID in (select P_ID from evenement_participation where E_CODE='.$renfort.')';
            }
            ...
            } elseif ($EO_TYPE == 'text' or $EO_TYPE == 'date' or $EO_TYPE == 'hour') {
                if ($EO_TYPE == 'text') {
                    $type = 'Texte libre';
                } elseif ($EO_TYPE == 'date') {
                    $type = 'Date';
...
if ($action == 'display') {
    ...
    if ($num > 0) {
        ...
        echo write_modal($url, 'formulaire_'.$evenement, "<input type='submit' class='btn btn-secondary' id='apercu' value='Aperçu' title=\"voir un aperçu du formulaire proposé à celui qui s'inscrit\">");
    }
    echo '</div>';
}
writefoot();
