...
if (check_rights($id, 71, "$S_ID")) {
    $responsable_consommable = true;
} else {
    $responsable_consommable = false;
}
...
if (isset($_GET['C_NOMBRE'])) {
    $C_NOMBRE = intval($_GET['C_NOMBRE']);
}
if (isset($_GET['C_MINIMUM'])) {
    $C_MINIMUM = intval($_GET['C_MINIMUM']);
}
if (isset($_GET['C_DESCRIPTION'])) {
    $C_DESCRIPTION = ($_GET['C_DESCRIPTION']);
}
if (isset($_GET['S_ID'])) {
    $S_ID = intval($_GET['S_ID']);
}
if (isset($_GET['C_DATE_ACHAT'])) {
    $C_DATE_ACHAT = $_GET['C_DATE_ACHAT'];
}
if (isset($_GET['C_DATE_PEREMPTION'])) {
    $C_DATE_PEREMPTION = $_GET['C_DATE_PEREMPTION'];
}
if (isset($_GET['C_LIEU_STOCKAGE'])) {
    $C_LIEU_STOCKAGE = $_GET['C_LIEU_STOCKAGE'];
}
...
while ($row = @mysqli_fetch_array($result2)) {
    ...
    if ($NEWTC_QUANTITE_PAR_UNITE > 1) {
        $NEWTUM_DESCRIPTION .= 's';
    }
    ...
    if ($NEWTCO_CODE == 'PE') {
        $label = $NEWTC_DESCRIPTION.' ('.$NEWTUM_DESCRIPTION.'s)';
    } elseif ($NEWTUM_CODE != 'un' or $NEWTC_QUANTITE_PAR_UNITE != 1) {
        $label = $NEWTC_DESCRIPTION.' ('.$NEWTCO_DESCRIPTION.' '.$NEWTC_QUANTITE_PAR_UNITE.' '.$NEWTUM_DESCRIPTION.')';
    } else {
        $label = $NEWTC_DESCRIPTION;
    }
...
if ($TC_PEREMPTION == 1) {
    ...
    echo "<input type='hidden' name='C_DATE_PEREMPTION' id='C_DATE_PEREMPTION' value=''>";
}
...
} elseif ($from == 'evenement' or $from == 'lot') {
