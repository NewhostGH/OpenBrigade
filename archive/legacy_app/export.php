...
if (isset($table) && isset($select) && isset($_GET['show'])) {
    ...
    if (! $result) {
        if (strpos(mysqli_error($dbc), "Incorrect key file for table '/tmp") !== false) {
            $errmsg = 'Veuillez re-essayer avec une plage de dates plus petite.';
        } else {
            $errmsg = "L'erreur suivante est apparue: <p><span style='color:red;font-family:Courier;'>".strip_tags(mysqli_error($dbc)).'</span><p> Veuillez transmettre ce message à votre administrateur.';
        }
...
// includes
if (substr($exp, 0, 4) == 'tcd_' && in_array($affichage, ['xls'])) {
...
if ($affichage == 'ecran') {
    ...
    if (isset($_GET['exp']) and isset($_GET['show'])) {
        ...
        if (substr($exp, 0, 1) == 1) {
            $param = 'du '.$dtdb.' au '.$dtfn;
        } elseif (substr($exp, 0, 1) == 2) {
            $param = 'année '.$yearreport;
        } else {
            $param = '';
        }
