...
if ($parent > 0) {
    $grand_parent = get_parent_folder($parent, 1);
} else {
    $grand_parent = 0;
}
...
if (check_rights($id, 47, "$filter")) {
    $granted_documentation = true;
} else {
    $granted_documentation = false;
}
...
if ($search != '') {
    $query .= " and d.D_NAME like '%".$search."%'";
} else {
    $query .= ' and d.DF_ID = '.$dossier;
}
if ($td != 'ALL') {
    $query .= " and d.TD_CODE = '".$td."'";
}
if ($yeardoc != 'all') {
    $query .= " and YEAR(d.D_CREATED_DATE) = '".$yeardoc."'";
}
...
if ($search != '') {
    $query .= " and df.DF_NAME like '%".$search."%'";
} else {
    $query .= ' and df.DF_PARENT = '.$dossier;
}
if ($td != 'ALL') {
    $query .= " and df.TD_CODE = '".$td."'";
}
if ($yeardoc != 'all') {
    $query .= " and YEAR(df.DF_CREATED_DATE) = '".$yeardoc."'";
}
$result = mysqli_query($dbc, $query);
$nb = mysqli_num_rows($result);
...
if ($number > 0) {
    ...
    for ($i = $low; $i < $high; $i++) {
        ...
        if ($granted_documentation) {
            echo write_modal($url, 'doc_'.$is_folder[$i].'_'.$id_arr[$i], $t_lib_arr[$i]);
        } elseif ($t_lib_arr[$i] != 'choisir') {
            echo $t_lib_arr[$i];
        }
        ...
        if ($document_security == 1) {
            if ($s_arr[$i] > 1) {
                $img = "<i class='fa fa-lock' style='color:#f64e60;' title=\"".$s_lib_arr[$i].'" ></i>';
            } else {
                $img = "<i class='fa fa-unlock' style='color:#1bc5bd' title=\"".$s_lib_arr[$i].'"></i>';
            }
            if (! $is_folder[$i]) {
                if ($granted_documentation) {
                    echo write_modal($url, 'doc_'.$is_folder[$i].'_'.$id_arr[$i], "<span class='btn btn-default btn-action' style='float:left'>$img</span>");
                } else {
                    echo $img;
                }
            }
...
if ($number == 0) {
    echo '<p align=center><i>Aucun document trouvé</i>';
}
