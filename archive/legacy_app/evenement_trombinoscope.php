...
while ($row2 = mysqli_fetch_array($result2)) {
    ...
    if ($S_ID2 != $prev) {
        if ($S_ID2 != $S_ID) {
            $renfort = 'Renfort de '.$S_CODE2.' - '.$S_DESCRIPTION2;
        } else {
            $renfort = 'Organisateur '.$S_CODE2.' - '.$S_DESCRIPTION2;
        }
    ...
    if ($c == 1) {
        $html .= '<tr><td align=center width=40><td>';
    }
    ...
    if ($with_nationality and $PAYS != '') {
        $html .= '<br><span class=small>Nationalité: '.$PAYS.'</span>';
    }
