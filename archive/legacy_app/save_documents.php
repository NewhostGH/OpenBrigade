...
function redirect(section, from, eid) {
    if(from == 'materiel'){
        url='upd_materiel.php?mid='+eid+'&tab=2';
    }
    else if(from == 'vehicule'){
        url='upd_vehicule.php?vid='+eid+'&tab=2';
    }
    else
...
if (isset($_POST['vehicule'])) {
    $eid = intval($_POST['vehicule']);
} elseif (isset($_POST['materiel'])) {
    $eid = intval($_POST['materiel']);
} else {
    $eid = 0;
}
...
if (isset($_FILES['userfile'])) {
    ...
    for ($i = 0; $i < count($_FILES['userfile']['name']); $i++) {
        if (check_rights($id, 47, "$S_ID")) {
            ...
            if ($dossier > 0) {
                $upload_dir = $filesdir.'/files_section/'.$S_ID.'/'.$dossier.'/';
            } else {
            ...
            if ($error == 0) {
                ...
                <input type='button' class='btn btn-secondary' value='Retour' onclick='javascript:history.back(1);'>", 10, 0);
                exit;
            }
            $operation = 'retour';
        }
    ...
    if ($isfolder == 0) {
        ...
        if ($oldfolder != $parentfolder) { // let's move
            ...
            if ($oldfolder == 0) {
                ...
                $source .= '/'.$oldfolder.'/'.$filename;
            }
            $dest = $filesdir.'/files_section/'.$S_ID;
            if ($parentfolder == 0) {
                $dest .= '/'.$filename;
            } else {
                $dest .= '/'.$parentfolder.'/'.$filename;
            }
            @rename($source, $dest);
        }
...
echo "<body onload=\"redirect('$S_ID', '$from', $eid);\">";
writefoot();
