...
function table_remplacements($evenement, $status, $date1, $date2, $replaced = 0, $substitute = 0, $section = 0, $show_add_button = 0)
{
    ...
    if (intval($evenement) > 0) {
        $query .= ' and r.E_CODE='.$evenement;
    }
    if ($status == 'VAL') {
        $query .= ' and r.APPROVED=1';
    } elseif ($status == 'REJ') {
        $query .= ' and r.REJECTED=1';
    } elseif ($status == 'ACC') {
        $query .= ' and r.ACCEPTED=1 and r.APPROVED=0 and r.REJECTED=0';
    } elseif ($status == 'DEM') {
        $query .= ' and r.ACCEPTED=0 and r.APPROVED=0 and r.REJECTED=0';
    } elseif ($status == 'ATT') {
        $query .= ' and r.APPROVED=0 and r.REJECTED=0';
    }
    if ($date1 != '') {
        $tmp = explode('-', $date1);
        $month1 = $tmp[1];
    ...
    if ($substitute > 0) {
        $query .= ' and r.SUBSTITUTE = '.$substitute;
    }
    if ($replaced > 0) {
        $query .= ' and r.REPLACED = '.$replaced;
    }
    if ($nbsections == 0 and intval($section) > 0) {
        $query .= ' and e.S_ID in ('.get_family(intval($section)).')';
    }
    ...
    if ($nbR > 0) {
        ...
        while ($row = @mysqli_fetch_array($result)) {
            ...
            if ($grade_replaced != '') {
                $grade_replaced = '<img src='.$grades_imgdir.'/'.$grade_replaced.".png style='PADDING:1px;' class='img-max-20'>";
            }
            $substitute = strtoupper($row['n2']).' '.my_ucfirst($row['p2']);
            $grade_substitute = $row['g2'];
            if ($grade_substitute != '') {
                $grade_substitute = '<img src='.$grades_imgdir.'/'.$grade_substitute.".png style='PADDING:1px;' class='img-max-20'>";
            }
            ...
            if ($SECTION_JOUR != 0) {
                $img = "<small><i class='badge badge".$SECTION_JOUR."' title='section $SECTION_JOUR'>".$SECTION_JOUR.'</i></small>';
            } else {
                $img = '';
            }
            ...
            if ($gardes) {
                $H .= "<td class='widget-text'>".$periode.'</td>';
            }
    ...
    if ($evenement > 0) {
        ...
        if ($assoc or $army) {
            $t = "l'activité concernée";
        } else {
            $t = 'la garde concernée';
        }
...
function get_garde_jour($section = 0, $eqid = 0, $date = 0)
{
    ...
    if ($eqid == 0) {
        ...
        if ($nbsections == 0) {
            $query .= ' where S_ID = '.intval($filter);
        }
        $query .= ' )';
    } else {
...
function get_inscrits_garde($evenement, $partie = 0, $status = '')
{
    ...
    if ($status != '') {
        if ($status == 'BEN') {
            $query .= " and p.P_STATUT in ('BEN','SAL')";
        } elseif ($status == 'SPV') {
            $query .= ' and ep.EP_FLAG1 = 0';
        } // prise en compte des gardes SPV par un SPP
        elseif ($status == 'SPP') {
            $query .= ' and ep.EP_FLAG1 = 1';
        } // prise en compte des gardes SPV par un SPP
        else {
...
function display_subgroup($status, $T, $year, $month, $day, $evenement, $section_jour, $section_nuit, $centre = 0)
{
    ...
    // $T= J, N, A, I
    if ($T == 'J') {
        $comment = 'de la section du jour';
    } elseif ($T == 'N') {
        $comment = 'de la section de nuit';
    } elseif ($T == 'A') {
        $comment = 'des autres sections';
    } elseif ($T == 'I' and $disponibilites) {
    ...
    while ($row1 = @mysqli_fetch_array($result1)) {
        ...
        foreach ($tmp_array as $pid) {
            if (intval($pid) > 0) {
                array_push($inscrits_demain, $pid);
            }
        }
    }
    ...
    if ($status == 'SPP') {
        ...
        if ($regime > 0) {
            if ($T == 'J') {
                $query .= ' and p.P_SECTION='.$section_jour;
            } elseif ($T == 'N') {
                $query .= ' and p.P_SECTION='.$section_nuit;
            } else {
        ...
        if ($regime > 0) {
            if ($T == 'J') {
                ...
                $query .= ' and p.P_ID in (select section_role.P_ID from section_role where GP_ID = '.get_specific_outside_role().' and S_ID = ('.get_section_parent($section_jour).'))';
            } else {
                $query .= ' and p.P_SECTION not in ('.$section_jour.','.$section_nuit.')';
                $query .= ' and p.P_SECTION in ('.get_family(get_section_parent($section_jour)).','.get_family(get_section_parent($section_nuit)).')';
            }
    ...
    if ($nb > 0) {
        if ($status == 'SPP') {
            $status_long = 'Personnel Professionnel';
        } elseif ($status == 'SPV') {
            $status_long = 'Personnel Volontaire';
        } else {
            $status_long = 'Personnel';
        }
        if ($T == 'I') {
            $ti = $status_long.' '.$comment;
        } elseif ($regime > 0) {
            $ti = $status_long.' '.$comment;
        } elseif ($status == 'SPP') {
            $ti = $status_long;
        } else {
            $ti = $status_long.' disponible';
        }
        if ($nb_parties == 2) {
            $col = 7;
        } else {
    ...
    while ($row = @mysqli_fetch_array($result)) {
        ...
        if ($P_STATUT == 'SPP' and $status == 'SPV') {
            $class = 'SPPV';
        } else {
            $class = $P_STATUT;
        }
        ...
        if ($P_STATUT == 'SPP' and $status != 'SPV') {
            $regime = "<span class=small title='Régime de travail $P_REGIME'>(".$P_REGIME.')</span>';
        } else {
            $regime = '';
        }
        ...
        if ($nb_heures == 0) {
            ...
            $nb_heures = "<span class=badge title=\"Nombre d'heures de gardes attribuées ce mois\">".$nb_heures.'</span>';
        }
        if ($DISPO == 30) {
            $check24 = 1;
        } else {
    ...
    foreach ($old_inscrits as $pid) {
        if (intval($pid) > 0) {
            if (! in_array($pid, $new_inscrits)) {
                ...
                if ($pid_statut == 'SPP' and $show_spp == 1) {
                    $nb = 1;
                } elseif ($pid_statut != 'SPP' and $show_indispos == 1) {
                    $nb = 1;
                } elseif ($pid_statut != 'SPP') {
...
function dispo_label($DISPO)
{
    ...
    // 12h
    elseif ($DISPO == 25) {
        $label = ' - disponible 12h nuit';
    } elseif ($DISPO == 5) {
        $label = ' - disponible 12h jour';
    } elseif ($DISPO == 10) {
        $label = ' - disponible matin et soir';
    } elseif ($DISPO == 13) {
        $label = ' - disponible après-midi et soir';
    } elseif ($DISPO == 17) {
        $label = ' - disponible matin et nuit';
    } elseif ($DISPO == 20) {
        $label = ' - disponible après-midi et nuit';
    }
    // 6h
    elseif ($DISPO == 1) {
        $label = ' - disponible matin seulement';
    } elseif ($DISPO == 4) {
        $label = ' - disponible après-midi seulement';
    } elseif ($DISPO == 9) {
        $label = ' - disponible soir seulement';
    } elseif ($DISPO == 16) {
        $label = ' - disponible nuit seulement';
    }
    // 18h
    elseif ($DISPO == 14) {
        $label = ' - disponible matin, après-midi et soir';
    } elseif ($DISPO == 21) {
        $label = ' - disponible matin, après-midi et nuit';
    } elseif ($DISPO == 26) {
        $label = ' - disponible matin, soir et nuit';
    } elseif ($DISPO == 29) {
        $label = ' - disponible après-midi, soir et nuit';
    } else {
        $label = '';
    }
...
function get_horaire($P_ID, $E_CODE)
{
    ...
    $RETURN = [$HORAIRE, $SP_TOTAL_TIME_PARTICIPATION_GARDE];
...
function display_postes($evenement, $vehicule, $showjour = true, $shownuit = true, $print_mode = false)
{
    ...
    if ($showjour) {
        $html .= '<td align=center><b>Jour</b></td>';
    }
    if ($shownuit) {
        $html .= '<td align=center><b>Nuit</b></td>';
    }
    $html .= '</tr>';
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        if ($grades) {
            $grade = '<img src='.$grades_imgdir.'/'.$P_GRADE.".png title='".$P_GRADE."' class='img-max-18'>";
        } else {
            $grade = '';
        }
        if ($P_ID == 0) {
            if ($print_mode) {
                $printName = '<span class="printable" style="color:darkgrey">'.ucfirst($ROLE_NAME).'</span> <span class="printable" style="color:darkgrey">'.$cmt.'</span>';
            } else {
                $name = "<span style='color:darkgrey'>".ucfirst($ROLE_NAME)."</span> <span style='color:darkgrey' class='hide_mobile'>".$cmt.'</span>';
            }
        } else {
            ...
            if (Gard24($P_ID) == true) {
                ...
                if ($EH_ID == 1) {
                    $periodeId = 1;
                    $font = "<span class='hide_mobile'><i style='float:right'class='fas fa-sun'></i></span>";
                } else {
                    $periodeId = 2;
                    $font = "<span class='hide_mobile'><i style='float:right' class='fas fa-moon'></i></span>";
                }
            }
...
{
    global $dbc, $list;
    static $pompierVeh = []; // statique pour incrémenter son contenu à chaque appel
    static $tabOccAffect; // tableau nombres d'affectation pour chaque pompier
    $listPiquet = []; // les pompiers déja affectés dans le même vehicule et la même periode
    ...
    $possiblePompTri = []; // Trier la liste des pompiers possibles selon le nombre d'affectation
        ...
        foreach ($possiblePompTri as $rang => $id) {// il a le nombre d'affectaion minimal
            if (isset($personnel[$periode][$id])) {// il a la même periode que le poste vacant
                if ($periode == 1) {
                    ...
                    $periodeCom = 1;
                }// préparation pour l'affectation de la periode complémentaire si le pompier est de garde 24h
                if (($PS_ID > 0) && (isset($comps[$id][$PS_ID])) || ($PS_ID == 0)) {// il a la compétence du poste vacant
                    ...
                    if (! findVehicule($pompierVehicules, $value, $TV_ID_vehicule)) {
                        $pompierVeh[$id] = $TV_ID_vehicule;
                    }// mettre à jours les informations concernant les vehicules auxquels le personnel est déja affecté
...
Vous pouvez voir le détail sur '.$cisname;
    if (@$_SERVER['HTTP_HOST'] != '127.0.0.1') {
        mysendmail2($email,$Subject,$Mailcontent,$SenderName,$_SESSION['SES_EMAIL'],$Attachment = 'None');
    }
}
