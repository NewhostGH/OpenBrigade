...
if ($numcav > 0) {
    ...
    if (check_rights($id, 15, (get_section_organisatrice($evenement)))) {
        $granted_update = true;
    }
}
...
if (isset($_POST['numcav']) and ($action == 'update' or $action == 'insert') and $granted_update) {
    ...
    $CAV_RESPONSABLE = intval($_POST['responsable']);
    if ($CAV_RESPONSABLE == 0) {
        $CAV_RESPONSABLE = 'null';
    }
    ...
    if (isset($_POST['ouvert'])) {
        $CAV_OUVERT = intval($_POST['ouvert']);
    } else {
        $CAV_OUVERT = 0;
    }
...
if (mysqli_num_rows($result) > 0) {
    ...
    if ($action != 'delete') {
        echo 'Centre accueil victmes non trouvé';
    }
    exit;
}
...
echo "<tr><td>Centre ouvert $asterisk</td>";
echo "<td><label class='switch'><input name='ouvert' id='ouvert' type=checkbox title='Cochez si le centre est ouvert' value='1' $disabled  $checked>
...
echo "<tr >
            <td><i>Commentaires</i><br>
                <input type='text' class='form-control form-control-sm' name='comptage' size='4' value='$textsize' readonly style='FONT-SIZE: 10pt;border:0px; font-weight:bold; width:50px'><br>
                <span class=small>500 max</td>
            <td colspan=3>
            <textarea name='commentaire' cols='40' rows='5' class='form-control form-control-sm'
...
if ($rowm['NB'] == 1 and $geolocalize_enabled == 1) {
    $map = '<a href=sitac.php?evenement='.$evenement." target=_blank><i class='fa fa-map fa-lg' style='color:green;' title='Voir la carte Google Maps' border=0></i></a>";
} else {
    $map = '';
}
...
if ($granted_update) {
    ...
    if ($numcav > 0) {
        echo " <input type='button'  class='btn btn-default' value='supprimer' onclick=\"deleteCav('".$numcav."');\">";
    }
}
if ($from == 'map') {
