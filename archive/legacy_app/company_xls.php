...
// search field
$possibleorders = ['TC_LIBELLE', 'C_NAME', 'S_CODE', 'C_DESCRIPTION', 'C_DESCRIPTION', 'C_PARENT'];
...
// columns
$columns_title = ['Id', 'Type', 'Nom', 'Description', 'Section',
...
if ($typecompany != 'ALL') {
    $query1 .= " AND c.TC_CODE='".$typecompany."'";
}
...
while (custom_fetch_array($result1)) {
    if (intval($MED) > 0) {
        $MED = get_prenom_nom($MED);
    }
    if (intval($RF) > 0) {
        $RF = get_prenom_nom($RF);
    }
    if (intval($RO) > 0) {
        $RO = get_prenom_nom($RO);
    }
    $columns_data = [$C_ID, $TC_LIBELLE, $C_NAME, $C_DESCRIPTION, $S_CODE,
