...
if ($pid != $id) {
    if ($P_STATUT == 'EXT') {
        check_all(37);
        if (! check_rights($id, 37, "$P_SECTION")) {
            check_all(24);
        }
    } else {
        check_all(2);
...
if ($id == $pid) {
    ...
    $text = 'Préférences de '.$P_PRENOM.' '.$P_NOM;
}
...
if ($tab == 2) {
    if (isset($_GET['saved'])) {
        ...
        if ($errcode == 'nothing') {
            echo "<div class='alert alert-info' role='alert'> Aucun changement à sauver.</div></div><p>";
        } elseif ($errcode == 0) {
            echo "<div class='alert alert-success' role='alert'> Préférences utilisateur sauvées.</div></div><p>";
        } else {
            echo "<div class='alert alert-danger' role='alert'> Erreur lors de la sauvegarde des préférences utilisateur.</div></div><p>";
        }
    }
    ...
    while ($row = @mysqli_fetch_array($result)) {
        ...
        if ($ID == 1) {
            ...
            if ($user_preferences[$ID] == 'FR') {
                $selected = 'selected';
            } else {
                $selected = '';
            }
            echo "<option value='FR' ".$selected.'>Français</option>';
            if ($user_preferences[$ID] == 'EN') {
                $selected = 'selected';
            } else {
                $selected = '';
            }
            ...
            echo "<td></td><tr $style><td colspan=2><select id='f$ID' name='f$ID' class='selectpicker smallcontrol2' data-container='body'>";
            if ($user_preferences[$ID] == 'hierarchique') {
                $selected = 'selected';
            } else {
                $selected = '';
            }
            echo "<option value='hierarchique' ".$selected.'>Ordre hiérarchique</option>';
            if ($user_preferences[$ID] == 'alphabetique') {
                $selected = 'selected';
            } else {
                $selected = '';
            }
    ...
    while ($row = @mysqli_fetch_array($result)) {
        ...
        if ($ID == 13 and $gardes == 0) {
            continue;
        } elseif ($ID == 10 and $disponibilites == 0) {
            continue;
        } elseif ($ID == 11 and $evenements == 0) {
            continue;
        } elseif ($ID == 12 and $evenements == 0) {
            continue;
        } else {
...
if ($tab == 3) {
    ...
    if (mysqli_num_rows($result) == 0) {
        ...
        $resultstat = mysqli_fetch_array($result)[0];
    }
    if ($resultstat == 1) {
        $checked = 'checked';
        $statvalue = 0;
...
if ($tab == 1) {
    ...
    if ($id != $pid) {
        check_all(2);
        if (! check_rights($id, 2, get_section_of("$pid"))) {
            check_all(24);
        }
    }
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        if (check_rights($pid, $F_ID) == 1) {
            ...
            $checkbox = "<label class='switchconfig' style='margin-left:auto;margin-right:10px'>
