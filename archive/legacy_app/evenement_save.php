...
if (isset($_GET['action']) or isset($_POST['action'])) {
    ...
    write_msgbox('ERREUR', $error_pic, "Une erreur est apparue<br>Veuillez recommencer.<p align=center><a href='evenement_choice.php' target='_self'><input type='submit' class='btn btn-secondary' value='Retour'></a> ", 10, 0);
    exit;
}
...
if (isset($_POST['copycheffrom'])) {
    $copycheffrom = intval($_POST['copycheffrom']);
} else {
    $copycheffrom = 0;
}
if (isset($_POST['copydetailsfrom'])) {
    $copydetailsfrom = intval($_POST['copydetailsfrom']);
} else {
    $copydetailsfrom = 0;
}
if (isset($_POST['copymode'])) {
    $copymode = $_POST['copymode'];
} else {
    $copymode = 'simple';
}
...
if (isset($_POST['open_to_ext'])) {
    $open_to_ext = intval($_POST['open_to_ext']);
} else {
    $open_to_ext = 0;
}
if (isset($_POST['allow_reinforcement'])) {
    $allow_reinforcement = intval($_POST['allow_reinforcement']);
} else {
...
if (isset($_POST['nb_vpsp'])) {
    $nb_vpsp = intval($_POST['nb_vpsp']);
} else {
    $nb_vpsp = 'null';
}
if (isset($_POST['nb_autres_vehicules'])) {
    $nb_autres_vehicules = intval($_POST['nb_autres_vehicules']);
} else {
    $nb_autres_vehicule = 'null';
}
if (isset($_POST['canceled'])) {
    $canceled = intval($_POST['canceled']);
} else {
    $canceled = 0;
}
if (isset($_POST['flag1'])) {
    $flag1 = intval($_POST['flag1']);
} else {
    $flag1 = 0;
}
if (isset($_POST['colonne'])) {
    $colonne = intval($_POST['colonne']);
} else {
    $colonne = 0;
}
if ($colonne == 1) {
    $allow_reinforcement = 1;
}
if (isset($_POST['visible_outside'])) {
    $visible_outside = intval($_POST['visible_outside']);
} else {
    $visible_outside = 0;
}
if (isset($_POST['mail1'])) {
    $mail1 = intval($_POST['mail1']);
} else {
    $mail1 = 0;
}
if (isset($_POST['mail2'])) {
    $mail2 = intval($_POST['mail2']);
} else {
    $mail2 = 0;
}
if (isset($_POST['mail3'])) {
    $mail3 = intval($_POST['mail3']);
} else {
    $mail3 = 0;
}
...
if (isset($_POST['contact_name'])) {
    $contact_name = secure_input($dbc, $_POST['contact_name']);
} else {
    $contact_name = '';
}
if (isset($_POST['contact_tel'])) {
    $contact_tel = secure_input($dbc, $_POST['contact_tel']);
} else {
    $contact_tel = '';
}
if (isset($_POST['e_tel'])) {
    $e_tel = secure_input($dbc, $_POST['e_tel']);
} else {
    $e_tel = '';
}
if (isset($_POST['whatsapp_group'])) {
    $whatsapp_group = secure_input($dbc, str_replace('"', '', $_POST['whatsapp_group']));
} else {
    $whatsapp_group = '';
}
$whatsapp_group = str_replace($whatsapp_chat_url.'/', '', $whatsapp_group);
$whatsapp_group = str_replace(' ', '', $whatsapp_group);
$whatsapp_group = str_replace('.', '', $whatsapp_group);
$whatsapp_group = str_replace('-', '', $whatsapp_group);

if (isset($_POST['webex_url'])) {
    $webex_url = secure_input($dbc, str_replace('"', '', $_POST['webex_url']));
} else {
    $webex_url = '';
}
$webex_url = str_replace(' ', '', $webex_url);

if (isset($_POST['webex_pin'])) {
    $webex_pin = secure_input($dbc, str_replace('"', '', $_POST['webex_pin']));
} else {
    $webex_pin = '';
}
$webex_pin = str_replace(' ', '', $webex_pin);

if (isset($_POST['webex_start'])) {
    $webex_start = secure_input($dbc, $_POST['webex_start']);
} else {
    $webex_start = '';
}
if ($webex_start == '') {
...
if (isset($_POST['cancel_detail'])) {
    $cancel_detail = secure_input($dbc, str_replace('"', '', $_POST['cancel_detail']));
} else {
    $cancel_detail = '';
}
...
if (isset($_POST['exterieur'])) {
    $exterieur = intval($_POST['exterieur']);
} else {
    $exterieur = '0';
}
if (isset($_POST['url'])) {
    $url = secure_input($dbc, $_POST['url']);
} else {
    $url = '';
}
$url = str_replace('"', '', $url);
...
if (isset($_POST['autoclose'])) {
    $autoclose = intval($_POST['autoclose']);
} else {
    $autoclose = '-1';
}
...
function attention_evenement_interdit($section, $type)
{
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        if ($row['SSE_BY'] != '') {
            $demande = '<i>A la demande de '.$P_PRENOM.' '.$P_NOM.'</i><br>';
        } else {
            $demande = '';
        }
        if ($TE_CODE == 'ALL') {
...
if ($action == 'delete') {
    check_all(19);
    if (! check_rights($id, 19, get_section_organisatrice($evenement))) {
        check_all(24);
    }
...
if ($action == 'document') {
    ...
    if (isset($_FILES['userfile'])) {
        for ($i = 0; $i < count($_FILES['userfile']['name']); $i++) {
            ...
            [$file_name, $error, $msgstring] = explode(';', $upload_result);
            if ($error > 0) {
                write_msgbox('ERREUR', $error_pic, $msgstring."<br><p align=center>
                            <a onclick=\"javascript:history.back(1);\"><input type='submit' class='btn btn-secondary' value='Retour'></a> ", 10, 0);
                exit;
            } elseif ($file_name != '') {
        ...
        $query = 'select count(1) as NB from document where E_CODE='.$evenement.' and D_NAME="'.$doc.'"';
        $result = mysqli_query($dbc, $query);
        $row = mysqli_fetch_array($result);
...
if ($action != 'delete' and $action != 'document') {
    ...
    if (isset($_POST['show_hide_option']) and check_rights($id, 9)) {
        $show_hide_option = intval($_POST['show_hide_option']);
    } else {
        $show_hide_option = 0;
    }

    $type = secure_input($dbc, $_POST['type']);
    if (isset($_POST['type_garde']) and $type == 'GAR') {
        $type_garde = intval($_POST['type_garde']);
    } else {
        $type_garde = 0;
    }
    ...
    if (isset($_POST['consignes'])) {
        $consignes = strip_tags(secure_input($dbc, str_replace('"', '', $_POST['consignes'])));
    } else {
        $consignes = '';
    }
    if (isset($_POST['custom_horaire'])) {
        $custom_horaire = strip_tags(secure_input($dbc, str_replace('"', '', $_POST['custom_horaire'])));
    } else {
        $custom_horaire = '';
    }
    if (isset($_POST['representant_legal'])) {
        $representant_legal = strip_tags(secure_input($dbc, str_replace('"', '', $_POST['representant_legal'])));
    } else {
        $representant_legal = '';
    }
    ...
    if (isset($_POST['date_envoi_convention'])) {
        if ($_POST['date_envoi_convention'] != '') {
            $tmp = explode('-', $_POST['date_envoi_convention']);
            $month1 = $tmp[1];
            $day1 = $tmp[0];
            $year1 = $tmp[2];
    ...
    if ($colonne == 1 and $j > 1) {
        write_msgbox('erreur', $error_pic, "Une colonne de renfort ne peut avoir que une partie.<br><p align=center><input type=submit  class='btn btn-secondary' value='Retour' onclick='javascript:history.back(1);'> ", 10, 0);
        echo '';
        exit;
    }
    ...
    if ($action == 'update') {
        ...
        $query = "update evenement set
            ...
            E_TEL='".$e_tel."',
            E_WHATSAPP=\"".$whatsapp_group.'",
            E_WEBEX_URL="'.$webex_url.'",
            E_WEBEX_PIN="'.$webex_pin.'",
            E_WEBEX_START='.$webex_start.',
            PS_ID='.$ps.",
            TF_CODE='".$tf."',
            E_EQUIPE=".$type_garde.',';
        if ($parent != 'null') {
        ...
        $query = 'insert into  evenement (E_CODE, TE_CODE, S_ID, E_LIBELLE, E_LIEU, E_LIEU_RDV, E_HEURE_RDV, E_NB, E_COMMENT, E_COMMENT2, E_CLOSED, E_CANCELED, E_FLAG1, E_CANCEL_DETAIL, 
                ...
                PS_ID, TF_CODE, E_TEL, E_WHATSAPP,E_WEBEX_URL, E_WEBEX_PIN, E_WEBEX_START,E_AUTOCLOSE_BEFORE,E_EQUIPE)
        ...
        $result = mysqli_query($dbc, $query) or exit('Erreur création événement');
    ...
    if ($canceled == 1) {
        ...
        // et rejeter les demandes de remplacements
        $query = 'update remplacement set REJECTED = 1, REJECT_DATE=NOW(), REJECT_BY ='.$id.'
                where APPROVED = 0 and REJECTED = 0
                and E_CODE in ( select E_CODE from evenement where E_CODE = '.$evenement.' or E_PARENT = '.$evenement.')';
        $result = mysqli_query($dbc, $query);
    }
    ...
    if ($action == 'create' or $action == 'copy' or $action == 'renfort') {
        ...
        if ($action == 'copy') {
            if (isset($_POST['vehicules'])) {
                $vehicules = intval($_POST['vehicules']);
            } else {
                $vehicules = 0;
            }
            if (isset($_POST['materiel'])) {
                $materiel = intval($_POST['materiel']);
            } else {
                $materiel = 0;
            }
            if (isset($_POST['personnel'])) {
                $personnel = intval($_POST['personnel']);
            } else {
                $personnel = 0;
            }
            ...
            if (isset($_POST['renforts'])) {
                $renforts = intval($_POST['renforts']);
            } else {
                $renforts = 0;
            }

            if ($copydetailsfrom != 0) {
            ...
            if ($options == 1) {
                $query = 'insert into evenement_option (E_CODE, EO_TITLE, EO_COMMENT, EO_TYPE, EOG_ID, EO_ORDER)
                       select '.$evenement.', EO_TITLE, EO_COMMENT, EO_TYPE, EOG_ID, EO_ORDER
                       from evenement_option
                       where E_CODE='.$copydetailsfrom;
                $result = mysqli_query($dbc, $query);

                $query = 'insert into evenement_option_group (E_CODE, EOG_TITLE, EOG_ORDER)
                       select '.$evenement.', EOG_TITLE, EOG_ORDER
                       from evenement_option_group
                       where E_CODE='.$copydetailsfrom;
                $result = mysqli_query($dbc, $query);
                // map options on groups
                if (intval(mysqli_affected_rows($dbc)) > 0) {
                    $query = 'select e1.EOG_ID, e2.EOG_ID EOG_ID_NEW
                            from evenement_option_group e1, evenement_option_group e2
                            where e1.E_CODE='.$copydetailsfrom.' and e2.E_CODE='.$evenement.'
                            and e1.EOG_TITLE = e2.EOG_TITLE
                            and e1.EOG_ORDER = e2.EOG_ORDER';
                    $result = mysqli_query($dbc, $query);
                    while (custom_fetch_array($result)) {
                        $query2 = 'update evenement_option set EOG_ID = '.$EOG_ID_NEW.'
                            where E_CODE = '.$evenement.' and EOG_ID = '.$EOG_ID;
                        $result2 = mysqli_query($dbc, $query2);
                    }
                }
                // duplication of dropdowns options
                $query = "select eod1.EOD_ID, eod1.EO_ID, eod1.EOD_ORDER, eod1.EOD_TEXTE, eo1.EO_TITLE, eo1.EO_COMMENT,
                        eo1.EO_TYPE, eo2.EO_ID EO_ID2
                        from evenement_option_dropdown eod1, evenement_option eo1, evenement_option eo2
                        where eod1.EO_ID = eo1.EO_ID
                        and eo1.EO_TYPE = eo2.EO_TYPE
                        and eo1.EO_TITLE = eo2.EO_TITLE
                        and eo1.EO_COMMENT = eo2.EO_COMMENT
                        and eo1.EO_ORDER = eo2.EO_ORDER
                        and eo1.EO_TYPE = 'dropdown'
                        and eo1.E_CODE=".$copydetailsfrom.'
                        and eo2.E_CODE='.$evenement;
                $result = mysqli_query($dbc, $query);
                $nb = mysqli_num_rows($result);
                if ($nb > 0) {
                    while (custom_fetch_array($result)) {
                        $query2 = 'insert into evenement_option_dropdown (EO_ID, EOD_ORDER, EOD_TEXTE)
                                values ('.$EO_ID2.','.$EOD_ORDER.',"'.$EOD_TEXTE.'")';
                        $result2 = mysqli_query($dbc, $query2);
                    }
                }
            }
            if ($personnel == 1) {
                ...
                $query = 'update evenement_participation ep set ep.EP_DUREE = (select eh.EH_DUREE from evenement_horaire eh where eh.E_CODE = ep.E_CODE and eh.EH_ID = ep.EH_ID 
                          and eh.E_CODE = '.$evenement.') where ep.E_CODE='.$evenement;
                $result = mysqli_query($dbc, $query);
            }
            ...
            if ($renforts == 1) {
                ...
                if ($nbr > 0) {
                    ...
                    while ($rowz = @mysqli_fetch_array($resultz)) {
                        ...
                        $resultk = mysqli_query($dbc, $queryk) or exit('Erreur création événement '.$renfort_label);
                        ...
                        $queryk = 'insert into evenement_competences (E_CODE,EH_ID,PS_ID,NB)
                                select '.$newr.',EH_ID,PS_ID,NB
                                from evenement_competences
                                where E_CODE='.$oldr;
                        $resultk = mysqli_query($dbc, $queryk);
