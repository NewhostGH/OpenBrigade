...
if ($cisname == 'Protection Civile') {
    $t = 'Fédération Nationale de la Protection Civile';
} else {
    $t = $organisation_name;
}
$pdf->Text(60, 20, $t);
...
$pdf->SetTextColor(0, 0, 0);
if ($date2 != $date) {
    $pdf->Text(60, 32, 'Pour la période du '.$date.' au '.$date2);
} else {
    $pdf->Text(60, 32, 'Pour le '.$date);
}
...
if ($num > 0) {
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        if ($_decede == 1) {
            $_nature = substr('DCD - '.$_nature, 0, 45);
        } elseif ($_detresse == 1) {
            $_nature = substr('Détresse Vitale - '.$_nature, 0, 45);
        }
...
if ($date2 == $date) {
    $t = 'Bulletin_renseignement_du_'.$date.'.pdf';
} else {
    $t = 'Bulletin_renseignement_du_'.$date.'_au_'.$date2.'.pdf';
}
$pdf->Output($t,'I');
