...
if ($TF_CODE == 'P' or $TF_CODE == 'S') {
    $type_formation = 'un '.$TF_LIBELLE;
} elseif ($TF_CODE == 'I') {
...
while ($data = mysqli_fetch_array($result)) {
    ...
    if ($date_nai != '' or $lieu_nai != '') {
        ...
        if ($nom_naissance != '') {
            $birthinfo .= ' '.$nom_naissance.',';
        }
        if ($date_nai != '') {
            $birthinfo .= ' le '.$date_nai;
        }
        if ($lieu_nai != '') {
            $birthinfo .= ' à '.$lieu_nai;
        }
        if ($dep_nai != '') {
            $birthinfo .= ' ('.$dep_nai.')';
        }
    ...
    elseif ($TF_CODE == 'R') {
        ...
        if ($PF_ADMIS > 0) {
            ...
            $y = $y + 10;
            $pdf->SetXY(15, $y);
            $pdf->MultiCell(180, 10, $civilite.' '.$nom_prenom, '', 'C');
            $y = $y + 6;
            $pdf->SetXY(15, $y);
            $pdf->MultiCell(180, 10, $birthinfo, '', 'C');
            $pdf->SetFont($police, '', $size);
            $y = $y + 14;
            $pdf->SetXY(25, $y);
            $pdf->MultiCell(180, 5, $attestation_complement2, '', 'L');
            $y = $y + 8;
            $pdf->SetFont($police, 'B', $size);
            $pdf->SetXY(35, $y);
            ...
            $pdf->SetFont($police, '', $size);
            $pdf->SetXY(25, $y);
            $pdf->MultiCell(160, 6, "qui s'est déroulée ".$periode.' à '.$lieu.'.', '', 'L');
            $y = $y + 10;
            $pdf->SetXY(25, $y);
    ...
    // ================================================
        ...
        if ($PF_EXPIRATION != '') {
            $expcomplement = " jusqu'au ".$PF_EXPIRATION;
        }

        $diplome = '';
        if ($PF_ADMIS == 1) {
            if ($type == 'PSC1') {
                $reussite = 'A suivi avec succès '.$type_formation;
            } else {
                $reussite = 'A fait l’objet d’un bilan favorable suite à '.$type_formation;
            }
            if ($PF_DIPLOME != '') {
                $diplome = ' et a obtenu le diplôme n°'.$PF_DIPLOME;
            }
        } else {
            $reussite = 'A participé à '.$type_formation;
        }

        if (substr($description, 0, 21) == 'Prévention et Secours') {
            $fonction = 'secouriste qualifié ';
        } else {
            $fonction = '';
        }
        ...
        if ($secourisme) {
            $complement .= ' '.$attestation_complement3;
        } else {
            $complement .= '.';
        }
        ...
        if ($S_IMAGE_SIGNATURE != '') {
            $signature_file = 'images/user-specific/'.$S_IMAGE_SIGNATURE;
            if (@is_file($signature_file)) {
                $pdf->Image($signature_file, 100, 236, 50);
            }
        }
    }
}
...
if ($pid > 0) {
    $name = 'attestation_'.$YEAR.'_'.$type.'_'.fixcharset(str_replace(' ','_',$nom_prenom)).'.pdf';
} else {
    $name = 'attestations_'.$YEAR.'_'.$type.'.pdf';
}
