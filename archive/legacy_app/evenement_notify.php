...
$query = "select E.E_CODE, EH.EH_ID _EH_ID, E.S_ID,E.TE_CODE, TE.TE_LIBELLE, E.E_LIEU, EH.EH_DATE_DEBUT _EH_DATE_DEBUT, EH.EH_DATE_FIN _EH_DATE_FIN,
    ...
    E.E_HEURE_RDV, E.E_LIEU_RDV, E.E_WHATSAPP, TE.ORDRE_MISSION
...
while (custom_fetch_array($result)) {
    if ($i == 1) {
        ...
        $need_check = ['created', 'enroll', 'closed', 'canceled'];
...
if ($action == 'created' and $E_VISIBLE_INSIDE == 1) {
    ...
    $ttte = '';
    if ($TE_CODE == 'DPS' and $TAV_ID == 5) {
        $ttt = 'un nouveau DPS de grande envergure';
    } elseif ($TE_CODE == 'DPS') {
        $ttt = 'un nouveau DPS';
    } elseif ($TE_CODE == 'ALERT') {
        $ttt = 'une nouvelle alerte des bénévoles';
        $ttte = 'e';
    } else {
        $ttt = 'un nouvel événement';
    }
    if ($E_EXTERIEUR == 1) {
...
Pour information, ".$ttt." vient d'être créé".$ttte.":\n";
    ...
    if (($S_EMAIL != '') and ($TE_CODE == 'DPS' or $TE_CODE == 'ALERT')) {
        $veille = true;
    }
    if ($S_EMAIL3 != '' and $TE_CODE == 'FOR') {
        $mail_formation = true;
    }
    if ($S_EMAIL2 != '') {
        $secretariat = true;
...
Voici la liste des personnes retenues:\n";
    ...
    $destid = get_inscrits($evenement, 'no').','.$admins.','.$id;
    if (count($chefs) > 0) {
        $destid .= ','.implode(',', $chefs);
    }
...
if (($action == 'inscription') and isset($_GET['P_ID'])) {
    ...
    if ($statut_of == 'SAL') {
        if (get_level("$usersection") == $nbmaxlevels - 1) {
            $level = 'parent';
        } else {
            $level = 'local';
        }
...
if ($destid != '') {
    if ($action == 'closed' and $ORDRE_MISSION == 1) {
        ...
        $nb = mysendmail("$destid", $_SESSION['id'], "$subject", "$message", $attached_file);
...
if ($COLMAIL != 'NO') {
    if ($nb2 == 1) {
        $addthis = '<br>Et à cette adresse aussi: '.$S_EMAIL;
    } else {
        $addthis = '';
    }
