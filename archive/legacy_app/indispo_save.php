...
if (isset($_GET['morning'])) {
    $morning = intval($_GET['morning']);
} else {
    $morning = 0;
}
if (isset($_GET['afternoon'])) {
    $afternoon = intval($_GET['afternoon']);
} else {
    $afternoon = 0;
}
if (isset($_GET['full_day'])) {
    $full_day = 1;
} elseif ($morning > 0 or $afternoon > 0) {
    $full_day = 2;
} else {
    $full_day = 0;
}
...
if ($type == '') {
...
} elseif ($dc2 == '' and $full_day != 2) {
    write_msgbox('Erreur date', $error_pic,
        " La date de fin doit être renseignée.<p align=center>
    <a href='indispo_choice.php?tab=1&person=$person'><input type='submit' class='btn btn-secondary' value='Retour'></a> ", 10, 0);
} elseif (($statut == 'SPV' or $statut == 'BEN' or $statut == 'ADH' or $statut == 'JSP')
            and $TI_FLAG == 1) {
    ...
    if ($dc1 == $dc2 or $full_day == 2) {
        ...
        $period = 'du '.$day1.'-'.$month1.'-'.$year1;
        if ($full_day == 0 and $morning == 0 and $afternoon == 0) {
            $period .= " ($debut)";
        }
