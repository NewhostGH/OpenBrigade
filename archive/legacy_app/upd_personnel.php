...
if (isset($_GET['self'])) {
    ...
    $pompier = intval($_GET['id']);
} elseif (isset($_GET['pompier'])) {
    $pompier = intval($_GET['pompier']);
} elseif (isset($_SESSION['pompier'])) {
    $pompier = $_SESSION['pompier'];
}

if ($pompier > 0) {
    $_SESSION['pompier'] = $pompier;
} else {
    unset($_SESSION['pompier']);
}
...
if ($id == $pompier) {
    $allowed = true;
} elseif ($mycompany == get_company($pompier) and check_rights($id, 45) and $mycompany > 0) {
    $allowed = true;
} else {
    if ($his_statut == 'EXT') {
        $perm = 37;
...
if ($from != 'inscriptions' and $from != 'cotisation') {
    $_SESSION['page'] = 1;
}

if (isset($_GET['tab'])) {
    $tab = intval($_GET['tab']);
} elseif ($from == 'cotisation' or $from == 'exportcotisation') {
    $tab = 8;
} elseif ($from == 'qualif') {
    $tab = 2;
} elseif ($from == 'formations') {
    $tab = 3;
} elseif ($from == 'inscriptions') {
    $tab = 4;
} elseif ($from == 'vehicules') {
    $tab = 5;
} elseif ($from == 'tenues') {
    $tab = 10;
} elseif ($from == 'document') {
    $tab = 6;
} elseif ($from == 'notes_de_frais') {
    $tab = 9;
} else {
    $tab = 1;
}
...
if (check_rights($id, 59, "$his_section") or ($id == $pompier and check_rights($id, 77))) {
    $compta_visible = true;
} elseif (check_rights($id, 59)) {
    $sec = get_highest_section_where_granted($id, 59);
    if (has_role_in_section($pompier, $sec)) {
        $compta_visible = true;
    }
}
if (check_rights($id, 4, "$his_section") or $id == $pompier) {
    $formations_visible = true;
} elseif (check_rights($id, 37, "$his_section") and $his_statut == 'EXT') {
    $formations_visible = true;
} else {
    $formations_visible = false;
}
if (check_rights($id, 70, "$his_section") or check_rights($id, 17, "$his_section") or $id == $pompier) {
    $materiel_visible = true;
} else {
    $materiel_visible = false;
}
if (check_rights($id, 15, "$his_section") or $id == $pompier) {
    $evenements_visible = true;
} elseif (check_rights($id, 15)) {
    // un responsable d'antenne voit les participations du dpartement
    if (is_children($his_section, $_SESSION['SES_PARENT']) or $his_section == $_SESSION['SES_PARENT']) {
        $evenements_visible = true;
    } else {
        $evenements_visible = false;
    }
} else {
    $evenements_visible = false;
}
if (check_rights($id, 2, "$his_section") or $id == $pompier) {
...
if ($P_STATUT == 'BEN' or ($P_STATUT == 'SAL' and $assoc)) {
    $NB5++;
} // passeport benevole
if ($syndicate == 1 and $NB8 > 0) {
    $NB5 = $NB5 + $NB11;
}  // attestation fiscale annes prcdentes
$NB5 = $NB5 + $NB12; // ASA/OM
...
if ((check_rights($id, 37, "$P_SECTION") or (check_rights($id, 37) and check_rights($id, 24))) and $P_STATUT == 'EXT') {
    $update_allowed = true;
} elseif ((check_rights($id, 2, "$P_SECTION") or (check_rights($id, 2) and check_rights($id, 24))) and $P_STATUT != 'EXT') {
    $update_allowed = true;
} else {
...
if (check_rights($id, 53, "$P_SECTION")) {
    $cotisations_allowed = true;
} else {
    $cotisations_allowed = false;
}

if (check_rights($id, 53, 0)) {
    $cotisations_national_allowed = true;
} else {
    $cotisations_national_allowed = false;
}

$notes_allowed = false;
if (check_rights($id, 73, "$P_SECTION")) {
    $notes_allowed = true;
} elseif (check_rights($id, 74, "$P_SECTION")) {
    $notes_allowed = true;
} elseif (check_rights($id, 75, "$P_SECTION")) {
    $notes_allowed = true;
}
...
if (check_rights($id, 4, "$P_SECTION")) {
    $change_formation_allowed = true;
} else {
    $change_formation_allowed = false;
}

if (check_rights($id, 70, "$P_SECTION")) {
    $update_tenues = true;
} else {
    $update_tenues = false;
}
...
if ($syndicate == 1 and (! check_rights($id, 1))) {
    $important_update_disabled = 'disabled';
}
...
if ($update_allowed == false and $P_STATUT != 'EXT' and check_rights($id, 2)) {
    ...
    if (check_rights($id, 70, "$EXTERNAL_SECTION") or check_rights($id, 17, "$EXTERNAL_SECTION")) {
        $materiel_visible = true;
    }
    if (check_rights($id, 15, "$EXTERNAL_SECTION")) {
        $evenements_visible = true;
    }
}
...
if ($P_EMAIL != '' && $P_OLD_MEMBER == 0 && check_rights($id, 43)) {
    ...
    if ($whatsapp != '') {
        ...
        if (substr($whatsapp, 0, 1) != '+') {
            $whatsapp = $phone_prefix.$whatsapp;
        }
...
if (($id == $pompier or $update_allowed) and isset($_GET['saved'])) {
    ...
    if ($errcode == 'nothing') {
        echo "<div class='alert alert-info' role='alert'> Aucun changement à sauver.";
    } elseif ($errcode == 0) {
        echo "<div class='alert alert-success' role='alert'> Fiche personnel sauvée.";
    } else {
        echo "<div class='alert alert-danger' role='alert'> Erreur lors de la sauvegarde de la fiche.";
    }
    echo '</div></div>';
}
...
if ($P_PHOTO != '') {
    ...
    if (file_exists($image)) {
        ...
        if ($P_OLD_MEMBER) {
            $grey_style = "style='filter: grayscale(100%);'";
        } else {
            $grey_style = '';
        }
...
$allowed_image_types = ['image/jpeg' => 'jpg', 'image/pjpeg' => 'jpg', 'image/jpg' => 'jpg', 'image/png' => 'png', 'image/x-png' => 'png'];
...
if (! $photo_found and $photo_obligatoire and $pompier == $id) {
    echo write_photo_warning($id);
}
echo write_competence_warning($pompier);
echo write_do_not_modify($ID_API);
...
if ($tab == 1) {
    ...
    echo insert_csrf('update_personnel');
    ...
    if ($P_CIVILITE == 1 and (! file_exists($profilpic))) {
        $defaultpic = $defaultboy;
    }
    if ($P_CIVILITE == 2 and (! file_exists($profilpic))) {
        $defaultpic = $defaultgirl;
    }
    if ($P_CIVILITE == 3 and (! file_exists($profilpic))) {
        $defaultpic = $defaultother;
    }
    if ($P_CIVILITE == 4 or $P_CIVILITE == 5 and (! file_exists($profilpic))) {
    ...
    if ($animaux == 1) {
        if ($block_personnel or $disabled == 'disabled') {
            ...
            $selected_humain = '';
            if ($is_chien) {
                $selected_animal = 'checked';
            } else {
                $selected_humain = 'checked';
            }
            echo "<td>
                    <form>
    ...
    if ($syndicate == 1) {
        ...
        if ($important_update_disabled == 'disabled') {
            ...
            while (custom_fetch_array($result2)) {
                if ($TP_CODE != '-') {
                    $TP_DESCRIPTION = $TP_CODE.' - '.$TP_DESCRIPTION;
                }
                if ($TP_CODE == $P_PROFESSION) {
                    $selected = 'selected';
                } else {
                    $selected = '';
                }
    ...
    if ($important_update_disabled == 'disabled') {
        echo '<span title="'.$TRT_DESC.'">'.$TRT_CODE.'</span>';
    } else {
    ...
    echo "<tr id='tsRow' $style class='pad0'>
          <td width=130>";
    echo write_modal($url, 'contrat_salarie', '<span title="Afficher ou modifier le détail des heures, CP, RTT ...">Contrat</span>');
    ...
    if ($block_personnel or $disabled_matricule == 'disabled') {
        ...
        if ($is_chien) {
            $query2 .= ' where TC_ID > 3';
        } else {
            $query2 .= ' where TC_ID < 4';
        }
    ...
    // =====================================================================
    if ($is_chien) {
        if (intval($P_MAITRE) > 0 or $P_CIVILITE > 3) {
            $style = '';
        } else {
            $style = "style='display:none'";
        }
        ...
        if ($update_allowed) {
            ...
            echo write_modal($url, $P_ID, "<i class='fa fa-user fa-lg' title='choisir le maitre' title='choisir le maître'/></i>");
        }
        echo '</td>
        </tr>';
    }
    ...
    if ($update_allowed or $pompier == $id) {
        if (($army == 0 and $pompiers == 0) or $P_CODE == 'admin') {
            $i = 'Identifiant';
        } else {
            $i = 'Matricule';
        }
        ...
        if ($syndicate == 1) {
            $disabled2 = $important_update_disabled;
        } else {
            $disabled2 = $disabled_matricule;
        }
    ...
    if ($disabled == 'disabled') {
        ...
        if ($update_allowed) {
            if ($P_STATUT == 'EXT') {
                ...
                $mysection = get_highest_section_where_granted($id, 2);
            }
            if ($mysection == '') {
                $mysection = $P_SECTION;
            }
    ...
    if ($infos_visible) {
        ...
        if (! check_rights($id, 9)) {
            ...
            if ($P_GP_ID2 != '') {
                $query2 .= " and groupe.GP_ID <> $P_GP_ID2 ";
            }
            $query2 .= ' )';
        }
        ...
        if ($update_allowed and (check_rights($id, 9) or check_rights($id, 25))) {
            $disabled2 = '';
        } else {
            $disabled2 = 'disabled';
        }

        if (check_rights($id, 2, "$S_PARENT")) {
            $disabled3 = '';
        } else {
            $disabled3 = 'disabled';
        }
        ...
        if ($disabled2 == 'disabled') {
            if ($GP_FLAG1 == 1) {
                echo " <i class='far fa-check-square' title=\"Et les permissions s'appliquent au niveau supérieur\"></i>";
            }
            ...
            if ($P_STATUT == 'EXT' or $P_STATUT == 'ADH') {
                $style = "style='display:none'";
            } else {
                $style = '';
            }
    ...
    if (mysqli_num_rows($result2) > 0) {
        ...
        while (custom_fetch_array($result2)) {
            ...
            if ((get_level("$_S_ID") + 1 == $nbmaxlevels) and ($nbsections == 0)) {
                if ($_GP_DESCRIPTION == 'Président (e)') {
                    $_GP_DESCRIPTION = "Responsable d'antenne";
                }
                if ($_GP_DESCRIPTION == 'Vice président (e)') {
                    $_GP_DESCRIPTION = 'Responsable adjoint';
                }
            }
            echo '<tr>
...
if ($tab == 1 and $infos_visible) {
    ...
    if ($syndicate == 0) {
        ...
        echo "<input type='hidden' name='pays' value=''>";
    }
    ...
    if ($is_chien) {
        ...
        if ($update_allowed) {
            ...
            if ($NPAI == 1) {
                echo " <i class='far fa-check-square' title=\"N'habite pas  l'adresse indique\"></i> ";
            }
            echo $DATE_NPAI;
        }
    }
...
if ($tab == 1 and $infos_visible) {
    ...
    if ($syndicate == 0) {
        ...
        if ($custom == 0 or $P_STATUT != 'JSP') {
            ...
            if ($PRENOM_MAITRE != '') {
                $P_RELATION_PRENOM = $PRENOM_MAITRE;
            }
            if ($disabled == 'disabled') {
    ...
    while (custom_fetch_array($result1)) {
        ...
        if ($CF_USER_VISIBLE == 1 or $update_allowed) {
            ...
            if (($CF_USER_MODIFIABLE == 1 and $id == $pompier) or $update_allowed) {
            ...
            if ($CF_TYPE == 'checkbox') {
                if ($custom_disabled) {
                    if ($CFP_VALUE == 1) {
                        echo "<i class='far fa-check-square' title='Oui'></i>";
                    }
                } else {
                    if ($CFP_VALUE == 1) {
                        $checked = 'checked';
                ...
                if ($CF_MAXLENGTH > 30) {
                    ...
                    $textsize = $CF_MAXLENGTH;
                }
                if ($CF_MAXLENGTH > 0) {
                    $sz = "maxlength='".$CF_MAXLENGTH."' size='".$textsize."'";
                } else {
                    $sz = 'size=10';
                }
                if ($CF_NUMERIC == 1) {
                    $chk = "onchange='checkNumberNullAllowed(form.custom_".$CF_ID.',"'.$CFP_VALUE."\");'";
                } else {
                    $chk = '';
                }
                if ($custom_disabled) {
                    ...
                    echo "<input type='text' class='form-control form-control-sm' ".$sz." name='custom_".$CF_ID."'  value=\"".$CFP_VALUE.'"  '.$chk.' title="'.$CF_COMMENT.'">';
                }
            } elseif ($CF_TYPE == 'textarea') {
                if ($CF_MAXLENGTH > 0) {
                    $sz = "maxlength='".$CF_MAXLENGTH."'";
                }
                if ($custom_disabled) {
                    echo "<div style='max-width:220px;font-size: 12px;'>".$CFP_VALUE.'</div>';
                } else {
    ...
    if ($is_chien) {
        ...
        if ($charte_active and ($update_allowed or $id == $P_ID)) {
            if ($P_ACCEPT_DATE == '') {
                $info = "N'a pas encore accept la<br><a href=charte.php title=\"Voir la charte d'utilisation\">charte d'utilisation</a>";
            } else {
                $info = "<a href=charte.php title=\"Voir la charte d'utilisation\">charte d'utilisation</a> acceptée <br>".$P_ACCEPT_DATE;
            }
        ...
        if ($info_connexion and ($update_allowed or $id == $P_ID)) {
            if ($P_ACCEPT_DATE2 == '') {
                $info = "N'a pas encore lu la <br><a href=specific_info.php title=\"Voir la note d'information\">Note d'information</a>";
            } else {
                $info = "<a href=specific_info.php title=\"Voir la note d'information\">Note d'information</a> lue <br>".$P_ACCEPT_DATE2;
            }
...
if ($tab == 2 and $infos_visible and ! $subPage) {
    ...
    if ($child == 1) {
        ...
        if ($n == 0) {
            ...
            while (custom_fetch_array($result2)) {
                ...
                $show = true;
                if ($PH_CODE != '') {
                    $hierarchie = '<span class=small2>('.$PH_CODE.' niveau '.$PH_LEVEL.')</span>';
                } else {
                    $hierarchie = '';
                }
                ...
                if ($Q_VAL == -1) {
                    ...
                    $mycolor = $widget_fggreen;
                } else {
                    $mycolor = $widget_fgblue;
                }
                if ($Q_EXPIRATION != '') {
                    if ($NB < $DAYS_WARNING) {
                        $mycolor = $widget_fgorange;
                    }
                    if ($NB <= 0) {
                        $mycolor = $widget_fgred;
                    }
                }
                if ($PS_FORMATION == 1) {
    ...
    if ($child == 2) {
            ...
            while (custom_fetch_array($result)) {
                ...
                if (intval($E_CODE) != 0) {
                    ...
                    if (isset($no_attestations_continue_secourisme) and $TF_CODE == 'R') {
                        if (in_array(str_replace(' ', '', $TYPE), ['PSC1', 'PSE1', 'PSE2', 'PAEPSC', 'PAEPS', 'FDFPSC', 'FDFPSE']) and intval($no_attestations_continue_secourisme) <= $YEAR) {
                            $enable_this = false;
                        }
                    }
        ...
        echo display_heures_formation($P_ID);
        echo '</div></div></div>';
    }
}
...
if ($tab == 4) {
    ...
    while ($row3 = @mysqli_fetch_array($result3)) {
        if ($type_evenement == $row3['TE_CODE']) {
            $selected = 'selected';
        } else {
            $selected = '';
        }
    ...
    and e.TE_CODE <> 'MC'";
    if ($type_evenement != 'ALL') {
        $query2 .= " and e.TE_CODE = '".$type_evenement."'";
    }
    if ((! check_rights($id, 9) and $id != $pompier) or $gardes == 1) {
    ...
    if ($number > 0) {
        ...
        while ($row = mysqli_fetch_array($res)) {
            ...
            if ($future == 0) {
                if ($e_code == 0) {
                    $te_libelle = 'Astreinte ancienne';
                } else {
                    $te_libelle = 'Activité Ancienne: '.$te_libelle;
                }
                $greystyle = 'filter: grayscale(100%);';
            }
            ...
            if ($e_code == 0) {
                ...
                if ($cmt != '') {
                    $txtimg = "<i class='far fa-".$txtimg."' title=\"".$cmt.'"></i>';
                } else {
                    $txtimg = '';
                }
                ...
                $EP_DAS = $row['ep_das'];
                if ($EP_ASA == 1) {
                    $asa = "<font size=1><a href=\"javascript:swalAlert('Autorisation spéciale absence');\" title=\"Autorisation spéciale d'absence\">ASA</a></font>";
                } else {
                    $asa = '';
                }
                if ($EP_DAS == 1) {
                    $das = "<font size=1><a href=\"javascript:swalAlert('Décharge activité de service');\" title=\"Décharge d'activité de service\">DAS</a></font>";
                } else {
                    $das = '';
                }
                ...
                if ($row['e_visible_inside'] == 0) {
                    $libelle .= " <i class='fa fa-exclamation-triangle' style='color:orange;' title='ATTENTION événement caché, seules les personnes ayant la permission n9 peuvent le voir'></i>";
                }
                if ($row['EQ_ICON'] == '') {
                    $img = 'images/evenements/'.$row['te_icon'];
                } else {
                    $img = $row['EQ_ICON'];
                }

                if (is_file($img)) {
                    $i = '<img border=0 src='.$img." style='padding-left:5px;$greystyle' class='img-max-35' title=\"".$te_libelle.'">';
                } else {
                    $i = '';
                }
                echo "<td class='widget-text' align=left style='max-width:20px; width: 50px;'>".$i.'</td>';
                ...
                if ($datedeb != $datefin) {
                    ...
                    echo "<td class='widget-text'>".$datedeb."<span class='only_mobile'><br>".$debut.'-'.$fin.'</span></td>';
                }
        ...
        if (is_dir($mypath)) {
            ...
            while ($row = @mysqli_fetch_array($result)) {
                ...
                if ($visible) {
                    ...
                    <td class='widget-text'><a href=showfile.php?section=$P_SECTION&pompier=$pompier&file=$file>$file</a></td>";
                } // target=_blank
                else {
                ...
                if ($author != '') {
                    $author = '<a href=upd_personnel.php?pompier='.$author.'>'.
                ...
                if ($document_security == 0) {
                    $img = '';
                } elseif ($securityid > 1) {
                    $img = "<span class='btn btn-default btn-action' href=#><i class='fa fa-lock' style='color:orange;' title=\"$securitylabel\" ></i></span>";
                } else {
                    $img = "<span class='btn btn-default btn-action' href=#><i class='fa fa-unlock' title=\"$securitylabel\" ></i></span>";
                }

                if (($update_allowed or $pompier == $id) and $fileid > 0) {
                    $url = 'document_modal.php?docid='.$fileid.'&pid='.$pompier;
                    echo write_modal($url, 'doc_'.$fileid, $img);
                } else {
                    echo $img.'</button>';
                }
                ...
                if ($update_allowed) {
                    ...
                    echo "<td class='widget-text'></div></td>";
                }
                echo '</tr>';
            }
        }
...
if ($tab == 8 and ($update_allowed or $P_ID == $pompier) && ! $subPage) {
    ...
    if ($cotisations_allowed) {
        ...
        echo write_modal($url, 'observation', "<div class='btn btn-default'><i class='".$fa." fa-file-alt fa-1x' title=\"Observations, Cliquez pour modifier \n".$OBSERVATION.'"></i></div>');
        echo '</div>';
    }
    echo '</div>';
    echo '</div>';
    ...
    if ($syndicate == 1) {
        echo ', soit '.$COTISATION_MENSUELLE.' '.$default_money_symbol.' / mois</td>';
    }

    $rembourse_style = 'color: Black;';
    if (intval($MONTANT_REGUL) != 0) {
        if ($MONTANT_REGUL > 0) {
            $rembourse_style = 'color: Red;';
        } else {
            $rembourse_style = 'color: Green;';
        }
    }
    if ($TP_ID == 1) {
        $ti = 'Si des prélèvements ont été rejetés et doivent être représentés à la banque, ou si des sommes doivent être remboursées (montant négatif), indiquer les montants cumulés ici. Ils seront automatiquement ajoutés au prochain prélèvement.';
    } else {
        $ti = 'Montant complémentaire à payer par la personne (si la valeur est positive ) ou à lui rembourser (si la valeur est négative)';
    }
    if ($MONTANT_REGUL < 0) {
        $ta = 'A rembourser';
    } elseif ($TP_ID == 1) {
    ...
    if ($bank_accounts == 1) {
        ...
        echo "<input type=hidden name='type_paiement' value='".$TP_ID."'>";
    }
    ...
    if ($bank_accounts == 1) {
        ...
        if ($UPDATE_DATE != '') {
            $UPDATE_DATE = '<span class=small>Modifié le '.$UPDATE_DATE.'</span>';
        }
    ...
    if ($number > 0) {
        ...
        echo "<tr>
             ...
             ";
        if ($cotisations_allowed) {
            echo '<td width=60 align=left> Actions</td>';
        }
        echo '</tr>';
        while (custom_fetch_array($result)) {
            $OBS = '';
            if ($NUM_CHEQUE != '') {
                $OBS = 'Chèque n'.$NUM_CHEQUE;
            }
            if ($IBAN != '') {
                $OBS = '<small>'.display_IBAN($IBAN).'</small>';
            } elseif ($COMPTE != '') {
                $OBS = $CODE_BANQUE.'-'.$ETABLISSEMENT.'-'.$GUICHET.'-'.$COMPTE;
            }
            if ($REJET == 1 and $COMMENTAIRE == '' and $REGUL_ID > 0) {
                $COMMENTAIRE = 'par '.$TR_DESCRIPTION;
            }
            if ($COMMENTAIRE != '') {
            ...
            if ($REJET == 1) { // rejet
                ...
                echo '</tr>';
            } elseif ($REMBOURSEMENT == 0) {  // paiement
                echo '<tr>
                <td align=center></td>
...
if ($tab == 9 && ! $subPage) {
    ...
    if ($number > 0) {
        ...
        while (custom_fetch_array($result)) {
            ...
            if ($NF_NATIONAL == 1) {
                $TM_DESCRIPTION = 'National '.$TM_DESCRIPTION;
            } elseif ($NF_DEPARTEMENTAL == 1) {
                $TM_DESCRIPTION = 'Départemental '.$TM_DESCRIPTION;
            }
            $COMPTABLE = $NF_CODE1.' / '.str_pad($NF_CODE2, 2, '0', STR_PAD_LEFT).' / '.str_pad($NF_CODE3, 3, '0', STR_PAD_LEFT);
            if ($E_CODE > 0) {
                $TM_DESCRIPTION .= ' (<a href=evenement_display.php?evenement='.$E_CODE.'&from=personnel_note&pid='.$pompier.' title="Voir activité">'.$E_CODE.'</a>)';
            }
            $author = '<a href=upd_personnel.php?pompier='.$NF_CREATE_BY.'>'.my_ucfirst($P_PRENOM).' '.strtoupper($P_NOM).'</a>';
            if ($NF_VALIDATED_BY != '') {
                $author2 = '<a href=upd_personnel.php?pompier='.$NF_VALIDATED_BY.'>'.my_ucfirst($P_PRENOM2).' '.strtoupper($P_NOM2).'</a>';
            } else {
                $author2 = '';
            }

            if ($NF_DON == 1 and $FS_CODE == 'REMB' and $assoc) {
                $FS_DESCRIPTION = "Don à l'association";
            }
            if ($syndicate == 1) {
                if ($FS_CODE == 'VAL') {
                    $FS_DESCRIPTION = 'Valide trésorier';
                }
                if ($FS_CODE == 'VAL1') {
                    $FS_DESCRIPTION = 'Valide président';
                }
            }

            $alertClass = '';
            if ($FS_CODE == 'ATTV') {
                $alertClass = 'alert-label alert-orange';
            } elseif ($FS_CODE == 'CRE') {
                $alertClass = 'alert-label alert-violet';
            } elseif ($FS_CODE == 'ANN') {
                $alertClass = 'alert-label alert-grey';
            } elseif ($FS_CODE == 'REJ') {
                $alertClass = 'alert-label alert-red';
            } elseif ($FS_CODE == 'VAL') {
                $alertClass = 'alert-label alert-green';
            } elseif ($FS_CODE == 'VAL2' or $FS_CODE == 'VAL1') {
                $alertClass = 'alert-label alert-green-apple';
            } elseif ($FS_CODE == 'REMB') {
                $alertClass = 'alert-label alert-blue';
            }
            ...
            if ($syndicate) {
                echo "<td class='hide_mobile widget-text'>".$COMPTABLE.'</td>';
            }
        ...
        echo 'Aucune note de frais trouvée.';
    }
}
