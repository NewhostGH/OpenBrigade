...
if (isset($_GET['V_KM'])) {
    $V_KM = intval($_GET['V_KM']);
} else {
    $V_KM = 0;
}
if (isset($_GET['V_KM_REVISION'])) {
    $V_KM_REVISION = intval($_GET['V_KM_REVISION']);
} else {
    $V_KM_REVISION = 0;
}
...
$V_TITRE_DATE = secure_input($dbc, $_GET['dc4']);
$S_ID = secure_input($dbc, $_GET['groupe']);
$operation = secure_input($dbc, $_GET['operation']);
if (isset($_GET['V_EXTERNE'])) {
    $V_EXTERNE = secure_input($dbc, $_GET['V_EXTERNE']);
} else {
    $V_EXTERNE = 0;
}
if (isset($_GET['V_FLAG1'])) {
    $V_FLAG1 = intval($_GET['V_FLAG1']);
} else {
    $V_FLAG1 = 0;
}
if (isset($_GET['V_FLAG2'])) {
    $V_FLAG2 = intval($_GET['V_FLAG2']);
} else {
    $V_FLAG2 = 0;
}
if (isset($_GET['V_FLAG3'])) {
    $V_FLAG3 = intval($_GET['V_FLAG3']);
} else {
    $V_FLAG3 = 0;
}
if (isset($_GET['V_FLAG4'])) {
    $V_FLAG4 = intval($_GET['V_FLAG4']);
} else {
    $V_FLAG4 = 0;
}
if (isset($_GET['affected_to'])) {
    $AFFECTED_TO = intval($_GET['affected_to']);
} else {
    $AFFECTED_TO = 'null';
}
if ($AFFECTED_TO == 0) {
...
if ($operation == 'update') {
    ...
    if ($V_ASS_DATE != '') {
        $tmp = explode('-', $V_ASS_DATE);
        $month1 = $tmp[1];
        $day1 = $tmp[0];
        $year1 = $tmp[2];
        $query .= "V_ASS_DATE='$year1-$month1-$day1',";
    } else {
        $query .= 'V_ASS_DATE= null,';
    }
    if ($V_CT_DATE != '') {
        $tmp = explode('-', $V_CT_DATE);
        $month2 = $tmp[1];
        $day2 = $tmp[0];
        $year2 = $tmp[2];
        $query .= "V_CT_DATE='$year2-$month2-$day2',";
    } else {
        $query .= 'V_CT_DATE= null,';
    }
    if ($V_REV_DATE != '') {
        $tmp = explode('-', $V_REV_DATE);
        $month3 = $tmp[1];
        $day3 = $tmp[0];
        $year3 = $tmp[2];
        $query .= "V_REV_DATE='$year3-$month3-$day3',";
    } else {
        $query .= 'V_REV_DATE= null,';
    }
    if ($V_TITRE_DATE != '') {
        $tmp = explode('-', $V_TITRE_DATE);
        $month4 = $tmp[1];
        $day4 = $tmp[0];
        $year4 = $tmp[2];
        $query .= "V_TITRE_DATE='$year4-$month4-$day4',";
    } else {
        $query .= 'V_TITRE_DATE= null,';
    }
    $query .= '   S_ID='.$S_ID.' 
           where V_ID='.$V_ID;
    $result = mysqli_query($dbc, $query);
...
if ($operation == 'insert') {
   ...
   (V_ID, VP_ID, TV_CODE, V_IMMATRICULATION, V_COMMENT, V_KM, V_KM_REVISION, V_ANNEE, 
     EQ_ID, V_MODELE, S_ID, V_ASS_DATE, V_CT_DATE, V_REV_DATE, V_TITRE_DATE, V_EXTERNE,V_INVENTAIRE,V_INDICATIF)
   values
