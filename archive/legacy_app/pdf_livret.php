...
// Identité
// =========================================================
if ($P_PHOTO != '' and is_file($P_PHOTO)) {
    $pdf->Image($P_PHOTO, 15, $y + 5);
}
...
if ($num > 0) {
    ...
    while ($row = @mysqli_fetch_array($result)) {
        ...
        if ($row['TF_CODE'] == 'I') {
            ...
            $TF_LIBELLE = substr($row['TF_LIBELLE'], 0, 18);
        }
        $datedeb = $row['datedeb'];
        ...
        if (intval($ep_duree) > 0) {
            $eh_duree = $ep_duree;
        }
        $TP_LIBELLE = substr($row['TP_LIBELLE'], 0, 18);
        if ($TP_LIBELLE == '') {
            $TP_LIBELLE = 'stagiaire';
        }
        $sum += intval($eh_duree);
...
if ($num > 0) {
    ...
    while ($row = @mysqli_fetch_array($result)) {
        ...
        if ($TE_CODE == 'DPS') {
            ...
            $TE_LIBELLE = substr($row['TE_LIBELLE'], 0, 18);
        }
        $datedeb = $row['datedeb'];
        $epdatedeb = $row['epdatedeb'];
        if ($epdatedeb != '') {
