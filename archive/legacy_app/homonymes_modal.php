...
if ($P_BIRTHDATE0 != '') {
    if ($P_SEXE == 'M') {
        $born0 = 'né le '.$P_BIRTHDATE0;
    } else {
        $born0 = 'née le '.$P_BIRTHDATE0;
    }
    if ($P_BIRTHPLACE0 != '') {
        $born0 .= ' à '.$P_BIRTHPLACE0;
    }
} else {
    $born0 = '?';
}
...
if (check_rights($id, 3, "$his_section")
    or (check_rights($id, 37, "$his_section") and $P_STATUT == 'EXT')) {
    $query .= '';
} else {
...
while (custom_fetch_array($result)) {
    if ($P_BIRTHDATE != '') {
        if ($P_SEXE == 'M') {
            $born = 'né le '.$P_BIRTHDATE;
        } else {
            $born = 'née le '.$P_BIRTHDATE;
        }
    } else {
        $born = '?';
    }
    if ($P_OLD_MEMBER > 0) {
        $P_STATUT = "<span style='background-color:grey;' title='ancien'>".$P_STATUT.'</span>';
    }
    $out .= "<tr style='background-color:white;'>
