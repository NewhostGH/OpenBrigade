...
$current = $row[0];
if (! is_dir('./images/vehicules/iconespersos')) {
    mkdir('./images/vehicules/iconespersos', 0755, true);
}
...
$categories = ['SECOURS', 'FEU', 'LOGISTIQUE', 'DIVERS'];
...
if ($competences == 1) {
    echo '<td colspan=2>Compétence requise</td>';
} else {
    ...
    if ($competences == 1) {
        ...
        while ($row2 = mysqli_fetch_array($result2)) {
            ...
            if ($prevEQ_ID != $NEWEQ_ID) {
                echo '<OPTGROUP LABEL="'.$NEWEQ_NOM."\" class='section'>";
            }
            $prevEQ_ID = $NEWEQ_ID;
            if ($PS_ID == $NEWPS_ID) {
                $selected = 'selected';
