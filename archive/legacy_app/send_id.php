...
if ($action == 'create') {
    if ($NBDAYS > 0 or $P_CREATE_DATE == '') {
        check_all(9);
    }
    check_all(1);
    if (! check_rights($id, 1, "$P_SECTION")) {
        check_all(24);
    }
    ...
    check_all(25);
    if (! check_rights($id, 25, "$P_SECTION")) {
        check_all(9);
    }
}
// select mode
if ($mode == 'unknown') {
    ...
    if ($mode == 'auto') {
        ...
        if ($P_EMAIL != '') {
            $texte .= "<br>- par mail à <a href='mailto:".$P_EMAIL."'>".$P_EMAIL.'</a>.';
        }
        if ($P_PHONE != '') {
            $texte .= '<br>- ou par téléphone au '.$P_PHONE.'.';
        }
    ...
    if ($action == 'create' and $P_STATUT != 'EXT') {
        $another = " <a class='btn btn-success' href='ins_personnel.php' title='Ajouter une autre fiche'><i class='fa fa-plus' ></i> Ajouter</a>";
    } else {
        $another = '';
    }
