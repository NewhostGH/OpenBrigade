...
} elseif ($gardeSP and check_rights($id, 6, $organisateur) and $sdis == 1) {
    $granted_personnel = true;
} elseif ($gardeSP and check_rights($id, 6) and $sdis == 0) {
    $granted_personnel = true;
} else {
...
if ($EP_BY != '') {
    $inscritPar = 'le '.$EP_DATE.' par '.my_ucfirst($P_PRENOM_BY).' '.strtoupper($P_NOM_BY);
} else {
    $inscritPar = '';
}

if ($EP_REMINDER == 1 and $cron_allowed == 1 and $P_EMAIL != '') {
    $checked_reminder = 'checked';
} else {
    $checked_reminder = '';
}
...
$out = "<div align=center >
    ...
    <p><table class='noBorder' cellspacing=0>
...
$out .= '<div align=center>';
if ($can_save) {
    $out .= "<input type='submit' class='btn btn-success' name='p".$pid."' value='Sauvegarder' title='cliquer pour valider' >
    </div>
    </form>
 </div>";
}
