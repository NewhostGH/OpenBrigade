...
check_all(0);
$id = $_SESSION['id'];
if (! check_rights($id, 2) and ! check_rights($id, 26) and ! check_rights($id, 43)) {
    check_all(2);
}
...
if (isset($_POST['SelectionMail'])) {
    $pids = explode(',', ($_POST['SelectionMail']));
} elseif (isset($_GET['destid'])) {
    $pids = explode(',', ($_GET['destid']));
}
...
while (custom_fetch_array($res)) {
    if ($P_EMAIL != '') {
        echo ucfirst(fixcharset($P_PRENOM)).','.fixcharset(strtoupper($P_NOM)).','.$P_EMAIL."\r\n";
    }
}
