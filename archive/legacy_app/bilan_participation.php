...
if (isset($_GET['groupJN'])) {
    $groupJN = intval($_GET['groupJN']);
}
if (isset($_GET['c1'])) {
    $c1 = intval($_GET['c1']);
} else {
...
if ($month == 100) {
    echo "<option value='100' selected >Bilan annuel</option>\n";
} else {
    echo "<option value='100'>Bilan annuel</option>\n";
}
echo '</select>';
...
// histogram
// =====================================================================
if ($gardes == 0 and $mode_garde == 0) {
    $legend_not_clickable = 1;
}
echo repo_bilan_participations($filter, $year, $month, $mode_garde, $groupJN, $c1, $c2, $c3);
echo '</div>';
writefoot();
