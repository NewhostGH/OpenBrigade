...
$moisCourt = ['Janv.', 'Févr.', 'Mars', 'Avr.', 'Mai', 'Juin', 'Juil.', 'Août', 'Sept.', 'Oct.', 'Nov.', 'Dec.'];
...
$supported_ext = ['.doc', '.docx', '.zip', '.pps', '.ppt', '.pptx', '.xls', '.xlsx', '.pdf', '.jpg', '.jpeg', '.png', '.odt', '.mp3'];

// Allowable file Mime Types. Add more mime types if you want
$supported_mimes = ['image/jpeg', 'image/jpg', 'image/gif', 'image/png', 'application/msword', 'application/octet-stream', 'application/oct-stream',
...
if ($MAX_POST_SIZE_MB <= $MAX_FILE_SIZE_MB) {
    $MAX_FILE_SIZE_MB = $MAX_POST_SIZE_MB - 1;
}
...
$bad_mail_domains = ['wanadoo.fr', 'orange.fr', 'yahoo.fr', 'gmail.fr'];
...
// colors report
$colors = [0xFF0000, 0x3300CC, 0x00CC00,
...
if (isset($noconnect)) {
    ...
    if ($army) {
        $grades_imgdir = 'images/grades_army';
    } else {
        $grades_imgdir = 'images/grades_sp';
    }
}
...
if ($syndicate == 1) {
    $renfort_label = 'participation';
} else {
    $renfort_label = 'renfort';
}
...
// Bridge mode includes this file inside a controller method scope.
// Mirror variables into $GLOBALS so legacy `global $var` access keeps working.
$__legacy_scope_vars = get_defined_vars();
foreach ($__legacy_scope_vars as $__legacy_key => $__legacy_value) {
    if ($__legacy_key === 'GLOBALS') {
        continue;
    }
    if (strpos($__legacy_key, '__legacy_') === 0) {
        continue;
    }
    if ($__legacy_key === '_SERVER' || $__legacy_key === '_GET' || $__legacy_key === '_POST' || $__legacy_key === '_COOKIE' || $__legacy_key === '_SESSION' || $__legacy_key === '_FILES' || $__legacy_key === '_ENV' || $__legacy_key === '_REQUEST') {
        continue;
    }
    $GLOBALS[$__legacy_key] = $__legacy_value;
}
unset($__legacy_scope_vars, $__legacy_key, $__legacy_value);
