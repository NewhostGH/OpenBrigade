...
if (isset($_GET['numlot'])) {
    $numlot = intval($_GET['numlot']);
} else {
    $numlot = 0;
}
...
if ($AFFECTED_TO != '') {
    ...
    if ($P_OLD_MEMBER == 1) {
        $warning = "<i clas='fa fa-exclamation-triangle' style='color:orange;' title=\"Attention $owner est un ancien membre\"></i>";
    } else {
        $warning = '';
    }
} else {
...
if ($tab == 1) {
    ...
    while ($row2 = @mysqli_fetch_array($result2)) {
        ...
        if ($prevTM_USAGE != $_TM_USAGE) {
            echo "<OPTGROUP class='categorie' LABEL='".$_TM_USAGE."'>";
        }
        if ($NEWTM_ID == $TM_ID) {
            $selected = 'selected';
        } else {
        ...
        if ($NEWTM_DESCRIPTION != '') {
            $addcmt = ' - '.$NEWTM_DESCRIPTION;
        } else {
            $addcmt = '';
        }
    ...
    if ($VP_OPERATIONNEL == -1) {
        ...
        $opcolor = $widget_fgblue;
    } elseif ($VP_OPERATIONNEL == 1) {
        $opcolor = $widget_fgred;
    } elseif ($VP_OPERATIONNEL == 2) {
        $opcolor = $widget_fgorange;
    } else {
        $opcolor = $widget_fggreen;
    }
    ...
    while ($row2 = @mysqli_fetch_array($result2)) {
        ...
        if ($NEWVP_OPERATIONNEL > 2) {
            $class = 'ope';
        } elseif ($NEWVP_ID == 'PRE') {
            $class = 'pre';
        } elseif ($NEWVP_OPERATIONNEL > 1) {
            $class = 'limited';
        } elseif ($NEWVP_OPERATIONNEL > 0) {
            $class = 'broken';
        } else {
            $class = 'off';
        }
    ...
    if ((! $found) and ($MA_ANNEE != '')) {
        echo "<option value='$MA_ANNEE' selected>$MA_ANNEE</option>";
    }
    ...
    if (my_date_diff(getnow(), $MA_REV_DATE) < 0) {
        $revision = $orange;
    }
    ...
    if ($TM_USAGE == 'Habillement') {
        ...
        $t2 = 'Prochaine révision ou péremption';
    }
    ...
    if (check_rights($id, 19, "$S_ID") or ($MA_ADDED != '' and $MA_HOURS < 24)) {
        if ($MA_ADDED != '' and $MA_HOURS < 24) {
            $t = 'Matériel ajouté il y moins de 24 heures, peut être supprimé';
        } else {
            $t = '';
        }
    ...
    } elseif ($from == 'personnel' and $AFFECTED_TO != '' and $TM_USAGE == 'Habillement') {
...
if ($TM_LOT == 1 and $tab == 3) {
    ...
    if ($NB2 > 0) {
        ...
        while ($row2 = @mysqli_fetch_array($result2)) {
            ...
            if ($_VP_OPERATIONNEL == -1) {
                $mytxtcolor = 'black';
            } elseif ($_VP_OPERATIONNEL == 1) {
                $mytxtcolor = $red;
            } elseif ($_NB_DAYS < 0) {
                $mytxtcolor = $orange;
                $_VP_LIBELLE = 'date dépassée';
            } elseif ($_VP_OPERATIONNEL == 2) {
                $mytxtcolor = $orange;
            } else {
                $mytxtcolor = $green;
            }
        ...
        while (custom_fetch_array($result4)) {
            ...
            if ($C_DATE_PEREMPTION != '') {
                if (my_date_diff(getnow(), $C_DATE_PEREMPTION) < 0) {
                    $class = 'red12';
                } elseif (my_date_diff(getnow(), $C_DATE_PEREMPTION) < 30) {
                    $class = 'orange12';
                } else {
                    $class = 'green12';
                }
            ...
            if ($TCO_CODE == 'PE') {
                $conditionnement = $TUM_DESCRIPTION.'s';
            } elseif ($TUM_CODE != 'un' or $TC_QUANTITE_PAR_UNITE != 1) {
                $conditionnement = $TCO_DESCRIPTION.' '.$TC_QUANTITE_PAR_UNITE.' '.$TUM_DESCRIPTION;
            } else {
                $conditionnement = '';
            }
