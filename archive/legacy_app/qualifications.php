...
if ($MYP_ID == 0) {
    ...
    if (intval($competence) > 0 and $action_comp == 'default') {
        $queryadd .= ' and exists ( select 1 from qualification q where q.P_ID = p.P_ID and q.PS_ID='.$competence.')';
    }
    ...
    if ($typequalif == 0) {
        ...
        if ($competence > 0 and $action_comp == 'update') {
            ...
            while (custom_fetch_array($result)) {
                ...
                if ($Q_EXPIRATION == '00-00-0000') {
                    $Q_EXPIRATION = '';
                }
                if ($Q_EXPIRATION != '') {
                    if ($NB <= 0) {
                        $myimg = "<i class='fa fa-circle' style='color:$widget_fgred;' title='date expiration dépassée' ></i>";
                    } elseif ($NB < $DAYS_WARNING) {
                        $myimg = "<i class='fa fa-circle' style='color:$widget_fgorange;' title='expiration dans $NB jours'></i>";
                    }
                }
                ...
                if ($grades == 1) {
                    $file = $G_ICON;
                    if (file_exists($file)) {
                        $t = "<img class='img-max-30' style='border-radius: 3px;' src=".$file." title='".str_replace("'", ' ', $G_DESCRIPTION)."'>";
                    } else {
                        $t = "<span title='".$G_DESCRIPTION."'>".$P_GRADE.'</span>';
                    }
        ...
        // ===============================================
            ...
            if ($typequalif != 0) {
                $query_k .= ' and e.EQ_ID='.$typequalif;
            }
            ...
            if ($number == 0) {
                ...
                while (custom_fetch_array($result2)) {
                    $w = strlen($TYPE) > 5 ? '3' : '2';
                    if ($PS_ID == $competence) {
                        $TYPE = "<span class='badge' style='background-color:yellow; color:$mydarkcolor'>".$TYPE.'</span>';
                    }
                ...
                while (custom_fetch_array($result)) {
                    ...
                    while (custom_fetch_array($result2)) {
                        ...
                        if (mysqli_num_rows($result3) > 0) {
                            ...
                            if ($Q_EXPIRATION != '') {
                                if ($NB < $DAYS_WARNING) {
                                    $mypic = "<i class='fa fa-circle' style='color:$widget_fgorange;' title='expiration dans $NB jours'></i>";
                                }
                                if ($NB <= 0) {
                                    $mypic = "<i class='fa fa-circle' style='color:$widget_fgred;' title='date expiration dépassée' ></i>";
                                }
                            }
                        } else {
                            $mypic = '';
...
// ===============================================
    ...
    echo '</select></div>';
    if ($disabled_base == 'disabled') {
        echo "<i class='fa fa-exclamation-triangle fa-lg' style='color:$widget_fgorange;'></i> <font size=1><i>Attention seules les compétences que vous avez le droit de modifier apparaissent</i></font><p>";
    }
