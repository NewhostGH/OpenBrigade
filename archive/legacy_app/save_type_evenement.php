...
if (isset($_POST['TE_MULTI_DUPLI'])) {
    $TE_MULTI_DUPLI = intval($_POST['TE_MULTI_DUPLI']);
} else {
    $TE_MULTI_DUPLI = 0;
}
if (isset($_POST['TE_MAIN_COURANTE'])) {
    $TE_MAIN_COURANTE = intval($_POST['TE_MAIN_COURANTE']);
} else {
    $TE_MAIN_COURANTE = 0;
}
if (isset($_POST['TE_VICTIMES'])) {
    $TE_VICTIMES = intval($_POST['TE_VICTIMES']);
} else {
    $TE_VICTIMES = 0;
}
if (isset($_POST['ACCES_RESTREINT'])) {
    $ACCES_RESTREINT = intval($_POST['ACCES_RESTREINT']);
} else {
    $ACCES_RESTREINT = 0;
}

if (isset($_POST['TE_PERSONNEL'])) {
    $TE_PERSONNEL = intval($_POST['TE_PERSONNEL']);
} else {
    $TE_PERSONNEL = 0;
}
if (isset($_POST['TE_VEHICULES'])) {
    $TE_VEHICULES = intval($_POST['TE_VEHICULES']);
} else {
    $TE_VEHICULES = 0;
}
if (isset($_POST['TE_MATERIEL'])) {
    $TE_MATERIEL = intval($_POST['TE_MATERIEL']);
} else {
    $TE_MATERIEL = 0;
}
if (isset($_POST['TE_CONSOMMABLES'])) {
    $TE_CONSOMMABLES = intval($_POST['TE_CONSOMMABLES']);
} else {
    $TE_CONSOMMABLES = 0;
}
if (isset($_POST['COLONNE_RENFORT'])) {
    $COLONNE_RENFORT = intval($_POST['COLONNE_RENFORT']);
} else {
    $COLONNE_RENFORT = 0;
}
if (isset($_POST['REMPLACEMENT'])) {
    $REMPLACEMENT = intval($_POST['REMPLACEMENT']);
} else {
    $REMPLACEMENT = 0;
}
if (isset($_POST['PIQUET'])) {
    $PIQUET = intval($_POST['PIQUET']);
} else {
    $PIQUET = 0;
}
if (isset($_POST['TE_MAP'])) {
    $TE_MAP = intval($_POST['TE_MAP']);
} else {
    $TE_MAP = 0;
}
...
if (isset($_POST['TE_DPS'])) {
    $TE_DPS = intval($_POST['TE_DPS']);
} else {
    $TE_DPS = 0;
}
if (isset($_POST['TE_DOCUMENT'])) {
    $TE_DOCUMENT = intval($_POST['TE_DOCUMENT']);
} else {
    $TE_DOCUMENT = 0;
}
if (isset($_POST['TE_BILAN'])) {
    $TE_BILAN = intval($_POST['TE_BILAN']);
} else {
    $TE_BILAN = 0;
}

if (isset($_POST['EVAL_PAR_STAGIAIRES'])) {
    $EVAL_PAR_STAGIAIRES = intval($_POST['EVAL_PAR_STAGIAIRES']);
} else {
    $EVAL_PAR_STAGIAIRES = 0;
}
if (isset($_POST['PROCES_VERBAL'])) {
    $PROCES_VERBAL = intval($_POST['PROCES_VERBAL']);
} else {
    $PROCES_VERBAL = 0;
}
if (isset($_POST['FICHE_PRESENCE'])) {
    $FICHE_PRESENCE = intval($_POST['FICHE_PRESENCE']);
} else {
    $FICHE_PRESENCE = 0;
}
if (isset($_POST['ORDRE_MISSION'])) {
    $ORDRE_MISSION = intval($_POST['ORDRE_MISSION']);
} else {
    $ORDRE_MISSION = 0;
}
...
if (isset($_POST['EVAL_RISQUE'])) {
    $EVAL_RISQUE = intval($_POST['EVAL_RISQUE']);
} else {
    $EVAL_RISQUE = 0;
}
if (isset($_POST['CONVOCATIONS'])) {
    $CONVOCATIONS = intval($_POST['CONVOCATIONS']);
} else {
    $CONVOCATIONS = 0;
}
if (isset($_POST['FACTURE_INDIV'])) {
    $FACTURE_INDIV = intval($_POST['FACTURE_INDIV']);
} else {
    $FACTURE_INDIV = 0;
}

$icon = secure_input($dbc, str_replace('images/evenements/', '', $_POST['icon']));
if (! file_exists('images/evenements/'.$icon)) {
    $icon = 'WHAT.png';
}
...
if (isset($_POST['nb_stats'])) {
    $nb_stats = $_POST['nb_stats'];
} else {
    $nb_stats = 0;
}
...
if ($operation == 'insert') {
    $query = 'insert into type_evenement (TE_CODE, TE_LIBELLE, CEV_CODE, TE_ICON, TE_MULTI_DUPLI, TE_MAIN_COURANTE, TE_VICTIMES, ACCES_RESTREINT,
            ...
            EVAL_PAR_STAGIAIRES, PROCES_VERBAL, FICHE_PRESENCE, ORDRE_MISSION, CONVENTION, EVAL_RISQUE, CONVOCATIONS, FACTURE_INDIV, REMPLACEMENT, 
            PIQUET, TE_MAP, CLIENT, TE_DPS, TE_DOCUMENT, TE_BILAN)
...
if ($operation == 'delete') {
    ...
    echo "<body onload=redirect('parametrage.php?tab=2&child=5') />";
}
