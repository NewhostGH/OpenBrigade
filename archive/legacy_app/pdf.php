...
if (isset($_POST['pdf'])) {
    $doc = secure_input($dbc, $_POST['pdf']);
} elseif (isset($_GET['pdf'])) {
    $doc = secure_input($dbc, $_GET['pdf']);
}

if (isset($_POST['id'])) {
    $devis = multi_secure_int_input($_POST['id']);
} elseif (isset($_GET['id'])) {
    $devis = multi_secure_int_input($_GET['id']);
}

if (isset($_POST['SelectionMail'])) {
    $badges = multi_secure_int_input($_POST['SelectionMail']);
} elseif (isset($_GET['SelectionMail'])) {
    $badges = multi_secure_int_input($_GET['SelectionMail']);
}
...
elseif (@$devis[0] != '') {
    if (is_chef_evenement($id, $devis[0])) {
        check_all(0);
    } elseif (get_cadre(get_section_organisatrice($devis[0])) == $id) {
        check_all(0);
    } elseif (! check_rights($id, 29, get_section_organisatrice($devis[0]))) {
        check_all(29);
        check_all(24);
    }
} else {
...
if (count($devis) > 0 && ($doc == 'devis' || $doc == 'facture' || $doc == 'relance' || $doc == 'facturepayee')) {
    ...
    while ($row1 = mysqli_fetch_array($res1)) {
        ...
        echo mysqli_errno($dbc) > 0 ? "<p>$sql<br>".mysqli_error($dbc).'</p>' : '';
        while ($row = mysqli_fetch_array($res)) {
            if ($doc == 'facture' && $payee) {
                $txtfin = stripslashes('Facture payée le '.$row['date2'].'. '.$row['paiement_comment']);
            }
            $pdf->addPage('P');
            ...
            if (mysqli_num_rows($resd) > 0) {
                ...
                while ($rowd = mysqli_fetch_array($resd)) {
                    ...
                    $pdf->Cell($colonnes[4], $hauteurligne, number_format($TotalLigne, 2, ',', ' ').' '.$default_money_symbol, 1, 1, 'R');
                }
            }
            $pdf->Ln();
            $pdf->SetFont('Arial', 'B', $fontsize + 2);
            ...
            if (isset($TotalLigne)) {
                ...
                if ($discount > 0) {
                    ...
                    $pdf->MultiCell(0, $hauteurligne, 'Total avant remises = '.number_format($TotalDocAvantRemise, 2, ',', ' ').' '.$default_money_symbol, 0, 'R', false);
                ...
                $pdf->SetFont('Arial', 'B', $fontsize + 2);
                $pdf->MultiCell(0, $hauteurligne, 'Total = '.number_format($TotalDoc, 2, ',', ' ').' '.$default_money_symbol, 0, 'R', false);
                $pdf->SetFont('Arial', '', 8);
    ...
    $pdf->Ln();
    if ($pdf->GetY() <= 197) {
        $pdf->SetXY($marge_left, 197);
    } // placer la signature au minimum à 19.7 cm du haut de la page
...
if (count($devis) > 0 && $doc == 'DPS') {
    ...
    $sql = "select ev.S_ID, sf.NIV, sf.S_PARENT, evf.dimP1, evf.dimP2, evf.dimE1, evf.dimE2, 
        ...
        ';
    $res = mysqli_query($dbc, $sql) or exit("$sql<br>".mysqli_error($dbc));
    $EvtDates = '';
    while ($row = mysqli_fetch_array($res)) {
    ...
    if ($S_IMAGE_SIGNATURE != '') {
        ...
        if (@is_file($signature_file)) {
            $pdf->Image($signature_file, 200, 177, 50);
        }
    }
