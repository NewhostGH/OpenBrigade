...
if (isset($_GET['file'])) {
    $file = secure_input($dbc, $_GET['file']);
} else {
    $file = '';
}
...
if ($sst == 1) {
    $filepath = $userdocdir.'/SST';
} elseif ($sst == 2) {
    $filepath = $userdocdir.'/PSC1';
} elseif ($diplome == 1) {
    $filepath = $filesdir.'/diplomes';
} elseif ($message > 0) {
    $filepath = $filesdir.'/files_message/'.$message;
} elseif ($evenement > 0) {
    $filepath = $filesdir.'/files/'.$evenement;
} elseif ($pompier > 0) {
    $filepath = $filesdir.'/files_personnel/'.$pompier;
} elseif ($vehicule > 0) {
    $filepath = $filesdir.'/files_vehicule/'.$vehicule;
} elseif ($victime > 0) {
    $filepath = $filesdir.'/files_victime/'.$victime;
} elseif ($intervention > 0) {
    $filepath = $filesdir.'/files_interventions/'.$intervention;
} elseif ($materiel > 0) {
    $filepath = $filesdir.'/files_materiel/'.$materiel;
} elseif ($note > 0) {
    $filepath = $filesdir.'/files_note/'.$note;
} elseif ($dossier > 0) {
    $filepath = $filesdir.'/files_section/'.$section.'/'.$dossier;
} elseif ($charte == 1) {
    $filepath = $filesdir.'/charte';
} else {
    $filepath = $filesdir.'/files_section/'.$section;
}
...
function SendFile($path, $filename)
{
    ...
    if (strtoupper($extension) == 'PDF') {
        ...
        if (! @is_file($filefullpath)) {
            ...
            write_msgbox('erreur', $error_pic, 'Le fichier '.$filename." est introuvable.<br><p align=center><input type=submit class='btn btn-secondary' value='Retour' onclick='javascript:history.back(-1);'> ", 10, 0);
            exit;
        }
...
if ($diplome == 1 or $message > 0 or $charte == 1) {
    ...
    $query = 'select ds.F_ID, d.D_CREATED_BY , td.TD_SECURITY
        ...
        and d.S_ID='.$section."
        and d.D_NAME='".$file."'";
    $query .= ' and d.E_CODE='.$evenement;
    $query .= ' and d.P_ID='.$pompier;
    $query .= ' and d.NF_ID='.$note;
    $query .= ' and d.EL_ID='.$intervention;
    $result = mysqli_query($dbc, $query);
    ...
    if (! check_rights($id, intval($row['TD_SECURITY']))) {
        ...
        <p><a class='btn btn-secondary' href=\"javascript:history.back(1)\">Retour</a>", 30, 90);
        exit;
    }
    ...
    if ($evenement > 0 and $row['F_ID'] == 120) {
    ...
    if ($allowed) {
        ...
        <a href=".$basedir.'/habilitations.php target=_blank>'.$miniquestion_pic."</a> 
        <a class='btn btn-secondary' href=\"javascript:history.back(1)\">Retour</a>", 30, 30);
    }
}
