...
if ($gardes == 1) {
    ...
    if (isset($_SESSION['filter'])) {
        $defaultsection = $_SESSION['filter'];
    } else {
        $defaultsection = $_SESSION['SES_SECTION'];
    }

    $query2 = 'select EQ_ID, EQ_NOM from type_garde ';
    if ($nbsections == 0 and $pompiers == 1) {
        $query2 .= ' where S_ID = '.$defaultsection;
    }
