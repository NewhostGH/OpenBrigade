...
function populate_evenement($evenement)
{
    ...
    if ($nbsections == 0 and ! check_rights($id, 6, $S_ID)) {
        check_all(24);
    }
...
function prepare_competences($evenement)
{
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        while ($nb > 0) {
            if (isset($_PCOMP[$PS_ID])) {
                array_push($_PCOMP[$PS_ID], 0);
            } else {
                $_PCOMP[$PS_ID] = [0];
            }
            $nb = $nb - 1;
...
// disponibles
// ---------------------------------------------------------------------
function prepare_dispos($section, $year, $month, $day, $prev_next = [])
{
    global $dbc;
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        if (intval($row['OTHER']) > 0) {
            $OTHER_GARDE = 1;
        } else {
            $OTHER_GARDE = 0;
        }
        if (in_array($P_ID, $prev_next)) {
            $AGAIN = 1;
        } else {
            $AGAIN = 0;
        }
        $nb_heures = intval(get_heures_gardes($P_ID, $year, $month));
        if (is_dispo_jour($DISPO)) {
            $JOUR = 1;
        } else {
            $JOUR = 0;
        }
        if (is_dispo_nuit($DISPO)) {
            $NUIT = 1;
        } else {
            $NUIT = 0;
        }
        $A[$P_ID] = ['P_NOM' => $P_NOM, 'DISPO' => $DISPO, 'JOUR' => $JOUR, 'NUIT' => $NUIT, 'SECTION' => $P_SECTION, 'AGAIN' => $AGAIN, 'OTHER_GARDE' => $OTHER_GARDE, 'HOURS' => $nb_heures];
    }
                ...
                if ($who == 0) {
                    ...
                    if ($who > 0) {
                        if (! isset($INSCRITS[$partie][$who])) {
                            ...
                            if (isset($INSCRITS[$partie])) {
                                $currently = count($INSCRITS[$partie]);
                            } else {
                                $currently = 0;
                            }
                            if ($partie == 1) {
