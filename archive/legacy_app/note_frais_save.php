...
function change_status_note($note, $new, $old, $reject_comment)
{
    ...
    if (multi_check_rights_notes($id, '0')) {
        $tresorier_national = true;
    } else {
        $tresorier_national = false;
    }
    if ($id == $person and $new == 'ATTV' and ($old == 'CRE' or $old = 'REJ')) {
        check_all(0);
    } else {
        if (! multi_check_rights_notes($id)) {
            check_all(73);
        }
        if (! multi_check_rights_notes($id, "$section")) {
            check_all(24);
        }
        if ($id == $person and ! $tresorier_national) {
            check_all(14);
        } // seul admin ou trésorier national peut changer le statut de ses propres notes
        if ($national == 1) {
            if (! $tresorier_national) {
                check_all(24);
            }
        }
        if ($departemental == 1) {
            if (! multi_check_rights_notes($id, "$departement")) {
                check_all(24);
            }
        }
    }
    ...
    if ($new != $old) {
        if ($new == 'REMB') {
            ...
            $query = "update note_de_frais set
                ...
                NF_REMBOURSE_BY = null
                where NF_ID = '.intval($note);
        } elseif ($new == 'CRE' or $new == 'ATTV' or $new == 'ANN') {
            $query = "update note_de_frais set
                FS_CODE='".$new."',
                NF_STATUT_DATE =  NOW(),
                NF_STATUT_BY = ".$id.',
...
if (isset($_GET['nfid'])) {
    $NF_ID = intval($_GET['nfid']);
} elseif (isset($_POST['nfid'])) {
    $NF_ID = intval($_POST['nfid']);
} else {
    $NF_ID = 0;
}
if (isset($_POST['section'])) {
    ...
    $section = get_section_of_note($NF_ID);
}
...
if (isset($_POST['justif_recus'])) {
    $justif_recus = intval($_POST['justif_recus']);
} else {
    $justif_recus = 0;
}
if (isset($_POST['frais_dep'])) {
    $frais_dep = intval($_POST['frais_dep']);
} else {
    $frais_dep = 0;
}
if (isset($_POST['national'])) {
    $national = intval($_POST['national']);
} else {
    $national = 0;
}
if (isset($_POST['departemental'])) {
    $departemental = intval($_POST['departemental']);
} else {
    $departemental = 0;
}
if (isset($_POST['motif'])) {
    $motif = secure_input($dbc, $_POST['motif']);
} else {
    $motif = '';
}
if (isset($_POST['nfcode1'])) {
    $nfcode1 = intval($_POST['nfcode1']);
} else {
    $nfcode1 = 0;
}
if ($nfcode1 == 0) {
...
if (isset($_POST['nfcode2'])) {
    $nfcode2 = intval($_POST['nfcode2']);
} else {
    $nfcode2 = 0;
}
if ($nfcode2 == 0) {
...
if (isset($_POST['nfcode3'])) {
    $nfcode3 = intval($_POST['nfcode3']);
} else {
    $nfcode3 = '';
}
if (isset($_POST['nfcomment'])) {
    $nfcomment = secure_input($dbc, str_replace('"', '', $_POST['nfcomment']));
} else {
    $nfcomment = '';
}
...
if (isset($_GET['reject_comment'])) {
    $reject_comment = secure_input($dbc, str_replace('"', '', $_GET['reject_comment']));
} else {
    $reject_comment = 'null';
}

if (get_level($section) < $nbmaxlevels - 1) {
    $departement = $section;
} else {
    $departement = get_section_parent("$section");
}
...
if (isset($_FILES['userfile'])) {
    for ($i = 0; $i < count($_FILES['userfile']['name']); $i++) {
        ...
        if ($error > 0) {
            ...
            <a onclick=\"javascript:self.location.href='note_frais_edit?action=update&nfid=".$nfid.'&id='.$person."';\"><input type='submit' class='btn btn-secondary' value='Retour'></a> ", 10, 0);
            exit;
        }
    }
...
if ($action == 'delete') {
    ...
    if ($from == 'export') {
        $action_link = "<input type=submit class='btn btn-default' value='fermer' onclick='window.close();'>";
    } else {
        $action_link = "<a href='upd_personnel.php?from=notes_de_frais&id=".$person."'><input type='submit' class='btn btn-secondary' value='Retour'></a>";
    }
    ...
    if ($action == 'update') {
        ...
        if (intval($previous_verified) != intval($verified) and (check_rights($id, 75, "$section"))) {
            ...
            if ($verified == 1) {
                $query .= ' NF_VERIFIED_BY='.$id.', NF_VERIFIED_DATE=NOW()';
            } else {
                $query .= ' NF_VERIFIED_BY=null, NF_VERIFIED_DATE=null';
            }
            $query .= ' where NF_ID = '.$NF_ID;
            $result = mysqli_query($dbc, $query);
        }
    } elseif ($action == 'submit' or $action == 'validate' or $action == 'validate1' or $action == 'rembourser' or $action == 'reject') {
        if ($action == 'submit') {
            $statut = 'ATTV';
        } elseif (in_array($action, ['validate', 'validate1']) and in_array($old, ['VAL', 'VAL1'])) {
            $statut = 'VAL2';
        } elseif ($action == 'validate1') {
            $statut = 'VAL1';
...
if ($NF_ID > 0) {
    ...
    if ($action == 'insert') {
        ...
        $keyword = 'envoyée pour validation';
    } else {
        $keyword = 'modifiée';
    }
    if ($from == 'export') {
    ...
    $query = 'select nf.NF_NATIONAL as national, nf.NF_DEPARTEMENTAL as departemental, nf.TOTAL_AMOUNT as total, nf.NF_DON as don, nf.E_CODE, nf.NF_FRAIS_DEP, s.S_CODE, s.S_DESCRIPTION
            from note_de_frais nf left join section s on s.S_ID = nf.S_ID where NF_ID='.$NF_ID;
    $result = mysqli_query($dbc, $query);
    custom_fetch_array($result);
    ...
    if ($action == 'submit') {
        ...
        $message .= "\nNote de frais".$K;
        if ($S_CODE != '') {
            $message .= ', créée au niveau '.$S_CODE.' '.$S_DESCRIPTION.'.';
        }
        ...
        if (intval($E_CODE) > 0) {
            ...
            eh.EH_ID, date_format(eh.EH_DATE_DEBUT,'%d-%m-%Y') SUGGESTED_DATE
            from evenement e, evenement_horaire eh, type_evenement te
            where eh.E_CODE = e.E_CODE and e.TE_CODE = te.TE_CODE and e.E_CODE = ".$E_CODE;
        ...
        if ($destid == '') {
            $destid = get_granted(73, "$section", 'parent', 'yes').','.get_granted(74, "$section", 'parent', 'yes');
        }
        if ($national == 1) {
            $destid .= ','.get_granted_everywhere(73, 'yes').','.get_granted_everywhere(74, 'yes');
        }
        ...
        @unlink($note_file);
        if ($destid != '') {
            $info = '<p>Un email a été envoyé aux personnes suivantes, pour validation:<p><span>'.show_names_dest($destid).'</span><p>';
        }

        // echo "<pre>".$message."</pre>";
    } elseif ($action == 'validate' or $action == 'rembourser' or $action == 'validate1' or $action == 'reject') {
        if ($action != 'reject' and $assoc and $don == 1) {
            $statut_description = "Don à l'association";
        } else {
            $statut_description = get_description_statut($statut);
        }
        ...
        if ($statut == 'VAL2' and $don == 0) {
            ...
            if ($destid == '') {
                $destid = get_granted(75, "$section", 'parent', 'yes');
            }
            if ($national == 1) {
                $destid .= ','.get_granted_everywhere(75, 'yes');
            }
            ...
            if ($destid != '') {
                $info .= '<p>Et un email a été envoyé aux personnes suivantes, pour remboursement:<p><span>'.show_names_dest($destid).'</span><p>';
            }
    ...
    if ($from == 'export') {
        $action_link = "<input type=submit class='btn btn-secondary' value='".$t."' onclick='window.close();'>";
    } elseif ($action == 'insert') {
        $action_link = "<input type='submit' class='btn btn-secondary' value=".$t." onclick=\"javascript:self.location.href='note_frais_edit.php?nfid=".$NF_ID.'&person='.$person."';\">";
    } else {
        $action_link = "<input type='submit' class='btn btn-secondary' value=".$t." onclick=\"javascript:self.location.href='upd_personnel.php?tab=9&nfid=".$NF_ID.'&from=notes_de_frais&id='.$person."';\">";
    }
    ...
    if ($action == 'insert' or ($action == 'update' and ($statut == '' or $old == $statut))) {
        ...
        if (intval($justif_recus) == 0 and $syndicate == 1 and $action == 'submit') {
            $cmt = " Attention, le remboursement n'interviendra que lorsque les justificatifs originaux auront été reçus.";
        } else {
            $cmt = '';
        }
        if ($action == 'reject') {
