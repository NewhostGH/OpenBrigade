...
if (check_rights($id, $perm, "$S_ID")) {
    $admin_evenement = true;
} elseif ($nbsections > 0 and check_rights($id, $perm)) {
    $admin_evenement = true;
}
if (check_rights($id, 19)) {
    $allowed_delete = true;
}
// -----------------------------------------------------
// save changes
...
if ($action != 'undefined') {
    ...
    if (isset($_GET['substitute'])) {
        $substitute = intval($_GET['substitute']);
    } else {
        $substitute = 0;
    }
    if (isset($_GET['periode'])) {
        $eh_id = intval($_GET['periode']);
    } else {
        $eh_id = 0;
    }
    if (isset($_GET['replaced'])) {
        $replaced = intval($_GET['replaced']);
    } else {
        $replaced = 0;
    }
    if ($rid == 0) {
...
if ($rid > 0) {
    if ($grades) {
        $html .= '<img src='.$grades_imgdir.'/'.$row['g1'].".png height=20 title='".$row['g1']."' style='PADDING:1px;' class='img-max-20'>";
    }
...
if ($nb_sessions == 2) {
    ...
    if ($replaced == 0) {
        $disabled = 'disabled';
    } elseif ($approved == 1 or $rejected == 1) {
        $disabled = 'disabled';
    } elseif ($rid == 0) {
        $disabled = '';
    ...
    } elseif ($id != $replaced and ! $admin_evenement) {
        $disabled = 'disabled';
    } else {
        $disabled = '';
    }
...
if ($approved == 1 or $rejected == 1) {
    if ($grades and $substitute > 0) {
        $html .= '<img src='.$grades_imgdir.'/'.$row['g8'].".png height=20 title='".$row['g8']."' style='PADDING:1px;' class='img-max-20'>";
    }
    $html .= ' <b>'.$substitute_name."</b><input type='hidden' name='substitute' id='substitute' value=".$substitute.'>';
} elseif ($admin_evenement or
    ...
    if ($S_PRINCIPAL != '') {
        $subquery .= ' or S_ID = '.$S_PRINCIPAL.' or S_PARENT='.$S_PRINCIPAL;
    }
    ...
    if ($nb_sessions == 2) {
        if (intval($periode) > 0) {
            $query .= ' and ep.EH_ID = '.$periode;
        } elseif (intval($P) > 0) {
            $query .= ' and ep.EH_ID = '.$P;
        }
    }
    ...
    $html .= '</select>';
    if ($nbR == 0) {
        $html .= '<br><small>Le responsable pourra choisir une autre remplaçant.</small>';
    }
} else {
    if ($grades and $substitute > 0) {
        $html .= '<img src='.$grades_imgdir.'/'.$row['g8'].".png height=20 title='".$row['g8']."' style='PADDING:1px;' class='img-max-20'>";
...
elseif ($rejected == 0 and $approved == 0) {
