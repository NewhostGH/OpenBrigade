...
if (isset($_GET['C_ID'])) {
    $consommable = intval($_GET['C_ID']);
} elseif (isset($_POST['C_ID'])) {
    $consommable = $_POST['C_ID'];
} else {
    $consommable = 0;
}
if (isset($_GET['TC_ID'])) {
    $type_consommable = intval($_GET['TC_ID']);
} else {
    $type_consommable = 0;
}
if (isset($_GET['EC_ID'])) {
    $EC_ID = intval($_GET['EC_ID']);
} elseif (isset($_POST['EC_ID'])) {
    $EC_ID = $_POST['EC_ID'];
} else {
    $EC_ID = 0;
}
...
if (! is_chef_evenement($id, $evenement)) {
    ...
    if (($E_OPEN_TO_EXT == 0) and (! check_rights($id, 71, "$S_ID")) and (! check_rights($id, 15, "$S_ID"))) {
        check_all(24);
    }
}
...
echo "<body onload=redirect('evenement_display.php?evenement=".$evenement."&from=consommables&tab=3&child=3');>";
