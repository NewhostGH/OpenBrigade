...
if ($TV_ICON == '') {
    ...
    $img = '<img src='.$TV_ICON." class='img-max-35' style='margin-right: 5px;margin-top: -3px;'>";
}
...
if ($AFFECTED_TO != '') {
    ...
    if ($P_OLD_MEMBER == 1) {
        $warning = "<i class='fa fa-exclamation-triangle' style='color:orange;' title=\"Attention $owner est un ancien membre\">";
    } else {
        $warning = '';
    }
} else {
...
if (check_rights($id, 17, "$S_ID")) {
    $responsable_vehicule = true;
} else {
    $responsable_vehicule = false;
}
...
if ($tab == 1) {
    ...
    if (! $found) {
        echo "<option value='$V_ANNEE' selected>$V_ANNEE</option>";
    }
    ...
    if (my_date_diff(getnow(), $V_ASS_DATE) < 0) {
        $assurance = $widget_fgred;
    } elseif (my_date_diff(getnow(), $V_ASS_DATE) < 30) {
        $assurance = $widget_fgorange;
    }

    if (my_date_diff(getnow(), $V_CT_DATE) < 0) {
        $controle = $widget_fgred;
    }

    if (my_date_diff(getnow(), $V_REV_DATE) < 0) {
        $revision = $widget_fgorange;
    }

    if ($V_TITRE_DATE == '') {
        $titre = $mydarkcolor;
    } elseif (my_date_diff(getnow(), $V_TITRE_DATE) < 0) {
        $titre = $widget_fgred;
    } elseif (my_date_diff(getnow(), $V_TITRE_DATE) < 30) {
        $titre = $widget_fgorange;
    }
    ...
    } elseif ($from == 'personnel' and $AFFECTED_TO != '') {
