...
if (($PS_DIPLOMA == 1) or ($PS_RECYCLE == 1)) {
    if ($action == 'list') {
        ...
        if ($Q_VAL != '') {
            if ($Q_EXPIRATION != '') {
                if ($NB <= 0) {
                    $cmt = "<div class='alert alert-danger' role='alert'> Compétence $TYPE expirée depuis $Q_EXPIRATION</div>";
                } elseif ($NB < $DAYS_WARNING) {
                    $cmt = "<div class='alert alert-warning' role='alert'>Compétence $TYPE expire dans $NB jours le $Q_EXPIRATION</div>";
                } elseif ($Q_VAL == 2) {
                    $cmt = "<div class='alert alert-success' role='alert'>Compétence secondaire $TYPE expire dans $NB jours le $Q_EXPIRATION</div>";
                } elseif ($Q_VAL == 1) {
                    $cmt = "<div class='alert alert-success' role='alert'>Compétence principale $TYPE expire dans $NB jours le $Q_EXPIRATION</div>";
                }
            } elseif ($Q_VAL == 2) {
                $cmt = "<div class='alert alert-success' role='alert'>Compétence secondaire $TYPE valide</div>";
            } elseif ($Q_VAL == 1) {
                $cmt = "<div class='alert alert-success' role='alert'>Compétence principale $TYPE valide</div>";
            }
        } else {
            $cmt = "<div class='alert alert-secondary' role='alert'>En formation pour obtenir la compétence $TYPE</div>";
        }
        echo $cmt;
        // =====================================================================
    ...
    if (($disabled == '')
        ...
        if ($PS_RECYCLE == 0) {
            $query2 .= " where TF_CODE not in ('R','M')";
        }
        ...
        if ($type == 'I') {
            $cmt = 'Diplôme délivré le';
        } else {
            $cmt = 'Date de formation';
        }
        echo '<tr>
        ...
        echo ' </tr>';
        if ($type == 'I') {
            $cmt = 'Diplôme délivré par';
        } else {
            $cmt = 'Responsable de la formation';
        }
        echo '<tr>
        ...
        if ($type == 'I') {
            ...
            echo "<input type=hidden name='numdiplome' value=''>";
        }
