...
if (isset($_SESSION['maptypeid'])) {
    $maptypeid = $_SESSION['maptypeid'];
} else {
    $maptypeid = 'roadmap';
}
...
while ($row = @mysqli_fetch_array($result1)) {
    ...
    if ($P_HIDE == 0) {
        $granted_address = true;
    } elseif (check_rights($id, 2) and check_rights($id, 24)) {
        $granted_address = true;
    } elseif ($P_SECTION == '') {
        $granted_address = true;
    } elseif (check_rights($id, 2, $P_SECTION)) {
        $granted_address = true;
    } else {
        $granted_address = false;
    }
    if ($granted_address) {
        $ADDRESS = str_replace("'", "\'", fixcharset($row['ADDRESS']));
    } else {
        $ADDRESS = '';
    }
    ...
    if ($ADDRESS != '') {
        $ADDRESS_HTML = '<b>Adresse GPS: </b>'.$ADDRESS;
    } else {
        $ADDRESS_HTML = '';
    }
...
if (check_rights($id, 23) and $SMS_CONFIG[1] != 0) {
    ...
    if (intval($credits) > 0 or $credits == 'Solde illimité' or $credits == 'OK') {
        $SMS = true;
    }
}
...
while ($row = mysqli_fetch_array($result)) {
    ...
    if (in_array($row['P_ID'], $localized)) {
        if ($row['P_NOM'] != '') {
            $msg .= ' '.my_ucfirst($row['P_PRENOM']).' '.strtoupper($row['P_NOM']).',';
        } else {
            $msg .= $phone_prefix.$row['P_ID'];
        }
