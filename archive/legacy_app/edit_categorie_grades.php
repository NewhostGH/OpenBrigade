...
if (isset($_GET['insertCat'])) {
    $insertCat = secure_input($dbc, $_GET['insertCat']);
} else {
    $insertCat = '';
}
if (isset($_GET['updCat'])) {
    $updCat = secure_input($dbc, $_GET['updCat']);
} else {
    $updCat = '';
}
...
if ($updCat == 1) {
    ...
    if ($numrows == 0) {
        ...
        echo "<input type='button' class='btn btn-danger' value='Supprimer' disabled title='Des grades sont associés à cette catégorie : Catégorie système'> ";
    }
} else {
    echo "<input type = 'submit' class = 'btn btn-success' name='operation' value = 'Sauvegarder'></form> ";
}
