...
$possibleorders = ['G_LEVEL', 'P_PHOTO', 'P_STATUT', 'P_NOM', 'P_PRENOM', 'S_CODE', 'P_DATE_ENGAGEMENT', 'P_FIN', 'C_NAME', 'P_PROFESSION', 'SERVICE', 'P_CODE', 'P_REGIME'];
...
if ($company == 0) {
    $queryadd .= ' and ( company.C_ID = 0 or company.C_ID is null )';
} elseif ($company > 0) {
...
if ($currentCategory != 'ALL') {
    ...
    } elseif ($currentCategory == 'NODOG') {
        $queryadd .= ' and P_CIVILITE <= 3';
    } // > 3 = considéré comme "Animal" dans la fiche personnel
    else {
...
if ($order == 'P_REGIME') {
    $queryorder = ' order by P_STATUT, P_REGIME desc';
} elseif ($order == 'G_LEVEL') {
    $queryorder = ' order by G_CATEGORY desc, G_LEVEL desc, G_DESCRIPTION, P_NOM asc, P_PRENOM asc';
} else {
    $queryorder = ' order by '.$order;
    if ($order == 'P_PHOTO' or $order == 'P_FIN' or $order == 'P_DATE_ENGAGEMENT') {
        $queryorder .= ' desc';
    }
}
if (isset($_GET['P_GRADE'])) {
    $P_GRADE = secure_input($dbc, $_GET['P_GRADE']);
} else {
    $P_GRADE = '';
}
if ($P_GRADE != '') {
...
if ($number > 0) {
    ...
    if ($disabledReason == '') {
        if ($disabledButton == 'disabled') {
            $disabledReason = "Vous n'avez pas la permission d'envoyer des messages (n°43).";
        } else {
            $disabledReason = "Pour activer l'envoi de message, rendez-vous dans la configuration";
        }
    }
