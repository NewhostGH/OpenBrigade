...
writehead();
echo "<script type='text/javascript' src='js/personnel.js'></script>";
echo '</head>';
...
if (isset($_GET['doublon_id'])) {
    $doublon_id = intval($_GET['doublon_id']);
} elseif (isset($_POST['doublon_id'])) {
    $doublon_id = intval($_POST['doublon_id']);
} else {
    $doublon_id = 0;
}
...
if ((check_rights($id, 37, "$pid_section") or (check_rights($id, 37) and check_rights($id, 24))) and $P_STATUT == 'EXT') {
    $update_allowed = true;
} elseif ((check_rights($id, 2, "$pid_section") or (check_rights($id, 2) and check_rights($id, 24))) and $P_STATUT != 'EXT') {
    $update_allowed = true;
} else {
...
if ((check_rights($id, 37, "$doublon_section") or (check_rights($id, 37) and check_rights($id, 24))) and $P_STATUT == 'EXT') {
    $update_doublon_allowed = true;
} elseif ((check_rights($id, 2, "$doublon_section") or (check_rights($id, 2) and check_rights($id, 24))) and $P_STATUT != 'EXT') {
    $update_doublon_allowed = true;
} else {
    $update_doublon_allowed = false;
}

if ($update_allowed and $update_doublon_allowed) {
...
if ($P_BIRTHDATE != '') {
    if ($P_SEXE == 'M') {
        $born = '<br>né le '.$P_BIRTHDATE;
    } else {
        $born = '<br>née le '.$P_BIRTHDATE;
    }
    if ($P_BIRTHPLACE != '') {
        $born .= ' à '.$P_BIRTHPLACE;
    }
...
if ($P_NOM_NAISSANCE != '') {
    if ($P_SEXE == 'M') {
        $nom1 = '<br>né '.$P_NOM_NAISSANCE;
    } else {
        $nom1 = '<br>née '.$P_NOM_NAISSANCE;
    }
}
if ($P_CREATE_DATE != '') {
...
$query = " select p.P_ID, p.P_NOM, p.P_NOM_NAISSANCE, p.P_PRENOM, p.P_PRENOM2, p.P_SEXE, p.P_SECTION,
            ...
            p.P_LICENCE, p.P_LICENCE_DATE, P_LICENCE_EXPIRY, p.ID_API
...
if ($P_BIRTHDATE1 != '') {
    if ($P_SEXE == 'M') {
        $born = 'né le '.$P_BIRTHDATE1;
    } else {
        $born = 'née le '.$P_BIRTHDATE1;
    }
    if ($P_BIRTHPLACE1 != '') {
        $born .= ' à '.$P_BIRTHPLACE1;
    }
}
if ($P_NOM_NAISSANCE != '') {
    if ($P_SEXE == 'M') {
        $nom1 = ' né '.$P_NOM_NAISSANCE;
    } else {
        $nom1 = ' née '.$P_NOM_NAISSANCE;
    }
}
...
if (isset($_POST['pid'])) {
    ...
    verify_csrf('fusion');
    if ($licences) {
        $query = 'update pompier set P_LICENCE = null, P_LICENCE_DATE = null, P_LICENCE_EXPIRY = null, ID_API = null where P_ID='.$doublon_id;
        mysqli_query($dbc, $query);
        if ($P_LICENCE_EXPIRY != '') {
            $exp = "P_LICENCE_EXPIRY = '".$P_LICENCE_EXPIRY."'";
        } else {
            $exp = 'P_LICENCE_EXPIRY = null';
        }
        $query = "update pompier set P_LICENCE = '".$P_LICENCE."', P_LICENCE_DATE = '".$P_LICENCE_DATE."', ".$exp.', ID_API = '.$ID_API.' where P_ID='.$pid;
        mysqli_query($dbc, $query);
    }
...
} elseif ($P_BIRTHDATE != '' and $P_BIRTHDATE1 != '') {
...
if (! $update_doublon_allowed) {
    $alert2 = "<div class='alert alert-warning' role='alert'> Vous n'avez pas les permissions de modifier la fiche 
...
$out .= "<div class='col-sm-5' align=center>
          <div class='card hide card-default graycarddefault' align=center>
            <div class='card-header graycard'>
                <div class='card-title'><h6><strong>Informations homonyme</strong></h6></div>
            </div>
...
if ($NB1 > 0 and $update_possible) {
    $move = " <label for='competences'>déplacer vers N°$pid</label> <input type='checkbox' id='competences' name='competences' checked value=1
...
if ($NB2 > 0 and $update_possible) {
    $move = " <label for='formations'>déplacer vers N°$pid</label> <input type='checkbox' id='formations' name='formations' checked value=1
...
if ($NB3 > 0 and $update_possible) {
    $move = " <label for='participations'>déplacer vers N°$pid</label> <input type='checkbox' id='participations' name='participations' checked value=1
...
$out .= " <p> <input type='button' class='btn btn-secondary' value='Annuler' onclick=\"bouton_redirect('upd_personnel.php?pompier=".$pid."');\">";
if ($update_possible) {
    $out .= " <input type='submit' class='btn btn-success' value='Fusionner'></form>";
}
$out .= '</div></div></div>';
