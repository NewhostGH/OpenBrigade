...
if (isset($_POST['DESCRIPTION'])) {
    $DESCRIPTION = secure_input($dbc, $_POST['DESCRIPTION']);
}
if (isset($_POST['PS_EXPIRABLE'])) {
    $PS_EXPIRABLE = intval($_POST['PS_EXPIRABLE']);
}
if (isset($_POST['PS_AUDIT'])) {
    $PS_AUDIT = intval($_POST['PS_AUDIT']);
}
if (isset($_POST['PS_DIPLOMA'])) {
    $PS_DIPLOMA = intval($_POST['PS_DIPLOMA']);
} else {
    $PS_DIPLOMA = 0;
}
if (isset($_POST['PS_NUMERO'])) {
    $PS_NUMERO = intval($_POST['PS_NUMERO']);
}
if (isset($_POST['PS_FORMATION'])) {
    $PS_FORMATION = intval($_POST['PS_FORMATION']);
}
if (isset($_POST['PS_SECOURISME'])) {
    $PS_SECOURISME = intval($_POST['PS_SECOURISME']);
}
if (isset($_POST['PS_NATIONAL'])) {
    $PS_NATIONAL = intval($_POST['PS_NATIONAL']);
}
if (isset($_POST['PS_PRINTABLE'])) {
    $PS_PRINTABLE = intval($_POST['PS_PRINTABLE']);
} else {
    $PS_PRINTABLE = 0;
}
if (isset($_POST['PS_PRINT_IMAGE'])) {
    $PS_PRINT_IMAGE = intval($_POST['PS_PRINT_IMAGE']);
} else {
    $PS_PRINT_IMAGE = 0;
}
if (isset($_POST['PS_RECYCLE'])) {
    $PS_RECYCLE = intval($_POST['PS_RECYCLE']);
}
if (isset($_POST['PS_USER_MODIFIABLE'])) {
    $PS_USER_MODIFIABLE = intval($_POST['PS_USER_MODIFIABLE']);
}
...
if (isset($_POST['PH_CODE'])) {
    $PH_CODE = secure_input($dbc, $_POST['PH_CODE']);
} else {
    $PH_CODE = '';
}
if (isset($_POST['DAYS_WARNING'])) {
    $DAYS_WARNING = intval($_POST['DAYS_WARNING']);
} else {
    $DAYS_WARNING = 0;
}
if (isset($_POST['PH_LEVEL'])) {
    $PH_LEVEL = intval($_POST['PH_LEVEL']);
}
if ($PH_CODE == '') {
    $PH_CODE = 'null';
    $PH_LEVEL = 'null';
