...
$query = "select e.TE_CODE, e.E_LIBELLE, e.E_CLOSED, e.E_CANCELED, e.E_OPEN_TO_EXT, e.S_ID, e.E_PARENT, eh.EH_ID,
        ...
        DATEDIFF(NOW(), EH_DATE_DEBUT) as NB_DAYS_START,
...
while ($row = @mysqli_fetch_array($result)) {
    ...
    $NB_DAYS_START[$i] = $row['NB_DAYS_START'];
    if ($EH_DUREE[$i] == '') {
        $EH_DUREE[$i] = 0;
    }
...
if ($pid > 0) {
    ...
    } elseif ($P_PHOTO != '' and file_exists($trombidir.'/'.$P_PHOTO)) {
        $title .= "<img src='".$trombidir.'/'.$P_PHOTO."'  border='0' style='border-radius:10px;max-width:50px;' > ";
    }
...
if ($vid > 0) {
    ...
    custom_fetch_array($result);
    if ($TV_ICON != '') {
        $img = '<img src='.$TV_ICON." class='img-max-40'>";
    } else {
        $img = '';
    }
    $title = $img."<span class='ebrigade-h4'> ".$TV_CODE.' '.$V_MODELE.' '.$V_INDICATIF.'</span> <i>'.$V_IMMATRICULATION.'</i>';
}
...
if (isset($_POST['pid'])) {
    ...
    for ($k = 1; $k <= $nbmaxsessionsparevenement; $k++) {
        ...
        if (isset($_POST['absent_'.$k])) {
            $absent[$k] = 1;
        } else {
            $absent[$k] = 0;
        }
        if (isset($_POST['excuse_'.$k])) {
            $excuse[$k] = 1;
        } else {
            $excuse[$k] = 0;
        }
        if (isset($_POST['astreinte_'.$k])) {
            $astreinte[$k] = 1;
        } else {
            $astreinte[$k] = 0;
        }
        if (isset($_POST['status_'.$k])) {
            $status[$k] = 1;
        } else {
            $status[$k] = 0;
        }
        ...
        if ($identique[$k] >= 0) {
            if ($nbp == 0) {
                ...
                if ($identique[$k] == 0) {
                    ...
                    $query .= 'null,null,null,null,'.$EH_DUREE[$k].',';
                }
                if ($EP_COMMENT != '') {
                    $query .= '"'.$EP_COMMENT.'",';
                } else {
            ...
            if (mysqli_affected_rows($dbc)) {
                insert_log('UPDHOR', $pid, 'horaires de '.my_ucfirst(get_prenom($pid)).' '.strtoupper(get_nom($pid)), $evenement);
            }
...
if ($pid > 0) {
    ...
    if ($gardeSP and check_rights($id, 6, $organisateur) and $nbsections == 0) {
        $granted_personnel = true;
    }
    if ($gardeSP and check_rights($id, 6) and $nbsections > 0) {
        $granted_personnel = true;
    }
    if (! $granted_personnel and $id != $pid) {
        check_all(10);
    }
    if (! $granted_personnel and $id != $pid) {
    ...
    echo "<div class='col-sm-12'>
            <div class='card hide card-default graycarddefault cardtab' style='margin-bottom:5px'>
                <div class='card-header graycard cardtab'>
                    <div class='card-title'><strong> Modifier horaires </strong></div>
                </div>
    ...
    for ($k = 1; $k <= $nbmaxsessionsparevenement; $k++) {
        if (isset($EH_ID[$k])) {
            ...
            if ($granted_personnel) {
                $granted_trash_add = true;
            } elseif (check_rights($id, 39) and $pid == $id) {
                ...
                if ($row1['NB_DAYS'] < 1 and $row1['NB_DAYS'] >= 0) {
                    $granted_trash_add = true;
                }
            }
...
if ($vid > 0) {
    if (check_rights($id, 17, $organisateur) or check_rights($id, 15, $organisateur)) {
        $granted_vehicule = true;
    } elseif (! $granted_vehicule) {
        check_all(17);
    }
    ...
    for ($k = 1; $k <= $nbmaxsessionsparevenement; $k++) {
        if (isset($EH_ID[$k])) {
            // si participation enregistree pour la session
            if (isset($EVH_ID[$k])) {
                $style = "style='background-color:transparent !important;'";
            ...
            // si pas de participation enregistree pour la session
                ...
                $disabled = 'disabled';
                $style = "style='display:none'";
                $antistyle = "style='background-color:transparent !important;'";
                $EV_DATE_DEBUT[$k] = $EH_DATE_DEBUT[$k];
                $EV_DATE_FIN[$k] = $EH_DATE_FIN[$k];
                $EV_DEBUT[$k] = $EH_DEBUT[$k];
                $EV_FIN[$k] = $EH_FIN[$k];
...
echo '</td></tr></table></div></div>'; // end cadre
