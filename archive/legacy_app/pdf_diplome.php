...
if ($mode == 3 and ! $printfulldiplome and ! $print_image) {
    $mode = 4;
}
...
$taille_org = [8, 9, 10, 11, 12, 14, 16, 18];
$style_org = ['', 'B', 'I', 'BI'];
$police_org = ['Courier', 'Arial', 'Times'];

// paramétrage global ou local?
if (get_level("$S_ID") + 1 == $nbmaxlevels) {
    $section_diplome = get_section_parent($S_ID);
} else {
    $section_diplome = $S_ID;
}
...
if ($pid > 0) {
    $query .= ' and pf.P_ID = '.$pid;
}
if ($numerotation == 1) {
    $query .= " and pf.PF_DIPLOME is not null and pf.PF_DIPLOME <> ''";
}
        ...
        if ($affichage[$j] == '12' and $actif[$j] == 1) { // importer une image de la signature du Président
            ...
            if ($S_IMAGE_SIGNATURE != '') {
                ...
                if (@is_file($signature_file)) {
                    $pdf->Image($signature_file, $pos_x[$j], $pos_y[$j], 40);
                }
            }
        } else {
            if ($affichage[$j] == '0') {
                $aff = my_ucfirst($data['P_PRENOM']).' '.strtoupper($data['P_NOM']);
            } elseif ($affichage[$j] == '1') {
                $aff = strtoupper($data['P_PRENOM']).' '.strtoupper($data['P_NOM']);
            } elseif ($affichage[$j] == '2') {
                $aff = my_ucfirst($data['P_PRENOM']).' '.my_ucfirst($data['P_NOM']);
            } elseif ($affichage[$j] == '3') {
                $aff = $data['PF_DATE'];
            } elseif ($affichage[$j] == '4') {
                $aff = $periode;
            } elseif ($affichage[$j] == '5') {
                $aff = $data['P_BIRTHPLACE'];
                if ($data['P_BIRTH_DEP'] != '') {
                    $aff .= ' ('.$data['P_BIRTH_DEP'].')';
                }
            } elseif ($affichage[$j] == '6') {
                $aff = $data['P_BIRTHDATE'];
            } elseif ($affichage[$j] == '7') {
                if ($mode > 1) {
                    $aff = $data['PF_DIPLOME'];
                } else {
                    $aff = '';
                }
            } elseif ($affichage[$j] == '8') {
                $aff = $fin;
            } elseif ($affichage[$j] == '9') {
                $aff = $annexe[$j];
            } elseif ($affichage[$j] == '10') {
                $aff = $organisateur;
            } elseif ($affichage[$j] == '11') {
                $aff = $organisateur_city;
            } elseif ($affichage[$j] == '13') {
                $aff = $debut;
            } elseif ($affichage[$j] == '14') {
                $aff = $moniteurs;
            } elseif ($affichage[$j] == '15') {
                $aff = $data['TC_LIBELLE'].' '.my_ucfirst($data['P_PRENOM']).' '.strtoupper($data['P_NOM']);
            } elseif ($affichage[$j] == '16') {
                $aff = $lieu;
            } elseif ($affichage[$j] == '17') {
                if ($mode > 1) {
                    $aff = $data['ANNEE'].' '.$data['PF_DIPLOME'];
                } else {
                    $aff = '';
                }
            } elseif ($affichage[$j] == '18') {
                $aff = $evenement;
            } elseif ($affichage[$j] == '19' and $actif[$j] == 1) { // Nom du Président national
