...
if ($type == 'OM') {
    $type_long = 'Ordre de Mission';
} else {
    $type_long = 'Autorisation Spéciale Absence';
}
...
$query = 'select E_LIEU, E_LIBELLE from evenement where E_CODE='.$evenement;
$result = mysqli_query($dbc, $query);
custom_fetch_array($result);
...
$A = 'à';
if (substr(strtolower($E_LIEU), 0, 5) == 'stade') {
    $A = 'au';
}
...
if ($type == 'ASA') {
    ...
    $pdf->SetFont('Times', '', 11);
    $pdf->SetXY($GoX, $y);
    $pdf->MultiCell(140, 5, $dates.', afin de se rendre '.$A.' '.$E_LIEU.' '.$txt2);
    GoDown(3);
    $pdf->SetXY($GoX, $y);
