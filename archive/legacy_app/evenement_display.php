...
if (isset($_GET['child'])) {
    $child = secure_input($dbc, $_GET['child']);
} else {
    $child = 0;
}

if (check_rights($_SESSION['id'], 25)) {
    $granted_for_all = true;
} else {
    $granted_for_all = false;
}
...
$possibleorders = ['E_PARENT', 'EE_NAME', 'TSP_ID', 'TP_LIBELLE', 'P_NOM', 'G_LEVEL'];
...
if (isset($_SESSION['from_interventions'])) {
    ...
    $from = secure_input($dbc, $_GET['from']);
} elseif (isset($_SESSION['eventabdoc'])) {
    $from = 'document';
...
if (isset($_GET['section'])) {
    $section = intval(secure_input($dbc, $_GET['section']));
} else {
    $section = $mysection;
}
if (isset($_GET['type'])) {
...
if (isset($_GET['day'])) {
    $day = secure_input($dbc, $_GET['day']);
} else {
    $day = '';
}
if (isset($_GET['pid'])) {
    $pid = secure_input($dbc, $_GET['pid']);
} else {
    $pid = '';
}
...
if ($nb == 0) {
    ...
    exit;
} elseif ($S_HIDE == 1 and $E_OPEN_TO_EXT == 0) {
...
while (custom_fetch_array($result)) {
    if ($i == 1) {
        if ($TE_CODE == 'MC') {
            check_feature('main_courante');
        } else {
            check_feature('evenements');
        }
        ...
        if ($E_EXTERIEUR == 1) {
            $E_LIEU .= " <span style='background-color:yellow;'>hors département</span>";
        }
    ...
    $tmp = explode('-', $EH_DATE_DEBUT[$i]);
    $year1[$i] = $tmp[0];
    $month1[$i] = $tmp[1];
    $day1[$i] = $tmp[2];
    $date1[$i] = mktime(0, 0, 0, $month1[$i], $day1[$i], $year1[$i]);
    $tmp = explode('-', $EH_DATE_FIN[$i]);
    $year2[$i] = $tmp[0];
    $month2[$i] = $tmp[1];
    $day2[$i] = $tmp[2];
...
if (isset($_GET['tab'])) {
    $tab = secure_input($dbc, $_GET['tab']);
} elseif ($from == 'inscription' || $from == 'gardes' || $from == 'calendar') {
    $tab = 2;
} elseif ($from == 'vehicule') {
    $tab = 3;
} elseif ($from == 'materiel') {
    $tab = 4;
} elseif ($from == 'consommables') {
    $tab = 9;
} elseif ($from == 'formation') {
    $tab = 5;
} elseif ($from == 'tarif') {
    $tab = 6;
} elseif ($from == 'document') {
    $tab = 7;
} elseif ($from == 'interventions') {
    $tab = 8;
} elseif ($from == 'choice' and $TE_CODE == 'MC') {
    $tab = 8;
} elseif ($from == 'piquets') {
    $tab = 11;
} elseif ($from == 'Logistique') {
    $tab = 14;
} else {
    $tab = '1';
}
...
if (is_operateur_pc($id, $evenement)) {
    $is_operateur_pc = true;
} else {
    $is_operateur_pc = false;
}
...
if (get_level("$organisateur") > $nbmaxlevels - 2) {
    $departement = get_family(get_section_parent("$organisateur"));
} else {
    $departement = get_family("$organisateur");
}
    ...
    if (get_section_parent("$mysection") != get_section_parent("$S_ID")) {
        ...
        if (! in_array($mysection, $list)) {
            ...
            if (! in_array($mysection, $list)) {
                ...
                if (get_level("$S_ID") == $nbmaxlevels - 2) {
                    $sections_roles_list = get_family("$S_ID");
                } elseif (get_level("$S_ID") == $nbmaxlevels - 1) {
                    $sections_roles_list = get_family(get_section_parent("$S_ID"));
                } else {
                    $sections_roles_list = "$S_ID";
                }
...
if (check_rights($id, 15, $organisateur) or $chef) {
    $granted_event = true;
} else {
    $granted_event = false;
}
if (check_rights($id, 10, $organisateur) or $chef) {
    $granted_personnel = true;
} else {
    $granted_personnel = false;
}
if (check_rights($id, 17, $organisateur) or $granted_event or $chef) {
    $granted_vehicule = true;
} else {
    $granted_vehicule = false;
}
if (check_rights($id, 19, $organisateur)) {
    $granted_delete = true;
} else {
    $granted_delete = false;
}
...
} elseif ($gardeSP and check_rights($id, 6) and $nbsections > 0) {
    $granted_inscription = true;
} elseif (($OPEN_TO_ME == 1) and (check_rights($id, 28) or check_rights($id, 10))) {
    $granted_inscription = true;
}
...
if ($granted_event or $granted_vehicule) {
    $granted_consommables = true;
} else {
...
if ($granted_event) {
    ...
    if ($FACTURE_INDIV == 1 and $E_TARIF > 0) {
        $NB4++;
    }
    if ($FICHE_PRESENCE == 1 and $E_CLOSED == 1) {
        $NB4++;
    }
    if ($PROCES_VERBAL == 1 and $E_CLOSED == 1 and $PS_ID != '' and in_array($TF_CODE, ['I', 'C', 'R', 'M'])) {
        $NB4++;
    }
    if ($CONVENTION == 1 and $E_PARENT == 0) {
        $NB4++;
    }
    if ($CONVENTION == 1 and $E_PARENT == 0 and signature_president_disponible($S_ID)) {
        $NB4++;
    }
    ...
    if ($CONVOCATIONS == 1 and $E_CLOSED == 1) {
        $NB4 = $NB4 + 2;
    } // convocations collective et individuelles
...
if ($granted_event or $is_present) {
    if ($ORDRE_MISSION == 1 and $E_CLOSED == 1) {
        $NB4++;
    }
}
...
if ($E_CLOSED == 1 and ($type_doc == 'SST' or $type_doc == 'PRAP') and $granted_event) {
    $NB4 = $NB4 + 1;
}
...
if ($pompiers == 1 and $nbsections == 0) {
    $cmt = $S_CODE.' - '.$cmt;
}
$buttons_container = '';
if (! $print) {
    ...
    if ($E_CLOSED == 0 and (! $print) and $public_can_enroll and $TE_PERSONNEL == 1) {
        if (my_date_diff(date('d').'-'.date('m').'-'.date('Y'), $very_end) >= 0) {
            if ($num == 0) {
                ...
                } elseif (($OPEN_TO_ME == -2) or ($OPEN_TO_ME == -3)) {
                    $buttons_container .= " <button type='button' title data-original-title=\"S'inscrire\" class='btn btn-default noprint' value=\"S'inscrire\" $disabled_inscr
...
// infos generales
if ($tab == 1 or $tab == 51 or $tab == 52 or $tab == 53 or $tab == 54 or $tab == 55 or $tab == 58 or $tab == 59 or $tab == 60 or $tab == 63) {
    $class = 'active';
} else {
    $class = '';
}
...
if ($ended > 0) {
    ...
    if (! $print) {
        echo "<table class='noBorder noprint'><tr><td><i class='fa fa-exclamation-triangle' style='color:$widget_fgorange' title=\"Cette activité n'est plus modifiable, sauf par les personnes ayant la permission n°19\"></i></td>
...
if (intval($E_PARENT) > 0) {
    $evts_list = $evenement.','.intval($E_PARENT);
} else {
    $evts_list = $evenement;
}
...
while ($rowe = @mysqli_fetch_array($resulte)) {
    array_push($equipes, [$rowe['EE_ID'], $rowe['EE_NAME']]);
}
$nbe = count($equipes);
...
if ($tab == 1 or $print) {
    ...
    if (! $print) {
        ...
        if ($E_AUTOCLOSE_BEFORE >= 0) {
            ...
            if (isset($REMAINING) and $REMAINING <= 0) {
                $t .= ' Et il est trop tard pour le réouvrir.';
            }
        }
        ...
        } elseif ($granted_delete and $gardeSP and ($E_PARENT != null or $E_EQUIPE == 0)) {
            $csrf = generate_csrf('delete');
            $laterBt .= " <input type='button' class='btn btn-danger' value='Supprimer' 
            onclick='bouton_redirect(\"evenement_save.php?action=delete&evenement=".$evenement.'&csrf_token_delete='.$csrf."\",\"delete\");'> ";
        }
    ...
    for ($i = 1; $i <= $nbmaxsessionsparevenement; $i++) {
        if ($nbsessions == 1) {
            $t = 'Dates et heures';
        } elseif (isset($EH_ID[$i])) {
            $t = 'Date Partie '.$EH_ID[$i];
        }
        if (isset($horaire_evt[$i])) {
            if ($description_partie[$i] != '') {
                $dp = ' - <i>'.$description_partie[$i].'</i>';
            } else {
                $dp = '';
            }
    ...
    if ($granted_event and ($E_CONTACT_LOCAL != '' or $E_CONTACT_TEL != '')) {
        ...
        if ($E_CONTACT_TEL != '') {
            if (is_iphone()) {
                $E_CONTACT_TEL = " - <a href='tel:".$E_CONTACT_TEL."'>".$E_CONTACT_TEL.'</a> ';
            } else {
                $E_CONTACT_TEL = '- '.$E_CONTACT_TEL;
            }
        }
    ...
    if ($C_ID != '' and $C_ID > 0) {
        ...
        if (check_rights($id, 37)) {
            $edit = "<a class='btn btn-default btn-action' href='$link'><i class='fa fa-edit fa-lg'></i></a>";
        } else {
            $edit = '';
        }
    ...
    if (! $print and $TE_PERSONNEL == 1) {
        if ($E_CANCELED == 1) {
            ...
            if ($E_CLOSED == 1) {
                echo "<div style='float:right;display:inline-block;$widget_all_orange;padding:5px;border-radius:5px;margin-top:-5px'>Inscriptions fermées</div>";
            } elseif ($OPEN_TO_ME == 0) {
                echo "<div style='float:right;display:inline-block;$widget_all_orange;padding:5px;border-radius:5px;margin-top:-5px'>Inscriptions interdites pour les personnes des autres ".$levels[3].'s</div>';
            } elseif ($OPEN_TO_ME == -1) {
                echo "<div style='float:right;display:inline-block;$widget_all_orange;padding:5px;border-radius:5px;margin-top:-5px'>Inscriptions possibles pour les personnes des autres ".$levels[3].'s par leur responsable</div>';
            } else {
                echo "<div style='float:right;display:inline-block;$widget_all_green;padding:5px;border-radius:5px;margin-top:-5px'>Inscriptions ouvertes</div>";
            }
        }
    }
    ...
    if ($E_URL != '') {
        if ($cisname == 'Protection Civile') {
            $t = 'Lien URL vers calendrier ADPC';
        } else {
    ...
    if ($syndicate == 1) {
        $t = "Gestionnaire de l'activité";
    } else {
        $t = 'Responsable '.$cisname;
    }
    ...
    if (count($chefs) > 0) {
        for ($c = 0; $c < count($chefs); $c++) {
            ...
            $phone = $rowz['P_PHONE'];
            $P_HIDE = $rowz['P_HIDE'];
            if ($syndicate == 1 or intval($E_TEL) > 0) {
                $phone = '';
            } elseif ($phone != '') {
                if (is_iphone()) {
    ...
    if ($TE_PERSONNEL == 1 and $TE_CODE != 'MC') {
        if ($nbsessions == 1) {
            $showcpt = "<tr style='color:#3f4254'><td width=30%>Personnel requis </td><td>";
        } else {
            $showcpt = '<tr><td colspan=2>Personnel requis ';
        }
        ...
        if (! $print and $granted_event and ! $gardeSP) {
            ...
            if ($nboptions > 0) {
                echo "<span class='badge'>".$nboptions.' options</span>';
            }
        ...
        while ($rowm = mysqli_fetch_array($resultm)) {
            $i = $rowm['EH_ID'];
            if ($nbsessions > 1) {
                $showcpt .= "</td></tr><tr style='color:#3f4254'><td align=right><font size=1>partie ".$i.'</font></td><td>';
            }
            $nbt = 0;
            ...
            while (custom_fetch_array($resultp)) {
                ...
                if ($inscrits < 2) {
                    $desc .= $inscrits.' participant ayant cette compétence valide.';
                } else {
            ...
            if ($granted_event and (! $print) and $changeallowed) {
                if ($competences) {
                    $cmt = 'Modifier les compétences demandées';
                } else {
                    $cmt = 'Modifier le nombre de personnes demandées';
                }
    ...
    if ($TE_CODE == 'DPS') {
        ...
        if ($TAV_ID == 1 or $TAV_ID == '') {
            ...
            if ($tagid != '' and (! $print)) {
                ...
                } elseif ($DPS_MAX_TYPE != '' and $DPS_MAX_TYPE < $TAV_ID) {
        ...
        if ($E_FLAG1 == 1) {
            $interassociatif = 'Inter-associatif, ';
        } else {
            $interassociatif = '';
        }
    ...
    if ($E_AUTOCLOSE_BEFORE > -1) {
        if ($E_AUTOCLOSE_BEFORE >= 24) {
            ...
            $TIME_BEFORE = 'au début, quand elle commence';
        }
    ...
    if ($C_ID != '' and $C_ID > 0) {
        ...
        $queryr = 'select p.P_ID, p.P_NOM, p.P_PRENOM, '.phone_display_mask('p.P_PHONE').' P_PHONE , tcr.TCR_DESCRIPTION
                    ...
                    and cr.C_ID='.$C_ID;
        if ($TE_CODE == 'FOR') {
            $queryr .= " and cr.TCR_CODE='RF'";
        } else {
            $queryr .= " and cr.TCR_CODE='RO'";
        }
        $resultr = mysqli_query($dbc, $queryr);
        $rowr = mysqli_fetch_array($resultr);
        ...
        if ($nb_renforts > 0 or $E_COLONNE_RENFORT == 1) {
            ...
            while (custom_fetch_array($resultA)) {
                ...
                while ($rowR = @mysqli_fetch_array($resultR)) {
                    ...
                    $EH_ID0[$j] = $rowR['EH_ID'];
                    if ($EH_DATE_DEBUT0[$j] != $EH_DATE_FIN0[$j]) {
                        $dates_renfort = $EH_DATE_DEBUT0[$j].' au '.$EH_DATE_FIN0[$j];
                    } else {
                        $dates_renfort = $EH_DATE_DEBUT0[$j];
                    }
                ...
                if ($E_COLONNE_RENFORT == 1) {
                    if (evenements_overlap($evenement, $CE_CODE)) {
                        echo "<i class='fa fa-clock fa-lg' style='color:#1bc5bd'  title=\"".$detail_renfort[$j].'"></i>';
                    } else {
                        echo "<i class='fa fa-ban fa-lg' style='color:$widget_fgred'  title=\"Les dates de ".$renfort_label." ne correspondent pas à celles de l'activité principale.\"></i>";
                    }
                    ...
                    for ($i = 1; $i <= $nbmaxsessionsparevenement; $i++) {
                        if (isset($EH_ID[$i])) {
                            if (isset($EH_ID0[$j])
                                ...
                                if ($nbpr == 1) {
                                    $c = ' <font size=1>'.$detail_renfort[$j].'</font>';
                                }
                                if ($CE_CANCELED == 0) {
                                    $clock = $widget_fggreen;
                                } else {
                                    $clock = 'red';
    ...
    if (! $print and $show_logistique) {
        ...
        if ($vehicules == 1 and $TE_VEHICULES == 1 and (! $gardeSP or $assoc == 1)) {
            ...
            if ($demandes > 0) {
                if ($inscrits >= $demandes) {
                    $colors = $widget_all_green;
                } else {
                    $colors = $widget_all_red;
                }
            ...
            echo $detail;
            if ($granted_event) {
                echo " <a class='btn btn-default btn-action' href='".$url."' title='Modifier les véhicules et matériel demandés'><i class='fa fa-edit fa-lg noprint' ></i></a>";
            }
            echo '</td><tr>';
        }
        // Matériel requis
    ...
    if ($TE_MAIN_COURANTE == 1 and intval($E_PARENT) == 0 and ! $print) {
        if ($granted_event) {
            $K = '<a href=evenement_display.php?from=interventions&evenement='.$evenement." title=\"Modifier les statistiques dans l'onglet Rapport\">Statistiques</a>";
        } else {
            $K = 'Statistiques';
        }
        echo "<div class='card hide card-default graycarddefault' align=center >
...
if ($tab == 2 or $tab == 61 or $print) {
    ...
    if ($child == 1) {
        ...
        if ($E_CLOSED == 0 and $E_CANCELED == 0 and ! $print and $changeallowed) {
            if ($granted_personnel or $granted_inscription) {
                ...
                } elseif ($nbsections > 0 and ! $gardeSP and check_rights($id, 6, $organisateur)) {
        ...
        if ($gardeSP and $E_VISIBLE_INSIDE == 0) {
            if ($nbsections == 0 and ! check_rights($id, 6, "$organisateur")) {
                $tableau_visible = false;
            }
            if ($nbsections > 0 and ! check_rights($id, 6)) {
                $tableau_visible = false;
            }
        }
        if ($tableau_visible) {
            ...
            } elseif ($_date != $last and $nbsessions > 1) {
            ...
            } elseif (check_rights($id, 56) and in_array($organisateur, [$_SESSION['SES_PARENT'], $_SESSION['SES_SECTION']])) {
                $personnel_visible = true;
            } else {
            ...
            if ($E_NB == 0) {
                $cmt = 'Pas de limite sur le nombre';
            } else {
                $cmt = "Requis <span class='badge'>$E_NB</span>";
            }
            if ($TE_CODE == 'FOR' and $E_NB_STAGIAIRES > 0) {
                $cmt .= ", dont places stagiaires $E_NB_STAGIAIRES";
            }

            $queryf = "select count(1) from type_participation where TE_CODE='".$TE_CODE."'";
            if ($gardeSP) {
                $queryf .= ' and EQ_ID in (0,'.intval($E_EQUIPE).')';
            }
            ...
            } elseif ($evenement_periode > 0 and $found2) {
                $query .= '    and ep.EH_ID='.$evenement_periode;
            }
            $query .= ' group by ep.P_ID';
            if ($order == 'EE_NAME') {
                $query .= ' order by EE_NAME desc, p.P_NOM';
            } elseif ($order == 'TP_LIBELLE') {
                $query .= ' order by TP_LIBELLE desc, p.P_NOM';
            } elseif ($order == 'TSP_ID') {
                $query .= ' order by TSP_ID, p.P_NOM';
            } elseif ($order == 'E_PARENT') {
                $query .= '    order by e.E_PARENT, ep.E_CODE asc, p.P_NOM';
            } elseif ($order == 'P_NOM') {
                $query .= '    order by p.P_NOM, ep.EP_DATE asc';
            } elseif ($order == 'G_LEVEL') {
                $query .= '    order by g.G_LEVEL desc, p.P_NOM';
            } elseif ($gardeSP and $grades) {
                $query .= '    order by e.E_PARENT, TP_NUM, ep.E_CODE asc, g.G_LEVEL desc, p.P_NOM';
            } else {
                $query .= ' order by p.P_NOM, ep.EP_DATE asc';
            }
            ...
            if ($nbparticipants > 0 or $NP > 0) {
                ...
                if ($grades) {
                    echo '<td width=40><a href=evenement_display.php?evenement='.$evenement."&tab=2&order=G_LEVEL title='trier par Grade décroissant'>Grade</a></td>";
                }
                if ($print) {
                    echo "<td style='min-width:300px;' >Personnel présent</td>";
                } else {
                    echo "<td style='min-width:300px;'>";
                    if ("$evts" != "$evenement") {
                        echo ' <a href=evenement_display.php?evenement='.$evenement."&tab=2&order=E_PARENT title='trier par ".$renfort_label."'>".ucfirst($renfort_label).'</a> /';
                    }
                    echo ' <a href=evenement_display.php?evenement='.$evenement."&tab=2&order=P_NOM title='trier par Nom'>Inscrits</a></td>";
                }
                if (! $print) {
                    echo "<td style='max-width:160px; min-width:60px' align=left>Téléphone</td>";
                }
                if ($nbfn > 0) {
                    if ($granted_event and (! $print) and $changeallowed) {
                        echo '<td width=170><a href=evenement_display.php?evenement='.$evenement."&tab=2&order=TP_LIBELLE title='trier par fonction'>Fonction</a></td>";
                    } else {
                        echo "<td style='min-width:80px;'>Fonction</td>";
                    }
                } else {
                    echo '<td></td>';
                }
                if ($nbe > 0) {
                    echo "<td style='min-width:80px;'><a href=evenement_display.php?evenement=".$evenement."&tab=2&order=EE_NAME title='trier par équipe'>Equipe</a></td>";
                }
                ...
                while (custom_fetch_array($result)) {
                    ...
                    if ($EC != $prevEC) {
                        ...
                        while ($rowR = @mysqli_fetch_array($resultR)) {
                            ...
                            if ($EH_DATE_DEBUT0[$n] != $EH_DATE_FIN0[$n]) {
                                $dates_renfort = $EH_DATE_DEBUT0[$n].' au '.$EH_DATE_FIN0[$n];
                            } else {
                                $dates_renfort = $EH_DATE_DEBUT0[$n];
                            }
                    ...
                    if ($grades) {
                        $P_GRADE = "<img src='".$G_ICON."' title='".$G_DESCRIPTION."' class='img-max-22' style='border-radius: 2px;' >";
                    } else {
                        $P_GRADE = '';
                    }
                    $F_PS_ID = $PS_ID;
                    ...
                    $listePompiers .= $P_ID.',';
                    if (in_array($P_ID, $arrayPompiers)) {
                        $warn_duplicate_pid = true;
                    } else {
                        $arrayPompiers[] = $P_ID;
                        $warn_duplicate_pid = false;
                    }

                    if (is_children($S_ID, $organisateur)) {
                        $prio = true;
                    } else {
                        $prio = false;
                    }

                    if (check_rights($id, 10, "$S_ID")) {
                        $granted_update = true;
                    } else {
                    ...
                    for ($i = 1; $i <= $nbmaxsessionsparevenement; $i++) {
                        ...
                        if (isset($EH_ID[$i])) {
                            if ((isset($EH_ID0[$j])
                                    and $EH_DATE_DEBUT1[$j] == $EH_DATE_DEBUT[$i]
                                    and ($EH_DEBUT0[$j] == $EH_DEBUT[$i] or ($nbsessions == 1 and count($EH_ID0) == 1))
                                ...
                                if (in_array($num_partie, $EH_ID1)) { // personne inscrite
                                    ...
                                    if ($EP_ABSENT[$key] == 1) {
                                        ...
                                        if ($EP_DATE_DEBUT[$key] == $EP_DATE_FIN[$key]) {
                                            $horaire_p[$key] = substr($EP_DATE_DEBUT[$key], 0, 5).', '.$EP_DEBUT[$key].'-'.$EP_FIN[$key];
                                        } else {
                                            $horaire_p[$key] = substr($EP_DATE_DEBUT[$key], 0, 5).' au '.substr($EP_DATE_FIN[$key], 0, 5).', '.$EP_DEBUT[$key].'-'.$EP_FIN[$key];
                                        }
                                        $subclock = "<i class='fa fa-clock fa-lg' style='color:".$normalicon2."' title=\"".$titleprefix."horaires différents de ceux $t \n".$horaire_p[$key].'"></i>';
                                    } elseif (isset($horaire_renfort[$i])) {
                                        $subclock = "<i class='fa fa-clock fa-lg' style='color:".$normalicon."'  title=\"".$titleprefix."horaires identiques à ceux $t \n".$horaire_renfort[$i].'"></i>';
                                    } else {
                                        $subclock = "<i class='fa fa-clock fa-lg' style='color:".$normalicon."' title=\"".$titleprefix.'horaires identiques à ceux de la partie n°'.$i." \n".$horaire_renfort[$j].'"></i>';
                                    }
                                    if ($num_partie == 1) {
                                        $tmp_arr = explode(':', $EH_DEBUT[$i]);
                                        $heure_deb = $tmp_arr[0];
                                        if (intval($heure_deb) >= 18) {
                                            $p2 = 1;
                                        } else {
                                            $p1 = 1;
                                        }
                                ...
                                } elseif ($E_COLONNE_RENFORT == 1) {
                                    if ($overlap) {
                                        $subclock = "<i class='fa fa-clock fa-lg' style='color:$widget_fggreen'></i>";
                                    } else {
                                        $subclock = "<i class='fa fa-ban fa-lg' style='color:$widget_fgred' title=\"Les dates de ".$renfort_label." ne correspondent pas à celles de l'activité principale.\"></i>";
                                    }
                                } else {
                                    $subclock = "<i class='far fa-circle fa-lg' style='color:grey' title=\"Pas inscrit(e) pour la Partie n°".$EH_ID[$i].'"></i>';
                                }
                                $j++;
                            } else {
                                if ($E_COLONNE_RENFORT == 1) {
                                    ...
                                    $subclock = "<i class='fa fa-ban fa-lg' style='color:$widget_fgred'  title=\"".ucfirst($renfort_label).' inactif pour la Partie n°'.$EH_ID[$i].'"></i>';
                                }
                            }
                        }
                        if ($CE_CANCELED == 1 and $subclock != '') {
                            $subclock = "<i class='fa fa-clock fa-lg' style='color:$widget_fgred'  title=\"annulé\"></i>";
                        }
                        if ($subclock != '') {
                    ...
                    if ($gardeSP and $P_STATUT == 'SPV') {
                        ...
                        $array_jour = [30, 5, 14, 21];
                        $array_nuit = [30, 25, 26, 29];
                        $array_aprem = [30, 5, 13, 4, 14, 20, 21, 29];
                    ...
                    if ($EP_FLAG1[1] == 1 and $EP_COMMENT != '') {
                        $txtimg = "sticky-note fa-lg' style='color:purple;";
                    } elseif ($EP_FLAG1[1] == 1) {
                        $txtimg = "sticky-note fa-lg' style='color:#7D62A4;";
                    } elseif ($EP_COMMENT != '') {
                        $txtimg = 'sticky-note fa-lg';
                    } else {
                        $txtimg = "sticky-note fa-lg' style='color:grey;";
                    }
                    ...
                    if ($gardeSP and $P_STATUT == 'SPP' and $EP_FLAG1[1] == 1) {
                    ...
                    if ($gardeSP) { // vérifier que pas inscrit sur 2 tableaux de gardes
                        ...
                        $autre_garde = $rowSP[0];
                        if ($autre_garde > 0) {
                            $myimg = "<i class='fa fa-exclamation' style='color:$widget_fgred' title='attention ce personnel est parallèlement inscrit sur une autre garde'></i>";
                        }
                    }
                    ...
                    } elseif ($gardeSP and $P_STATUT == 'SPP' and $EP_FLAG1 == 0) {
                    ...
                    if ($P_CIVILITE > 3) {
                        ...
                        if ($AGE < 18) {
                            $cmt = "<span class='badge' style='background-color:$widget_fgred; color:white; font-size:9px; padding:2px;' title=\"mineur au début de l'activité\">-18</span>";
                        }
                    }
                    ...
                    if (! $print or ! $full_absent) {
                        ...
                        if ($file != '' and file_exists($path)) {
                            ...
                            if ($P_CIVILITE == 1) {
                                $defaultpic = $defaultboy;
                            } elseif ($P_CIVILITE == 2) {
                                $defaultpic = $defaultgirl;
                            } elseif ($P_CIVILITE == 3) {
                                $defaultpic = $defaultother;
                            } elseif ($P_CIVILITE == 4 or $P_CIVILITE == 5) {
                                $defaultpic = $defaultdog;
                            }
                            $src = $defaultpic;
                        }
                        if (! $print) {
                        ...
                        if ($grades) {
                            echo '<td align=left >'.$altcolor.$P_GRADE.'</font></td>';
                        }
                        ...
                        if ($warn_duplicate_pid) {
                            echo "<i class='fa fa-exclamation-triangle' style='color:$widget_fgorange' title='Attention cette personne apparaît plusieurs fois dans la liste'></i>";
                        }
                        ...
                        } elseif ($required_comp > 0) {
                            $null = get_competences($P_ID, $TE_CODE);
                        }
                        ...
                        if (($granted_personnel or $granted_event or $granted_update or ($granted_inscription and $gardeSP))
                            ...
                            if ($nbfn > 0) {
                                ...
                                if (($granted_event or ($granted_inscription and $gardeSP)) and $changeallowed) {
                                    ...
                                    if ($TP_ID == '' or $TP_ID == 0) {
                                        $TP_LIBELLE = "<div id='divfn".$P_ID."' class='noprint' title='sélectionner une fonction'>Choisir</div>";
                                    } else {
                                        $TP_LIBELLE = "<div id='divfn".$P_ID."' title='changer la fonction'>".$TP_LIBELLE.' '.$warnflag.'</div>';
                                    }
                                    echo '<td>';
                                    echo write_modal($url, 'fonction_'.$P_ID, $TP_LIBELLE);
                                    echo '</td>';
                                } else {
                                    if ($TP_ID == '' or $TP_ID == 0) {
                                        $TP_LIBELLE = '';
                            ...
                            if ($nbe > 0) {
                                if (! $granted_event or ! $changeallowed) {
                                    ...
                                    if ($EE_ID == '' or $EE_ID == 0) {
                                        $EE_NAME = "<div id='divpe".$P_ID."' title='Choisir une équipe' class='noprint'>Choisir équipe</div>";
                                    } else {
                                        $EE_NAME = "<div id='divpe".$P_ID."' title='Changer équipe'>".$EE_NAME.' <a></a></div>';
                                    }
                                    echo '<td>';
                                    echo write_modal($url, 'equipe_'.$P_ID, $EE_NAME);
                                    echo '</td>';
                                }
                            } else {
                                echo '<td></td>';
                        ...
                        if (! $print and $changeallowed) {
                            ...
                            if ($EP_KM != '') {
                                $_km = "<span class='btn btn-default btn-action' title='$EP_KM km parcourus en véhicule personnel'>".$EP_KM.' </span>';
                            } else {
                                $_km = '';
                            }
                            if ($EP_ASA == 1) {
                                $_asa = "<span class='btn btn-default btn-action' title=\"Autorisation spéciale d'absence\">ASA</span>";
                            } else {
                                $_asa = '';
                            }
                            if ($EP_DAS == 1) {
                                $_das = "<span class='btn btn-default btn-action' title=\"Décharge d'activité de service\">DAS</span>";
                            } else {
                                $_das = '';
                            }
                        ...
                        if (! $print) {
                            ...
                            } elseif ($granted_event or ($P_ID == $id and $E_CLOSED == 0 and $PARENT_CLOSED == 0) or ($granted_update and $E_CLOSED == 0 and $PARENT_CLOSED == 0)
                                or ($granted_inscription and (check_rights($id, 15, "$organisateur") or $gardeSP))) {
                                echo write_modal($url, 'Horaires_'.$P_ID, $clock);
                                echo "$warnclock";
                            } else {
                                echo "$clock $warnclock";
                            }
                ...
                echo 'Aucun personnel inscrit. ('.$cmt.').<br>';
            }
        ...
        if (! $print) {
            ...
            if (! $addAction) {
                ...
                if ($evenement > 0) {
                    ...
                    if ($nbsections > 0 and check_rights($id, 6)) {
                        $label = '<i class="fa fa-plus-circle"></i><span class="hide_mobile"> Remplacement</span>';
                    } elseif ($gardes and check_rights($id, 6, $S_ID)) {
                        $label = '<i class="fa fa-plus-circle"></i><span class="hide_mobile"> Remplacement</span>';
                    } elseif (($assoc or $army) and check_rights($id, 15, $S_ID)) {
                        $label = '<i class="fa fa-plus-circle"></i><span class="hide_mobile"> Remplacement</span>';
                    } elseif (is_inscrit($id, $evenement)) {
                ...
                echo '</div>';
                echo table_remplacements($evenement, $status = '', $date1 = '', $date2 = '', $section = 0);
            } else {
    ...
    if ($child == 3) {
        ...
        $personnel = []; // put personnel in a array
        ...
        $list .= '0';
        $comps = []; // put competences in a array
        ...
        if ($nbsessions == 2) {
            ...
            if (intval(substr($EH_DEBUT[1], 0, 2)) > 16) {
                $shownuit = true;
            } else {
                $showjour = true;
            }
        ...
        if (mysqli_num_rows($result) == 0) {
            echo "<div class='alert alert-info' role='alert'> Pour utiliser les affectations, il faut configurer les équipes et les véhicules de l'activité </div>";
        }
        write_debugbox($query);
...
if ((($tab == 3) or ($print)) and (($vehicules) || ($materiel) || ($consommables))) {
    ...
    if ($child == 1) {
        ...
        while ($rowf = @mysqli_fetch_array($resultf)) {
            array_push($fonctions, [$rowf['TFV_ID'], $rowf['TFV_NAME']]);
        }
        $nbfn = count($fonctions);
        ...
        if ($nbvehic > 0) {
            ...
            while (custom_fetch_array($result)) {
                if ($TV_ICON == '') {
                    ...
                    $vimg = '<img src='.$TV_ICON." class='img-max-50'>";
                }

                if ($V_MODELE == '') {
                    $vehicule_string = $TV_CODE;
                } else {
                    $vehicule_string = $TV_CODE.' - '.$V_MODELE;
                }
                ...
                if ($VP_OPERATIONNEL == -1) {
                    ...
                    $mytxtcolor = 'blue';
                } elseif ($VP_OPERATIONNEL == 1) {
                    $mytxtcolor = $red;
                } elseif (my_date_diff(getnow(), $V_ASS_DATE) < 0) {
                    $mytxtcolor = '#f64e60';
                    $fgcolor = $widget_fgred;
                    $bgcolor = $widget_bgred;
                       ...
                       and V_ID='.$V_ID;
                        ...
                        if ($EH_ID != '') {
                            ...
                            if ($nbsessions == 1) {
                                ...
                                $t = " de la partie n°$EH_ID";
                            }
                            if ($EV_DATE_DEBUT != '') {
                                if ($EV_DATE_DEBUT1 == $EH_DATE_DEBUT0[$i] and $EV_DATE_FIN1 == $EH_DATE_FIN0[$i]) {
                                    $horaire_v = $EV_DEBUT.'-'.$EV_FIN;
                                } elseif ($EV_DATE_DEBUT == $EV_DATE_FIN) {
                                    $horaire_v = substr($EV_DATE_DEBUT, 0, 5).', '.$EV_DEBUT.'-'.$EV_FIN;
                                } else {
                                    $horaire_v = substr($EV_DATE_DEBUT, 0, 5).' au '.substr($EV_DATE_FIN, 0, 5).', '.$EV_DEBUT.'-'.$EV_FIN;
                                }
                                $clock .= "<i class='fa fa-clock fa-lg noprint' style='color:$widget_fgorange'></i>";
                            } else {
                                $clock .= "<i class='fa fa-clock fa-lg noprint' style='color:$widget_fggreen'></i>";
                            }
                        } else {
                            $clock .= "<i class='fa fa-clock fa-lg noprint' style='color:$widget_fgred'></i>";
                        }
                        $clock .= '</span>';
                    }
                }
                ...
                if (! $print) {
                    ...
                    if ($TFV_ID == '' or $TFV_ID == 0) {
                        $TFV_NAME = "<div id='divfn".$V_ID."' title='sélectionner une fonction'>Choisir</div>";
                    } else {
                        $TFV_NAME = "<div id='divfn".$V_ID."' title='changer la fonction'>".$TFV_NAME.'</div>';
                    }
                    echo "<td style='min-width:5px;' align=center class='hide_mobile2'>";
                    echo write_modal($url, 'fonction_'.$V_ID, $TFV_NAME);
                    echo '</td>';
                }
                ...
                if ($nbe > 0) {
                    if (! $granted_event or ! $changeallowed or $print) {
                        ...
                        if ($EE_ID == '' or $EE_ID == 0) {
                            $EE_NAME = "<div id='divpe".$V_ID."' title='Choisir une équipe' class='noprint'>Choisir équipe</div>";
                        } else {
                            $EE_NAME = "<div id='divpe".$V_ID."' title='Changer équipe'>".$EE_NAME.'</div>';
                        }
                        echo "<td align=center style='min-width:80px;' class='hide_mobile2'>";
                        echo write_modal($url, 'equipe_'.$V_ID, $EE_NAME);
                        echo '</td>';
                    }
                } else {
                    echo "<td class='hide_mobile2'></td>";
                }
                ...
                echo '<td>';
                if ($readonly == '') {
                    echo write_modal($url, 'km_'.$V_ID, $showEV_KM);
                } elseif ($EV_KM > 0) {
                    echo "$EV_KM km";
                }
                echo '</td>';
                ...
                if (! $print and $changeallowed) {
                    if ($granted_event or ($granted_vehicule and $E_CLOSED == 0)) {
                        ...
                        echo write_modal($url, 'Horaire_'.$V_ID, $clock);
                    } else {
                        echo $clock;
                    }
                }
        ...
        if ($nbmat > 0) {
            ...
            while (custom_fetch_array($result)) {
                ...
                if ($VP_OPERATIONNEL == -1) {
                    ...
                    $mytxtcolor = $widget_fgblue;
                } elseif ($VP_OPERATIONNEL == 1) {
                    $mytxtcolor = $widget_fgred;
                } elseif (my_date_diff(getnow(), $MA_REV_DATE) < 0) {
                    $mytxtcolor = $widget_fgorange;
                    $VP_LIBELLE = 'date dépassée';
                } elseif ($VP_OPERATIONNEL == 2) {
                    $mytxtcolor = $widget_fgorange;
                } else {
                    $mytxtcolor = $widget_fggreen;
                }

                $element = "<font color=$mylightcolor>.....";
                if ($TM_LOT == 1) {
                    $element .= "</font><i class='fa fa-plus-square fa-lg' title=\"Ceci est un lot de matériel\"></i> ";
                } elseif ($MA_PARENT > 0) {
                    $element .= "...</font><i class='fa fa-minus'  title=\"élément d'un lot de matériel\"></i> ";
                } else {
                    $element .= "</font><i class='fa fa-caret-right fa-lg' title=\"Ne fait pas partie d'un lot\"></i> ";
                }
                ...
                if ($nbe > 0) {
                    if (! $granted_event or ! $changeallowed) {
                        ...
                        if ($EE_ID == '' or $EE_ID == 0) {
                            $EE_NAME = "<div id='divpe".$MA_ID."' title='Choisir une équipe'><i>Choisir équipe</i></div>";
                        } else {
                            $EE_NAME = "<div id='divpe".$MA_ID."' title='Changer équipe'>".$EE_NAME.'</div>';
                        }
                        echo '<td width=120>';
                        echo write_modal($url, 'equipe_'.$MA_ID, $EE_NAME);
                        echo '</td>';
                    }
                } else {
                    echo '<td></td>';
                ...
                if ($MA_NB > 1) {
                    ...
                    if ($readonly == '') {
                        ...
                        echo write_modal($url, 'nombre_'.$MA_ID, $EM_NB);
                    } else {
                        echo "$EM_NB pièces";
                    }
                }
    ...
    if ($child == 3) {
        ...
        if ($nbmat > 0) {
            ...
            while (custom_fetch_array($result)) {
                ...
                if ($TCO_CODE == 'PE') {
                    $label = $TC_DESCRIPTION.' ('.$TUM_DESCRIPTION.'s) '.$C_DESCRIPTION;
                } elseif ($TUM_CODE != 'un' or $TC_QUANTITE_PAR_UNITE != 1) {
                    $label = $TC_DESCRIPTION.' ('.$TCO_DESCRIPTION.' '.$TC_QUANTITE_PAR_UNITE.' '.$TUM_DESCRIPTION.') '.$C_DESCRIPTION;
                } else {
                    $label = $TC_DESCRIPTION.' '.$C_DESCRIPTION;
                }
                ...
                if (! $print) {
                    echo '<td width=120>';
                    if ($readonly == '') {
                        echo write_modal($url, 'nombre_'.$EC_ID, $EC_NOMBRE);
                    } else {
                        echo "$EC_NOMBRE unités";
                    }
                    echo '</td>';
                }
            ...
            echo "Aucun consommable n'est renseigné.<br>";
        }
    }
}
...
if (($tab == 5) and (! $print) and ($TE_CODE == 'FOR') and ($PS_ID != '') and ($TF_CODE != '')) {
    ...
    if ($granted_event and (check_rights($id, 4, $organisateur) or $chef)) {
        $disabledtf = '';
    } else {
        $disabledtf = 'disabled';
    }
    ...
    if ($_PS_PRINTABLE == 1) {
        if ($_PS_NATIONAL == 1) {
            if (check_rights($id, 48, '0')) {
                $printdiplomes = true;
            }
        } elseif (check_rights($id, 48, "$S_ID")) {
            $printdiplomes = true;
        }
    }
    ...
    if ($_TYPE != '') {
        if ($_TF_LIBELLE != '') {
            $tt = $_TF_LIBELLE.' pour '.$_TYPE;
        } else {
            $tt = 'formation pour '.$_TYPE;
        }
    } else {
        $tt = 'formation';
    }
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            while (custom_fetch_array($result)) {
                ...
                if (($granted_event or ($P_ID == $id and $E_CLOSED == 0) or ($granted_update and $E_CLOSED == 0)) and $changeallowed) {
                    ...
                    if ($n1 < $nbsessions) {
                        $warn = "<i class='fa fa-clock fa-lg' style='color:$widget_fgred'  title=\"Attention n'est pas présent à toutes les parties de la formation\" border=0></i>";
                    } elseif ($n2 > 0) {
                        $warn = "<i class='fa fa-clock fa-lg' style='color:$widget_fgorange'  title='Attention horaires différents de ceux de la formation' border=0></i>";
                    } else {
                        $warn = "<i class='fa fa-clock fa-lg' style='color:$widget_fggreen'  title='Présence totale sur la formation' border=0></i>";
                    }
                }
                ...
                where q.P_ID='.$P_ID." and p.PS_ID=q.PS_ID and p.TYPE='PSE2'";
                        ...
                        $NB = $row3['NB'];
                        if ($NB == 1) {
                            $cmt2 = '<font size=1 color=blue>Possède la compétence supérieure PSE2</font>';
                        }
                    }
                }
        ...
        if ($E_CLOSED == 0) {
            ...
            if (($printdiplomes or $granted_event) and $nbstagiaires > 0) {
                ...
                if (isset($no_attestations_continue_secourisme) and $TF_CODE == 'R') {
                    if (in_array(str_replace(' ', '', $_TYPE), ['PSC1', 'PSE1', 'PSE2', 'PAEPSC', 'PAEPS', 'FDFPSC', 'FDFPSE']) and intval($no_attestations_continue_secourisme) <= $YEAR) {
                        $enable_this = false;
                    }
                }
                if ($enable_this) {
...
if (($tab == 6) and (! $print) and ($E_TARIF > 0) and $granted_event) {
    ...
    while (custom_fetch_array($result)) {
        ...
        if ($C_ID == 0) {
            ...
            $company = '<a href=upd_company.php?C_ID='.$C_ID.'>'.$colorbegin.$C_NAME.$colorend.'</a>';
        }
        $for = strtoupper($P_NOM).' '.my_ucfirst($P_PRENOM);
        ...
        if ($changeallowed) {
            echo write_modal($url, $P_ID, $displayed);
        } else {
            echo $displayed;
        }
        echo '</td>';
    ...
    if ($total_to_pay > $total_paid) {
        $T = '<font color=red>'.round($total_paid, 2).' '.$default_money_symbol.'</font>';
    } else {
        $T = "<font color=$widget_fggreen>".round($total_paid, 2).' '.$default_money_symbol.'</font>';
    }
...
if ($tab == 7 and ! $print) {
    ...
    if (! $addAction) {
        ...
        if (intval($E_PARENT) > 0) {
            $table .= show_attached_docs($E_PARENT);
        }
        ...
        if ($granted_event) {
            if ($FICHE_PRESENCE == 1 and $E_CLOSED == 1) {
                ...
                if ($type_doc == 'SST') {
                    $table .= show_auto_doc('Fiche de présence SST', '8', true);
                } elseif ($type_doc == 'PRAP') {
                    $table .= show_auto_doc('Fiche de présence PRAP', '8', true);
                } else {
                    $table .= show_auto_doc('Fiche de présence', '1', true);
                }
                if ($type_doc == 'SST') {
                    $table .= show_auto_doc('Attestations de présence SST', '10', true);
                } elseif ($type_doc == 'PRAP') {
                    $table .= show_auto_doc('Attestations de présence PRAP', '10', true);
                }
            }
            if ($PROCES_VERBAL == 1 and $E_CLOSED == 1 and $PS_ID != '' and in_array($TF_CODE, ['I', 'C', 'R', 'M'])) {
            ...
            if ($EVAL_PAR_STAGIAIRES == 1 and $competences == 1) {
                ...
                if ($type_doc == 'SST' and (file_exists($sstspec)) and $level < 3) {
                    $table .= show_auto_doc("Fiche d'évaluation de la formation SST", '9', false);
                } elseif ($type_doc == 'PRAP' and (file_exists($sstspec)) and $level < 3) {
                    $table .= show_auto_doc("Fiche d'évaluation de la formation PRAP", '9', false);
                } else {
                    $table .= show_auto_doc("Fiche d'évaluation de la formation", '3', false);
                }
            }
            if ($FACTURE_INDIV == 1 and $E_TARIF > 0) {
                $table .= show_auto_doc('Factures individuelles', '7', false);
            }
            ...
            if ($CONVENTION == 1 and $E_PARENT == 0) {
                ...
                if (signature_president_disponible($S_ID)) {
                    $table .= show_auto_doc('Convention signée par le président', $docnum, $secured = false, $signed = true);
                }
            }
            if ($EVAL_RISQUE == 1 and dim_ready($evenement)) {
            ...
            if (! $gardeSP and $TE_PERSONNEL == 1 and $TE_CODE != 'MC' and $TE_CODE != 'FOR' and $ORDRE_MISSION == 1 and ($assoc or $army)) {
                if ($E_ALLOW_REINFORCEMENT == 1) {
                    $title = 'Demande de '.$renfort_label.'s';
                } else {
                    $title = 'Demande de personnels et de moyens';
                }
        ...
        if ($granted_event or $is_present) {
            if ($ORDRE_MISSION == 1 and $E_CLOSED == 1) {
                $table .= show_auto_doc('Ordre de mission', '4', false);
            }
        }
...
if ($tab == 8 and ! $print and $TE_MAIN_COURANTE == 1) {
    ...
    if ($TE_VICTIMES == 0 || $E_PARENT > 0) {
        ...
        while (custom_fetch_array($result)) {
            ...
            if (abs($NEW) < 10) {
                $new = "<i class='fa fa-star' style='color:yellow;' title=\"Cette ligne a été ajoutée il y a moins de 10 minutes\" ></i>";
            } else {
                $new = '';
            }

            if ($granted_event or $is_operateur_pc) {
                $title_msg = '<a href=intervention_edit.php?evenement='.$E_CODE.'&numinter='.$EL_ID.'&action=update&type='.$TEL_CODE." title='Cliquer pour éditer' >".$EL_TITLE.'</a>';
            } else {
                $title_msg = $EL_TITLE;
            }
    ...
    // ==========================
        ...
        if ($nbcav > 0) {
            ...
            while ($row = @mysqli_fetch_array($result)) {
                ...
                if ($CAV_RESPONSABLE != 0) {
                    $resp = '<a href=upd_personnel.php?pompier='.$CAV_RESPONSABLE." title='Voir la fiche du responsable'>".$P_PRENOM.' '.$P_NOM.'</a>';
                } else {
                    $resp = '';
                }
        ...
        if ($nbinter > 0) {
            ...
            while (custom_fetch_array($result)) {
                ...
                if ($E_PARTIES > 1) {
                    ...
                    if ($key == '') {
                        $key = array_search(end($EH_DATE_DEBUT), $EH_DATE_DEBUT);
                    }
                    if ($key != $prev_key) {
                        $tmp = explode('-', $EH_DATE_DEBUT[$key]);
                        $month1 = $tmp[1];
                        $day1 = $tmp[2];
                        $year1 = $tmp[0];
                        $tmp = explode('-', $EH_DATE_FIN[$key]);
                        $month2 = $tmp[1];
                        $day2 = $tmp[2];
                        $year2 = $tmp[0];
                ...
                } elseif ($DATE_DEBUT != $prev_DATE_DEBUT) {
                    $tmp = explode('-', $DATE_DEBUT);
                    $month1 = $tmp[1];
                    $day1 = $tmp[0];
                    $year1 = $tmp[2];
                ...
                if (abs($NEW) < 10) {
                    $new = "<i class='fa fa-star' style='color:$widget_fgorange' title=\"Cette ligne a été ajoutée il y a moins de 10 minutes\" ></i>";
                } else {
                    $new = '';
                }
                if ($TEL_CODE == 'I') {
                ...
                if ($dateaffiche == $olddate) {
                    ...
                    $datedisplay = $dateaffiche;
                }

                if ($HEURE_SLL != '') {
                    $HEURE_SLL = "<br><span title='heure sur les lieux'>".$HEURE_SLL.'</span>';
                }
                if ($HEURE_FIN != '') {
                    $HEURE_FIN = "<br><span title='heure de fin'>".$HEURE_FIN.'</span>';
                }
            ...
            if ($nbrow > 0) {
                ...
                while (custom_fetch_array($result)) {
                    ...
                    if ($CAV_SORTIE != '') {
                        ...
                        if ($CAV_SORTIE[1] != null) {
                            $CAV_SORTIE = $CAV_SORTIE[0].':'.$CAV_SORTIE[1];
                        } else {
                            $CAV_SORTIE = '';
                        }
                    }
                    if ($CAV_REGULATED == 1) {
                        $CAV_REGULATED = "<i style='color:#dc3545' class='far fa-registered fa-lg' title='A été régulé par le médecin'></i>";
                    } else {
                        $CAV_REGULATED = '';
                    }
                    ...
                    if ($VI_DETRESSE_VITALE == 1 or $VI_DECEDE == 1 or $VI_MALAISE == 1 or $VI_MEDICALISE == 1 or $VI_IMPLIQUE == 1 or $VI_TRAUMATISME == 1 or $VI_SOINS == 1 or $VI_TRANSPORT == 1 or $VI_REPOS == 1) {
                        $displayinfo = 1;
                        $infotitle = '';
                        if ($VI_DETRESSE_VITALE == 1) {
                            $infotitle .= 'Détresse vitale <br>';
                        }
                        if ($VI_DECEDE == 1) {
                            $infotitle .= 'Décédé.e <br>';
                        }
                        if ($VI_MALAISE == 1) {
                            $infotitle .= 'Malaise <br>';
                        }
                        if ($VI_MEDICALISE == 1) {
                            $infotitle .= 'Médicalisé.e <br>';
                        }
                        if ($VI_IMPLIQUE == 1) {
                            $infotitle .= 'Impliqué.e <br>';
                        }
                        if ($VI_TRAUMATISME == 1) {
                            $infotitle .= 'Traumatisme <br>';
                        }
                        if ($VI_SOINS == 1) {
                            $infotitle .= 'Soins <br>';
                        }
                        if ($VI_TRANSPORT == 1) {
                            $infotitle .= 'Transporté.e <br>';
                        }
                        if ($VI_REPOS == 1) {
                            $infotitle .= 'Repos <br>';
                        }
                    ...
                    echo "<tr>
                        ...
                        <td align=center>";
                    if ($displayinfo == 1) {
                        echo "<a href=# data-html='true' title data-original-title='$infotitle'> <i class='fa fa-info-circle fa-lg'></i></a>";
                    }
                    echo '</td></tr>';
                }
                echo '</table>';
