...
if (is_lowest_level($filter) and $pompiers) {
...
if ($nbtypesgardes == 0) {
    ...
    if ($tableau_garde_display_mode == 'month') {
        ...
        while ($m <= 12) {
            $monmois = $mois[$m - 1];
            if ($m == $month) {
                echo "<option value='$m' selected >".ucfirst($monmois)."</option>\n";
            } else {
                echo "<option value= $m >".ucfirst($monmois)."</option>\n";
            }
            $m = $m + 1;
        }
        echo '</select>';
    ...
    if ($year == $yearminus5) {
        echo "<option value='$yearminus5' selected>".$yearminus5.'</option>';
    } else {
        echo "<option value='$yearminus5' >".$yearminus5.'</option>';
    }
    if ($year == $yearminus4) {
        echo "<option value='$yearminus4' selected>".$yearminus4.'</option>';
    } else {
        echo "<option value='$yearminus4' >".$yearminus4.'</option>';
    }
    if ($year == $yearminus3) {
        echo "<option value='$yearminus3' selected>".$yearminus3.'</option>';
    } else {
        echo "<option value='$yearminus3' >".$yearminus3.'</option>';
    }
    if ($year == $yearminus2) {
        echo "<option value='$yearminus2' selected>".$yearminus2.'</option>';
    } else {
        echo "<option value='$yearminus2' >".$yearminus2.'</option>';
    }
...
if ($created and $show_table) {
    ...
    if ($nbcol > 0) {
        ...
        while ($day <= $lastday) {
            ...
            if ($SNUM == 0 and in_array($S_CODE, ['A', 'B', 'C', 'D', 'E', 'F', 'G'])) {
                $SNUM = $S_CODE;
            }
            $SECTION_JOUR = $SNUM;
            ...
            if ($E_ANOMALIE == 1) {
                ...
                $rowtitle = 'garde du '.date_fran($month, $day, $year);
            }
            ...
            if ($E_CODE > 0) {
                ...
                while (custom_fetch_array($result)) {
                    ...
                    $HORAIRE = $HORAIRE[0];
                    if (! empty($TP_LIBELLE)) {
                        $FONCTION_DISPLAY = "<br><span style='font-size:9px;color:grey'>".$TP_LIBELLE.'</span>';
                    } else {
                        $FONCTION_DISPLAY = '';
                    }
                    if ($horaires_tableau_garde == 1) {
                    ...
                    if ($P_ID == $person) {
                        $FORMATED_NAME = str_replace($RAW_NAME, "<span  STYLE='background-color:$green; color:yellow; font-weight: bold; font-size: 18px;'>".$RAW_NAME.'</span>', $FORMATED_NAME);
                    }
                    if (! isset($noms[$P_ID])) {
                        ...
                        if ($P_STATUT == 'SPP') {
                            if ($STATUS == 1) {
                                ...
                                $SPPV = " <span class='smallblue' title='Garde en tant que SPV, au moins pour une partie de la garde' >SPV</span>";
                            }
                            $noms[$P_ID] .= ' </td><td class=red12>'.$FORMATED_NAME.$SPPV.'</td></tr>';
                        } else {
                            $noms[$P_ID] .= ' </td><td class=blue12>'.$FORMATED_NAME.'</td></tr>';
                        }
                        $noms[$P_ID] .= '</table>';
                    }
                    // positionner jour
                ...
                for ($l = 1; $l <= count($nightly_remain); $l++) {
                    for ($k = 1; $k <= $nbcol; $k++) {
                        if ($night_id[$k] == 0) {
            ...
            if ($regime > 0) {
                $data .= '<td align=center>'.$img.'</td>';
            }
            if ($EQ_JOUR == 1 and $EQ_NUIT == 1) {
                $t = "<i style='color:$mydarkcolor'>J-<b>".$jour."</b><br><style='color:$mydarkcolor>N-<b>".$nuit.'</b>';
            } elseif ($EQ_JOUR == 1) {
                $t = 'J';
            } elseif ($EQ_NUIT == 1) {
                $t = 'N';
            ...
            for ($i = 1; $i <= $nbcol; $i++) {
                ...
                if ($EQ_NUIT == 1 and $EQ_JOUR == 1) {
                    if ($night_id[$i] == 0) {
                        if ($day1_id[$i] > 0) {
                            $case .= " <span STYLE='background-color:$yellow; color:#00000; font-size: 10px;' > jour seulement</span>";
                        } else {
                            $case .= '<br> -';
                        }
                    } elseif ($night_id[$i] != $day2_id[$i] and $night_id[$i] != $day1_id[$i]) {
                        $case .= $noms[$night_id[$i]];
                    }
                }
