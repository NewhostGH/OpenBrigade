...
if ($pid != $id) {
    check_all(59);
    if (! check_rights($id, 59, $his_section)) {
        check_all(24);
    }
}
...
if ($row1[2] != '') {
    if ($row1[3] == 'M') {
        $NOM = 'Le Président Fédéral, ';
    } else {
        $NOM = 'La Présidente Fédérale, ';
    }
...
if ($signature_file != '') {
    if (@is_file($signature_file)) {
        $pdf->Image($signature_file, 100, $y + 10, 50);
    }
}
...
if ($tofile == 1) {
    ...
    if (@is_file($courrier_file)) {
        unlink($courrier_file);
    }
