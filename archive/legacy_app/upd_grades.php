...
if ($operation == 'insert') {
    ...
    if ($NB_grade_actif == 0) {
        $badge = "<span class='badge $typeclass $class' style='float:right'>$NB_grade_actif personne$s associée$s</span>";
    } else {
        $badge = "<span class='badge $typeclass $class' style='float:right'><a href='personnel.php?position=actif&category=INT&P_GRADE=$G_GRADE'>$NB_grade_actif personne$s associée$s </a> </span>";
    }
...
if ($NB_grade_actif > 0) {
    echo "<button class='btn btn-danger'  title='Suppression impossible: Grade associé' disabled>Supprimer</button>";
} elseif ($operation == 'update') {
    echo "<input type='button' class='btn btn-danger' value='Supprimer' onclick=\"suppress('".$G_GRADE."');\"> ";
}
