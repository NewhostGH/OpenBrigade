...
if (isset($_POST['evenement'])) {
    if ($update_allowed) {
        ...
        if (isset($_POST['point'])) {
            $POINT_REGROUPEMENT = secure_input($dbc, str_replace('"', '', $_POST['point']));
        } else {
            $POINT_REGROUPEMENT = '';
        }
        if (isset($_POST['specifique'])) {
            $DEMANDE_SPECIFIQUE = secure_input($dbc, str_replace('"', '', $_POST['specifique']));
        } else {
            $DEMANDE_SPECIFIQUE = '';
        }
        if (isset($_POST['vehicule'])) {
            $NB_VEHICULES = intval($_POST['vehicule']);
        } else {
            $NB_VEHICULES = 0;
        }
