...
$possibleorders = ['VI_DECEDE', 'VI_NUMEROTATION', 'VI_DETRESSE_VITALE', 'VI_MALAISE', 'VI_TRANSPORT', 'VI_REPOS', 'VI_TRAUMATISME', 'VI_IMPLIQUE', 'VI_AGE', 'VI_NOM', 'PAYS',
...
if ($type_victime == 'cav') {
    ...
    $picture = 'h-square';
    $color = $mydarkcolor;
} elseif ($type_victime == 'intervention') {
    $title = 'Victimes sur interventions';
    $picture = 'medkit';
    $color = $mydarkcolor;
} elseif (intval($type_victime) > 0) {
...
if (intval($type_victime) > 0) {
    ...
    if ($responsable == $id) {
        $granted_update = true;
    } elseif (check_rights($id, 15, (get_section_organisatrice($evenement)))) {
        $granted_update = true;
    }
} elseif ($evenement_victime > 0) {
    if (check_rights($id, 15, (get_section_organisatrice($evenement_victime)))) {
        $granted_update = true;
    }
}
...
if (in_array($id, $chefs_parent)) {
    $granted_update = true;
}
if (is_operateur_pc($id, $evenement_victime)) {
    $granted_update = true;
}
...
if ($type_victime == 'intervention') {
    $queryadd .= ' and victime.EL_ID in (select EL_ID from evenement_log where E_CODE='.$evenement_victime.')';
} elseif ($type_victime == 'cav') {
    $queryadd .= ' and victime.CAV_ID in (select CAV_ID from centre_accueil_victime where E_CODE='.$evenement_victime.')';
} elseif (intval($type_victime) > 0) {
    $queryadd .= ' and victime.CAV_ID = '.intval($type_victime);
} else {
    $queryadd .= ' and ( victime.EL_ID in (select EL_ID from evenement_log where E_CODE='.$evenement_victime.')
...
// filtering
if ($in_cav == 1) {
    $queryadd .= ' and ((CAV_SORTIE is null and victime.CAV_ID > 0 ) or (EL_FIN is null and victime.CAV_ID is null ))';
}
if ($a_reguler == 1) {
    $queryadd .= ' and CAV_REGULATED = 0 ';
}

// order
if ($order == 'CAV_ID') {
    $query .= $queryadd.' order by CAV_NAME,EL_TITLE';
} elseif ($order == 'VI_NOM') {
    $query .= $queryadd.' order by VI_NOM,VI_PRENOM';
} else {
    $query .= $queryadd.' order by '.$order;
}
if (! in_array($order, ['CAV_ID', 'PAYS', 'VI_NOM', 'VI_NUMEROTATION', 'VI_AGE'])) {
    $query .= ' desc';
}
$querycnt .= $queryadd;
...
if (! $granted_update) {
    if (is_chef_evenement($id, $evenement_victime)) {
        $granted_update = true;
    } elseif (is_operateur_pc($id, $evenement_victime)) {
        $granted_update = true;
    }
}
...
if ($nb_cav > 0) {
    ...
    while (custom_fetch_array($result2)) {
        if ($CAV_RESPONSABLE == $id) {
            $granted_update = true;
        }
        if ($CAV_OUVERT == 0) {
            $CAV_NAME .= ' - fermé';
        } else {
            $cav_ouvert_found = true;
        }
        if (intval($type_victime) == $CAV_ID) {
            $selected = 'selected';
        } else {
            $selected = '';
        }
...
if ($number > 0) {
    ...
    while (custom_fetch_array($result)) {
        ...
        if ($VI_DETRESSE_VITALE == 1) {
            $VI_DETRESSE_VITALE = "<i class='fa fa-exclamation-circle fa-lg' style='color:red;' title='détresse vitale'></i>";
        } else {
            $VI_DETRESSE_VITALE = '';
        }
        if ($VI_TRAUMATISME == 1) {
            $VI_TRAUMATISME = "<i class='fa fa-exclamation-circle fa-lg' style='color:orange;' title='traumatisme'></i>";
        } else {
            $VI_TRAUMATISME = '';
        }
        if ($VI_SOINS == 1) {
            $VI_SOINS = "<i class='fa fa-exclamation-circle fa-lg' style='color:orange;' title='soins donnés à la victime'></i>";
        } else {
            $VI_SOINS = '';
        }
        if ($VI_DECEDE == 1) {
            $VI_DECEDE = "<i class='fa fa-exclamation-circle fa-lg' style='color:red;'  title='décédé'></i>";
        } else {
            $VI_DECEDE = '';
        }
        if ($VI_MALAISE == 1) {
            $VI_MALAISE = "<i class='fa fa-exclamation-circle fa-lg' style='color:orange;' title='malaise'></i>";
        } else {
            $VI_MALAISE = '';
        }
        if ($VI_TRANSPORT == 1) {
            $VI_TRANSPORT = "<i class='fa fa-exclamation-circle fa-lg' style='color:orange;'  title=\"transport vers ".$D_NAME.' par '.$T_NAME.'"></i>';
        } else {
            $VI_TRANSPORT = '';
        }
        if ($VI_REPOS == 1) {
            $VI_REPOS = "<i class='fa fa-exclamation-circle fa-lg' style='color:orange;'  title=\"Repos sous surveillance\"></i>";
        } else {
            $VI_REPOS = '';
        }
        if ($VI_IMPLIQUE == 1) {
            $VI_IMPLIQUE = "<i class='fa fa-exclamation-circle fa-lg' style='color:orange;'  title=\"Impliqué indemne\"></i>";
        } else {
            $VI_IMPLIQUE = '';
        }
        if ($VI_INFORMATION == 1) {
            $VI_INFORMATION = "<i class='fa fa-exclamation-circle fa-lg' style='color:orange;'  title=\"Personne assistée\"></i>";
        } else {
            $VI_INFORMATION = '';
        }
        if ($VI_MEDICALISE == 1) {
            $VI_MEDICALISE = "<i class='fa fa-exclamation-circle fa-lg' style='color:red;'  title=\"Victime médicalisée\"></i>";
        } else {
            $VI_MEDICALISE = '';
        }
        ...
        if ($CAV_NAME != '') {
            $localisation = $CAV_NAME;
        } else {
            $localisation = 'Sur intervention';
        }

        if ($EL_ID > 0) {
            $inter_icon = "<i class = 'fa fa-medkit' title='Victime initialement prise en compte sur intervention'></i>";
        } else {
            $inter_icon = '';
        }
        ...
        if ($REGULATED == 1) {
            $REGUL_ICON = "<i class='fa fa-check-square fa-lg' style='color:green;' title='régulation faite par le médecin ou le PC'></i>";
        } elseif ($REGULATED == 0) {
            $REGUL_ICON = "<i class='fa fa-exclamation-triangle fa-lg' style='color:orange;' title='à réguler par le médecin ou le PC'></i>";
        } else {
            $REGUL_ICON = '';
        }
        ...
        if ($REGULATED == 0 and $CAV_ID > 0) {
            $span = "<span class ='red12' title='à réguler par médecin ou le PC'>";
        } elseif ($HEURE_SORTIE == '') {
            $span = "<span class ='green12' title='en cours de traitement'>";
        } else {
            $span = '<span>';
        }
