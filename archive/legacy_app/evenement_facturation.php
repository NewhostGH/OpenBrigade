...
if ($resevt) {
    ...
    if ($evtCompany != '') {
        ...
        if (substr($rowC['C_PHONE'], 0, 2) == '06') {
            $evtMobile = $rowC['C_PHONE'];
        } else {
            $evtTel = $rowC['C_PHONE'];
        }
        $evtFax = $rowC['C_FAX'];
...
function Buttons($tabn = 'null')
{
    ...
    echo write_modal($url, 'facturation_'.$evenement, "<input type='submit' class='btn btn-secondary' id='btaction".$tabn."' value='Détail' title=\"voir le détail de l'activité\">");
    echo '</td>';
...
if (isset($_GET['status'])) {
    ...
    $status = get_etat_facturation($evenement, 'code');
}
if (isset($_POST['frmaction'])) {
    if ($_POST['frmaction'] == 'CopierDevis') {
        $status = 'facture';
    }
}
...
if (isset($_GET['child'])) {
    ...
    $child = 1;
} elseif ($status == 'facture') {
    $child = 2;
} elseif ($status == 'relance') {
    $child = 3;
} elseif ($status == 'paiement') {
    $child = 4;
} else {
    $child = 1;
}
...
if ($devisAccepte == 1) {
    $cmt = 'Devis du '.$devisDate.' (accepté)';
} elseif ($devisDate != '') {
    $cmt = 'Devis du '.$devisDate;
} else {
    $cmt = 'Devis';
}
...
if ($child == 1) {
    ...
    if ($devisDate == '') {
        $warn = " <i class='fa fa-exclamation-triangle' style='color:orange; padding-left:5px;' title='saisissez la date et sauvez'></i>";
    } else {
        $warn = '';
    }
...
if ($child == 2) {
    ...
    echo "<table cellspacing=0 class='noBorder'>";
    if ($factMontant == $devisMontant) {
        $cmt = '';
    } else {
        $cmt = 'Devis = '.$devisMontant;
    }
    if ($factDate == '') {
        $warn = " <i class='fa fa-exclamation-triangle' style='color:orange; padding-left:5px;' title='saisissez la date et sauvez'></i>";
    } else {
