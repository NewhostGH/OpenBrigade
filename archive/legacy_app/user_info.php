...
if ($AGE > 0) {
    $P_BIRTHDATE .= ' - <b>'.$AGE.' ans</b>';
}
if ($P_BIRTHPLACE == '') {
    $P_BIRTHPLACE = '?';
}
if ($P_OLD_MEMBER == 0) {
    $position = "<span style='font-weight:800;color:green;' >Actif</span>";
} else {
    $position = "<span style='font-weight:800;color:red;' >Ancien</span>";
}
if ($P_EMAIL != '') {
...
if ($grades and ! $syndicate) {
    $pic1 = '<img src='.$grades_imgdir.'/'.$P_GRADE.".png class='img-max-50' title='".$P_DESCRIPTION."'>";
} else {
    $pic1 = '';
}
if ($P_PHOTO != '') {
    if (file_exists($trombidir.'/'.$P_PHOTO)) {
        ...
        $pic = "<i class='fa fa-user fa-2x' title='Photo non trouvée'></i> ";
    }
} else {
    if ($P_CIVILITE >= 4) {
        $pic = 'dog.png';
