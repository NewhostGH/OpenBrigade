...
if ($nbsections == 0) {
    ...
    if ($responsable_gardes) {
        ...
        if (! is_children($filter, $mysection)) {
            $mysection = $filter;
        }
    } else {
        $mysection = $S_ID;
    ...
    if (check_rights($id, 24)) {
        ...
        if ($disabled == '') {
            display_children2($mysection, $level + 1, $S_ID, $maxL);
        }
    }
    if ($eqid > 0) {
        $detail = '<br><small>'.$cmt.'</small>';
    } else {
        $detail = '';
    }
...
if ($EQ_ADDRESS != '' and $geolocalize_enabled == 1) {
    ...
    custom_fetch_array($resultm);
    if ($NB == 1) {
        $map = ' <a href=map.php?type=G&code='.$EQ_ID." target=_blank><i class='fa fa-map noprint' style='color:green' title='Voir la carte Google Maps' class='noprint'></i></a>";
    }
}
...
if ($pompiers) {
    if ($EQ_ID == 0) {
        ...
        $section_today = get_section_pro_jour($EQ_ID, date('Y'), date('n'), date('d'), 'J');
    }
    ...
    echo "<tr id='row_eq1' style='display:none'>";
}
...
if ($pompiers) {
    if ($EQ_ID == 0) {
        ...
        $section_today = get_section_pro_jour($EQ_ID, date('Y'), date('n'), date('d'), 'N');
    }
...
echo '<tr>
    ...
    <td align=left>';
if ($nbDefaut > 0 && $EQ_ID != $row['EQ_ID']) {
    echo "<i class='fas fa-lock' title='".$row['EQ_NOM']." est déjà défini par défaut pour cette section'></i>";
} else {
    echo "<input type='checkbox' name='EQ_DEFAULT' value='1' $checked  title = \"Définir ce type de garde par défaut. Il sera présélectionné à l'ouverture de l'application \" $disabled>";
}
echo '</tr>';
...
if ($operation == 'insert') {
    echo '<option value='.($count + 1).' selected>'.($count + 1).'</option>';
}
