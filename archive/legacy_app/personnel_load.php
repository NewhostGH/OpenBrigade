...
if (is_chef($id, $filter)) {
    $hide_phone = false;
} elseif ((is_children($filter, $mysection)) or (check_rights($id, 24))) {
    if (check_rights($id, 2) and $category == 'INT') {
        $hide_phone = false;
    }
    if (check_rights($id, 37) and $category == 'EXT') {
        $hide_phone = false;
    }
    if (check_rights($id, 12) and $category == 'INT') {
        $hide_phone = false;
    }
}
...
if (isset($_GET['data'])) {
    ...
    while (custom_fetch_array($result)) {
        ...
        if ($grades == 1) {
            $file = $G_ICON;
            if (file_exists($file) and $G_CATEGORY != 'PATS') {
