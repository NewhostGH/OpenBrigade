...
while (custom_fetch_array($result1)) {
    $r++;
    $columns_data = [strtoupper($P_NOM), my_ucfirst($P_PRENOM)];
    if ($grades) {
    ...
    for ($i = 1; $i <= $nbjoursdumois; $i++) {
        $color = 'none';
        [$status, $title, $style] = get_status($P_ID, $year, $month, $i, $type = 'excel');
        if ($style == 'participe') {
