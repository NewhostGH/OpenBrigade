...
if ($C_ID == 0) {
    $C_ID = intval($_SESSION['SES_COMPANY']);
}

if ($C_ID == 0) {
    check_all(29);
} elseif ($C_ID == $_SESSION['SES_COMPANY'] and ! check_rights($_SESSION['id'], 45)) {
    check_all(45);
} elseif (! check_rights($_SESSION['id'], 45) or $C_ID != $_SESSION['SES_COMPANY']) {
    check_all(29);
}
...
} elseif (basename($_SERVER['PHP_SELF']) == 'evenement_display.php') {
...
while ($row = @mysqli_fetch_array($result)) {
    if ($C_PARENT == $row['C_ID']) {
        $selected = 'selected';
    } else {
        $selected = '';
    }
    $code = $row['C_NAME'];
    if ($row['C_DESCRIPTION'] != '') {
        $code .= ' - '.substr($row['C_DESCRIPTION'], 0, 20);
    }
...
if ($S_ID > 0) {
    $query .= ' where tcr.TCR_FLAG is null';
}
...
while ($row = @mysqli_fetch_array($result)) {
    ...
    echo '<td  align=left>';
    if (($disabled == '')) {
        echo write_modal('upd_company_role.php?C_ID='.$C_ID.'&TCR_CODE='.$c, 'personne_'.$c, "<i class='fa fa-user fa-lg' title='choisir une personne pour ce rôle'></i>");
    }

    echo '<a href=upd_personnel.php?pompier='.$CURPID.'>'.strtoupper($CURPNOM).' '.ucfirst($CURPPRENOM).'</a>';
    if ($CURSECTIONCODE != '') {
        echo ' <font size=1>('.$CURSECTIONCODE.')</font>';
    }
