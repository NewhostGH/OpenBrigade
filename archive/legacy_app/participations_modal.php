...
while (custom_fetch_array($result)) {
    ...
    if ($TP_LIBELLE != '') {
        $E_LIBELLE = $E_LIBELLE.'<br><span class=small>'.$TP_LIBELLE.'</span>';
    }
    echo "<tr>
    ...
    $periode = '';
    if ($RAW_DATE_DEBUT != $date) {
        $periode = 'du '.$EH_DATE_DEBUT.' à '.$EH_DEBUT;
    } else {
        $periode = ' de '.$EH_DEBUT;
    }
    if ($RAW_DATE_FIN != $date) {
        $periode .= ' au '.$EH_DATE_FIN.' à '.$EH_FIN;
    } else {
        $periode .= ' à '.$EH_FIN;
    }
