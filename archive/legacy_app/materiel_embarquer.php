...
if ($what == 'materiel') {
    ...
    while (custom_fetch_array($result)) {
        ...
        if ($prevTM_USAGE != $TM_USAGE) {
            echo "<OPTGROUP LABEL='".$TM_USAGE."' class='categorie option-ebrigade'>";
        }
        ...
        if ($big_device) {
            $text = $TM_CODE.' - '.$MA_MODELE.$add.$add2.'. '.$MA_LIEU_STOCKAGE;
        } else {
            $text = $TM_CODE.' - '.$MA_MODELE;
        }
    ...
    if ($where == 'vehicule') {
        $url = 'upd_vehicule.php?vid='.$KID.'&tab=3';
    } else {
