...
if (($I_STATUS == 'ANN') or ($I_STATUS == 'REF')) {
    $mytxtcolor = 'red';
}
if (($I_STATUS == 'ATT') or ($I_STATUS == 'PRE')) {
    $mytxtcolor = 'orange';
}
...
if ($I_JOUR_COMPLET == 0) {
    if ($abs == 1) {
        if (substr($IH_FIN, 0, 1) == '0') {
            $fin = substr($IH_FIN, 1, 1);
        } else {
            $fin = substr($IH_FIN, 0, 2);
        }
        if (substr($IH_DEBUT, 0, 1) == '0') {
            $debut = substr($IH_DEBUT, 1, 1);
        } else {
            $debut = substr($IH_DEBUT, 0, 2);
        }
        $abs = $fin - $debut;
        $abs .= ' heure(s)';
    } else {
...
if ($TI_FLAG == 1) {
    ...
    if ($nbcp == 1 and $I_JOUR_COMPLET == 2) {
        $d = '0,5 jour ';
    } else {
        $d = $nbcp.' jour';
    }
    if ($TI_CODE == 'RTT' or $TI_CODE == 'CP') {
        $d .= ' de '.$TI_CODE;
    }
...
if ($TI_FLAG == 1) {
    $label = "<font style='font-weight:600'><font color=$mytxtcolor>".$I_STATUS_LIBELLE.'</font></font>';
} else {
    $label = "<span class=small title='voir détail'>".$I_STATUS_LIBELLE.'</span>';
}
...
if ($TI_FLAG == 1) {
    ...
    } elseif (check_rights($id, 13, $section)) {
        echo " <input type=submit class='btn btn-danger' value='Supprimer' 
          onclick=\"bouton_redirect('indispo_status.php?code=$code&action=supprimer','supprimer');\">";
    }
} elseif (check_rights($id, 12, $section)) {
...
if ($from == 'calendar') {
    $s = 'calendar.php?pompier='.$P_ID;
} elseif ($from == 'personnel') {
    $s = 'upd_personnel.php?from=default&tab=18&pompier='.$P_ID.'&person='.$P_ID.'&table=1';
} else {
    $s = 'indispo_choice.php?tab=2';
}
