...
if ($critere == 'habilitation') {
    ...
    if ($s > 0) {
        $query .= ' and s.S_ID in ('.get_family($s).')';
    }
    if (! $externe) {
    ...
    $columns_title = ['Nom', 'Prénom', 'Civilite', 'Email', 'Section', 'Principal', 'Secondaire', 'Téléphone'];
    ...
    while ($row = @mysqli_fetch_array($result)) {
        ...
        $columns_data = [strtoupper($P_NOM), ucfirst($P_PRENOM), $civilite, $P_EMAIL, $section, $groupe1, $groupe2, $phone];
        foreach ($columns as $c => $letter) {
            $objPHPExcel->getActiveSheet()->setCellValue($letter.$i, utf8_encode($columns_data[$c]));
            $objPHPExcel->getActiveSheet()->getStyle($letter.$i)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_TEXT);
        }
        $i++;
    }
    if ($s != 0) {
        $cmt = ' de '.get_section_code("$s");
    } else {
        $cmt = ' de '.$cisname;
    }
...
// ===============================================
    ...
    switch ($critere) {
        case 'et':
            ...
            if ($nb == 1) {
                $query .= ' AND po.PS_ID = z.PS_ID AND z.P_ID = p.P_ID and z.PS_ID = '.$q[0];
            }
    ...
    $columns_title = ['Nom', 'Prénom', 'Civilité', 'Statut', 'Email', 'Section', 'Compétence', 'Expiration', 'Téléphone'];
    foreach ($columns as $c => $letter) {
        $objPHPExcel->getActiveSheet()->setCellValue($letter.'1', utf8_encode($columns_title[$c]));
        $objPHPExcel->getActiveSheet()->getColumnDimension($letter)->setAutoSize(true);
        $objPHPExcel->getActiveSheet()->getStyle($letter.'1')->getAlignment()->setWrapText(true);
    ...
    while ($row = @mysqli_fetch_array($result)) {
        ...
        $columns_data = [$P_NOM, $P_PRENOM, $civilite, $p_statut, $P_EMAIL, $section, $Competence, $Expire, $phone];
        foreach ($columns as $c => $letter) {
            $objPHPExcel->getActiveSheet()->setCellValue($letter.$i, utf8_encode($columns_data[$c]));
            $objPHPExcel->getActiveSheet()->getStyle($letter.$i)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_TEXT);
        }
