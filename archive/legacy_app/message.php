...
if (isset($_GET['mode_garde'])) {
    $mode_garde = intval($_GET['mode_garde']);
} else {
    $mode_garde = 0;
}

if (isset($catmessage) or isset($_POST['catmessage']) or isset($_SESSION['catmessage'])) {
    if (isset($_POST['catmessage'])) {
        $catmessage = secure_input($dbc, $_POST['catmessage']);
    } elseif (isset($_SESSION['catmessage'])) {
        $catmessage = secure_input($dbc, $_SESSION['catmessage']);
    }
    if ($catmessage != 'amicale') {
        $catmessage = 'consigne';
    }
    $error = 0;
} else {
...
if ($error == 0) {
    ...
    if (isset($_POST['objet']) or isset($_POST['message'])) {
        ...
        if (substr($message, 0, 3) == '<p>') {
            $message = substr($message, 3);
        }
        if (substr($message, -4) == '</p>') {
    ...
    if (($gardes == 1) and (check_rights($id, 8))) {
        $mycatmessage = 'consigne';
    } else {
        $mycatmessage = 'amicale';
    }
...
if ($write) {
    if (check_rights($id, $numfonction)) {
        ...
        if ($catmessage == 'amicale') {
            ...
            $target = 'tableau_garde.php?tab=2&mode_garde='.$mode_garde;
        }
        ...
        if ($highestsection == '') {
            $highestsection = $mysection;
        }
        if (($highestsection != '') and check_rights($id, 24)) {
            $highestsection = 0;
        }
...
// ============================================================
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        $grade = $row['P_GRADE'];
        $photo = $row['P_PHOTO'];
        if (! file_exists($trombidir.'/'.$photo)) {
            $photo = '';
        }
        ...
        if ($category == 'informatique') {
            echo "<span class='act-message-type' style='background-color:$color;'>".strtoupper($category).'</span>';
        }
        ...
        if ($row['M_FILE'] != '') {
            echo '<a href=showfile.php?section='.$S_ID.'&evenement=0&message='.$mid.'&file='.$file.'>Pièce jointe</a>';
        }
        echo '</div></td></tr>';
    }
    echo '</table></div>';
