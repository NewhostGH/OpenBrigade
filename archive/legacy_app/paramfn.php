...
$possibleorders = ['TE_CODE', 'TP_NUM', 'TP_LIBELLE', 'INSTRUCTOR', 'DESCRIPTION', 'DESCRIPTION2'];
...
if ($type_evenement != 'ALL' and $type_evenement != 'ALLBUTGARDE') {
...
if ($order == 'TE_CODE') {
    $query .= ' order by tp.TE_CODE, tp.EQ_ID, tp.TP_NUM';
} else {
...
if ($number > 0) {
    ...
    while (custom_fetch_array($result)) {
        ...
        if ($INSTRUCTOR == 1) {
            $ins = "<i class='fa fa-check-square fa-lg' style='color:green;' title='Instructeur ou moniteur'></i>";
        } else {
            $ins = '';
        }
