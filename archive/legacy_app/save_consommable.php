...
if (isset($_POST['quantity'])) {
    $C_NOMBRE = intval($_POST['quantity']);
} else {
    $C_NOMBRE = 0;
}
if (isset($_POST['minimum'])) {
    $C_MINIMUM = intval($_POST['minimum']);
} else {
    $C_MINIMUM = 0;
}
if (isset($_POST['C_DATE_ACHAT'])) {
    $C_DATE_ACHAT = secure_input($dbc, $_POST['C_DATE_ACHAT']);
} else {
    $C_DATE_ACHAT = '';
}
if (isset($_POST['C_DATE_PEREMPTION'])) {
    $C_DATE_PEREMPTION = secure_input($dbc, $_POST['C_DATE_PEREMPTION']);
} else {
    $C_DATE_PEREMPTION = '';
}
if (isset($_POST['C_DESCRIPTION'])) {
    $C_DESCRIPTION = secure_input($dbc, $_POST['C_DESCRIPTION']);
} else {
    $C_DESCRIPTION = '';
}
$C_DESCRIPTION = str_replace('"', '', $C_DESCRIPTION);
if (isset($_POST['C_LIEU_STOCKAGE'])) {
    $C_LIEU_STOCKAGE = secure_input($dbc, $_POST['C_LIEU_STOCKAGE']);
} else {
    $C_LIEU_STOCKAGE = '';
}
...
if ($C_DATE_PEREMPTION != '') {
    $tmp = explode('-', $C_DATE_PEREMPTION);
    $month1 = $tmp[1];
    $day1 = $tmp[0];
    $year1 = $tmp[2];
    $C_DATE_PEREMPTION = '"'.$year1.'-'.$month1.'-'.$day1.'"';
} else {
    $C_DATE_PEREMPTION = 'null';
}
