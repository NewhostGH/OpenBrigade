...
if (! in_array($filter, $grantedsections)) {
    ...
    if (! in_array($filter, $list) and ! check_rights($id, 24)) {
        $filter = $highestsection;
    }
}

$possibleorders = ['P_STATUT', 'P_NOM', 'P_SECTION', 'P_PROFESSION', 'TP_DESCRIPTION', 'COMMENTAIRE', 'PC_ID', 'P_DATE_ENGAGEMENT', 'P_FIN', 'PC_DATE'];
...
if ($syndicate == 1) {
    $columns_title = ['Nom Prénom', 'Profession', 'Mode paiement', 'Statut', 'Section', 'Entrée', 'Sortie', 'Payé', 'Montant', 'Date payé', 'Commentaire'];
    $columns = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K'];
} else {
    $columns_title = ['Nom Prénom', 'Statut', 'Section', 'Entrée', 'Sortie', 'Payé', 'Montant', 'Date payé', 'Commentaire'];
...
if ($period_month != '0') {
    $query .= " and ( p.P_DATE_ENGAGEMENT < DATE_ADD('".$year.'-'.$period_month."-01', INTERVAL 1 MONTH) or p.P_DATE_ENGAGEMENT is null )";
...
if ($paid == 1) {
    $query .= ' and pc.PC_DATE is not null';
} elseif ($paid == 0) {
    $query .= ' and pc.PC_DATE is  null';
}
if ($include_old == 0) {
    $query .= ' and p.P_OLD_MEMBER = 0 and p.SUSPENDU = 0';
}
...
while (custom_fetch_array($result)) {
    if ($MONTANT == '') {
        ...
        if ($periode == 'A' and ($YEAR_ENGAGEMENT == $year or $YEAR_FIN == $year)) {
            ...
            if ($MONTH_ENGAGEMENT != '' and $YEAR_ENGAGEMENT == $year) {
    ...
    if ($PC_DATE == '') {
        $PAID = 'Non';
        if ($COMMENTAIRE == '' and $number_months_to_pay != '' and $bank_accounts == 1) {
            $COMMENTAIRE = $number_months_to_pay.' mois';
        }
    } else {
        $PAID = 'Oui';
    }
    ...
    if ($syndicate == 1) {
        $columns_data = [strtoupper($P_NOM).' '.my_ucfirst($P_PRENOM), $P_PROFESSION, $TP_DESCRIPTION, $P_STATUT, $S_CODE, $P_DATE_ENGAGEMENT, $P_FIN, $PAID, $MONTANT, $PC_DATE, $COMMENTAIRE];
    } else {
        $columns_data = [strtoupper($P_NOM).' '.my_ucfirst($P_PRENOM), $P_STATUT, $S_CODE, $P_DATE_ENGAGEMENT, $P_FIN, $PAID, $MONTANT, $PC_DATE, $COMMENTAIRE];
    }
