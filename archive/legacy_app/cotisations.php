...
if ((isset($_GET['old']) and $_GET['old'] == 1) or (isset($_GET['include_old']) and $_GET['include_old'] == 1)) {
    $old = 1;
} else {
    $old = 0;
}
if (isset($_GET['year'])) {
    $year = secure_input($dbc, $_GET['year']);
} else {
    $year = date('Y');
}
...
if ($order == 'P_DATE_ENGAGEMENT' or $order == 'P_FIN' or $order == 'PC_DATE') {
    $query .= ' desc';
}
...
if ($syndicate == 1) {
    $title = 'Cotisations des adhérents';
} else {
    $title = 'Cotisations du personnel';
}
if (isset($_GET['tab'])) {
...
if (check_rights($_SESSION['id'], 53)) {
    ...
    if ($bank_accounts == 1) {
        ...
        if ($order == 'P_DATE_ENGAGEMENT' or $order == 'P_FIN' or $order == 'PC_DATE' or $order == 'MONTANT') {
            $query3 .= ' desc';
        }
...
$nb = 0;
if ($paid == 0 and $check_all == 1) {
    $nb = min($pages->items_per_page, $pages->items_total);
}
    ...
    while (custom_fetch_array($result)) {
        ...
        if ($PC_DATE != '') {
            if ($check_all == 1) {
                ...
                if ($MONTANT >= $EXPECTED_MONTANT) {
                    $montant_style = 'color: Green;';
                } else {
                    $montant_style = 'color: Orange;';
                }
            } else {
                $montant_style = 'color: Black;';
            }
        } else {
            if ($COMMENTAIRE == '' and $number_months_to_pay != '' and $bank_accounts == 1) {
                $COMMENTAIRE = $number_months_to_pay.' mois';
