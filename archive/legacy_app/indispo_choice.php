...
if (! isset($_GET['table'])) {
    ...
    $onlyTable = intval($_GET['table']);
}

$possibleorders = ['P_NOM', 'TI_CODE', 'I_STATUS', 'I_DEBUT', 'I_FIN', 'I_COMMENT'];
...
if (check_rights($id, 12) and intval($person) > 0) {
    $query .= "\nand  p.P_ID = ".$person;
} elseif (! check_rights($id, 12)) {
    $query .= "\nand  p.P_ID = ".$id;
}
if ($validation != 'ALL') {
...
if ($order == 'I_COMMENT' or $order == 'I_DEBUT' or $order == 'I_FIN') {
    $query .= ' desc';
}
...
if ($number > 0) {
    ...
    while (custom_fetch_array($result)) {
        ...
        if ($I_FIN != $I_DEBUT) {
            $mobiletxt = "<span class='only_mobile'><br>".$I_FIN.'</span>';
        } else {
            $mobiletxt = '';
        }
    ...
    if ($type_indispo == 'CP' || $type_indispo == 'RTT') {
        ...
        } elseif ($type_indispo == 'RTT' and (intval($person) > 0)) {
            // droits CP / an
