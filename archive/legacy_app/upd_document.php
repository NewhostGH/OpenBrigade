...
if (isset($_SESSION['dossier'])) {
    $dossier = intval($_SESSION['dossier']);
} else {
    $dossier = 0;
}
...
if ($evenement == 0 and $pompier == 0 and $vehicule == 0 and $note == 0 and $dossier == 0 and $materiel == 0 and $victime == 0 and $numinter == 0) {
    ...
    $parent = 'A la racine';
} elseif (isset($_SESSION['dossier']) and $_SESSION['dossier'] > 0 and $evenement == 0 and $pompier == 0 and $vehicule == 0 and $note == 0 and $materiel == 0) {
...
if ($note == 0 and $document_security == 1) {
    ...
    echo "<input type='hidden' name='security' value='1'>";
}
...
if ($note > 0) {
    $onclick = "javascript:self.location.href='note_frais_edit.php?nfid=".$note."&action=update'";
} else {
