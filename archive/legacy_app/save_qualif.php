...
while (custom_fetch_array($result)) {
    ...
    $year = '';
    $day = '';
    if (isset($_POST['updated_'.$PS_ID])) {
        $updated = intval($_POST['updated_'.$PS_ID]);
    } else {
        $updated = 0;
    }
    if ($updated == 1) {
        ...
        if ($PS_USER_MODIFIABLE == 1 and $id == $P_ID) {
            $competence_allowed = true;
        } elseif (check_rights($id, $F_ID, "$P_SECTION")) {
            $competence_allowed = true;
        } else {
            $competence_allowed = false;
        }
