...
function widget_condition($type, $value)
{
    ...
    $configs = ['evenements', 'gardes', 'disponibilites', 'assoc', 'vehicules', 'consommables', 'remplacements', 'syndicate', 'army', 'notes', 'pompiers', 'main_courante'];
    $ret = false;
    if ($type == 'multi_check_rights_notes') {
        ...
        if (check_rights($id, $value)) {
            $ret = true;
        }
    } elseif (in_array($type, $configs)) {
        global $$type;
        if ($$type == $value) {
            $ret = true;
        }
    ...
    if ($num != 0) {
        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            if ($row['P_PHOTO'] == '') {
                if ($row['P_CIVILITE'] == '1') {
                    $img2 = 'images/boy.png';
                } elseif ($row['P_CIVILITE'] == '2') {
                    $img2 = 'images/girl.png';
                } elseif ($row['P_CIVILITE'] == '3') {
                    $img2 = 'images/autre.png';
                } elseif ($row['P_CIVILITE'] == '4' or $row['P_CIVILITE'] == '5') {
                    $img2 = 'images/chien.png';
                }
            } elseif (! file_exists($trombidir.'/'.$row['P_PHOTO'])) {
                if ($row['P_CIVILITE'] == '1') {
                    $img2 = 'images/boy.png';
                } elseif ($row['P_CIVILITE'] == '2') {
                    $img2 = 'images/girl.png';
            ...
            if ($date_comment == 'Demain') {
                $color = 'orange';
            } elseif ($date_comment == 'Après-demain') {
                $color = 'violet';
            } else {
                $color = 'blue';
            }
...
function missing_field($row, $field, $description, $txt = '')
{
    ...
    if ($row[$field] == '0' or $row[$field] == '') {
        if (isset($GLOBALS['nbAlertMissingFields'])) {
            $GLOBALS['nbAlertMissingFields']++;
        } else {
            $GLOBALS['nbAlertMissingFields'] = 1;
        }
...
function show_alerts_consommables()
{
    ...
    if ($nbsections > 0 and check_rights($id, 71, 0)) {
        $mysection = 0;
    } else {
        $mysection = get_highest_section_where_granted($id, 71);
    }
    if (($assoc or $army) and $mysection == 0) {
...
function show_alerts_vehicules()
{
    ...
    if ($nbsections > 0 and check_rights($id, 17, 0)) {
        $mysection = 0;
    } else {
...
function show_alerts_cp()
{
    ...
    $id = intval(@$_SESSION['id']);
    $GLOBALS['nbAlertCp'] = 0;
    if ($nbsections > 0 and check_rights($id, 13, 0)) {
        $mysection = 0;
    } else {
        $mysection = get_highest_section_where_granted($id, 13);
    }
    // des CP à valider?
...
function show_alerts_horaires()
{
    ...
    if ($mysection == 0 and $nbsections == 0) {
        $list = '0';
    } else {
        $list = get_family("$mysection");
    }
...
function show_alerts_remplacements()
{
    ...
    if ($nbsections > 0) {
        $sid = 0;
    } elseif ($gardes == 1 and check_rights($id, 24) and check_rights($id, 6)) {
        $sid = 0;
    } else {
        $sid = intval(@$_SESSION['SES_FAVORITE']);
        if ($pompiers == 1 and $nbsections == 0 and get_level("$sid") == $nbmaxlevels - 1) {
            $sid = get_section_parent("$sid");
        }
    }
    ...
    if ($gardes == 1 and check_rights($id, 6)) {
        ...
        $txt = 'À approuver';
        $status = 'ATT';
    } elseif ($gardes == 1 and check_rights($id, 61)) {
        $query = "select date_format(min(eh.EH_DATE_DEBUT),'%d-%m-%Y') DEBUT, date_format(max(eh.EH_DATE_FIN),'%d-%m-%Y') FIN, count(1) as NB
        from remplacement r, evenement_horaire eh, evenement e
        where eh.E_CODE = r.E_CODE 
        and eh.EH_ID=1
    ...
    if ($query != '') {
        ...
        if ($nb > 0) {
            ...
            $out .= "<span class='no-content'>Aucun remplacement en cours.</span>";
        }
    }
...
function show_proposed_remplacements()
{
    ...
    if ($out == '') {
        $out = "<span class='no-content'>Aucun recherche de remplaçant.</span><a href='remplacements.php?filter=".$sid."&replaced=0&substitute=0'><i class = 'fa fa-ellipsis-h fa-lg three-points-icon'></i></a>";
    }
...
function show_attestation_fiscale()
{
    ...
    if ($montant == 0) {
        $out = '<td>Aucune cotisation enregistrée pour '.$fiscal_year.'.</td>';
    } else {
...
function show_about()
{
    ...
    $row = mysqli_fetch_array($result);
    if (@$row['P_EMAIL'] != '') {
        $display_mail = $row['P_EMAIL'];
    } else {
        $display_mail = $admin_email;
    }
    ...
    if (check_rights($id, 14)) {
        $link = 'configuration.php?tab=conf7';
    } else {
        $link = 'about.php';
    }
...
function show_events()
{
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        if ($E_CLOSED == 1) {
            $myimg = "<i class='fas fa-arrow-right event-arrow event-arrow-closed' title='inscriptions fermées'></i>";
        } else {
            $myimg = "<i class='fas fa-arrow-right event-arrow event-arrow-opened' title='inscriptions ouvertes'></i>";
        }
        if ($EH_ID > 1) {
            $sess = ' session n°'.$EH_ID;
        } else {
            $sess = '';
        }
        if (is_file('images/evenements/'.$TE_ICON)) {
            $img = '<img src=images/evenements/'.$TE_ICON.' width=40>';
        } else {
            $img = '';
        }
    ...
    if ($out == '') {
        $out = "<span class='no-content'>Pas d'événements prévus</span>";
    }
...
function show_factures()
{
    ...
    if ($number > 0) {
        ...
        while ($row = mysqli_fetch_array($res)) {
            ...
            if ($relance_date != '') {
                $c = "<span  class='alert-blue alert-label'>Relance depuis ".$relance_depuis.' j</span>';
            } elseif ($facture_date != '') {
                $c = "<span class='alert-orange alert-label'>Facturé depuis ".$facture_depuis.' j</span>';
            } else {
                $c = "<span class='alert-red alert-label'>A facturer depuis ".$termine_depuis.' j</span>';
            }
            if (intval($montant) == 0) {
                $montant = $row['devis_montant'];
            }
...
function show_stats_manquantes()
{
    ...
    if ($out == '') {
        $out = "<span class='no-content'>Aucune statistique manquante</span>";
    }
...
function show_notes()
{
    ...
    if (get_level("$section_perm1") < $nbmaxlevels - 1) {
        ...
        $perm_dep = true;
    } elseif (get_level("$section_perm2") < $nbmaxlevels - 1) {
        $section_perm = $section_perm2;
        $perm_dep = true;
    } elseif (get_level("$section_perm3") < $nbmaxlevels - 1) {
        $section_perm = $section_perm3;
        $perm_dep = true;
    } else {
        $perm_dep = false;
    }
    if ($section_perm == 0 and $nbsections == 0) {
        $list = '0';
    } else {
        $list = get_family("$section_perm");
    }
    ...
    if ($perm_dep) {
        ...
        } elseif (! empty($list) and ($syndicate == 0 or ! multi_check_rights_notes($id, '0'))) {
    ...
    if ($number == 0) {
        ...
        while ($row = mysqli_fetch_array($res)) {
            ...
            $cmt = '';
            if ($departemental == 1) {
                $cmt .= "<span title='Note de frais départementale'>D</span>";
            } elseif ($national == 1) {
                $cmt .= "<span title='Note de frais nationale'>N</span>";
            }
            if ($syndicate == 1) {
                if ($fscode == 'VAL') {
                    $statut = 'Validée trésorier';
                } elseif ($fscode == 'VAL1') {
                    $statut = 'Validée président';
                }
            }
            if ($verified == 1) {
                $v = " <i class='fas fa-check' title='vérifié par la comptabilité'></i>";
            } else {
                $v = '';
            }
            ...
            } elseif ($statut == 'Validée deux fois') {
                $alertClass = 'alert-label alert-green-apple';
            } elseif (substr($statut, 0, 3) == 'Val') {
                $alertClass = 'alert-label alert-green';
            } elseif ($statut == 'Remboursée') {
...
function show_participations($type = 'ALL')
{
    ...
    if ($number == 0) {
        ...
        while ($row = mysqli_fetch_array($res)) {
            ...
            $te_code = $row['te_code'];
            if ($te_code == 'GAR' and $gardes == 1 and $pompiers == 1) {
                $gardeSP = true;
            } else {
                $gardeSP = false;
            }
            ...
            if ($e_code == 0 and $type == 'ALL') {
                ...
                if ($datedeb != $datefin) {
                    $out .= "<td class='activity-date' style='min-width:100px;'><div class='widget-text'>".$datedeb."</div><span class='subtitle-small'>au ".$datefin.'</span></td>';
                } else {
                    $out .= "<td class='activity-date'><div class='widget-text'>".$datedeb.'</div></td>';
                }
                ...
                if ($cmt != '') {
                    $txtimg = "<i class='fa fa-".$txtimg."' title=\"".$cmt.'" ></i>';
                } else {
                    $txtimg = '';
                }
                ...
                if ($row['e_visible_inside'] == 0) {
                    $libelle .= " <i class='fa fa-exclamation-triangle' style='color:orange;' title='ATTENTION événement caché, seules les personnes inscrites ou ayant la permission n°9 peuvent le voir'></i>";
                }
                if ($row['EQ_ICON'] == '') {
                ...
                if ($te_code != 'MC') {
                    $out .= "<span class='widget-subtitle'><br>Durée ".$duree."h<br>$tp_libelle</span>";
                }
...
function show_stats()
{
    ...
    $nbTotalAlert = 0;
    if (isset($GLOBALS['nbAlertVehicule'])) {
        $nbTotalAlert += $GLOBALS['nbAlertVehicule'];
    }
    if (isset($GLOBALS['nbAlertConsommable'])) {
        $nbTotalAlert += $GLOBALS['nbAlertConsommable'];
    }
    if (isset($GLOBALS['nbAlertCp'])) {
        $nbTotalAlert += $GLOBALS['nbAlertCp'];
    }
    if (isset($GLOBALS['nbAlertHoraire'])) {
        $nbTotalAlert += $GLOBALS['nbAlertHoraire'];
    }
    if (isset($GLOBALS['nbAlertRemplacement'])) {
        $nbTotalAlert += $GLOBALS['nbAlertRemplacement'];
    }
    if (isset($GLOBALS['nbAlertMissingFields'])) {
        $nbTotalAlert += $GLOBALS['nbAlertMissingFields'];
    }
    // section
