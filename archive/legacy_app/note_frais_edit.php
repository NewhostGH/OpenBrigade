...
function write_select($number, $value, $disabled = '', $new = 'false')
{
    ...
    while ($row1 = @mysqli_fetch_array($result1)) {
        ...
        if ($TF_CODE1 == $value) {
            $selected = 'selected';
            if ($class == 'restricted') {
                $warn = "<i class='fa fa-exclamation-triangle' style='color:red' title=\"".$TF_COMMENT.'"></i>';
            }
        } else {
            $selected = '';
        }
        if ($TF_PRIX_UNITAIRE != '') {
            ...
            if ($new) {
                $TF_DESCRIPTION1 = $TF_DESCRIPTION1.' ('.$TF_PRIX_UNITAIRE.' '.$default_money_symbol.'/'.$TF_UNITE.')';
            }
        }
...
if (get_level("$S_ID") < $nbmaxlevels - 1) {
    $departement = $S_ID;
} else {
    $departement = $S_PARENT;
}
...
if (check_rights($id, 14)) {
    $administrateur = true;
} elseif ($syndicate == 1 and check_rights($id, 9) and $tresorier_national) {
    $administrateur = true;
} else {
    $administrateur = false;
}

if ($NF_NATIONAL == 1 and $tresorier_national) {
    $granted_update = true;
} elseif ($person == $id and $FS_CODE != 'CRE' and $FS_CODE != 'ATTV' and $FS_CODE != 'REJ') {
    $granted_update = false;
} elseif (multi_check_rights_notes($id, "$S_ID")) {
    $granted_update = true;
} elseif ($administrateur) {
    $granted_update = true;
} elseif ($person == $id) {
    $granted_update = true;
}
...
if ($administrateur) {
    $granted_validation = true;
} elseif ($person == $id) {
    $granted_validation = false;
} else {
    $granted_validation = $granted_update;
}

if ($administrateur or $tresorier_national) {
    $all_status_changes_allowed = true;
} else {
    $all_status_changes_allowed = false;
}
...
echo get_specific_section_option($person, $P_SECTION, $S_ID);
...
while (custom_fetch_array($result1)) {
    ...
    if ($assoc and intval($E_PARENT) > 0 and strtolower(substr($E_LIBELLE, 0, 7)) != 'renfort') {
        $E_LIBELLE = 'Renfort '.$E_LIBELLE;
    }
    ...
    if ($nfid == 0 and $action == 'insert') {
        if ($syndicate) {
            ...
            $lst = ['DPS', 'AIP', 'HEB', 'ALERT', 'AH', 'GAR', 'NAUT', 'COOP', 'MED'];
            $lst2 = ['FOR', 'MAN', 'EXE'];
            if (in_array($TE_CODE, $lst)) {
                $TM_CODE = 'OP';
            } elseif (in_array($TE_CODE, $lst2)) {
                $TM_CODE = 'FG';
            } else {
                $TM_CODE = 'AUT';
            }
...
while ($row1 = @mysqli_fetch_array($result1)) {
    ...
    if ($MOTIF_LEVEL != $PREV_MOTIF) {
        if ($MOTIF_LEVEL == 'N') {
            $mlname = 'Niveau Fédéral seulement';
        } elseif ($MOTIF_LEVEL == 'D') {
            $mlname = 'Niveau Départemental seulement';
        } else {
            $mlname = 'Pour tous les niveaux';
        }
...
if ($nfid > 0) {
    if (intval($NF_CREATE_BY) == 0) {
        $nom1 = '';
    } else {
        $nom1 = 'par '.my_ucfirst($P_PRENOM1).' '.strtoupper($P_NOM1);
    }
    $cmt = "créée le $NF_CREATE_DATE $nom1";
    if ($NF_STATUT_DATE != '' and $FS_CODE == 'REJ') {
        if (intval($NF_STATUT_BY) == 0) {
            $nom2 = '';
        } else {
            $nom2 = 'par '.my_ucfirst($P_PRENOM2).' '.strtoupper($P_NOM2);
        }
        $cmt .= " , rejetée le $NF_STATUT_DATE $nom2";
    } elseif ($NF_STATUT_DATE != '' and $FS_CODE == 'ATTV') {
        if (intval($NF_STATUT_BY) == 0) {
            $nom2 = '';
        } else {
            $nom2 = 'par '.my_ucfirst($P_PRENOM2).' '.strtoupper($P_NOM2);
    ...
    if ($NF_VALIDATED_DATE != '') {
        if (intval($NF_VALIDATED_BY) == 0) {
            $nom4 = '';
        } else {
            $nom4 = 'par '.my_ucfirst($P_PRENOM4).' '.strtoupper($P_NOM4);
        }
        $cmt .= " , validée le $NF_VALIDATED_DATE $nom4";
    }
    if ($NF_VALIDATED2_DATE != '') {
        if (intval($NF_VALIDATED2_BY) == 0) {
            $nom5 = '';
        } else {
            $nom5 = 'par '.my_ucfirst($P_PRENOM5).' '.strtoupper($P_NOM5);
        }
        $cmt .= " , validée le $NF_VALIDATED2_DATE $nom5";
    }
    if ($NF_REMBOURSE_DATE != '') {
        if (intval($NF_REMBOURSE_BY) == 0) {
            $nom3 = '';
        } else {
            $nom3 = 'par '.my_ucfirst($P_PRENOM3).' '.strtoupper($P_NOM3);
        }
    ...
    if ($administrateur) {
        ...
        if ($color == 'green') {
            ...
            $allcolor = "background-color:$FS_COLOR; color:white";
        }
        ...
        $result1 = mysqli_query($dbc, $query1);
        while (custom_fetch_array($result1)) {
            if ($FC == 'VAL' and $syndicate == 1) {
                $FD = 'Validée trésorier';
            }
            if ($FC == 'VAL1' and $syndicate == 1) {
                $FD = 'Validée président';
            }
            if ($FC == 'VAL' and $syndicate == 0) {
                $FD = 'Validée';
            }
            if ($FC == 'VAL1' and $syndicate == 0) {
                $FD = 'Validée autre';
            }
            if ($NF_DON == 1 and $FC == 'REMB' and $assoc) {
                $FD = "Don à l'association";
            }
            if ($FC == $FS_CODE) {
                $selected = 'selected';
            } else {
            ...
            if ($color == 'green') {
                ...
                $allcolor = "background-color:$color; color:white";
            }
    ...
    // note vérifiée?
    if ($syndicate) {
        if (($id != $person and check_rights($id, 75)) or $administrateur) {
            $disabled_verified = '';
        } else {
        ...
        if ($NF_VERIFIED == 1) {
            if (intval($NF_VERIFIED_BY) == 0) {
                $nom6 = '';
            } else {
                $nom6 = '<i> - par '.my_ucfirst($P_PRENOM6).' '.strtoupper($P_NOM6).' le '.$NF_VERIFIED_DATE.'</i>';
            }
...
if ($action == 'insert') {
    if ($KM != '') {
        $selectForm = write_select(1, 'KM', '', true);
    } else {
        $selectForm = write_select(1, '', '', true);
    }
    $i = 1;
...
if (intval($NF_VALIDATED_BY) != $id and intval($NF_VALIDATED2_BY) != $id) {
    $not_validated_by_me = true;
} else {
    $not_validated_by_me = false;
}
...
if ($granted_update) {
    ...
    if ($id != $person or $all_status_changes_allowed) {
        if ($not_validated_by_me) {
            // bouton valider A
            if (in_array($FS_CODE, ['ATTV', 'VAL1']) and check_rights($id, 73, "$S_ID")) {
                if ($syndicate == 1) {
                    $va = 'Valider trésorier';
                } elseif ($FS_CODE == 'VAL1') {
                    $va = 'Valider 2';
                } else {
                    $va = 'Valider';
            ...
            // bouton valider B
            if (in_array($FS_CODE, ['ATTV', 'VAL']) and check_rights($id, 74, "$S_ID")) {
                if ($syndicate == 1) {
                    $vb = 'Valider président';
                } elseif ($FS_CODE == 'VAL') {
                    $vb = 'Valider 2';
                } else {
                    $vb = 'Valider';
        ...
        // bouton rejeter
        if (in_array($FS_CODE, ['ATTV', 'VAL', 'VAL1', 'VAL2'])) {
    ...
    if ($id != $person and check_rights($id, 75, "$S_ID")
        and ((in_array($FS_CODE, ['VAL', 'VAL1', 'VAL2']) and $syndicate == 0)
