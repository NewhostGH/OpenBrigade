...
if ($domain != -1) {
    $query1 .= ' and tf.TF_ID = '.$domain;
} else {
    $query1 .= ' and tf.TF_ID is not null';
}
...
if ($gardes == 0) {
    $query .= " where TF_DESCRIPTION <> 'gardes'";
}
...
if ($nbg > 0) {
    ...
    while (custom_fetch_array($result2)) {
        ...
        if ($GP_DESCRIPTION == 'Président (e)') {
            $title = $GP_DESCRIPTION." ou responsable d'antenne.";
        } elseif ($GP_DESCRIPTION == 'Vice président (e)') {
            $title = $GP_DESCRIPTION." ou responsable adjoint d'antenne.";
        } else {
            $title = '';
        }
        ...
        if ($GP_USAGE == 'externes') {
            $class = 'green12';
        } elseif ($GP_USAGE == 'all') {
            $class = 'orange12';
        } elseif ($GP_ASTREINTE == 1 and $cron_allowed == 1) {
            $class = 'purple12';
            $title = 'Ce rôle peut être attribué pour des astreintes';
        } else {
            $class = '';
        ...
        if (check_rights($id, 9)) {
            ...
            if ($title != '') {
                $GP_DESCRIPTION = "<a title='$title'><span class=$class>$GP_DESCRIPTION</span></a>";
            }
    ...
    while (custom_fetch_array($result1)) {
        if (($gardes == 1) or ($F_TYPE != 1)) {
            ...
            if ($F_FLAG == 1 and $nbsections == 0) {
                $cmt = " <i class='fa fa-asterisk' style='color:red; position: relative; top: -3px; font-size:0.5em;' title='Permission valide au niveau départemental seulement'></i>";
            } else {
                $cmt = '';
            }
            $help_link = ' ';
