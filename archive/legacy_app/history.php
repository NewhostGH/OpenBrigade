...
$possibleorders = ['LH_STAMP', 'LT_DESCRIPTION', 'P_ID', 'LH_COMPLEMENT', 'P_NOM', 'COMPLEMENT_CODE', 'P_NOM2'];
...
if ($lcid > 0) {
    if ($lccode == 'P') {
        ...
        if ($granted_for_all) {
            $what2 = '<a href=history.php?ltcode='.$ltcode."&lccode=P&lcid=0 title='historique pour tout le personnel'>Voir tout</a>";
        }
    ...
    if ($lccode == 'S') {
        ...
        if ($granted_for_all) {
            $what2 = '<a href=history.php?ltcode='.$ltcode."&lccode=S&lcid=0 title='historique pour toutes les sections'>Voir tout</a>";
        }
    ...
    if ($lccode == 'E') {
        ...
        if ($granted_for_all) {
            $what2 = '<a href=history.php?ltcode='.$ltcode."&lccode=E&lcid=0 title='historique pour tous les événements'>Voir tout</a>";
        }
    }
} elseif ($lccode == 'U' or $lccode == 'C') {
    check_all(20);
} else { // $lcid=0
    check_all(49);
    $query .= " and lt.LC_CODE='P'";
    ...
    if ($lccode == 'E') {
        if ($filter > 0) {
            $query .= ' and e.S_ID in ('.get_family("$filter").')';
        }
        $query .= " union select lh.LH_ID,lh.P_ID, date_format(lh.LH_STAMP, '%d-%m-%Y %k:%i:%s') DATE, LH_STAMP, lh.LT_CODE,lh.LH_WHAT,lh.LH_COMPLEMENT,lh.COMPLEMENT_CODE,
        lt.LT_CODE,lt.LT_DESCRIPTION,p.P_NOM, p.P_PRENOM, e.E_CODE, e.E_LIBELLE, '' P_NOM2, '' P_PRENOM2, '' P_SECTION
        from log_type lt, pompier p, log_history lh, evenement e
    ...
    if ($lccode == 'S') {
        ...
        if ($filter > 0) {
            $query .= ' and s.S_ID in ('.get_family("$filter").')';
        }
        $query .= " union select lh.LH_ID,lh.P_ID, date_format(lh.LH_STAMP, '%d-%m-%Y %k:%i:%s') DATE, LH_STAMP, lh.LT_CODE,lh.LH_WHAT,lh.LH_COMPLEMENT,lh.COMPLEMENT_CODE,
        lt.LT_CODE,lt.LT_DESCRIPTION,p.P_NOM, p.P_PRENOM,s.S_CODE P_NOM2, '' P_PRENOM2, s.S_ID P_SECTION, '' E_CODE, '' E_LIBELLE
        from log_type lt, pompier p, log_history lh, section s where p.P_ID = lh.P_ID and lh.LT_CODE=lt.LT_CODE and s.S_ID = lh.COMPLEMENT_CODE";
...
if ($order == 'LH_STAMP' or $order == 'COMPLEMENT_CODE') {
    $query .= ' desc';
}
...
if ($lccode == 'A') {
    ...
    $c = 'Modifications sur les '.$days_log.' derniers jours';
} else {
    $c = '';
}
...
if ($number > 0) {
    ...
    while (custom_fetch_array($result)) {
        ...
        } elseif ($lccode == 'P' and ($LT_CODE == 'ADDROLE' or $LT_CODE == 'DELROLE')) {
            $COMPLEMENT = '';
        } elseif ($E_LIBELLE != '') {
        ...
        if ($lccode == 'P' and ($LT_CODE == 'UPDMAIL' or $LT_CODE == 'UPDPHONE' or $LT_CODE == 'UPDADR')) {
            if (! check_rights($id, 2, get_section_of($LH_WHAT))) {
                $LH_COMPLEMENT = '*********';
            }
        }
