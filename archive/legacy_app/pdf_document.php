...
if (isset($_GET['victime'])) {
    $victime = intval($_GET['victime']);
} else {
    $victime = 0;
}

if (isset($_GET['numinter'])) {
    $numinter = intval($_GET['numinter']);
} else {
    $numinter = 0;
}

if (isset($_GET['signed'])) {
    $signed = intval($_GET['signed']);
} else {
    $signed = 1;
}
...
if (isset($_GET['tofile'])) {
    $tofile = intval($_GET['tofile']);
} else {
    $tofile = 0;
}

// specifique rapport
if (isset($_GET['show_responsable'])) {
    $show_responsable = intval($_GET['show_responsable']);
} else {
    $show_responsable = 0;
}
if (isset($_GET['show_nombres'])) {
    $show_nombres = intval($_GET['show_nombres']);
} else {
    $show_nombres = 0;
}
if (isset($_GET['show_statistiques'])) {
    $show_statistiques = intval($_GET['show_statistiques']);
} else {
    $show_statistiques = 0;
}
if (isset($_GET['show_vehicules'])) {
    $show_vehicules = intval($_GET['show_vehicules']);
} else {
    $show_vehicules = 0;
}
if (isset($_GET['show_materiel'])) {
    $show_materiel = intval($_GET['show_materiel']);
} else {
    $show_materiel = 0;
}
if (isset($_GET['show_cav'])) {
    $show_cav = intval($_GET['show_cav']);
} else {
    $show_cav = 0;
}
...
function write_victime_comments($comments)
{
    ...
    if ($VI_DETRESSE_VITALE == 1) {
        $comments .= 'La victime est en détresse vitale. ';
    }
    if ($VI_SOINS == 1) {
        $comments .= "Des soins ont été réalisés par l'équipe de secouristes. ";
    }
    if ($VI_MEDICALISE == 1) {
        $comments .= 'La victime a été médicalisée. ';
    }
    if ($VI_INFORMATION == 1) {
        $comments .= 'La personne a été assistée, ou des renseignements et informations lui ont été donnés. ';
    }
    if ($VI_DECEDE == 1) {
        $comments .= 'La victime est décédée. ';
    }
    if ($VI_MALAISE == 1) {
        $comments .= 'La victime a eu un malaise. ';
    }
    if ($VI_TRAUMATISME == 1) {
        $comments .= "La victime souffre d'un traumatisme. ";
    }
    if ($VI_REPOS == 1) {
        $comments .= 'La victime a été mise au repos sous surveillance. ';
    }
    if ($VI_TRANSPORT == 1) {
        $comments .= 'La victime a été transportée par '.$T_NAME.', destination: '.$D_NAME.'. ';
    }
    if ($HEURE_HOPITAL != '') {
        $comments .= 'Arrivée à '.$HEURE_HOPITAL.'. ';
    }
    if ($VI_VETEMENT == 1) {
        $comments .= 'Des vêtements ou une couverture ont été offerts à la victime. ';
    }
    if ($VI_ALIMENTATION == 1) {
        $comments .= 'Des aliments ou une boisson ont été offerts à la victime. ';
    }
    if ($VI_REFUS == 1) {
        $comments .= "La victime a refusé d'être prise en charge. ";
    }
    if ($VI_IMPLIQUE == 1) {
        $comments .= 'La personne est seulement impliquée, indemne. ';
    }
    if ($VI_REPARTI == 1) {
        $comments .= 'La victime est repartie par ses propres moyens. ';
    }
...
while ($row = mysqli_fetch_array($result)) {
    ...
    if ($contact_tel != '') {
        $contact = ''.$row['E_CONTACT_LOCAL'].' (tél. '.$contact_tel.')';
    }
    $section = $row['S_ID'];
    $description = stripslashes($row['E_LIBELLE']);
    $S_URL = $row['S_URL'];
    if ($row['S_EMAIL2'] != '') {
        $S_EMAIL = $row['S_EMAIL2'];
    } else {
        $S_EMAIL = $row['S_EMAIL'];
    }
    $S_PHONE = $row['S_PHONE'];
    ...
    $TAV_ID = $row['TAV_ID'];
    if ($row['E_FLAG1'] == 1) {
        $interassociatif = true;
    } else {
        $interassociatif = false;
    }
...
if (count($chefs) > 0) {
    $id_responsable_evt = intval($chefs[0]);
} else {
    $id_responsable_evt = 0;
}

if ($id_responsable_evt > 0) {
    $responsable_evt_phone = get_phone($id_responsable_evt);
} else {
    $responsable_evt_phone = '';
}
...
if ($niv == $nbmaxlevels - 1) {
    ...
    if (stristr($antenne_affiche, 'canotier') != '') {
        $antenne_affiche = '';
    } elseif (stristr($antenne_affiche, 'non embarqué') != '') {
        $antenne_affiche = '';
    }
    $tmpS = $row2['S_ID'];
...
if ($num_resulty == 0) {
    ...
    if ($responsable_antenne == '') {
        $responsable_antenne = $chef;
    }
}
...
if ($S_URL != '' or $S_EMAIL != '') {
    if ($S_EMAIL != '') {
        $mailinfos .= 'Email : '.$S_EMAIL;
    }
    if ($S_URL != '' and $S_EMAIL != '') {
        $mailinfos .= ' - ';
    }
    if ($S_URL != '') {
        $mailinfos .= 'Site : '.$S_URL;
    }
}

if ($S_PHONE != '' or $S_FAX != '') {
    if ($S_PHONE != '') {
        $phoneinfos = 'Téléphone : '.$S_PHONE;
    }
    if ($S_FAX != '' and $S_PHONE != '') {
        $phoneinfos .= ' - ';
    }
    if ($S_FAX != '') {
        $phoneinfos .= 'Télécopie : '.$S_FAX;
    }
} else {
    $phoneinfos = '';
}

if ($S_ADDRESS_COMPLEMENT != '') {
    $C = $S_ADDRESS_COMPLEMENT."\n";
} else {
    $C = '';
}
$adr = $S_ADDRESS."\n".$C.$S_ZIP_CODE.' '.$S_CITY;
...
if (($mode == 13 or $mode == 19 or $mode == 20) and $evenement == 0) {
    ...
    if ($note > 0) {
        if ($pid != $id) {
            check_all(59);
            if (! check_rights($id, 59, get_section_of($pid))) {
                check_all(24);
            }
        }
    } elseif ($mode == 4) {
        if (is_chef_evenement($id, $evenement)) {
            $allowed = true;
        } elseif (check_rights($id, 15, $section)) {
            $allowed = true;
        } elseif (is_present($id, $evenement)) {
            $allowed = true;
        } else {
            check_all(15);
    ...
    elseif (! is_chef_evenement($id, $evenement)) {
        if ($mode == 11 and (is_operateur_pc($id, $evenement))) {
            check_all(0);
        } elseif ($mode == 16 or $mode == 17) {  // voir fiche victime tout le monde peut
            check_all(0);
        } else {
            check_all(15);
            if ((! check_rights($id, 15, "$section"))) {
...
if ($mode == 15) {
    ...
    while ($data = mysqli_fetch_array($result)) {
        ...
        if ($data['TP_ID'] > 0) {
            $fonction = ' - '.$data['TP_LIBELLE'];
        } else {
            $fonction = '';
        }
        if ($data['E_CODE'] != $evenement) {
            $renfort = ' (renfort '.$data['S_CODE'].')';
        } else {
            $renfort = '';
        }
        if ($data['C_NAME'] != '' and intval($data['C_ID']) > 0) {
            $c = ' ('.$data['C_NAME'].')';
        } else {
            $c = '';
        }
    ...
    if ($S_IMAGE_SIGNATURE != '' and $signed == 1) {
        ...
        if (@is_file($signature_file)) {
            $pdf->Image($signature_file, 100, $y + 6, 40);
        }
    }
...
if ($mode == 25) {
    if ($E_ALLOW_REINFORCEMENT == 1) {
        $title = 'Demande de renfort au titre de la Solidarité Nationale';
    } else {
    ...
    if ($id_responsable_evt > 0) {
        $_chef = $responsable_evt;
    } else {
        $_chef = $section_president_prefix.' '.$section_president;
    }
    ...
    if ($E_ALLOW_REINFORCEMENT == 1) {
        $txt = "Dans le cadre d'une demande de renfort, ".$txt;
    } else {
        $txt = "Dans le cadre de l'événement  \"".$description.'", '.$txt;
    }
    ...
    if ($E_ALLOW_REINFORCEMENT == 1 or $evt_principal > 0) {
        $t = ', au titre de la mutualisation des compétences et moyens opérationnels des '.$levels[3].'s ';
    } else {
        $t = ' ';
    }
    ...
    if ($E_ALLOW_REINFORCEMENT == 1 or $evt_principal > 0) {
        ...
        if ($evt_principal > 0) {
            if ($nbsessions == 1) {
                $t = 'le '.$EH_DATE_DEBUT[1];
            } else {
                $t = 'du '.$EH_DATE_DEBUT[1].' au '.$EH_DATE_FIN[$nbsessions];
            }
        } else {
            $t = 'du . . . . . . . . . . . . . .   au . . . . . . . . . . . . ';
        }
        ...
        $pdf->SetXY(25, $y + 35);
        if ($evt_principal > 0) {
            $t = $EH_DATE_DEBUT[1].' à '.$EH_DEBUT[1];
        } else {
            $t = '. . . . . . . . . . . . . ';
        }
    ...
    $pdf->MultiCell(100, 5, $printed_by, '', 'L');
    if ($E_ALLOW_REINFORCEMENT == 1) {
        $name = 'Demande_de_renfort.pdf';
    } else {
        $name = 'Demande_de_personnels_et_de_moyens.pdf';
    }
    $pdf->Output($name, 'I');
}
...
if ($mode == 4) {
    ...
    while ($data = mysqli_fetch_array($result)) {
        if ($data['E_PARTIES'] == 1) {
            $horaires = get_horaires_personne($data['P_ID'], $evts);
        } else {
            $horaires = '';
        }
        $nom_prenom = strtoupper($data['P_NOM']).' '.my_ucfirst($data['P_PRENOM']);
        ...
        if ($data['P_PHONE'] != '' and intval($data['P_HIDE']) == 0) {
            $phone = ' - '.phone_display_format($data['P_PHONE']);
        } else {
            $phone = '';
        }
        if ($data['E_CODE'] != $evenement) {
    ...
    if ($tofile == 1) {
        $evt_dir = $filesdir.'/files/'.$evenement.'/';
        @mkdir($filesdir.'/files/', 0755);
        @mkdir($evt_dir, 0755);
        $evt_file = $evt_dir.'/Ordre_de_mission_'.$evenement.'.pdf';
        if (@is_file($evt_file)) {
            unlink($evt_file);
        }
        $pdf->Output($evt_file, 'F');
    } else {
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        if ($cs[$i] == 'setxy') {
            ...
            if ($S_IMAGE_SIGNATURE != '' and $signed == 1) {
                ...
                [$x, $y] = explode(',', $ct[$i]);
                if (@is_file($signature_file)) {
                    $pdf->Image($signature_file, $x, $y, 40);
                }
            }
        } elseif ($cs[$i] == 'chapter') {
            $pdf->SetFont('Arial', '', 9);
...
elseif ($mode == 3) {
    ...
    if ($lieu != '' and strlen($libelle) < 60) {
        $libelle .= ' ('.substr($lieu, 0, 75 - strlen($libelle)).'.)';
    }
    ...
    $pdf->SetXY(5, 54);
    if ($cid > 0) {
        $pdf->Text(5, 55, 'pour le compte de : '.$company);
    }
...
elseif ($mode == 1) {
    ...
    if ($nbsessions > 1) {
        $nb_cours = $nbsessions;
    } elseif ($E_DUREE_TOTALE > 4) {
        $nb_cours = 2;
    } else {
        $nb_cours = 1;
    }
    if ($nb_cours > 8) {
        $largeur = 180 / $nb_cours;
    } else {
        $largeur = 150 / $nb_cours;
    }
    $hauteur = 10;
    $smallhauteur = 8;
    ...
    while ($data = mysqli_fetch_array($result) or ($i < 12)) {
        if ($i % 12 == 0) { // nouvelle page
            ...
            if ($cid > 0) {
                $pdf->Text(55, 26, 'pour le compte de : '.$company);
            }
            $pdf->SetXY(0, 10);
            $pdf->SetFont('Arial', '', 10);
            ...
            if ($te_code == 'FOR') {
                $pdf->MultiCell(140, 5, $formateurs, 0, 'L', 0);
            }
            $pdf->SetDrawColor(0, 0, 0);
            $pdf->SetTextColor(0, 0, 0);
            ...
            for ($k = 1; $k != $nb_cours + 1; $k++) {
                ...
                if ($nbsessions == 1 and $EH_DATE_DEBUT[1] == $EH_DATE_FIN[1]) {
                    $day = $EH_DATE_DEBUT[1];
                    if ($nb_cours == 2 and $k == 1) {
                        $day = $EH_DATE_DEBUT[1].' - matin';
                    }
                    if ($nb_cours == 2 and $k == 2) {
                        $day = $EH_DATE_DEBUT[1].' - après-midi';
                    }
                } elseif ($k == 1 or $nbsessions > 1) {
                    if ($EH_DATE_DEBUT[$k] == $EH_DATE_FIN[$k]) {
                        $day = $EH_DATE_DEBUT[$k];
                    } else {
                        $day = $EH_DATE_DEBUT[$k];
                    }
        ...
        for ($k = 1; $k != $nb_cours + 1; $k++) {
            ...
            if ($pid != '') {
                ...
                if (mysqli_num_rows($result2) == 0) {
                    $abs = '---';
                } elseif ($data2['EP_ABSENT'] == 1) {
                    $abs = 'Absent';
                } elseif ($data2['EP_ABSENT'] == 0) {
                    $abs = '';
                } else {
                    $abs = '---';
                }
...
elseif ($mode == 8) {
    ...
    if ($level < 3) {
        $special_template = $basedir.'/images/user-specific/documents/template_SST.pdf';
    }
    $pdf = new PDFEB;
    $pdf->SetCreator('EBrigade - FPDF');
    $pdf->SetAuthor("$cisname");
    ...
    $query = 'SELECT distinct e.E_CODE, s.S_CODE, p.P_ID, p.P_NOM, p.P_PRENOM, c.C_NAME
             ...
             and s.S_ID = e.S_ID
             and e.E_CODE = ep.E_CODE';
    if ($te_code != 'REU' and $te_code != 'CER' and $CEV_CODE != 'C_STA') {
        $query .= ' and ep.TP_ID = 0 ';
    }
    $query .= ' and ( e.E_CODE='.$evenement.' or e.E_PARENT='.$evenement.')
             order by p.P_NOM, p.P_PRENOM asc';
    ...
    while ($data = mysqli_fetch_array($result)) {
        if ($i % 12 == 0) { // nouvelle page
            ...
            for ($k = 1; $k != $nb_cours + 1; $k++) {
                ...
                $pdf->SetXY($pos, $y);
                if (isset($EH_DEBUT[$k])) {
                    $horaires = $EH_DEBUT[$k].' à '.$EH_FIN[$k];
                } else {
                    $horaires = '';
                }
...
elseif ($mode == 5) {
    ...
    if (! $empty) {
        ...
        while ($data = mysqli_fetch_array($result)) {
            ...
            if ($i % 12 == 0) { // nouvelle page
                ...
                if ($TF_CODE == 'I') {
                    $type_formation = 'une formation initiale';
                } elseif ($TF_CODE != '') {
                    $type_formation = 'une '.$TF_LIBELLE;
                }
                $txt = 'à '.$type_formation;
                if (substr($type, 0, 3) == 'PSE') {
                    $txt .= ' aux premiers secours en équipe ('.$type.')';
                }
                $pdf->Text(30, 21, 'Suite '.$txt.',');
                $pdf->Text(30, 25, "qui s'est déroulée ".$periode);
                $pdf->Text(30, 29, 'à '.$lieu);
                if ($cid > 0) {
                    $pdf->Text(30, 33, 'pour le compte de : '.$company);
                }
                $pdf->SetXY(0, 10);
                $pdf->SetFont('Arial', '', 10);
                $pdf->MultiCell(0, 4, $adr."\n".$phoneinfos."\n".$mailinfos, 0, 'R', 0);
    ...
    if ($S_IMAGE_SIGNATURE != '' and $signed == 1) {
        $signature_file = 'images/user-specific/'.$S_IMAGE_SIGNATURE;
        if (@is_file($signature_file)) {
            $pdf->Image($signature_file, 220, $y + 8, 40);
        }
    }
    $pdf->SetXY(10, 200);
    $pdf->SetFont('Arial', '', 6);
...
if ($mode == 7 or $mode == 18) {
    ...
    if ($mode == 7) {
        $t = 'Facture de formation';
    } else {
        $t = 'Convocation formation';
    }
    $pdf = new PDFEB;
    $pdf->AliasNbPages();
    $pdf->SetCreator($organisateur);
    ...
    $query = 'SELECT distinct p.P_ID, p.P_SEXE, p.P_NOM, p.P_PRENOM, ep.EP_TARIF, e.E_TARIF , ep.EP_PAID,
             ...
             and ( e.E_CODE='.$evenement.' or e.E_PARENT='.$evenement.')';
    if ($mode == 7) {
        $query .= " and ( ep.TP_ID = 0 or e.TE_CODE <> 'FOR' )";
    }

    if ($pid > 0) {
        $query .= ' and p.P_ID = '.$pid;
    }
    ...
    while (custom_fetch_array($result)) {
        ...
        if ($te_code == 'FOR') {
            if ($TF_CODE == 'P') {
                $type_evenement = $TF_LIBELLE;
            } elseif ($TF_CODE == 'I') {
                $type_evenement = 'formation initiale';
            } elseif ($TF_CODE != '') {
                $type_evenement = $TF_LIBELLE;
            }
        }
        ...
        if (strlen($P_ADDRESS) > 0) {
            ...
            $nbret = count_returns($P_ADDRESS);
            if ($nbret == 0 and strlen($P_ADDRESS) > 35) {
                $nbret = 1;
            }
            $y = $y + 6 * $nbret;
        }
        ...
        if ($E_ADDRESS != '') {
            ...
            if (strlen($E_ADDRESS) > 180) {
                $y = $y + 12;
            } elseif (strlen($E_ADDRESS) > 80) {
                $y = $y + 6;
            }
        }
        ...
        if ($mode == 7) {
            ...
            if ($EP_PAID == 1) {
                $txt = $facture4;
                if (intval($MODE_PAIEMENT) > 0) {
                    $txt .= ' par '.$TP_DESCRIPTION;
                }
                if ($NUM_CHEQUE != '') {
                    $txt .= ', numéro de chèque: '.$NUM_CHEQUE;
                }
                $txt .= '.';
            } else {
                $txt = $facture3;
        ...
        if ($S_IMAGE_SIGNATURE != '' and $signed == 1) {
            $signature_file = 'images/user-specific/'.$S_IMAGE_SIGNATURE;
            if (@is_file($signature_file)) {
                $pdf->Image($signature_file, 110, $y + 60, 40);
            }
        }
...
if ($mode == 19) {
    ...
    if ($S_IMAGE_SIGNATURE != '') {
        $signature_file = 'images/user-specific/'.$S_IMAGE_SIGNATURE;
        if (@is_file($signature_file)) {
            $pdf->Image($signature_file, 100, $y + 79, 40);
        }
    }
...
if ($mode == 11) {
    ...
    if ($te_code == 'REU' or $te_code == 'WEB' or $CEV_CODE == 'C_STA') {
        $pdf->MultiCell(150, 12, 'Compte rendu de réunion', '1', 'C');
    } else {
        $pdf->MultiCell(150, 12, 'Rapport: '.$type_evenement, '1', 'C');
    }
    ...
    if ($te_code == 'REU' or $te_code == 'WEB' or $CEV_CODE == 'C_STA') {
        ...
        if ($show_nombres == 1) {
            ...
            if ($mode == 12 or $mode == 14) {
                $msg = get_messages_stats($evenement, $important = true);
            } else {
                $msg = get_messages_stats($evenement, $important = false);
            }
            $pdf->Image('./images/bullet_black_small.png', 15, $y, 4);
    ...
    if ($show_cav == 1) {
        ...
        if (@mysqli_num_rows($result) > 0) {
            ...
            while (custom_fetch_array($result)) {
                ...
                if ($HEURE_SORTIE != '') {
                    $txt .= ', sortie à '.$HEURE_SORTIE;
                }
                $txt .= ' - Victime: ';
                if ($VI_NUMEROTATION != '') {
                    $txt .= 'V'.$VI_NUMEROTATION.', ';
                }
    ...
    WHERE tel.TEL_CODE = e.TEL_CODE
    AND e.E_CODE = evenement.E_CODE
    AND evenement.E_PARENT = $evenement
    AND e.EL_IMPRIMER=1
    AND e.EL_IMPORTANT = 1";
    ...
    if (@mysqli_num_rows($result) > 0) {
        ...
        while (custom_fetch_array($result)) {
            ...
            if ($TEL_CODE == 'I') {
                if ($EL_IMPORTANT == 1) {
                    $img = 'intervention_important.png';
                } else {
                    $img = 'intervention.png';
                }
            } elseif ($EL_IMPORTANT == 1) {
                $img = 'important.png';
            } else {
                $img = '';
            }
            ...
            if ($te_code == 'REU' or $te_code == 'WEB' or $CEV_CODE == 'C_STA') {
                ...
                $pdf->SetFont('Arial', 'B', 11);
                if ($E_CODE != $evenement) {
                    $pdf->MultiCell(180, 6, "Renfort ($E_LIBELLE) ".$HEURE_DEBUT.': '.$TEL_DESCRIPTION.' '.$fromto.' '.$EL_TITLE, '', 'L');
                } else {
                    $pdf->MultiCell(180, 6, $HEURE_DEBUT.': '.$TEL_DESCRIPTION.' '.$fromto.' '.$EL_TITLE, '', 'L');
                }

                if (strlen($fromto.' '.$EL_TITLE) > 75) {
                    $y = $y + 7;
                } else {
                    $y = $y + 2;
                }
            ...
            if ($nbequipes > 0) {
                if ($nbequipes == 1) {
                    $detail .= 'Equipe engagée: ';
                } else {
                    $detail .= 'Equipes engagées: ';
                }
                $detail .= rtrim($eqnames, ',').'. ';
            }

            if ($EL_RESPONSABLE != '') {
                $detail .= 'Responsable: '.$P_PRENOM.' '.$P_NOM.'. ';
            }
            if ($HEURE_SLL != '') {
                $detail .= 'Arrivée sur les lieux: '.$HEURE_SLL.'. ';
            }
            if ($HEURE_FIN != '') {
                $detail .= 'Fin intervention: '.$HEURE_FIN.'. ';
            }
            if ($detail != '') {
                $pdf->SetFont('Arial', '', 10);
                $y = $y + 6;
            ...
            $pdf->SetFont('Arial', '', 9);
            $y = $pdf->WriteBigTextBlock($EL_COMMENTAIRE, 28, $y);
...
elseif ($mode == 13) {
    ...
    if ($NF_FRAIS_DEP == 1) {
        // specific
        if ($cisname == 'FA/SPP-PATS' and $S_ID > 1) {
            $BENEFICIAIRE = 'SA/SPP-PATS '.substr($S_CODE, 0, 2);
        } else {
            $BENEFICIAIRE = $S_CODE;
        }
        if ($S_DESCRIPTION != '') {
            $BENEFICIAIRE .= ' -'.$S_DESCRIPTION;
        }
        $BENEFICIAIRE .= ' (frais pour '.my_ucfirst($P_PRENOM).' '.strtoupper($P_NOM).')';
    } else {
        $BENEFICIAIRE = my_ucfirst($P_PRENOM).' '.strtoupper($P_NOM).' ('.$S_CODE.' - '.$S_DESCRIPTION.')';
    }
    ...
    while (custom_fetch_array($result)) {
        if ($AMOUNT > 0) {
            $AMOUNT = my_number_format($AMOUNT).' '.$default_money_symbol;
        }
        ...
        $pdf->SetXY(110, $y);
        if (strlen($TF_DESCRIPTION) > 20) {
            $pdf->SetFont('Arial', '', 8);
        }
        $pdf->MultiCell(50, $hauteur, $TF_DESCRIPTION, 1, 'C', true);
        $pdf->SetFont('Arial', '', 10);
        $pdf->SetXY(160, $y);
        if (strlen($NFD_DESCRIPTION) > 30) {
            $pdf->SetFont('Arial', '', 8);
        }
    ...
    if (@is_file($signature_file1)) {
        $pdf->Image($signature_file1, 15, $y + 1, 40);
    } else {
        $pdf->Text(15, $y + 4, "Pas d'image pour la signature");
    }
    $pdf->SetXY(78, $y);
    $pdf->MultiCell(68, $hauteur, '', 1, 'L', true);
    if ($valid != '') {
        if (@is_file($signature_file2)) {
            $pdf->Image($signature_file2, 82, $y + 1, 40);
        } else {
            $pdf->Text(82, $y + 4, "Pas d'image pour la signature");
        }
    }
    $pdf->SetXY(146, $y);
    $pdf->MultiCell(67, $hauteur, '', 1, 'L', true);
    if ($valid2 != '') {
        if (@is_file($signature_file3)) {
            $pdf->Image($signature_file3, 150, $y + 1, 40);
        } else {
            $pdf->Text(150, $y + 4, "Pas d'image pour la signature");
        }
    }
    $pdf->SetXY(213, $y);
    $pdf->MultiCell(67, $hauteur, '', 1, 'L', true);
    if ($rembourse != '') {
        if (@is_file($signature_file4)) {
            $pdf->Image($signature_file4, 216, $y + 1, 40);
        } else {
            $pdf->Text(216, $y + 4, "Pas d'image pour la signature");
        }
    }
    $pdf->SetXY(10, 200);
    $pdf->SetFont('Arial', '', 6);
    ...
    if (is_dir($mypath)) {
        ...
        while ($file = readdir($dir)) {
            ...
            if ($ext == 'JPG' or $ext == 'PNG') {
                if ($ext == 'PNG') {
                    $f = fopen($mypath.'/'.$file, 'rb');
                    $res = fread($f, 1); // read 1 byte from stream
                    $bpc = ord($res);
                    // test 16-bit depth
                    if ($bpc > 8) {
                        continue;
...
elseif ($mode == 17 or $mode == 16) {
    ...
    if ($mode == 17) {
        ...
        while (custom_fetch_array($result2)) {
            $BVP_VALUE = str_replace(["\n", "\t", "\r"], '', $BVP_VALUE);
            $BVP_TEXT = str_replace(["\n", "\t", "\r"], '', $BVP_TEXT);

            // cas particuliers des traumatismes 2 et 3
            if ($BVP_ID == 531 or $BVP_ID == 535) {
                $bi = 500;
            } elseif ($BVP_ID == 532 or $BVP_ID == 536) {
                $bi = 510;
            } elseif ($BVP_ID == 533 or $BVP_ID == 537) {
                $bi = 520;
            } elseif ($BVP_ID == 534 or $BVP_ID == 538) {
                $bi = 530;
            } else {
                $bi = $BVP_ID;
            }
            ...
            if (substr($BVP_TITLE, 0, 11) == 'Traumatisme') {
                $pdf->SetFont('Arial', 'B', 9);
            } else {
                $pdf->SetFont('Arial', '', 9);
            }
