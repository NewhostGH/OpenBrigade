...
if (isset($_GET['rejet_id'])) {
    $rejet_id = intval($_GET['rejet_id']);
} elseif (isset($_POST['rejet_id'])) {
    $rejet_id = intval($_POST['rejet_id']);
} else {
    $rejet_id = -1;
}
...
} elseif (isset($_POST['paiement_id'])) {
...
if (isset($_GET['rembourse'])) {
    $rembourse = intval($_GET['rembourse']);
} elseif (isset($_POST['rembourse'])) {
    $rembourse = intval($_POST['rembourse']);
} else {
    $rembourse = 0;
}
    ...
    if ($action == 'insert') {
        ...
        if (mysqli_affected_rows($dbc) == 1 and $MONTANT_REGUL > 0) {
            if ($REPRESENTER == 1 and $REGULARISE == 0) { // incrementer somme à représenter
                $query = 'update pompier set MONTANT_REGUL = ROUND( MONTANT_REGUL + '.$MONTANT_REGUL.' , 2 ) where P_ID='.$pid;
                $result = mysqli_query($dbc, $query);
            } elseif ($REGULARISE == 1) { // réduire somme à représenter
                $query = 'update pompier set MONTANT_REGUL = ROUND( MONTANT_REGUL - '.$MONTANT_REGUL.' , 2 ) where P_ID='.$pid;
                $result = mysqli_query($dbc, $query);
...
if (isset($_POST['paiement_id']) and ($action == 'update' or $action == 'insert')) {
    ...
    if (isset($_POST['compte_a_debiter'])) {
        $compte_a_debiter = intval($_POST['compte_a_debiter']);
    } else {
        $compte_a_debiter = 0;
    }
    if ($compte_a_debiter == 0) {
        $compte_a_debiter = 'null';
    }
    $etablissement = 'null';
    $guichet = 'null';
    $compte = 'null';
...
if ($rejet_id >= 0) {
    ...
    if ($MONTANT_REJET == '') {
        ...
        $DEFMONTANT_REJET = $MONTANT_REJET;
    }
    ...
    if ($REPRESENTER == 1 or ($bank_accounts == 1 and $TP_ID == 1)) {
     ...
     <td><label class='switch'>
        ...
        echo "<input type='hidden' name='representer' id='representer' value='0'>";
    }
    ...
    if ($bank_accounts == 0) {
        $query2 .= ' where ( TR_ID < 3 or TR_ID ='.$REGUL_ID.')';
    }
...
<input type='button' class='btn btn-secondary' value='Retour' onclick='goback();'>
...
} elseif ($paiement_id >= 0) {
    // =====================================================================
    // Editer Paiement
    // =====================================================================

    if ($rembourse == 1) {
        $img = "<i class='fa fa-money fa-3x' style='color:black;'></i> ";
    } else {
        $img = "<i class='fa fa-euro-signfa-3x' style='color:blue;'></i> ";
    }
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if ($PERIODE_CODE == 'A' and ($YEAR_ENGAGEMENT == $ANNEE or $YEAR_FIN == $ANNEE)) {
            ...
            if ($MONTH_ENGAGEMENT != '') {
                $number_months_to_pay = $number_months_to_pay - $MONTH_ENGAGEMENT + 1;
            }
            if ($MONTH_FIN != '') {
                $number_months_to_pay = $number_months_to_pay - (12 - $MONTH_FIN);
            }
    ...
    if ($COMMENTAIRE == '' and $number_months_to_pay != '' and $bank_accounts == 1 and $REMBOURSEMENT == 0) {
        $COMMENTAIRE = $number_months_to_pay.' mois, soit '.$MONTANT.$default_money_symbol;
    }
    ...
    if ($REMBOURSEMENT == 0) {
        $t2 = "Paiement ou prélèvement pour $P_PRENOM $P_NOM";
    } else {
        $t2 = "$t pour $P_PRENOM $P_NOM";
    }
    echo "<form action=cotisation_edit.php method=POST>
    ...
    if ($REMBOURSEMENT == 0) {
        ...
        if ($action == 'insert') {
            echo "<select class='form-control select-control maxsize' name='periode' id='periode' onchange=\"javascript:changedPeriode('".$pid."');\">";
        } else {
    ...
    if ($REMBOURSEMENT == 1 and $TP_ID != 1) {
        $query2 .= ' where ( TP_ID <> 1 )';
    } // pas de prélèvement
    $result2 = mysqli_query($dbc, $query2);
    ...
    if (($TP_ID == '1' or $TP_ID == '2') and $bank_accounts) {
        $style = '';
    } else {
        $style = "style='display:none'";
    }
    ...
    if ($REMBOURSEMENT == 1) {
        ...
        while ($row2 = @mysqli_fetch_array($result2)) {
            ...
            if ($_CB_ID <= 1) {
                ...
                $_S_DESCRIPTION = '- '.$row2['S_DESCRIPTION'];
            }
            if ($COMPTE_DEBITE == $_CB_ID) {
                $selected = 'selected';
            } else {
                $selected = '';
            }
