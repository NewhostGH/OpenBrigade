...
<?php
$possibleorders = ['TE_CODE', 'TE_LIBELLE', 'CEV_DESCRIPTION', 'TE_MAIN_COURANTE', 'TE_VICTIMES', 'TE_MULTI_DUPLI', 'ACCES_RESTREINT',
...
if ($order == 'TE_VICTIMES' or $order == 'TE_MULTI_DUPLI' or $order == 'TE_MAIN_COURANTE' or $order == 'ACCES_RESTREINT'
    ...
    or $order == 'COLONNE_RENFORT' or $order == 'TE_BILAN' or $order == 'REMPLACEMENT' or $order == 'TE_DOCUMENT') {
    $query .= ' desc';
}
...
if ($number > 0) {
    ...
    while (custom_fetch_array($result)) {
        ...
        if ($TE_MAIN_COURANTE == 1) {
            $TE_MAIN_COURANTE = "<i class='fa fa-check' title=\"Il est possible d'écrire une main courante pour ce type d'activité\"></i>";
        } else {
            $TE_MAIN_COURANTE = '';
        }

        if ($TE_VICTIMES == 1) {
            $TE_VICTIMES = "<i class='fa fa-check'  title=\"Il est possible d'enregistrer des victimes sur ce type d'activités\"></i>";
        } else {
            $TE_VICTIMES = '';
        }

        if ($TE_MULTI_DUPLI == 1) {
            $TE_MULTI_DUPLI = "<i class='fa fa-check'  title=\"Il est possible de faire des duplications multiples pour ce type d'activité\"></i>";
        } else {
            $TE_MULTI_DUPLI = '';
        }

        if ($ACCES_RESTREINT == 1) {
            $ACCES_RESTREINT = "<i class='fa fa-check'  title=\"Les activités de ce type ne sont visibles que par les inscrits et les responsables\"></i>";
        } else {
            $ACCES_RESTREINT = '';
        }

        if ($TE_PERSONNEL == 1) {
            $TE_PERSONNEL = "<i class='fa fa-check'  title=\"On peut inscrire du personnel sur ce type d'activité\"></i>";
        } else {
            $TE_PERSONNEL = '';
        }

        if ($TE_VEHICULES == 1) {
            $TE_VEHICULES = "<i class='fa fa-check'  title=\"Les véhicules peuvent être engagés sur ce type d'activités\"></i>";
        } else {
            $TE_VEHICULES = '';
        }

        if ($TE_MATERIEL == 1) {
            $TE_MATERIEL = "<i class='fa fa-check'  title=\"Du matériel peut êtree engagé sur ce type d'activités\"></i>";
        } else {
            $TE_MATERIEL = '';
        }

        if ($TE_CONSOMMABLES == 1) {
            $TE_CONSOMMABLES = "<i class='fa fa-check'  title=\"Des consommations de produits peuvent être enregistrées sur ce type d'activité\"></i>";
        } else {
            $TE_CONSOMMABLES = '';
        }

        if ($COLONNE_RENFORT == 1) {
            $COLONNE_RENFORT = "<i class='fa fa-check'  title=\"Les activités de ce type peuvent avoir la propriété colonne de renfort activée.\"></i>";
        } else {
            $COLONNE_RENFORT = '';
        }

        if ($TE_BILAN == 1) {
            $TE_BILAN = "<i class='fa fa-check'  title=\"Les activités de ce typesont prise en compte dans les bilans PDFs annuels.\"></i>";
        } else {
            $TE_BILAN = '';
        }

        if ($REMPLACEMENT == 1) {
            $REMPLACEMENT = "<i class='fa fa-check'  title=\"Les remplacements de personnel sont possibles sur ce type d'activité.\"></i>";
        } else {
            $REMPLACEMENT = '';
        }

        if ($TE_DOCUMENT == 1) {
            $TE_DOCUMENT = "<i class='fa fa-check'  title=\"Des documents peuvent être attachés sur ce type d'activité\"></i>";
        } else {
            $TE_DOCUMENT = '';
        }
