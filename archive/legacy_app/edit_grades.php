...
if (isset($_GET['upd'])) {
    $upd = secure_input($dbc, $_GET['upd']);
} else {
    $upd = '';
}
if (isset($_GET['activ'])) {
    $activ = secure_input($dbc, $_GET['activ']);
} else {
    $activ = 2;
}
if (isset($_GET['error'])) {
    $error = secure_input($dbc, $_GET['error']);
} else {
    $error = '';
}
...
if (check_rights($_SESSION['id'], 18)) {
    ...
    // =====================================================================
    if ($catGrade == 'ALL') {
        echo "<div class='dropdown-right' style='float:right' align=right > <a class='btn btn-secondary disabled' value='Modifier' name='modifierCategorie' ><i class=\"fas fa-edit \"></i> Catégorie</a></div>";
    } else {
        echo "<div class='dropdown-right' style='float:right' align=right> <a class='btn btn-primary' value='Modifier' name='modifierCategorie' onclick=\"bouton_redirect('parametrage.php?tab=5&child=14&ope=edit_cat&updCat=1');\"><i class=\"fas fa-edit \"></i> Catégorie</a></div>";
    }
