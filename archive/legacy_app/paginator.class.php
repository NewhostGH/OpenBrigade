...
class Paginator
{
    ...
    public function __construct()
    {
        ...
        } elseif (! empty($_SESSION['ipp'])) {
            $this->items_per_page = $_SESSION['ipp'];
        } else {
            $this->items_per_page = $this->default_ipp;
        }
    }

    public function paginate($add_get = '')
    {
    ...
    public function display_items_per_page($add_get = '')
    {
        $items = '';
        if (! empty($_GET['ipp'])) {
            $this->items_per_page = $_GET['ipp'];
        ...
        $ipp_array = [10, 20, 30, 50, 80, 100, 'Toutes'];
    ...
    public function display_jump_menu($add_get = '')
    {
        $option = '';
        for ($i = 1; $i <= $this->num_pages; $i++) {
            if ($i == $this->current_page) {
