...
if ($chef) {
    $granted_event = true;
} elseif (check_rights($id, 15, $organisateur)) {
    $granted_event = true;
} elseif ($gardesp and check_rights($id, 6) and $nbsections > 0) {
    $granted_event = true;
} elseif ($gardesp and check_rights($id, 6, $organisateur) and $nbsections == 0) {
    $granted_event = true;
} else {
    $granted_event = false;
}
...
if ($pid > 0) {
    ...
    $title = '';
    if ($TV_ICON != '') {
        $title .= '<img src='.$TV_ICON." style='max-width:50px;' > ";
    }
    $title .= $TV_CODE;
    if ($V_INDICATIF != '') {
        $title .= ' - '.$V_INDICATIF;
    ...
    $title .= $label;
} elseif ($action == 'colonne') {
    $title = 'Rattacher un refort à la colonne';
} elseif ($action == 'facturation') {
...
elseif ($action == 'fonction') {
    ...
    if ($gardes) {
        $queryf .= ' and tp.EQ_ID in (0,'.intval($E_EQUIPE).')';
    }
    ...
    while ($rowf = @mysqli_fetch_array($resultf)) {
        array_push($fonctions, [$rowf['TP_ID'], $rowf['TP_LIBELLE'], $rowf['TYPE'], $rowf['TYPE2']]);
    }
...
elseif ($action == 'equipe' or $action == 'vequipe' or $action == 'mequipe') {
    if ($action == 'mequipe') {
        $ident = $mid;
        $cat = 'M';
    } elseif ($action == 'vequipe') {
        $ident = $vid;
        $cat = 'V';
    } else {
        $ident = $pid;
    ...
    if (intval($principal) > 0) {
        $evts_list = $evenement.','.$principal;
    } else {
        $evts_list = $evenement;
    }
...
elseif ($action == 'cnombre') {
    ...
    if ($STOCK_ID > 0) {
        $out .= "    <i>stock restant $C_NOMBRE</i> ";
    }
...
elseif ($action == 'facturation') {
    ...
    if ($evtKm != '') {
        $out .= "<tr><td><b>Kilomètres parcourus</td><td> $evtKm </td></tr>";
    }
    $out .= '</table><br>';
}
