...
if ($child == 10 and $tab == 5) {
    check_all(5);
} elseif (! check_rights($id, 18) and ! check_rights($id, 54) and ! check_rights($id, 29)) {
    check_all(18);
}
...
if ($tab == 4) {
    ...
    if ($client == 1) {
        $where = 'S_ID='.$filter;
        if ($type_element != 'ALL') {
            $where .= " and TEF_CODE='".$type_element."'";
        }
...
if ($tab == 5) {
    ...
    if (check_rights($id, 18) and $grades) {
        ...
        if ($catGrade != 'ALL') {
            ...
            $query = 'select count(1) from grade WHERE G_CATEGORY = G_CATEGORY ';
        }
        if ($activ == 0 || $activ == 1) {
            $query .= "\nand G_FLAG='".$activ."'";
        }
...
$rights = [18, 18, 18, 18, 18, 18, 18, 18, 18, 5, 54, 29, 29, 18];
