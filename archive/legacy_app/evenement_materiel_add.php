...
if (isset($_GET['MA_ID'])) {
    $materiel = intval($_GET['MA_ID']);
} else {
    $materiel = intval($_POST['MA_ID']);
}
if (isset($_POST['nb'])) {
    $nb = intval($_POST['nb']);
} else {
...
if (isset($_GET['filtermateriel'])) {
    $filtermateriel = $_GET['filtermateriel'];
} else {
    $filtermateriel = 'ALL';
}
...
if (! is_chef_evenement($id, $evenement)) {
    ...
    if (($E_OPEN_TO_EXT == 0) and (! check_rights($id, 70, "$S_ID")) and (! check_rights($id, 15, "$S_ID"))) {
        check_all(24);
    }
}
