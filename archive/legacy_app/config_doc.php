...
if ($TF_CODE == 'R') {
    ...
    if ($type == 'PSC1') {
        ...
        $attestation_arretes = "
        • Vu l'arrété du 21 décembre 2020 portant organisation de la formation continue dans le domaine des premiers secours
...
if ($mode == 10) {
    if ($type == 'SST') {
        ...
        $lig2 = 'Je soussigné, '.$responsable_evt.',  Formateur en Sauvetage Secourisme du Travail de la '.$organisation_name.',';
        $lig3 = 'Certifie que :';
    ...
    if ($type == 'PRAP') {
        ...
        $lig2 = 'Je soussigné, '.$responsable_evt.",  Formateur en Prévention des Risques liés à l'Activité Physique de la ".$organisation_name.',';
        $lig3 = 'Certifie que :';
...
if ($mode == 26) {
    ...
    $cs[] = 'small';
    $ct[] = ' 1.1) Type de formation: '.$TF_LIBELLE.' '.$type;
    $cs[] = 'small';
    $ct[] = ' 1.2) Durée de la formation: '.$E_DUREE_TOTALE.' h';
    $cs[] = 'small';
    $ct[] = ' 1.3) Date de la formation: '.$dates_heures;
    $cs[] = 'small';
    $ct[] = ' 1.4) Lieu de la formation: '.$lieu;
    $cs[] = 'small';
    $ct[] = ' 1.5) Nombre de participants: '.$nb_stagiaires;
    $cs[] = 'small';
    $ct[] = ' 1.6) Noms des participants: ';
    $inscrits = get_noms_inscrits($evenement, $mode = 'stagiaires');
    $cs[] = 'chapter';
    $ct[] = $inscrits;
    $cs[] = 'small';
    $ct[] = ' 1.7) Nom des intervenants prévus: ';
    $formateurs = get_noms_inscrits($evenement, $mode = 'formateurs');
    $cs[] = 'chapter';
    $ct[] = $formateurs;
    $cs[] = 'small';
    $ct[] = ' 1.8) Nom et adresse de facturation: '.$company.' ( Adresse : '.$company_address.' '.$company_cp.' '.$company_city." )\n";
    $cs[] = 'title2';
    $ct[] = "\n2°) ".$company." s'engage à:";
    $cs[] = 'small';
    $ct[] = ' 2.1) Verser à '.$association.', après la réalisation de l’action de formation et à émission de sa facture, la somme de '.$E_TARIF.' '.$default_money_symbol.' par personne pour la prestation (exonéré de TVA) par mandat administratif.';
    $total_amount = floatval($E_TARIF) * intval($nb_stagiaires);
    $cs[] = 'small';
    $ct[] = ' 2.2)  Soit un total de '.$total_amount.' '.$default_money_symbol.' pour la session de '.$nb_stagiaires.' candidats au total.';
    $cs[] = 'small';
    $ct[] = "\n\n\nFait à ".$organisateur_city.', le '.date('d-m-Y');

    $cs[] = 'italic';
    $ct[] = 'Pour '.$section_prefix.' '.$section_affiche.",\n".$chef;
    if ($company == '') {
        $o = "l'organisateur";
    } else {
        $o = $company;
    ...
    $ct[] = '15,260';
    $cs[] = 'italic';
    $ct[] = 'Convention établie en trois exemplaires originaux, dont deux à retourner signés pour accord au siège de '.$association.'.';
}
...
if ($mode == 6) {
    ...
    $cs[] = 'normal';
    $ct[] = 'Deux exemplaires de la convention précisant les modalités de notre accord. Vous voudrez bien les compléter et nous retourner un exemplaire signé.';
    $cs[] = 'normal';
    $ct[] = "Dans l'attente, veuillez, Madame, Monsieur, accepter nos salutations les meilleures.";
    $cs[] = 'righti';
    ...
    $d = $description;
    if ($num_convention != '') {
        $d = $description.' - convention n°'.$num_convention;
    }
    $cs[] = 'title1';
    $ct[] = "Convention pour la mise en place d'un Dispositif Prévisionnel de Secours \n".$d;
    $cs[] = 'setxy';
    $ct[] = '15,70';
    $cs[] = 'title1';
    $ct[] = '1. Association Prestataire';
    $cs[] = 'normal';
    $ct[] = ' '.ucfirst($section_prefix).' '.$section_affiche.' '.$antenne_affiche.
    ...
    $cs[] = 'italic';
    $ct[] = 'Association ayant reçu notamment une autorisation d’exercice déconcentrée pour les missions de sécurité civile de type D (Certificat Original d’Affiliation en annexe) par sa régulière affiliation à la '.$organisation_name.' ('.$cisname."), association agréée de sécurité civile au plan national par arrêté ministériel.\n\n";
    $cs[] = 'title1';
    $ct[] = "2. Organisateur de l'évènement";
    $cs[] = 'normal';
    $ct[] = " Raison sociale de l'organisateur : ".$company.''.
    ...
    $cs[] = 'title1';
    $ct[] = '3. Objet de la convention';
    $cs[] = 'title2';
    $ct[] = '3.1 Objet';
    $cs[] = 'normal';
    $ct[] = 'La présente convention a pour but de fixer les modalités de fonctionnement entre :';
    $cs[] = 'italic';
    $ct[] = ucfirst($section_prefix).' '.rtrim($section_affiche).' '.rtrim($antenne_affiche).", qui peut régulièrement exercer, d'une manière déconcentrée les missions de Dispositifs prévisionnels de Secours.";
    $cs[] = 'normal';
    $ct[] = 'et';
    $cs[] = 'italic';
    $ct[] = ' '.$company;
    $cs[] = 'normal';
    $ct[] = 'pour la mise en place d’un Dispositif Prévisionnel de Secours, ceci afin de bien clarifier le cadre juridique de la prestation de service assurée.';
    $a = 'La mise en place du Dispositif Prévisionnel de Secours concerne';
    if ($is_acteurs > 0) {
    ...
    $ct[] = '15,'.$y_after_new_page;
    $cs[] = 'title2';
    $ct[] = "3.2 Descriptif de l'évènement";
    if (strlen(preg_replace('/\s+/', '', $custom_horaire)) > 0) {
        $periode_long = str_replace(["\n", "\r"], ' ', $custom_horaire);
    }

    $cs[] = 'normal';
    $ct[] = " Nom de l'évènement : ".$description.''."\n Date(s) : ".$periode_long.''."\n Lieu : ".$lieu."\n Adresse précise : ".$E_ADDRESS;
    $cs[] = 'title2';
    $ct[] = "3.3 Grille d'évaluation des risques";
    $cs[] = 'normal';
    $ct[] = "Cet évènement a fait l'objet par l'organisateur d'une évaluation des risques dont la grille figure en annexe de la présente convention.";
    $cs[] = 'title2';
    $ct[] = '3.4 Autorisations';
    $cs[] = 'normal';
    $ct[] = "L'organisateur reconnait posséder toutes les autorisations nécessaires au déroulement de la dite manifestation et avoir souscrit une assurance responsabilité civile organisateur.";
    $cs[] = 'title2';
    $ct[] = '3.5 Responsabilités';
    $cs[] = 'normal';
    $ct[] = "Conformément aux textes réglementaires, l'organisateur est responsable de l'ensemble de l'organisation et des mesures prises en liaison avec l'autorité de police compétente (maire, préfet).".
    ...
    $cs[] = 'bold';
    $ct[] = ' '.$type_evenement.' '.$cmt;
    $cs[] = 'title2';
    $ct[] = '4.2 : Composition du dispositif ';
    $cs[] = 'normal';
    $ct[] = "Nombre d'intervenants secouristes : ".intval($nb_is).''.
    "\nVéhicules de Premiers Secours : ".intval($nb_vpsp).''.
    ...
    $ct[] = '15,'.$y_after_new_page;
    $cs[] = 'title2';
    $ct[] = '4.3 : Informations concernant le dispositif ';
    $cs[] = 'title3';
    $ct[] = '4.3.1 : Les intervenants ';
    $cs[] = 'normal';
    $ct[] = "- Les équipiers secouristes sont titulaires du Diplôme de premiers Secours en équipe de niveau 2 (PSE2), validés dans leur aptitude opérationnelle conformément à la réglementation en vigueur et portés sur les listes d’aptitude opérationnelles.
- Les secouristes sont titulaires du Diplôme de premiers Secours en équipe de niveau 1 (PSE1), validés dans leur aptitude opérationnelle et portés sur les listes d’aptitudes opérationnelles.
...
- En fonction de la taille du DPS , un ou des chef(s) de  poste, chef(s) de section, chef(s) de secteur, chef de dispositifs, cadres opérationnels  (est  ou sont) désigné(s)  par l'association prestataire.";
    $cs[] = 'title3';
    $ct[] = '4.3.2 : Moyens matériels ';
    $cs[] = 'normal';
    $ct[] = '- Les  différents lots de matériels mis à disposition sont conformes au RNDPS du 7 novembre 2006.';
    $cs[] = 'normal';
    $ct[] = "- Les Véhicules de Premiers Secours à Personnes (V.P.S.P), utilisés  comme Postes de Secours Mobiles ou Fixes, sont  dotés d’une cellule de soins  adaptée et des matériels permettant d’assurer les premiers secours ainsi que le conditionnement d’une victime.\n";
    $cs[] = 'title2';
    $ct[] = '4.4 Missions ';
    $cs[] = 'normal';
    $ct[] = "Les moyens mis en place par l'association prestataire sont destinés à assurer une présence préventive pendant la manifestation faisant l'objet de cette convention :";
    $cs[] = 'bold';
    $ct[] = "Points d'alertes et de premiers secours :";
    $cs[] = 'normal';
    $ct[] = ' 1° Reconnaître et analyser la situation accidentelle,';
    $cs[] = 'normal';
    $ct[] = ' 2° Prendre les premières mesures adaptées de sécurité et de protection,';
    $cs[] = 'normal';
    $ct[] = ' 3° Alerter les secours publics,';
    $cs[] = 'normal';
    $ct[] = ' 4° Prodiguer à la victime des gestes de premiers secours réalisables à 2 intervenants,';
    $cs[] = 'normal';
    $ct[] = ' 5° Accueillir les secours et faciliter leur intervention.';
    $cs[] = 'bold';
    $ct[] = 'Poste de secours : ';
    $cs[] = 'normal';
    $ct[] = ' 1° Reconnaître et analyser la situation accidentelle,';
    ...
    $ct[] = ' 2° Prendre les premières mesures adaptées de sécurité et de protection,';
    $cs[] = 'normal';
    $ct[] = ' 3° Faire un bilan et porter les premiers secours nécessaires à une victime,';
    $cs[] = 'normal';
    $ct[] = ' 4° Prodiguer des conseils adaptés à une victime qui pourrait partir par ses propres moyens,';
    $cs[] = 'normal';
    $ct[] = " 5° Contribuer à la mise en place de la chaîne des secours allant de l'alerte jusqu'à la prise en charge de la victime par les pouvoirs publics,";
    $cs[] = 'normal';
    $ct[] = ' 6° Accueillir les secours et faciliter leur intervention';
    $cs[] = 'bold';
    $ct[] = 'Une équipe de secours peut prendre en charge : ';
    $cs[] = 'normal';
    $ct[] = "- Une seule victime atteinte d'une détresse vitale
- Un nombre de victimes sans gravités, équivalent à celui des intervenants qui la compose";
    $cs[] = 'addpage';
    $ct[] = '';
    $cs[] = 'setxy';
    $ct[] = '15,'.$y_after_new_page;
    $cs[] = 'title2';
    $ct[] = '4.5 Transport des victimes ';
    $cs[] = 'normal';
    if ($transport == 1) {
        $ct[] = "Les Véhicules de Premiers Secours à Personnes (V.P.S.P) de l'association prestataire, conventionnée au titre de l'article L725-4 du Code de la Sécurité Intérieure, peuvent, après accord du médecin régulateur du service d’aide médicale d’urgence et sous son autorité, participer en complément des secours publics, à l’acheminement des victimes vers une structure d’accueil, de soins ou un point relais.";
    } else {
    ...
    $cs[] = 'title2';
    $ct[] = '4.6 Modalités opérationnelles ';
    $cs[] = 'normal';
    $ct[] = '- Les intervenants  sont revêtus de leur tenue officielle.
- Ils interviennent sous la direction de l’encadrement mis en place par '.$section_prefix.' '.$section_affiche.' '.$antenne_affiche;
    if ($responsable_evt != '') {
        $cs[] = 'normal';
        $ct[] = "- L'association est représentée opérationnellement par ".$responsable_evt.', qui est joignable au: '.$responsable_evt_phone.", qui a procédé à la désignation du chef d'équipe (ou chef de poste, ou chef de section).";
    }
    $cs[] = 'normal';
    $ct[] = "- Le chef de poste prendra contact avec le bénéficiaire dès son arrivée sur site pour vérifier la concordance avec les clauses techniques de la convention, mettre en place le dispositif et déterminer les modalités opératoires liées à l’évènement.
- Les intervenants et véhicules sont dotés de moyens radio sur fréquence propres. Ces moyens peuvent constituer un réseau qui nécessite la mise en place de matériels spécifiques et la présence d’opérateurs radio.\n\n";
    $cs[] = 'title1';
    $ct[] = "5. Engagements de l'organisateur";
    $cs[] = 'title2';
    $ct[] = '5.1 Aspects logistiques';
    $cs[] = 'title3';
    $ct[] = '5.1.1 Locaux, matériels, moyens de communication';
    $cs[] = 'normal';
    ...
    $ct[] = '15,'.$y_after_new_page;
    $cs[] = 'title3';
    $ct[] = "5.1.2 Dispositf d'alerte des secours publics";
    $cs[] = 'normal';
    $ct[] = "L'organisateur s'engage à mettre à la disposition des équipes de secours, un moyen d'appel des secours publics.";
    $cs[] = 'title3';
    $ct[] = '5.1.3 Conditions de vie';
    if ($repas == 1) {
        $cs[] = 'normal';
        $ct[] = 'L’organisateur s’engage à fournir des repas ou paniers-repas équilibrés et boissons sans alcool en quantités suffisantes pour l’ensemble des intervenants.';
    } else {
        $cs[] = 'normal';
        $ct[] = "Les repas et les boissons des secouristes présents ne seront pas pris en charge par l'organisateur.";
    }

    $cs[] = 'title2';
    $ct[] = '5.2 Modalités opérationnelles';
    $cs[] = 'title3';
    $ct[] = "5.2.1 Correspondant de l'organisateur";
    if ($contact == '') {
        $contact = 'Une personne à désigner,';
    }
    $cs[] = 'normal';
    $ct[] = ''.$contact." membre de l'organisateur, sera interlocuteur de l'association prestataire le jour de la manifestation.";

    $cs[] = 'title3';
    $ct[] = '5.2.2 Chaîne de commandement du DPS';
    $cs[] = 'normal';
    $ct[] = "Le commandement du dispositif sera assuré par l'association prestataire.";

    if ($interassociatif) {
        $cs[] = 'title3';
        $ct[] = "5.2.3 Cas particulier d'un DPS Inter associatif";
        $cs[] = 'normal';
        $ct[] = "L'organisateur désignera le coordinateur inter-associatif.";
    }
    $cs[] = 'title2';
    $ct[] = '5.3 Modalités financières';
    $cs[] = 'title3';
    $ct[] = '5.3.1 Montant de la participation';
    ...
    if ($montant_devis != '') {
        $a .= ', estimés à '.$montant_devis.' '.$default_money_symbol.'.';
    } else {
        $a .= ", pour un montant défini dans le devis remis par l'association.";
    }
    $cs[] = 'normal';
    $ct[] = $a;

    $cs[] = 'title3';
    $ct[] = '5.3.2 Conditions de paiement';
    $cs[] = 'normal';
    $ct[] = "Cette somme sera réglée par virement ou par chèque libellé à l'ordre de : ".$section_prefix.' '.$section_affiche.' '.$antenne_affiche;

    $cs[] = 'title1';
    $ct[] = '6. Engagement des deux parties';
    $cs[] = 'title2';
    $ct[] = '6.1 Durée de la convention';
    $cs[] = 'normal';
    $ct[] = "Cette convention est signée pour la durée de l'événement objet de la présente.";
    $cs[] = 'title2';
    $ct[] = '6.2 Condition de réalisation';
    $cs[] = 'normal';
    $ct[] = "L'engagement de l'association prestataire est lié :".
    "\n- à l'acceptation de la présente convention par l'organisateur.".
    "\n- à l'autorisation de l'événement par les pouvoirs publics.\n\n";

    $cs[] = 'title1';
    $ct[] = "7. Grille d'évalution des risques";
    $cs[] = 'normal';
    $ct[] = "Cette grille remplie sous la responsabilité de l'organisateur figure en annexe de la présente convention.\n\n";
    ...
    $ct[] = '15,'.$y_after_new_page;
    $cs[] = 'title1';
    $ct[] = '8. Clauses particulières';
    $cs[] = 'normal';
    $ct[] = ''.$clause_particuliere_1.''."\n\n".$clause_particuliere_2."\n";

    $cs[] = 'title1';
    $ct[] = '9. Litiges';
    $cs[] = 'normal';
    $ct[] = "En cas de litige pendant et après la manifestation, à défaut d'entente entre l'association prestataire et l'organisateur, le contentieux pourra faire l'objet de recours devant les tribunaux compétents.";
    $cs[] = 'normal';
...
if ($mode == 50) {
    $nouvel_adherent1 = "
Cher adhérent,\n
Nous vous souhaitons tout d’abord la bienvenue au sein de la Fédération Autonome SPP-PATS et nous vous remercions chaleureusement de la confiance que vous nous avez témoignée en rejoignant notre Fédération. Votre adhésion prend effet à partir du ".$P_DATE_ENGAGEMENT.".\n
La FA/SPP-PATS est la 1ère force syndicale dans la corporation des SPP, représentant plus de 6000 adhérents répartis dans plus de 60 départements. Elle est démocratique, apolitique et a pour but la défense des intérêts collectif et individuel des SPP et PATS.\n
L’adhésion à la FA/SPP-PATS vous apporte une aide à l’assistance et à la protection juridique professionnelle ainsi que la certitude d’être informé sur les différents dossiers intéressant les Sapeurs-Pompiers Professionnels et les Personnels Administratifs Techniques et Spécialisés.\n
De même que vous pouvez désormais entrer dans l’accès réservé de notre site internet (www.faspp-pats.org) :\n
Votre identifiant : ".$P_CODE."\n
Votre carte d’adhérent numérisée est dès à présent disponible et téléchargeable depuis votre espace.\n";

    $nouvel_adherent2 = "
Si vous nous avez fourni une adresse email, un courriel vous indiquant votre mot de passe vous a été envoyé lors de votre inscription dans notre base d’adhérents.\n
Pour tout oubli ou problème de mot de passe, rendez-vous sur notre base d’adhérents : ".$cisurl.", dans la rubrique Mot de passe perdu afin d’en générer un nouveau à l’aide de votre identifiant et de votre adresse email.
(N’oubliez pas de changer votre mot de passe dès votre première visite).
Si vous n’avez pas indiqué d’adresse mail, merci de prendre contact auprès de notre service adhérent par mail ou téléphone.\n";

    $nouvel_adherent3 = '
Vous avez opté pour le mode de paiement par prélèvement SEPA dont voici les caractéristiques :
Nom du créancier : FA/SPP-PATS
Identifiant Créancier SEPA (ICS) : FR57ZZZ428791
Référence unique de Mandat (RUM) : 0000007590
Date de début des échéances : '.$date_debut_echeances.'
D’un montant mensuel de : '.$montant_mensuel;

    $nouvel_adherent4 = "Pour recevoir les informations en temps réel, nous vous invitons dès aujourd’hui à nous communiquer votre adresse mail. Pour cela, rien de plus simple, envoyez un mail à l’adresse suivante : adherent@faspp-pats.org en nous indiquant votre nom et les coordonnées mail auxquelles vous souhaitez recevoir les informations.\n
Enfin, pour toute question, n’hésitez pas à nous contacter par téléphone, par courrier ou par mail, l’équipe de la Fédération Autonome, votre correspondant comme votre président départemental se tiennent à votre entière disposition.\n
Nous vous prions de recevoir, cher adhérent, nos salutations amicales.\n";
}
