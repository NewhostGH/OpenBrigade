...
if (isset($_GET['dtdb'])) {
    ...
    $dtdb = date('d-m-Y', strtotime('-1 month'));
}
if (isset($_GET['dtfn'])) {
    $dtfn = $_GET['dtfn'];
} else {
...
if ($tab == 1) {
    ...
    $possibleorders = ['CC_CODE', 'TC_ID', 'C_NOMBRE', 'C_MINIMUM', 'TUM_DESCRIPTION', 'S_CODE', 'C_DESCRIPTION', 'C_DATE_PEREMPTION', 'C_LIEU_STOCKAGE'];
    if (! in_array($order, $possibleorders) or $order == '') {
        $order = 'tc.CC_CODE,tc.TC_DESCRIPTION';
    }

    if ($order == 'CC_CODE') {
        $query .= $queryadd.' order by tc.CC_CODE,tc.TC_ID';
    } elseif ($order == 'TC_ID') {
        $query .= $queryadd.' order by tc.TC_DESCRIPTION';
    } elseif ($order == 'C_NOMBRE') {
        $query .= $queryadd.' order by '.$order.' desc';
    } elseif ($order == 'C_MINIMUM') {
        $query .= $queryadd.' order by DIFF desc';
    } elseif ($order == 'TUM_DESCRIPTION') {
        $query .= $queryadd.' order by tum.TUM_DESCRIPTION asc';
    } elseif ($order == 'S_CODE') {
        $query .= $queryadd.' order by S_CODE asc';
    } elseif ($order == 'C_DESCRIPTION') {
        $query .= $queryadd.' order by C_DESCRIPTION asc';
    } elseif ($order == 'C_DATE_PEREMPTION') {
        $query .= $queryadd.' order by NBDAYSPEREMPTION asc';
    } elseif ($order == 'C_LIEU_STOCKAGE') {
        $query .= $queryadd.' order by c.C_LIEU_STOCKAGE desc';
    } else {
...
if ($tab == 2) {
    ...
    $possibleorders = ['TC_DESCRIPTION', 'E_LIBELLE', 'EC_NOMBRE', 'EC_DATE_CONSO'];
    if (! in_array($order, $possibleorders) or $order == '') {
        $order = 'EC_DATE_CONSO';
    }
    if ($type_conso != 'ALL') {
        $addsql = " and (tc.TC_ID='".$type_conso."' or tc.CC_CODE='".$type_conso."')";
    } else {
        $addsql = '';
    }
    if ($order != '') {
