...
if (count($horaire_evt) == 1) {
    $header .= utf8_encode($horaire_evt[1]);
}
...
$styleFont->getColor()->setARGB(Color::COLOR_DARKBLUE);
...
$sheet->setCellValue('B'.$r, utf8_encode($The_Organizer));
$r++;

if (intval($E_TEL) > 0) {
    $sheet->mergeCells('B'.$r.':L'.$r);
    $sheet->setCellValue('A'.$r, 'Telephone contact');
    $sheet->setCellValue('B'.$r, utf8_encode($E_TEL));
    $r++;
}

$sheet->mergeCells('B'.$r.':L'.$r);
$sheet->setCellValue('A'.$r, 'Responsable');
$sheet->setCellValue('B'.$r, utf8_encode($prenom_chef.' '.$nom_chef.' '.$phone_chef));
$r++;

$sheet->mergeCells('B'.$r.':L'.$r);
$sheet->setCellValue('A'.$r, 'Pour le compte de');
$sheet->setCellValue('B'.$r, utf8_encode(get_company_name("$C_ID")));
$r++;

$sheet->mergeCells('B'.$r.':L'.$r);
$sheet->setCellValue('A'.$r, 'Contact local');
$sheet->setCellValue('B'.$r, utf8_encode($E_CONTACT_LOCAL.' '.$E_CONTACT_TEL));
$r++;

$sheet->mergeCells('B'.$r.':L'.$r);
$sheet->setCellValue('A'.$r, 'Lieu');
$sheet->setCellValue('B'.$r, utf8_encode($E_LIEU));
$r++;

$sheet->mergeCells('B'.$r.':L'.$r);
$sheet->setCellValue('A'.$r, utf8_encode('Durée'));
$sheet->setCellValue('B'.$r, $E_DUREE_TOTALE.'h');
$r++;
...
$query = "select e.E_CODE as EC, p.P_CODE, p.P_ID, p.P_NOM,p.P_PHONE, p.P_EMAIL, p.P_PRENOM, p.P_GRADE, s.S_ID, p.P_HIDE, p.P_SEXE,
...
if (mysqli_num_rows($result) > 0) {
    ...
    $sheet->setCellValue("B$r", utf8_encode($t));
    ...
    $sheet->setCellValue("E$r", utf8_encode('téléphone'));
    ...
    if ($syndicate == 0) {
        ...
        $sheet->setCellValue("K$r", utf8_encode('équipe'));
        $sheet->setCellValue("L$r", utf8_encode('Statut'));
        $sheet->setCellValue("M$r", utf8_encode('Date naissance'));
        $sheet->setCellValue("N$r", utf8_encode('compétences valides'));
        $last = 'N';
    } else {
        $last = 'I';
    }
    ...
    while ($row = @mysqli_fetch_array($result)) {
        ...
        if ($P_ID != $prevpid) {
            ...
            if ($AGE > 0 and $AGE < 18) {
                ...
                $styleFontM->getColor()->setARGB(Color::COLOR_RED);
            }
            $prevpid = $P_ID;
        } else {
            $NewPID = false;
        ...
        if ($NewPID) {
            ...
            $birth = $P_BIRTHDATE.' à '.$P_BIRTHPLACE;
            ...
            $sheet->setCellValue("F$r", utf8_encode($email));
            $sheet->setCellValue("G$r", utf8_encode($adresse));
            $sheet->setCellValue("H$r", utf8_encode($fullcomment));
            if ($syndicate == 0) {
                $sheet->setCellValue("I$r", utf8_encode($horaire));
                $sheet->setCellValue("J$r", utf8_encode($TP_LIBELLE));
                $sheet->setCellValue("K$r", utf8_encode($EE_NAME));
                $sheet->setCellValue("L$r", utf8_encode($TSP_CODE));
                $sheet->setCellValue("M$r", utf8_encode($birth));
                $sheet->setCellValue("N$r", utf8_encode($postes));
            }
            $curval = $horaire;
            $r++;
        } else {
            $p = $r - 1;
            $curval = $curval.', '.$horaire;
            $sheet->setCellValue("I$p", utf8_encode($curval));
        }
    }
}
...
if ($materiel == 1) {
    ...
    if ($nbmat > 0) {
        ...
        $sheet->setCellValue("B$r", utf8_encode('modle'));
        $sheet->setCellValue("C$r", utf8_encode($section_label));
...
if ($nboptions > 0) {
    ...
    while ($row = @mysqli_fetch_array($result)) {
        ...
        if ($syndicate == 1) {
            $p = $P_PROFESSION;
        } elseif (intval($E_PARENT) > 0) {
            $p = ucfirst($renfort_label).' '.$S_CODE;
        } else {
            $p = '';
        }
...
// =======================================================
// quatrime onglet : personnel avec horaires
// =======================================================
...
while ($row = @mysqli_fetch_array($result)) {
    ...
    if ($E_PARENT > 0) {
        $p = ucfirst($renfort_label).' '.$S_CODE1;
    } else {
        $p = '';
    }
    $sheet->setCellValue("A$r", utf8_encode($p));
...
if ($syndicate == 0) {
    ...
    while ($row = @mysqli_fetch_array($result)) {
        ...
        if ($P_PRENOM2 != '' and $P_PRENOM2 != 'none') {
            $P_PRENOM .= ', '.$P_PRENOM2;
        }
        $PAYS = $row['PAYS'];
        $S_CODE1 = ' '.$row['S_CODE'];
