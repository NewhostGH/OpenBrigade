...
$columns_title = ['Numero', 'Type', 'DPS', 'Activité', 'Statut', 'Organisateur',
...
if ($type_evenement != 'ALL' and $type_evenement != '') {
    ...
    $q1 = "select TB_NUM, TB_LIBELLE from type_bilan where TE_CODE='".$type_evenement."' order by TB_NUM";
    ...
    while ($row = @mysqli_fetch_array($r1)) {
        ...
        array_push($stats, 'T'.$TB_NUM);
        $TB_LIBELLE = str_replace("'", '', $TB_LIBELLE);
        array_push($columns_title, $TB_LIBELLE);
        $last = next_letter($last);
        array_push($columns, $last);
        $fields .= 'case when EH.eh_id=1 then be'.$i.".BE_VALUE else '0' end as 'T".$TB_NUM."',";
...
$query = "select E.TE_CODE, TE.TE_LIBELLE, E.E_LIEU, EH.EH_ID,
    ...
    E.E_PARENT, E.TAV_ID, S.S_HIDE, S.S_PARENT, SF.NIV ".$fields.'
    from evenement E '.$from_join.", type_evenement TE, section S, section_flat SF, evenement_horaire EH
...
while (custom_fetch_array($result)) {
    ...
    if ($E_CANCELED == 1) {
        $state = 'événement annulé';
    } elseif ($E_CLOSED == 1) {
        $state = 'inscriptions fermées';
    } elseif ($E_OPEN_TO_EXT == 0) {
        $state = 'inscriptions interdites pour personnes extérieures';
    } else {
        $state = 'inscriptions ouvertes';
    }
    ...
    $columns_data = [$E_CODE, $TE_CODE, $TA_SHORT, $E_LIBELLE, $state, $organisateur,
        $renfort, $E_LIEU, $EH_DATE_DEBUT, $EH_DATE_FIN, $EH_DEBUT,
        $EH_FIN, $EH_DUREE, $NP, $E_NB, $myfact];

    foreach ($stats as $stat) {
        array_push($columns_data, ${$stat});
    }
...
if (check_rights($id, 15, intval($section))) {
    ...
    $columns_title = ['Nom', 'Prénom', 'Adresse', 'Code postal', 'Ville', 'Téléphone', 'Email', 'Section'];
    ...
    while ($row = @mysqli_fetch_array($result)) {
        ...
        $P_PHONE = ' '.$row['P_PHONE'];
        $S_CODE = ' '.$row['S_CODE'];
        $columns_data = [$P_NOM, $P_PRENOM, $P_ADDRESS, $P_ZIP_CODE, $P_CITY, $P_PHONE, $P_EMAIL,  $S_CODE];
        foreach ($columns as $c => $letter) {
            $objPHPExcel->getActiveSheet()->setCellValue($letter.$i, utf8_encode($columns_data[$c]));
        }
        $i++;
    ...
    $columns_title = ['Numero', 'Evenement', 'Debut', 'Durée', 'Equipe', 'Nom', 'Prénom', 'Fonction', 'Adresse', 'Code postal', 'Ville', 'Téléphone', 'Email', 'Section'];
    ...
    while ($row = @mysqli_fetch_array($result)) {
        ...
        $E_CODE = $row['E_CODE'];
        $columns_data = [$E_CODE, $E_LIBELLE, $EH_DATE_DEBUT, $EP_DUREE, $EE_NAME, $P_NOM, $P_PRENOM, $TP_LIBELLE, $P_ADDRESS, $P_ZIP_CODE, $P_CITY, $P_PHONE, $P_EMAIL,  $S_CODE];
        foreach ($columns as $c => $letter) {
            $objPHPExcel->getActiveSheet()->setCellValue($letter.$i, utf8_encode($columns_data[$c]));
        }
        $i++;
    ...
    // Add the columns heads
    $columns = ['A', 'B', 'C', 'D', 'E', 'F'];
    $columns_title = ['Type', 'Immatriculation', 'Modele', 'Indicatif', 'Section', 'Km parcourus'];
    ...
    while ($row = @mysqli_fetch_array($result)) {
        ...
        $S_CODE = ' '.$row['S_CODE'];
        $KM = $row['KM'];
        $columns_data = [$TV_CODE, $V_IMMATRICULATION, $V_MODELE, $V_INDICATIF, $S_CODE, $KM];
        foreach ($columns as $c => $letter) {
            $objPHPExcel->getActiveSheet()->setCellValue($letter.$i, utf8_encode($columns_data[$c]));
        }
        $i++;
