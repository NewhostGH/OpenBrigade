...
$possibleorders = ['P_STATUT', 'P_NOM', 'P_SECTION', 'P_PROFESSION', 'MONTANT', 'COMMENTAIRE', 'PC_ID', 'P_DATE_ENGAGEMENT', 'P_FIN', 'PC_DATE'];
...
while ($row = @mysqli_fetch_array($result1)) {
    ...
    if ($MONTANT > 0) {
        ...
        if ($COMMENTAIRE != '') {
            $libelle = str_pad($COMMENTAIRE, 31);
        } else {
        ...
        if ($COMPTE == '' or $ETABLISSEMENT == '' or $GUICHET == '') {
            $err++;
        }
        $i++;
