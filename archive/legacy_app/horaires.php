...
if (isset($_GET['action'])) {
    ...
    if ($action == 'pointer' or $action == 'depointer') {
        if ($action == 'pointer') {
            fill_horaires_start($id);
        }
        if ($action == 'depointer') {
            fill_horaires_end($id);
        }
...
if (intval($person) > 0) {
    ...
    if (check_rights($id, 14)) {
        $update_allowed = true;
    } elseif ($syndicate == 1 and $id == $person) {
        $update_allowed = false;
    } elseif (check_rights($id, 13, $P_SECTION)) {
        $update_allowed = true;
    } else {
        $update_allowed = false;
    }
...
if ($view == 'week') {
    ...
    if (jddayofweek($jd) == 0 and $year % 4 > 0) {
        $maxweek = 52;
    } else {
        $maxweek = 53;
    }
    ...
    if ($year > $yearprevious) {
        echo "<option value='$yearprevious'>".$yearprevious.'</option>';
    } else {
        echo "<option value='$yearprevious' selected>".$yearprevious.'</option>';
    }
    if ($year != $yearcurrent) {
    ...
    if ($year < $yearnext) {
        echo "<option value='$yearnext' >".$yearnext.'</option>';
    } else {
        echo "<option value='$yearnext' selected>".$yearnext.'</option>';
    }
    echo '</select>';
    ...
    if ($syndicate == 0 and $id == $person) {
        if ($HS_CODE == 'SEC' or $HS_CODE == '') {
            if ($id == $person) {
                $query1 .= " where HS_CODE in ('SEC','ATTV') or HS_CODE = '".$HS_CODE."'";
            }
        } else {
            $disabled = 'disabled';
        }
    ...
    for ($i = 0; $i <= 6; $i++) {
        ...
        $debut1 = @$row[0];
        $fin1 = @$row[1];
        if ($debut1 == '00:00' and $fin1 == '00:00') {
            $debut1 = '';
            $fin1 = '';
        }
        ...
        $debut2 = @$row[2];
        $fin2 = @$row[3];
        if ($debut2 == '00:00' and $fin2 == '00:00') {
            $debut2 = '';
            $fin2 = '';
        }
        ...
        if ($duree2_minutes == 0) {
            ...
            $duree2 = convert_hours_minutes($duree2_minutes);
        }
        if ($duree2 == '0h') {
            $duree2 = '';
        }
        ...
        if ($duree_minutes == 0) {
            ...
            $duree_affichage = convert_hours_minutes($duree_minutes);
        }
        ...
        if (($HS_CODE == 'SEC' or $HS_CODE == '') and $person == $id) {
            $disabled2 = '';
        } else {
            $disabled2 = $disabled;
        }
        ...
        if ($H_COMMENT == '') {
            $cmt = 'texte libre permettant de noter le détail du travail de la journée';
        } else {
            $cmt = $H_COMMENT;
        }
    ...
    if ($HS_CODE != '') {
        ...
        $selectForm = "<span class='".$HS_CLASS."'><b>".$HS_DESCRIPTION.'</b></span>';
    }
    ...
    if (! $onlyTable) {
        if ($from == 'export') {
            echo " <input type=submit class='btn btn-secondary' value='Fermer' onclick='window.close();'>";
        } elseif ($from == 'list') {
            echo " <input type='button' class='btn btn-secondary' value='Retour' onclick='javascript:self.location.href=\"upd_personnel.php?tab=12&pompier=".$person.'&person='.$person."&view=list&table=1\";'>";
        } elseif ($from == 'save') {
            echo " <input type='button' class='btn btn-secondary' value='Retour' onclick='javascript:history.go(-3);'>";
        } else {
    ...
    if ($annuelles > 0) {
        ...
        if ($year > date('Y') or $heures_sup_decimal < -200) {
            $heures_sup = 0;
        }
        if ($heures_sup < 0) {
            $color = 'red';
    ...
    if ($number > 0) {
        ...
        while (custom_fetch_array($result)) {
            ...
            if ($horaire_list_mode == 'W') {
                ...
                if (dateCheckPublicholiday(mktime(0, 0, 0, $MOIS, $DAY, $ANNEE))) {
                    $daystyle = "style='background-color:orange;'";
                } elseif ($dow == 0 or $dow == 6) {
                    $daystyle = "style='background-color:yellow;'";
                } else {
                    $daystyle = '';
                }
            ...
            if (intval($TS_HEURES) > 0) {
                ...
                if ($SUP_SEMAINE_MINUTES > 0) {
                    $sup = '<span class=green12>'.$SUP_SEMAINE_HEURES.'</span>';
                } elseif ($SUP_SEMAINE_MINUTES < 0 and $HEURES > 0 and ($SEMAINE < date('W') or $ANNEE != date('Y'))) {
                    $sup = '<span class=red12>'.$SUP_SEMAINE_HEURES.'</span>';
                }
            }
    ...
    echo "$later<p>";
    if ($from == 'export') {
        echo "<input type=submit class='btn btn-default' value='fermer' onclick='window.close();'>";
    } elseif ($from == 'accueil') {
        echo "<input type='button' class='btn btn-secondary' value='Retour' onclick='javascript:history.back(1);'>";
    } else {
        echo "<input type='button' class='btn btn-secondary' value='Retour' onclick=\"change_display('".$person."','".$week."','".$year."','week', '".$from."','".$horaire_list_mode."');\">";
    }
