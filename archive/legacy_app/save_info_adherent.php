...
if (! check_rights($id, 53, get_section_of("$P_ID"))) {
    check_all(24);
}
...
if ($bank_accounts == 1) {
    ...
    if (isset($_POST['iban'])) {
        $iban = str_replace(' ', '', secure_input($dbc, $_POST['iban']));
    } else {
        $iban = '';
    }
    ...
    if ($OLDBIC != $bic or $OLDIBAN != $iban) {
        ...
        if ($OLDBIC != $bic) {
            insert_log('UPDBIC', $P_ID, 'ancien BIC: '.$OLDBIC);
        }
        if ($OLDIBAN != $iban) {
            insert_log('UPDIBAN', $P_ID, 'ancien IBAN: '.$OLDIBAN);
        }
    }
