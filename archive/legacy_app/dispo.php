...
if ($tab == 1 || $onlyTable) {
    ...
    if ($pompiers == 1) {
        ...
        if ($nbsections > 0) {
            $mysection = 0;
        } elseif ($level_section == $nbmaxlevels - 1) {
            $mysection = get_section_parent($lasection);
        } else {
            $mysection = $lasection;
        }
    ...
    while ($m <= 12) {
        ...
        if ($m == $month) {
            echo "<option value='$m' selected >".$monmois."</option>\n";
        } else {
            echo "<option value= $m >".$monmois."</option>\n";
        }
        $m = $m + 1;
    }
    echo '</select>';
    ...
    if (check_rights($id, 5, $lasection)) {
        $admin = true;
    } else {
        $admin = false;
    }
    ...
    if ($dispo_periodes == 1) {
        $queryA .= ' where DP_ID= 1';
    }
    if ($dispo_periodes == 2) {
        $queryA .= ' where DP_ID in (1,4)';
    }
    if ($dispo_periodes == 3) {
        $queryA .= ' where DP_ID in (1,2,4)';
    }
    ...
    if ($jj == 0) {
        $jj = 7;
    } // on affecte 7 au dimanche, (lundi=1)
        ...
        while (($jj <= 7) && ($i <= $d)) { // boucle des jours de la semaine
            ...
            if ($lasection != 0) {
                if ($s_garde_jour != $s_garde_nuit) {
                    if ($s_garde_jour == $lasection) {
                        $mycolor = $widget_bggreen;
                    }
                    if ($s_garde_nuit == $lasection) {
                        $mycolor = $widget_bgblue;
                    }
                } elseif ($s_garde_jour == $lasection) {
                    $mycolor = $widget_bggreen;
                }
            }
            if (is_out($person, $year, $month, $i) != 0) {
                $mycolor = $widget_bgorange;
            }
            if ($DAYDATE < $CURDATE) {
            ...
            if ($blockdispo and ! $admin) {
                $disabled_cell = 'disabled';
            } else {
                $disabled_cell = '';
            }
            ...
            if ($dispo_periodes == 1) {
                ...
                if ($disabled_cell == 'disabled' and $checked[1]) {
                    echo "<input type=hidden name='save_1".$i."' id='save_1' value='$checked[1]'></td>";
                }
            }
            if ($dispo_periodes == 2) {
                ...
                if ($disabled_cell == 'disabled' and $checked[1]) {
                    echo "<input type=hidden name='save1_".$i."' id='save_1' value='$checked[1]'>";
                }
                if ($disabled_cell == 'disabled' and $checked[4]) {
                    echo "<input type=hidden name='save4_".$i."' id='save_4' value='$checked[4]'>";
                }
                echo '</td>';
            }
            if ($dispo_periodes == 3) {
                ...
                if ($disabled_cell == 'disabled' and $checked[2]) {
                    echo "<input type=hidden name='save2_".$i."' id='save_2' value='$checked[2]'>";
                }
                if ($disabled_cell == 'disabled' and $checked[4]) {
                    echo "<input type=hidden name='save4_".$i."' id='save_4' value='$checked[4]'>";
                }
            ...
            if ($dispo_periodes == 4) {
                ...
                if ($disabled_cell == 'disabled' and $checked[3]) {
                    echo "<input type=hidden name='save3_".$i."' id='save_3' value='$checked[3]'>";
                }
                if ($disabled_cell == 'disabled' and $checked[4]) {
                    echo "<input type=hidden name='save4_".$i."' id='save_4' value='$checked[4]'>";
                }
...
if ($tab == 2) {
    ...
    } elseif ($dispo_periodes == 2) {
        $periodes[1] = 'Jour';
        $periodes[3] = 'Nuit';
    } elseif ($dispo_periodes == 3) {
        $periodes[1] = 'Matin';
        $periodes[2] = 'Après-midi';
        $periodes[3] = 'Nuit';
    } elseif ($dispo_periodes == 4) {
        $periodes[1] = 'Matin';
        $periodes[2] = 'Après-midi';
        $periodes[3] = 'Soir';
        $periodes[4] = 'Nuit';
    ...
    if ($poste != 0) {
        $query1 .= " and q.P_ID=p.P_ID and q.PS_ID=$poste";
    }
    ...
    if (mysqli_num_rows($result1) == 0) {
        ...
        while (custom_fetch_array($result)) {
            ...
            foreach ($periodes as $period => $DP_NAME) {
                if (! isset($dispos[$period][$P_ID])) {
                    $dispos[$period][$P_ID] = 0;
                }
            }
            if ($gardes) {
        ...
        foreach ($names as $pid => $value) {
            ...
            if ($grades == 1) {
                $file = $value['icone'];
                if (file_exists($file)) {
                    $t = "<img class='img-max-30 ' style='border-radius: 3px;' src=\"".$file.'" title="'.$value['grade_desc'].'">';
                } else {
                    $t = '<span>'.$value['grade'].'</span>';
                }
...
if ($tab == 4) {
    ...
    echo repo_dispo_view($filter,$year,$month);
}
