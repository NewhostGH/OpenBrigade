...
`plugin.json` manifest, booted through a **standard Laravel service
provider**. No plugin API layer: an enabled plugin's provider is registered
like any application provider, with the same privileges.
...
- **No composer dependencies**: the autoloader is composer-less (app's
  `vendor/` + your PSR-4 sources only). Bundle a needed library's
  (license-permitting) sources under your PSR-4 tree, or contribute it to core.
- Blade conventions apply to shipped views (i18n via `__()`, no inline
  `<style>`); `php artisan ob:views:lint` won't check plugin views, but broken
  compiled Blade surfaces as a load failure.
...
1. Work directly in `plugins/<slug>/` (gitignored in the app repo: keep the
   plugin in its own repo) and enable it once; code changes apply immediately
   since the provider re-registers per request.
