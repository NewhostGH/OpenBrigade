...
**Single source of truth for how code is written.** Binding for all migrated
and new code.

`tests/Feature/ConventionsTest.php` enforces the most mechanical rules in CI.
...
**Raw rows are not exempt**: if a query builder returns `stdClass` rows needing a
derived value, either return Eloquent models or put the derivation in a shared
service method called from both paths.

## 2. Models

- **One model per table** is the goal.
- Where two models intentionally map to the same table (`User` = auth concerns,
  `Personnel` = domain concerns, both on `pompier`), shared behavior **MUST** live
...
Collision-safe Perl one-liner (lookbehind/lookahead prevent double-prefixing
`ob-ob-*` and protect design tokens):

```bash
perl -i -pe 's/(?<![a-zA-Z0-9_\-])(OLD_CLASS)(?![a-zA-Z0-9_\-])/ob-new-class/g' file...
```

The lookbehind doesn't exclude `$`, so a JS variable sharing a token name (e.g.
jQuery `$siglet`) can be wrongly renamed. Always run `npm run build` after each
module to catch it; fix by reverting just the variable name.
...
Any link, asset path, or redirect pointing at the legacy bridge
(`/legacy/...`, `*.php?...`, `archive/legacy_app/...`) **MUST** carry a marker
comment on the same or preceding line: Blade `{{-- TODO: Migrate code --}}`,
PHP/JS `// TODO: Migrate code`. Keeps every remaining coupling greppable (the
Phase 4 decommission work list): `grep -rn "TODO: Migrate code" resources/ app/`.

A legacy URL **without** a `/legacy/` prefix (e.g. `url('/ins_personnel.php')`) is a
**routing bug**, not a bridge: fix the route, or use `route()` when a native
...
New list and detail pages use the reusable `ob-*` component set: follow these
patterns rather than hand-rolling markup.
...
| `ob-badge-archive`                         | Archived / neutral chip             | Gray                          |
| `ob-badge-bloqued`                         | Blocked / canceled                  | Red                           |
...
A row is "in scope" when its `section_id ≤ 0` (global) or names a section in the
active chain (section + ancestors). Global groups (`ob_personnel_group`) always
apply. Within a tier, **deny wins**. Defaults keep this backward compatible:
`ob_group_permission.effect` defaults to `allow`, `ob_user_permission` is
empty, so legacy data reduces to "section deny, then group/role allow."
...
Base habilitation data is owned natively (not back-filled from legacy). SSOT:
`config/habilitations.php` + `App\Support\Habilitations\BaseHabilitations`
(the builder both the rebuild migration and `CoreSeeder` call, so they never drift).

- **Super-admin is the account flag `pompier.P_SUPERADMIN`**, not a group.
  Short-circuits `PermissionResolver::allows()` to ALLOW everything (after
  `isBlocked`), **uncappable** by any section ceiling. At least one always
  exists; controllers refuse to clear/delete the **last** one
  (`PermissionResolver::isLastSuperAdmin`). Gate with `isSuperAdmin()`; never
  read `P_SUPERADMIN` directly (use `hasPermission()`).
- **Four base groups**: Admin, Auditor, User, Guest (reserved ids in
  `config('habilitations.base_groups')`), `is_system`/`isProtected()` so the
  admin UI can't rename or delete them. Grants are **seeded with defaults**,
  then **freely editable** per-permission: never re-derived at runtime.
- **`ob_permission`** is the canonical catalog (`id` = legacy
  `fonctionnalite.F_ID`). Classified on `domain` (`config`|`data`), `is_read`,
  `is_critical` (legacy `F_FLAG`); drives **seeded** base-group defaults only
  (Admin = all non-critical, Auditor = reads, User = non-critical data, Guest
  = data reads); not a runtime enforcement path.
- **Section roles are per organisation type** (`ob_group.org_type`, keyed by
  `config('brigade.organisation_types')`); the seeder creates every type's role set.
- **Seeding split by environment**: `DatabaseSeeder` always runs `CoreSeeder`
  (production-canonical, idempotent) and runs `DevelopmentDataSeeder`
  (throwaway fixtures) **only** outside production. Canonical data goes in
  `CoreSeeder` via `BaseHabilitations`, never `DevelopmentDataSeeder`.
...
**Single authority:** `App\Services\SectionScopeService` is the sole arbiter of
data visibility/edit rights when `multi_site` is on. Computes one **visible
set** per request:
...
The **navbar section switcher** shows the base set (membership + descendants,
never narrowed by the current choice, or switching sideways becomes
impossible), via `switcherSections()` in **org-chart order** (`S_ORDER`, `S_CODE`).

The **active operation scope** (`activeOperationScope()`) is what's actually
filtered when viewing/editing data: intersects the base set with the navbar
choice, `null` when unrestricted (multi_site off).
...
This ensures the org chart root appears first, not last.
`SectionScopeService::descendantIds()` expands a section to itself + all
descendants: use it for subtrees in scope checks or cycle prevention.

**Form membership multiselect:** editing `ob_personnel_section` memberships
only touches rows within the editor's visible scope, so a narrowly-scoped
editor can't strip a member's wider-scope memberships. Same for
`ob_user_assignment` role rows.

**Section dropdown component:** `<x-ob-section-select>`
(`App\View\Components\ObSectionSelect`) renders nothing when multi_site is off
and self-feeds from `SectionScopeService` otherwise: one rendering source for
every section select.
...
**The entire application layer is written in English.** Remaining French is
strictly the legacy *data* layer: immutable, not a style choice.

All identifiers are English: controller/model/service/Form Request class
names, route names, URL path segments, Blade view folders/files, CSS/JS file
names, CSS classes/ids, JS variables, `config/navigation.php` `code`/`key`.

### French stays only in the data layer

**Data mirrored from legacy**, must not be renamed:
...
- [brand-identity.md](brand-identity.md): user-facing copy, colors, icons, images
