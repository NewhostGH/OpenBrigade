...
Replaces Laravel's `php artisan down`: administrable from the UI, survives
deployments, never locks the admin out. Enforced by
...
(`--force`, bypassing the toggle). Tables are briefly locked, so prefer
off-peak hours.
...
| Purger les journaux          | `ob:logs:prune` (honors the configured retention)             |
...
organization type id and the active-member count rounded to the nearest
